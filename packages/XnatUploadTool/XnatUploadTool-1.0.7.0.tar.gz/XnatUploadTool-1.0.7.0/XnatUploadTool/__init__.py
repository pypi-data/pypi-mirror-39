...
class XnatUploadTool:
        ...
        for mysubject, mysubjectdata in self.ingestdata.items():
            ...
            for mysession, mysessiondata in mysubjectdata['sessions'].items():
                if self.check_session(create=True, subject=mysubject, session=mysession,
                                      ...
                                      fields=mysessiondata['fields']) is not True:
                    self.logger.error("Session %s does not exist and cannot be created for project %s/subject %s" %
                                      (mysession, self.project, mysubject))
                    exit(1)
                ...
                for subfile in f:
                    if subfile.startswith('.'):
                        ...
                        self.ingestdata[thisingest['subject']]['sessions']\
                            [thisingest['session']]['scans'][thisingest['scan']] = \
                            {
                                'type': mime_type,
                                'path': mypath,
    ...
    def pull_ingestion_field(self, pattern, filename):
        ...
        if 'split' in pattern:
            ...
            if 'subsplit' in pattern:
                parts = []
                if pattern['source'] == 'filename':
                    parts = os.path.basename(filename).split(pattern['subsplit'][0])
                elif pattern['source'] == 'filename-no-ext':
                    parts = os.path.basename(filename.rsplit(".", 1)[0]).split(pattern['subsplit'][0])
                elif pattern['source'] == 'filepath':
                    parts = filename.split('/')
            ...
            if pattern['source'] == 'filename-ext':
        ...
        if 'range' in pattern:
            if len(pattern['range']) == 1:
                myoutput = myoutput[pattern['range'][0]:]
            else:
                myoutput = myoutput[pattern['range'][0]:pattern['range'][1]]
    ...
    def check_subject(self, create=False, subject=None, fields=None):
        ...
        if create is True:
            ...
            if response.status_code == 201:
                ...
                self.logger.debug('Unable to create subject %s: %s' % (subject, response.text))
    ...
    def check_session(self, create=False, subject=None, session=None, xmlnamespace=None, datatype=None, fields=None):
        ...
        if subject is None:
            self.logger.errot('Session cannot be checked without subject, required for this upload')
            return False
        ...
        if fields is not None:
            for fieldname, fieldvalue in fields.items():
                proc_fields['%s:%sSessionData/%s' % (xmlnamespace, datatype, fieldname)] = fieldvalue
        ...
        if create is True:
            ...
            if response.status_code == 201:
                if self.verbose:
                    self.logger.debug('Created new session %s as %s:%s' % (session, xmlnamespace, datatype))
                return True
            else:
                self.logger.debug('Unable to create session %s: %s' % (session, response.text))
    ...
    def delete_session(self, create=False, subject=None, session=None):
        ...
        if response.status_code == 200:
            ...
            self.logger.debug('Unable to delete session %s: %s' % (session, response.text))
            return False
    ...
    def check_scan(self, create=False, subject=None, session=None, scan=None, xmlnamespace=None, datatype=None,
        ...
        if create is True:
            ...
            if response.status_code == 201 or response.status_code == 200:
                ...
                self.logger.debug('Unable to create scan %s: %s' % (scan, response.text))
    ...
    def renew_session(self):
        ...
        if self.lastrenew is None or ((self.lastrenew + self.sessiontimeout) < datetime.datetime.now()):
            ...
            self.httpsess.config['keep_alive'] = False
            self.httpsess.close()
