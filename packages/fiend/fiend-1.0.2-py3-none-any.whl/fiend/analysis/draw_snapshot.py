...
def draw_snapshot():
    ...
    parser.add_argument("--time", type=float,
                        help=("Which instant of time to draw"
                              "(will use the closest available)")
                        )
    ...
    save_interval = int(prop_config['save_interval'])
    if args['time'] == 'final':
        args['time'] = Afield.pulse_duration()
    else:
        args['time'] = float(args['time'])
    iteration_exact = np.rint(
        (args['time']) / float(prop_config['delta_t']))

    iteration = int(save_interval * np.rint(iteration_exact / save_interval))
    print("Plotting iteration nbr: ", iteration)
    psi_re = load_function('data/tdse_wavefunction/realpart_iteration_%d_real' % iteration,
                           funspace)

    psi_im = load_function('data/tdse_wavefunction/imagpart_iteration_%d_real' % iteration,
                           funspace)
    ...
    if args['save']:
        ...
        fig.savefig('data/figures/snapshot_t%g.pdf' % time)
