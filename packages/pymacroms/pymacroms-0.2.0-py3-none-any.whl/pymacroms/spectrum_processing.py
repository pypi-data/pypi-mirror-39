...
class Spectrum:
    ...
    def __init__(self,
        ...
        self.spectrumCSV = pd.read_csv(filename, "\t", names=["mz", "abundance"])
        self.peakData = self.spectrumCSV.copy(deep=True)
        self.peakData.columns = ["mz", "relAbundance"]
        # Save the raw unfilteres spectrum for later reference
        ...
        # spectrum.to_csv("TN60P_filtered.csv", index=False)
        # print("Filtered spectrum saved")

        # for row in spectrum["toBeDeleted"]:
        #     if row:
        #         spectrum.drop(spectrum["toBeDeleted"], 0)
    ...
    def saveComposition(self):
        ...
        export.to_csv(self.filename + "_composition." + self.extension, index=False)
        ...
        export.to_csv(self.filename + "_isobaric." + self.extension, index=False)
