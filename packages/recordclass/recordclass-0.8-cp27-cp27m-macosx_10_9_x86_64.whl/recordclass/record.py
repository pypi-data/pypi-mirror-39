...
""" 
    ...
    #new_func_def += '\n'.join("    _self[%s]=%s" % (i,n) for i,n in enumerate(field_names)) + "\n"
    ...
    def __repr__(self):
        'Return a nicely formatted representation string'
        args = tuple(self)
        return self.__class__.__name__ + "(" + (repr_fmt % args) + ")" 
...
def join(name, cls, other):
    if cls.__bases__ != (ArrayClass,) or other.__bases__ != (ArrayClass,):
        raise TypeError('All arguments should be child of ArrayClass')
    if not hasattr(cls, '_fields') or not hasattr(other, '_fields'):
        raise TypeError('Missing _fields')
    readonly = not hasattr(cls, '__getitem__') or not hasattr(cls, '__getitem__')
    fields = cls._fields + other._fields
    return structclass(name, fields, readonly=readonly)
