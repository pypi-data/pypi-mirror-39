...
class functions:
    ...
    def choose(lg,df1,df2):
        return functions.standardisation(df1.where(cond=lg.astype(bool),other=df2))
    def absolute(df):
        return functions.standardisation(df.abs())
    def log(df):
        return functions.standardisation(np.log(df.abs()+1))
    def sign(df):
        return np.sign(df)
    def add(df,both):
        return functions.standardisation(df.add(both))
    def subtract(df,both):
        return functions.standardisation(df.subtract(both))
    def multiply(df,both):
        return functions.standardisation(df.multiply(both))
    def divide(df,both):
        if (type(both) in (float,int)) and both==0:
            return functions.standardisation(df)
        elif type(both) == pd.DataFrame:
            both=both.replace(0,np.nan)
        return functions.standardisation(df.divide(both))
    ...
    def correlation(df1,df2,num):
        ...
        r1=df1.rolling(d)
        r2=df2.rolling(d)
        return functions.standardisation(r1.corr(r2).replace(np.inf,np.nan).replace(-np.inf,np.nan))
    def covariance(df1,df2,num):
        ...
        r1=df1.rolling(d)
        r2=df2.rolling(d)
        return functions.standardisation(r1.cov(r2))
    def scale(df):
        return functions.standardisation(df.divide(df.abs().sum(axis=1),axis=0))
    def delta(df,num):
        d=int(np.ceil(abs(num)))
        return functions.standardisation(df-df.shift(d))
    def signedpower(df,num):
        if abs(num)>=3:
            num=3
        return functions.standardisation(df.pow(num))
    def linear_decay(df,num):
        d=int(np.ceil(abs(num)))
        return functions.standardisation(df.rolling(d).apply(lambda x: np.average(x,weights=range(1,d+1))))
    def indneutralize(df,group):
        ...
        return functions.standardisation(result)
    def ts_min(df,num):
        d=int(np.ceil(abs(num)))
        return functions.standardisation(df.rolling(d).min())
    def ts_max(df,num):
        d=int(np.ceil(abs(num)))
        return functions.standardisation(df.rolling(d).max())
    ...
    def ts_sum(df,num):
        d=int(np.ceil(abs(num)))
        return functions.standardisation(df.rolling(d).sum())
    def ts_product(df,num):
        d=int(np.ceil(abs(num)))
        return functions.standardisation(df.rolling(d).apply(np.product))
    def ts_std(df,num):
        d=int(np.ceil(abs(num)))
        return functions.standardisation(df.rolling(d).std())
    def reg_cap(df,cap):
        ...
        return functions.standardisation(r)
