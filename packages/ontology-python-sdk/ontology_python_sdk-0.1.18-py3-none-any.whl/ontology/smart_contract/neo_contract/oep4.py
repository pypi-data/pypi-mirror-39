...
class Oep4(object):
    def __init__(self, sdk):
        self.__sdk = sdk
        self.__bytearray_contract_address = bytearray()

    def set_contract_address(self, contract_address: str or bytearray or bytes):
        if len(contract_address) == 20:
            if isinstance(contract_address, bytes):
                self.__bytearray_contract_address = bytearray(contract_address)
            elif isinstance(contract_address, bytearray):
                self.__bytearray_contract_address = contract_address
            else:
            ...
            self.__bytearray_contract_address = bytearray(binascii.a2b_hex(contract_address))
            self.__bytearray_contract_address.reverse()
        else:
    ...
    def __get_token_setting(self, func_name: str) -> str:
        ...
        res = self.__sdk.neo_vm().send_transaction(self.__bytearray_contract_address, None, None, 0, 0, func, True)
        return res
    ...
    def get_contract_address(self, is_hex: bool = True) -> str or bytearray:
        if is_hex:
            array_address = self.__bytearray_contract_address.copy()
            array_address.reverse()
            return binascii.b2a_hex(array_address).decode('ascii')
        else:
            return self.__bytearray_contract_address
    ...
    def init(self, acct: Account, payer_acct: Account, gas_limit: int, gas_price: int) -> str:
        ...
        tx_hash = self.__sdk.neo_vm().send_transaction(self.__bytearray_contract_address, acct, payer_acct, gas_limit, gas_price,
                                                       func, False)
        return tx_hash
    ...
    def get_total_supply(self) -> int:
        ...
        total_supply = self.__sdk.neo_vm().send_transaction(self.__bytearray_contract_address, None, None, 0,
                                                            0, func, True)
        try:
    ...
    def balance_of(self, b58_address: str) -> int:
        ...
        balance = self.__sdk.neo_vm().send_transaction(self.__bytearray_contract_address, None, None, 0, 0, func, True)
        try:
    ...
    def transfer(self, from_acct: Account, b58_to_address: str, value: int, payer_acct: Account, gas_limit: int,
        ...
        tx_hash = self.__sdk.neo_vm().send_transaction(self.__bytearray_contract_address, from_acct, payer_acct, gas_limit,
    ...
    def transfer_multi(self, args: list, payer_acct: Account, signers: list, gas_limit: int, gas_price: int):
        ...
        params.append(0x67)
        for i in self.__bytearray_contract_address:
            params.append(i)
    ...
    def approve(self, owner_acct: Account, b58_spender_address: str, amount: int, payer_acct: Account, gas_limit: int,
        ...
        tx_hash = self.__sdk.neo_vm().send_transaction(self.__bytearray_contract_address, owner_acct, payer_acct, gas_limit,
                                                       gas_price, func, False)
        return tx_hash
    ...
    def allowance(self, b58_owner_address: str, b58_spender_address: str):
        ...
        allowance = self.__sdk.neo_vm().send_transaction(self.__bytearray_contract_address, None, None, 0, 0, func, True)
