...
def deprecated(
    since, message="", name="", alternative="", pending=False, obj_type=None
):
    """
    ...
    def deprecate(
        obj, message=message, name=name, alternative=alternative, pending=pending
    ):
        if obj_type is None:
        ...
        if not message or type(message) == type(deprecate):
            if pending:
                ...
                message = (
                    "The %(func)s %(obj_type)s is deprecated and may "
