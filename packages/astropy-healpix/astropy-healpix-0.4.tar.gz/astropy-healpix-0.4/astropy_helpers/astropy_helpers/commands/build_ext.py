...
def should_build_with_cython(package, release=None):
    ...
    try:
        version_module = __import__(
            package + ".cython_version", fromlist=["release", "cython_version"]
        )
    except ImportError:
        version_module = None
...
def get_compiler_version(compiler):
    ...
    # cases anyway
    flags = ["--version", "--Version", "-version", "-Version", "-v", "-V"]
...
def generate_build_ext_command(packagename, release):
    ...
    class build_ext(SetuptoolsBuildExt, object):
        ...
        _broken_compiler_mapping = [("i686-apple-darwin[0-9]*-llvm-gcc-4.2", "clang")]
        ...
        def _final_class(cls):
            ...
            boolean_options = base_cls.boolean_options + [
                opt
                for opt in cls._boolean_options
