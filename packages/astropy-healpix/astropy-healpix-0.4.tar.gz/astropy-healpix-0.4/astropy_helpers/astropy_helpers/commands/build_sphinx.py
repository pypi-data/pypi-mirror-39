...
class AstropyBuildDocs(SphinxBuildDoc):
    ...
    user_options.append(
        ("no-intersphinx", "n", "Skip intersphinx, even if conf.py says to use it")
    )
    user_options.append(
        (
        ...
        if self.clean_docs:
            ...
            for d in dirstorm:
                if os.path.isdir(d):
                    ...
                    log.info(
                        "Not cleaning directory " + d + " because "
    ...
    def run(self):
        ...
        if self.no_intersphinx:
            ...
            subproccode = subproccode.replace(
                "confoverrides = {}", "confoverrides = {'intersphinx_mapping':{}}"
            )

        log.debug(
            "Starting subprocess of {0} with python code:\n{1}\n"
        ...
        if proc.returncode == 0:
            if self.open_docs_in_browser:
                if self.builder == "html":
                    ...
                    log.warn(
                        "open-docs-in-browser option was given, but "
                        "the builder is not html! Ignoring."
                    )
        else:
            log.warn(
                "Sphinx Documentation subprocess failed with return "
...
class AstropyBuildSphinx(AstropyBuildDocs):  # pragma: no cover
    ...
    def run(self):
        warnings.warn(
            'The "build_sphinx" command is now deprecated. Use' '"build_docs" instead.',
