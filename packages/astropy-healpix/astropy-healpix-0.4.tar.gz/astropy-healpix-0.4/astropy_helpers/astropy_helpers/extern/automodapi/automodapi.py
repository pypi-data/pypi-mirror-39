...
_automodapirex = re.compile(
    r"^(?:\.\.\s+automodapi::\s*)([A-Za-z0-9_.]+)" r"\s*$((?:\n\s+:[a-zA-Z_\-]+:.*$)*)",
    flags=re.MULTILINE,
)
...
def automodapi_replace(sourcestr, app, dotoctree=True, docname=None, warnings=True):
    """
    ...
    if len(spl) > 1:  # automodsumm is in this document
        ...
        for grp in range(len(spl) // 3):
            ...
            if top_head:
                newstrs.append(
                    automod_templ_modheader.format(
                        modname=modnm,
                        modhds=h1 * len(modnm),
            ...
            if hasfuncs:
                newstrs.append(
                    automod_templ_funcs.format(
                        modname=modnm, funchds=h2 * 9, clsfuncoptions=clsfuncoptionstr
                    )
                )

            if hascls:
                newstrs.append(
                    automod_templ_classes.format(
                        modname=modnm, clshds=h2 * 7, clsfuncoptions=clsfuncoptionstr
                    )
                )

            if allowothers and hasother:
                newstrs.append(
                    automod_templ_vars.format(
                        modname=modnm, otherhds=h2 * 9, clsfuncoptions=clsfuncoptionstr
                    )
                )
        ...
        if app.config.automodapi_writereprocessed:
            ...
            if docname is None:
                with io.open(
                    os.path.join(app.srcdir, "unknown.automodapi"), "a", encoding="utf8"
                ) as f:
                    f.write(u"\n**NEW DOC**\n\n")
                    f.write(ustr)
            else:
                ...
                with io.open(
                    os.path.join(app.srcdir, filename), "w", encoding="utf8"
                ) as f:
                    f.write(ustr)
...
def _mod_info(modname, toskip=[], onlylocals=True):
    ...
    ispkg = (
        hasattr(pkg, "__file__")
        and isinstance(pkg.__file__, str)
        and os.path.split(pkg.__file__)[1].startswith("__init__.py")
    )
