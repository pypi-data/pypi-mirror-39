    ...
    def run(self):
        ...
        try:
            ...
            if [clsonly, funconly, varonly].count(True) > 1:
                self.warning(
                    "more than one of functions-only, classes-only, "
            ...
            if "skip" in self.options:
                ...
                if len(option_skipnames) > 0:
                    self.warn(
                        "Tried to skip objects {objs} in module {mod}, "
            ...
            if funconly:
                ...
                cont = []
                for nm, obj in zip(localnames, objs):
                    if nm not in skipnames and not (
...
_automodsummrex = re.compile(
    _hdrex + "(" + _oprex1 + "?" + _oprex2 + "*)", re.MULTILINE
)
...
def automodsumm_to_autosummary_lines(fn, app):
    ...
    with io.open(fullfn, encoding="utf8") as fr:
        ...
        if automodapi.__name__ in extensions:
            ...
            filestr = automodapi.automodapi_replace(
                fr.read(), app, True, docname, False
            )
        else:
            filestr = fr.read()
    ...
    for i, (i1, i2, modnm, ops, rem) in enumerate(
        ...
        if [funcsonly, clssonly, varsonly].count(True) > 1:
            msg = (
                "Defined more than one of functions-only, classes-only, "
    ...
    for name, path, template_name, inherited_mem in sorted(items):
        ...
        try:
            ...
            if doc.objtype == "module":
                ...
                ns["functions"], ns["all_functions"] = get_members_mod(obj, "function")
                ns["classes"], ns["all_classes"] = get_members_mod(obj, "class")
                ns["exceptions"], ns["all_exceptions"] = get_members_mod(
                    obj, "exception"
                )
                ...
                ns["methods"], ns["all_methods"] = get_members_class(
                    obj, "method", api_class_methods, include_base=include_base
                )
                ns["attributes"], ns["all_attributes"] = get_members_class(
                    obj, "attribute", include_base=include_base
                )
                ns["methods"].sort()
                ns["attributes"].sort()
            ...
            if not os.path.isdir(
                os.path.join(base_path, mod_name_dir)
            ) and os.path.isdir(
                os.path.join(base_path, mod_name_dir.rsplit("/", 1)[0])
            ):
