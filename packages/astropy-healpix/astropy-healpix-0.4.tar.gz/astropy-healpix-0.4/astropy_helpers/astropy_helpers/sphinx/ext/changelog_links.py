    ...
    for item in doctree.traverse():
        ...
        for block in BLOCK_PATTERN.finditer(item):
            ...
            for m in ISSUE_PATTERN.finditer(block):
                ...
                children.append(
                    reference(text=issue_number, name=issue_number, refuri=refuri)
                )
                prev_end = end
...
def setup_patterns_rexes(app):
    app.changelog_links_rexes = [
        re.compile(pat) for pat in app.config.changelog_links_docpattern
    ]
...
def setup(app):
    ...
    app.add_config_value(
        "changelog_links_docpattern", [".*changelog.*", "whatsnew/.*"], True
    )
