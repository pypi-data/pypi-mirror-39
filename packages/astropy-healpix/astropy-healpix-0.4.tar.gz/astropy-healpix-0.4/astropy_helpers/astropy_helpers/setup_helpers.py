...
def get_debug_option(packagename):
    ...
    try:
        current_debug = get_pkg_version_module(packagename, fromlist=["debug"])[0]
...
def run_command_hooks(cmd_obj, hook_kind):
    ...
    *cmd_obj* is a finalized command object; *hook_kind* is either
    ...
    for modname, hook in hooks:
        ...
        log.info(
            "running {0} from {1} for {2} command".format(
...
def generate_test_command(package_name):
    ...
    return type(
        package_name.title() + "Test", (AstropyTest,), {"package_name": package_name}
    )
...
def get_package_info(srcdir=".", exclude=()):
    ...
    ext_modules.extend(get_cython_extensions(srcdir, packages, ext_modules, ["numpy"]))
...
def get_cython_extensions(
    srcdir, packages, prevextensions=tuple(), extincludedirs=None
):
    """
    ...
    for package_name in packages:
        ...
        for extmod, pyxfn in iter_pyx_files(package_path, package_name):
            ...
            if sourcepath not in prevsourcepaths:
                ext_modules.append(
                    Extension(extmod, [pyxfn], include_dirs=extincludedirs)
                )
...
def use_system_library(library):
    ...
    """
    return get_distutils_build_or_install_option(
        "use_system_{0}".format(library)
...
class FakeBuildSphinx(Command):
    ...
    def initialize_options(self):
        try:
            raise RuntimeError(
                "Sphinx and its dependencies must be installed " "for build_docs."
            )
        except:
            log.error(
                "error: Sphinx and its dependencies must be installed "
                "for build_docs."
            )
            sys.exit(1)
