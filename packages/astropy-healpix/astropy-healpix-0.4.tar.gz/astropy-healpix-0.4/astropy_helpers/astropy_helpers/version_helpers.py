...
def _get_version_py_str(packagename, version, githash, release, debug, uses_git=True):
    ...
    if not header:  # If _generate_git_header fails it returns an empty string
        header = _FROZEN_VERSION_PY_STATIC_HEADER.format(
            verstr=version, githash=githash
        )

    return _FROZEN_VERSION_PY_TEMPLATE.format(
...
def generate_version_py(
    packagename, version, release=None, debug=None, uses_git=True, srcdir="."
):
    ...
    if (
        last_generated_version != version
