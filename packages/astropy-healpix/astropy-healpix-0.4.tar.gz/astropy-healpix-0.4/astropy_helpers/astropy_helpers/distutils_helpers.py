...
def get_dummy_distribution():
    ...
    # the option is set.
    dist = Distribution(
        {"script_name": os.path.basename(sys.argv[0]), "script_args": sys.argv[1:]}
    )
...
def get_distutils_build_or_install_option(option):
    ...
    """
    return get_distutils_option(option, ["build", "build_ext", "build_clib", "install"])
...
def add_command_option(command, name, doc, is_bool=False):
    ...
    if (
        hasattr(cmdcls, "_astropy_helpers_options")
...
def get_distutils_display_options():
    ...
    display_commands = set(
        ["clean", "register", "setopt", "saveopts", "egg_info", "alias"]
    )
