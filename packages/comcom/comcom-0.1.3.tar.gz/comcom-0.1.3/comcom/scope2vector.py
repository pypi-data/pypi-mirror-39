...
def scope2list(scope_str_original):
    #关键词列表要单独提取出来做分析
    stop_list   = ml3.tools.load_json_file("stop_list.json")
    keywords    = ml3.tools.load_json_file("keywords.json")
    ignores     = ml3.tools.load_json_file("ignores.json")
    gbbs_dict   = ml3.tools.load_json_file("gbt4547.json")
...
def scope2vector(scope_str_original):
    ...
    gbt4547_tmpl = ml3.tools.load_json_file("gbt4547_tmpl.json")
