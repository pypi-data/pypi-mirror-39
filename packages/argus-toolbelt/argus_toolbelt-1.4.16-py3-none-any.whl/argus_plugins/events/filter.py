...
from argus_cli.utils import time, output
...
OUTPUT_METHODS = {
    "csv": lambda headers, data: output.csv(
        headers,
        data,
        lambda row, headers: [str(getattr(row, field)) for field in headers]
    ),
...
def get_updated(start: time.date_time_to_timestamp, end: time.date_time_to_timestamp,
