...
from argus_cli.utils.time import timestamp_to_date, timestamp_to_period, time_diff, date_time_to_timestamp
from argus_cli.utils.output import csv, formatted_string
...
def escalate(before: date_time_to_timestamp, customer: get_customer_id, dry: bool = False,
...
def print_statistics(statistics: dict) -> None:
    ...
    for case_id, data in statistics.items():
        ...
        body = "#%s," % case_id
        body += csv(headers[1:], [output], show_headers=False).strip()
        print(body)


@register_command(extending=SLA_PLUGIN, alias="incident")
def case_times(start: date_time_to_timestamp, end: date_time_to_timestamp, customer: get_customer_id,
               case_type: utils.CASE_TYPES = None, status: utils.STATUSES = None,
...
def print_incident_statistics(cases: dict, format: str = "csv", format_string: str = None) -> None:
    ...
    if format == "csv":
        print(FORMATS["csv"](headers, [data for data in cases.values()]))
    elif format == "string":
        print(FORMATS["string"](headers, [data for data in cases.values()], format_string))


@register_command(extending=SLA_PLUGIN, alias="soc")
def time_to_case(
        start: date_time_to_timestamp,
