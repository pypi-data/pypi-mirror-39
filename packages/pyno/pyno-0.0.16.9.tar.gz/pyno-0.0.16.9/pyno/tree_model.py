...
import inspect
from pyno.flask_integration import FlaskInterface
...
class ClassyMaker:
    ...
    def __call__(self, *args, **kwargs):
        return self.node(*args, **{k: v for k, v in kwargs.items() if k.lower() != 'class'},
                         Class=' '.join(x.replace('_', '-') for x in [*self.Class, kwargs.get('class'), kwargs.get('Class', '')] if x is not None))


class TreeNode(FlaskInterface):
    ...
    def construct(self, *args, **kwargs):
        ...
        def value_exp(value):
            if value is None:
                ...
                return f'="{value}"'
        properties = (' '+' '.join([f'{name.replace("_", "-")}{value_exp(value)}' for name, value in property_args.items()])) \
