...
import numpy as np
import pandas as pd
from . import components as compos
...
class CDFMRegressor():
    ...
    def _model_equation(
        ...
        y_hat = self.b \
            + np.dot(self.w, x) \
            + compos.compute_Iec(tar_index, com_indices, self.Ve, self.Vc) \
            + compos.compute_Ief(tar_index, x, self.Ve, self.Vf) \
            + compos.compute_Iff(x, self.Vf)
        return y_hat
    ...
    def fit(self,
        ...
        for epoch_idx in range(self.n_iterations):
            for group_id in group_ids:
                ...
                # Prediction phase
                for i, (tar, com) in enumerate(zip(entity_indices, com_indices)):
                    x: np.ndarray = group_data.iloc[i][FEATURES]
                    pred_labels[i] = self._model_equation(tar, com, x)
                # Param update phase
                pred_errors = obs_labels - pred_labels
                for i, (err, tar, com) in enumerate(zip(pred_errors, entity_indices, com_indices)):
                    x: np.ndarray = group_data.iloc[i][FEATURES]
                    stride = err / n_entities
                    coef_w = x - np.multiply(self.l2_w, w_cur)
                    coef_ve = diff.diff_Iec_with_ve(com, Vc_cur) \
                        + diff.diff_Ief_with_ve(x, Vf_cur) \
                        - np.multiply(self.l2_V, Ve_cur[tar])
                    coef_vc = diff.diff_Iec_with_vc(tar, Ve_cur) \
                        - np.multiply(self.l2_V, Vc_cur[com])
                    coef_vf = diff.diff_Ief_with_vf(tar, x, Ve_cur) \
                        + diff.diff_Iff_with_vf(x, Vf_cur) \
