...
def imresize(tensor_or_size,size_or_tensor):
    ...
    if isinstance(siz,(int,float)):
        return torch.nn.functional.upsample(x,scale_factor=siz,mod='bilinear',align_corners=False)
    else:
        h,w = siz
        return torch.nn.functional.upsample(x,size=(h,w),mode='bilinear',align_corners=False)
...
def imcrop_center(tensor_or_size,size_or_tensor):
    ...
    *_,h,w = x.shape
    ...
    return torch.nn.functional.upsample(patch,size=siz,mode='bilinear',align_corners=False)
