...
from Profils.profilbase2 import ProfilBasePlus
...
class Oboe000(OrigineStr):
    ...
    def evtF(self, duree, ProfilVarFreq):
        """
        ...
        w = ondeTable(self.table, mf) #Tables/ondepartable.py
        return w / extremum(w)

    def evtH(self, duree, profilprony):
        """
        retourne une onde dont la hauteur suit profilprony
        et de durée 'duree'.
        'profilprony' : profil-liste en Prony
        """
        mf = ProfilBase(profilprony).temps(duree)
        mf = prony2freq(mf)
        w = ondeTable(self.table, mf) #Tables/ondepartable.py
        return w / extremum(w)
...
#        return cg.a, cd.a
    ...
    def evtMono(self, lfreqs, duree, dt, pEnv):
        """
        guirlande de sons haut-sylvestres ;
        Env est renouvelé à chaque appel ;
        'LFreqs' : liste des fréquences
        'duree' : duree d'un ton de guirlande
        'dt' : temps de transition entre deux tons
        'Env' : enveloppe (profil-liste) de l'ensemble
        duree, dt = 1./12, 1./96
        pEnv = [[recta, (dt), 0.0, 1.0],
                [recta, 5.0, 1.0, 1.0],
                [recta, 4.0, 1.0, 0.0]]
        """
        self.e = ProfilBase(pEnv)
        #mf = self.g.traitG(duree, dt, LFreqs)
        mf = self.g.evt(duree, dt, lfreqs)
        w = ondeTable(self.table, mf)
        env = self.e.samples(w.size)
