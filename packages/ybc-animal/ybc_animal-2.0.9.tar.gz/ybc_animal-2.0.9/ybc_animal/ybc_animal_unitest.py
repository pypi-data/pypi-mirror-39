...
class MyTestCase(unittest.TestCase):
    ...
    def test_sound_ParameterTypeError(self):
        with self.assertRaisesRegex(ParameterTypeError, "^参数类型错误 : 调用sound方法时，'animal'参数类型错误。$"):
            sound(1)

    def test_sound_ParameterValueError(self):
        with self.assertRaisesRegex(ParameterValueError, "^参数数值错误 : 调用sound方法时，'animal'参数不在允许范围内。$"):
            sound('')
