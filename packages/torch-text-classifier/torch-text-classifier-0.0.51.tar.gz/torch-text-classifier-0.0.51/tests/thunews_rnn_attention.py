...
def main():
    print('read data')
    x_data, y_data = json_reader('/tmp/thunews.json', max_len=200)
    x_data, y_data = shuffle(x_data, y_data, random_state=222)
    # x_data, y_data = x_data[:10000], y_data[:10000]
    print('split data')
    x_train, x_val, y_train, y_val = train_test_split(
        x_data, y_data, test_size=0.2, random_state=222)
