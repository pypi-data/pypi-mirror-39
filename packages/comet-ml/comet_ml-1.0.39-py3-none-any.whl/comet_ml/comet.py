...
class Streamer(threading.Thread):
    ...
    def _process_upload_message(self, message):
        ...
        upload_thread = upload_file_thread(
            ...
            experiment_id=message.experimentKey,
    ...
    def _process_ws_message(self, message):
        try:
            data = format_messages_for_ws([message])
    ...
    def _wait_for_empty(self, timeout, verbose=False, sleep_time=1):
        ...
        while self._is_finished() is False and time.time() < end_time:
            if verbose is True:
...
class OfflineStreamer(threading.Thread):
    ...
    def _process_upload_message(self, message):
        ...
        data = {"type": "file_upload", "payload": message.repr_json()}
        self.file.write(compact_json(data) + "\n")
        self.file.flush()

    def _process_ws_message(self, message):
        data = {"type": "ws_msg", "payload": message.repr_json()}
        self.file.write(compact_json(data) + "\n")
        self.file.flush()
