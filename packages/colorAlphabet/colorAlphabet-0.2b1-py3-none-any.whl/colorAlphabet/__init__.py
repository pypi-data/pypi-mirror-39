...
BW_COLOR_ALPHABET = COLOR_ALPHABET.copy()
BW_COLOR_ALPHABET['White'] = (255, 255, 255)
BW_COLOR_ALPHABET['Black'] = (  0,   0,   0)
BW_COLOR_ALPHABET.move_to_end('Black', last=False)

NORM_COLOR_ALPHABET = OrderedDict()
for c in COLOR_ALPHABET:
    NORM_COLOR_ALPHABET[c] = tuple([i / 255 for i in COLOR_ALPHABET[c]])

BW_NORM_COLOR_ALPHABET = OrderedDict()
for c in BW_COLOR_ALPHABET:
    BW_NORM_COLOR_ALPHABET[c] = tuple([i / 255 for i in BW_COLOR_ALPHABET[c]])
#------------------------------------------------------------------------------
    ...
    """ Return XTerm string to set the background color to `colorname`
        ...
        Args:
            colorname (str): color name from COLOR_ALPHABET
                or BW_COLOR_ALPHABET
            full (boolean): if True use BW_COLOR_ALPHABET
    """
    try:
        if full:
            r, g, b = BW_COLOR_ALPHABET[colorname]
        else:
...
if __name__ == '__main__':
    for i, color in enumerate(BW_COLOR_ALPHABET):
