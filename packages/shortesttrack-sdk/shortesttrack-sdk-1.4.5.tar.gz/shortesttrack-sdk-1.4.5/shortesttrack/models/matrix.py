...
class Matrix(Model):
    ...
    @property
    def name(self) -> str:
        return self.metadata.get('id') or self.metadata.get('name') or 'unknown'
    ...
    def _set_up_content(self, data: dict) -> None:
        """
        Also set up name in Matrix content
        """
        self._content = data
        if not self._content.get('name'):
            self._content['name'] = self.name

    def read(self) -> 'Matrix':
        self._set_up_content(self.get_matrix(self.id))
        return self
