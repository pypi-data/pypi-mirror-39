...
"""
mpi framework for python mapping of functions
...
class Mpimap(object):
	...
	def stop(self):
		"""
		Stop all worker processes from listening for jobs
		and continue interpreting the main script.
		"""
		if self.type != 'header':
			return
		if self.usempi is False:
	...
	def exit(self):
		...
		for i in range(1, self.size):
			self.comm.send('EXIT', dest=i, tag=i)
