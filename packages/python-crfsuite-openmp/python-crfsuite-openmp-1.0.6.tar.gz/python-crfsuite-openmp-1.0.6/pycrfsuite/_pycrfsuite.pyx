...
cdef crfsuite_api.Item to_item_dense(patterns, x, crfsuite_api.Item* prev_item_1, prev_x_1,
                                     crfsuite_api.Item* prev_item_2, prev_x_2) except+:
    """ Convert a Python object to an Item. """
    cdef crfsuite_api.Item c_item
    cdef double c_value
    cdef string c_key
    ...
    c_item = crfsuite_api.Item()
    c_item.reserve(len(patterns))  # at least this amount is required

    for column, positions in patterns:

        str_key = []
        invalid_sequence = False

        str_key_prev_1 = []
        invalid_sequence_prev_1 = False
        should_do_sequence_prev_1 = False

        str_key_prev_2 = []
        invalid_sequence_prev_2 = False
        should_do_sequence_prev_2 = False


        for pos in positions:
            if pos == 0:
                str_key.append(x[column])

                if prev_x_1 is not None:
                    str_key_prev_1.append(prev_x_1[column])
                if not invalid_sequence_prev_2 and (prev_x_2 is not None):
                    str_key_prev_2.append(prev_x_2[column])

            elif pos == -1:
                if prev_x_1 is not None:
                    str_key.append(prev_x_1[column])

                if not invalid_sequence_prev_1 and (prev_x_2 is not None):
                    str_key_prev_1.append(prev_x_2[column])
                invalid_sequence_prev_2 = True

            elif pos == -2:
                if prev_x_2 is not None:
                    str_key.append(prev_x_2[column])
                invalid_sequence_prev_1 = True
                invalid_sequence_prev_2 = True
            elif pos == 1:

                str_key_prev_1.append(x[column])

                if not invalid_sequence_prev_2 and (prev_x_1 is not None):
                    str_key_prev_2.append(prev_x_1[column])

                should_do_sequence_prev_1 = True

                # We defer adding this element for now
                invalid_sequence = True

            elif pos == 2:

                str_key_prev_2.append(x[column])

                # We defer adding this element for now
                invalid_sequence = True
                should_do_sequence_prev_2 = True
                invalid_sequence_prev_1 = True

        if not invalid_sequence:
            c_item.push_back(crfsuite_api.Attribute((str(positions) + ("".join(str_key))).encode('utf8'), 1.0))

        if (prev_x_1 is not None) and should_do_sequence_prev_1 and not invalid_sequence_prev_1:
            prev_item_1.push_back(crfsuite_api.Attribute((str(positions) + ("".join(str_key_prev_1))).encode('utf8'), 1.0))

        if (prev_x_2 is not None) and should_do_sequence_prev_2 and not invalid_sequence_prev_2:
            prev_item_2.push_back(crfsuite_api.Attribute((<unicode>(str(positions) + ("".join(str_key_prev_2)))).encode('utf8'), 1.0))
...
cdef crfsuite_api.ItemSequence to_seq_pattern(pyseq, pattern) except+:
    """
    Convert an iterable to an ItemSequence.
    Elements of an iterable could be:
    ...
    * {"string_prefix": [...]} dicts: nested list is processed and
    ...
    cdef crfsuite_api.Item* prev_item_1 = NULL
    cdef crfsuite_api.Item* prev_item_2 = NULL
    ...
    for x in pyseq:
        ...
        if seq_size > 0:
            prev_item_1 = &c_seq.at(seq_size - 1)

        if seq_size > 1:
            prev_item_2 = &c_seq.at(seq_size - 2)

        c_seq.push_back(to_item_dense(pattern, x, prev_item_1, prev_1, prev_item_2, prev_2))
        seq_size+= 1
        prev_2 = prev_1
        prev_1 = x
...
cdef class BaseTrainer(object):
    ...
    def append_pattern(self, xseq, yseq, pattern, int group=0):
        ...
        group : int, optional
            ...
            select subset of data for heldout evaluation.
        """
        self.c_trainer.append(to_seq_pattern(xseq, pattern), yseq, group)
...
cdef class Tagger(object):
    ...
    cpdef set_pattern(self, xseq, pattern) except +:
        ...
        """
        self.c_tagger.set(to_seq_pattern(xseq, pattern))
