...
cdef crfsuite_api.Item to_item_dense(patterns, x, crfsuite_api.Item* prev_item_1, prev_x_1,
    ...
    c_item = crfsuite_api.Item()
    c_item.reserve(len(x)*2)  # at least this amount is required

    for column, positions in patterns:

        invalid_sequence = False
        str_key = ""
        have_1_next_pos = False
        have_2_next_pos = False

        for pos in positions:
            if pos == 0:
                str_key += str(pos)+ "_" + x[column]
            elif pos == -1 and prev_x_1 is not None:
                str_key += str(pos)+ "_" + prev_x_1[column]
            elif pos == -2 and prev_x_2 is not None:
                str_key += str(pos)+ "_" + prev_x_2[column]
            else:
                if pos == 1:
                    have_1_next_pos = True
                else:
                    have_2_next_pos = True
                invalid_sequence = True

        if not invalid_sequence:
            c_value = 1.0

            c_item.push_back(crfsuite_api.Attribute((<unicode>str_key).encode('utf8'), c_value))

        # Vamos ver o passado se tiver
        if (prev_x_1 is not None) and have_1_next_pos:
            str_key = ""
            ...
            for pos in positions:
                if pos == 0:
                    str_key += str(pos) + "_" + prev_x_1[column]
                elif pos == -1 and prev_x_2 is not None:
                    str_key += str(pos) + "_" + prev_x_2[column]
                elif pos == 1:
                    str_key += str(pos) + "_" + x[column]
                else:
            ...
            if not invalid_sequence:
                c_value = 1.0
                prev_item_1.push_back(crfsuite_api.Attribute((<unicode>str_key).encode('utf8'), c_value))

        if (prev_x_2 is not None) and have_2_next_pos:
            str_key = ""
            ...
            if not invalid_sequence:
                c_value = 1.0
                prev_item_2.push_back(crfsuite_api.Attribute((<unicode>str_key).encode('utf8'), c_value))
