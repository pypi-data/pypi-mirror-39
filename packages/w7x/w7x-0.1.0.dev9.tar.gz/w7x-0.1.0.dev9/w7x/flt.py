...
class MagneticConfig(Base):
    ...
    def coil_currents(self, unit='rw'):
        ...
        scs = self.coilsIdsCurrents[70:72]
        npcs = np.array(npcs)
        plcs = np.array(plcs)
        scs = np.array(scs)
        I_n = npcs[0]
    ...
    def poincare_in_phi_plane(self, phi_list_rad,
        ...
        for i_seed in range(len(seeds)):
            ...
            res = w7x.run_service(fieldLineServer.service.trace,
                                  seed_point.as_input(),
                                  self.as_input(),
                                  task,
                                  errors=errors)
    ...
    def magnetic_characteristics(self, points3D, taskStepSize=0.2, returnType=list):
        ...
        result = w7x.run_service(fieldLineServer.service.trace,
                                 points3D.as_input(),
                                 self.as_input(),
    ...
    def find_axis_at_phi(self, phi=0, size=w7x.Defaults.Poincare.stepSize, settings=None):
        ...
        result = w7x.run_service(fieldLineServer.service.findAxisAtPhi,
                                phi,
                                size,
                                self.as_input(),
                                settings)
...
class Run(object):
    ...
    def find_lcfs(self, step=0.001, settings=None, max_time=None):
        ...
        lCFSPoint = w7x.run_service(fieldLineServer.service.findLCFS,
                                   step,
    ...
    def line_diffusion(self,
        ...
        Examples:
            ...
            >>> initial_points = w7x.flt.Points3D([[6.0,0.0001,0]], coord_sys='cylinder')
            >>> conn_len, comp_load, start_points = run.line_diffusion(initial_points)
        ...
        for iShift, additionalShift in enumerate(additionalShifts):
            ...
            resForward = w7x.run_service(fieldLineServer.service.trace,
        ...
        # max_time could be something like int(round(line.numSteps / 4000. * 2000)))
        resInvers = w7x.run_service(fieldLineServer.service.trace,
                                   start_points.as_input(),
                                   config,
                                   fldTask,
                                   machine,
...
if __name__ == "__main__":
    import doctest
    doctest.run_docstring_examples(MagneticConfig.geiger_string, globals())
    quit()
    doctest.testmod()
