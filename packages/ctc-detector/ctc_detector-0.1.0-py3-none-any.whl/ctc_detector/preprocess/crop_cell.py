...
def apply_heuristic_filter(df, rule):
    if rule == 'MN':
        # missing_nuclei rule: check mask id start with '_n_' or 'xxxx-xxxx-' 
        return df.filter(regex='^_n_|^[0-9a-z]+-[0-9a-z]+-', axis=0)
    elif rule == 'FP':
        # false positive rule: high confidence level (>50%) but not labeled
        if 'TumorProb' not in df.columns:
            return
        if 'Label' not in df.columns:
            df['Label'] = np.NaN
        return df.loc[ (df['TumorProb'] > 0.5) & (df['Label'].isna()) ]
    elif rule == 'FN':
        # false negative rule: low confidence level (<= 50%) but was labeled
        if 'TumorProb' not in df.columns:
            return
        if 'Label' not in df.columns:
            return
        return df.loc[ (df['TumorProb'] <= 0.5) & (df['Label']) ]
    elif rule == 'CT':
...
def run(root, args):
    ...
    for mask_csv in tqdm(glob.iglob(os.path.join(src, '**', 'mask.csv'), recursive=True)):
        ...
        df_cad = apply_heuristic_filter(df, args.rule)
...
def main(args=None):
    ...
    parser.set_defaults(src='test', crop=300, all=True, rle=True, rule='FP')
