...
class Pad():
    ...
    def pad(self, t, border):
        ...
        if self.mode == 'constant':
            return np.pad(t, b[:t.ndim], self.mode, constant_values=self.fill)
        else:
            return np.pad(t, b[:t.ndim], self.mode)
...
class RandomUniformCrop(RandomCrop):
    ...
    def get_params(self, x, y, output_size):
        ...
        num_grids_h, num_grids_w = math.ceil(h / th), math.ceil(w / tw)
        gh, gw = math.ceil(h / num_grids_h), math.ceil(w / num_grids_w)
        row, col = random.randint(0, num_grids_h-1), random.randint(0, num_grids_w-1)
        # randomly move around slightly
        i = round((row + random.triangular(-0.3, 0.3, 0)) * gh)
        j = round((col + random.triangular(-0.3, 0.3, 0)) * gw)
...
class RandomScale():
    ...
    """
    def __init__(self, scale=(0.2, 2.0), mode='tri'):
...
class RandomGaussianBlur():
    def __init__(self, p=0.75, mode='reflect'):
        self.mode = mode
        self.p = p
    ...
    def gaussian(self, t):
        if t is None:
            return t
        t = filters.gaussian(t, mode=self.mode, multichannel=True)
        return t
...
class ElasticDistortion():
    ...
    def distort(self, t, indices, spline_order=1, mode='nearest'):
        if t is None:
            return t
        if t.ndim == 2:
            t = np.expand_dims(t, -1)
