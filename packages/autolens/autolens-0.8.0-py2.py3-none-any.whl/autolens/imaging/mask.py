...
class ImagingGrids(object):
    ...
    def __init__(self, image, sub, blurring, pix=None):
        ...
        The regular - the (y,x) coordinate at the center of every padded pixel.
...
class ImageGrid(np.ndarray):
    ...
    An *RegularGrid* is a ndarray of shape [image_pixels, 2], where image_pixels is the total number of padded \
    ...
    Below is a visual illustration of an regular, where a total of 10 pixels are padded and are included in the \
    grid.
    ...
    def unlensed_grid(self):
        return ImageGrid(arr=grid_util.regular_grid_1d_masked_from_mask_pixel_scales_and_origin(mask=self.mask,
                                                                                                pixel_scales=self.mask.pixel_scales),
                         mask=self.mask)

    @property
    def unlensed_unmasked_grid(self):
        return ImageGrid(arr=grid_util.regular_grid_1d_from_shape_pixel_scales_and_origin(shape=self.mask.shape,
                                                                                          pixel_scales=self.mask.pixel_scales),
                         mask=self.mask)
    ...
    @property
    def yticks(self):
        """Compute the yticks labels of this grid, used for plotting the y-axis ticks when visualizing an regular"""
        return np.linspace(np.min(self[:, 0]), np.max(self[:, 0]), 4)

    @property
    def xticks(self):
        """Compute the xticks labels of this grid, used for plotting the x-axis ticks when visualizing an regular"""
...
class ImageGridBorder(np.ndarray):
    ...
    def __new__(cls, arr, *args, **kwargs):
        """The border of an regular, containing the pixel-index's of all padded pixels that are on the \
