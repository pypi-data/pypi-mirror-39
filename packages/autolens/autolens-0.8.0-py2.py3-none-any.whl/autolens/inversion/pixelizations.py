...
from autolens.model.inversion import pixelization_util, mappers


class ImagePlanePixelizationGrid(scaled_array.RectangularArrayGeometry):

    def __init__(self, pix_grid_shape, pixel_scales, image_grid):
        """Abstract class which handles the uniform regular-grid whose pixel centers are used to form an adaptive grid's \
        pixelization pixel-centers.

        This is performed by over-laying a mask over the regular-grid, such that only pixels within masked pixels are \
