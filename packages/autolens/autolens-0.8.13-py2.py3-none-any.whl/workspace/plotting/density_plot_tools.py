...
class SLACS(object):
  ...
  #      tracer = ray_tracing.TracerImageSourcePlanes(lens_galaxies=[self.lens_galaxy], source_galaxies=[self.source_galaxy],
  #                                                   image_plane_grids=[data_grids], cosmology=cosmo.Planck15)

        # REMEMBER TO REMOVE DARK MATTER HALO WHEN COMPUTING STELLAR MASS TO LIGHT RATIOS

      #  print(self.lens_galaxy.luminosity_within_circle(radius=1.04))
      #  print(self.lens_galaxy.mass_within_circle(radius=1.04, conversion_factor=self.critical_density_arcsec))
      #  stop
    ...
    def masses_of_all_samples(self, radius_kpc):
        ...
        for i in range(len(self.pdf.samples)):
            ...
            if self.pdf.weights[i] > 1e-6:
                ...
                bulge_mass =  self.critical_density_arcsec * self.lens_galaxy.bulge.mass_within_circle(radius=radius_arc)
                disk_mass = self.critical_density_arcsec * self.lens_galaxy.disk.mass_within_circle(radius=radius_arc)
                halo_mass = self.critical_density_arcsec * self.lens_galaxy.halo.mass_within_circle(radius=radius_arc)

                total_mass = bulge_mass + disk_mass + halo_mass
                stellar_mass = (bulge_mass + halo_mass)
                dark_mass = halo_mass
        ...
        for r in range(number_bins):
            ...
            for i in range(len(self.pdf.samples)):
                ...
                if self.pdf.weights[i] > weight_cut:
                    ...
                    bulge_density = self.critical_density_arcsec * (self.lens_galaxy.bulge.mass_within_circle(radii[r+1]) -
                                     self.lens_galaxy.bulge.mass_within_circle(radii[r])) / annuli_area

                    disk_density = self.critical_density_arcsec * (self.lens_galaxy.disk.mass_within_circle(radii[r+1]) -
                                     self.lens_galaxy.disk.mass_within_circle(radii[r])) / annuli_area

                    halo_density = self.critical_density_arcsec * (self.lens_galaxy.halo.mass_within_circle(radii[r+1]) -
                                     self.lens_galaxy.halo.mass_within_circle(radii[r])) / annuli_area
    ...
    def plot_density(self, image_name='', labels=None, xaxis_is_physical=True, yaxis_is_physical=True):
        ...
        plt.title('Decomposed surface density profile of ' + image_name, size=30)

        if xaxis_is_physical:
            plt.xlabel('Distance From Galaxy Center (kpc)', size=30)
        else:
            plt.xlabel('Distance From Galaxy Center (")', size=30)

        if yaxis_is_physical:
            plt.ylabel(r'Surface Mass Density $\Sigma$ ($M_{\odot}$ / kpc $^2$)', size=30)
        else:
            pass
        ...
        plt.savefig(path + 'density/slacs1250.png', bbox_inches='tight')
        plt.show()