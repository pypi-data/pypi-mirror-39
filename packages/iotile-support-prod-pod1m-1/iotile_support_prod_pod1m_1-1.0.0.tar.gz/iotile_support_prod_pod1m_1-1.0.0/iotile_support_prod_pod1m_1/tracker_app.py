...
class LogisticsTracker(IOTileApp):
    ...
    def trip_info(self):
        ...
        if hwversion == 'btc1_v3':   #FIXME: Use HW TAG when available
            uptime = current_time
        else:
    ...
    def start_trip(self):
        ...
        #FIXME: there must be a simpler way to check. 
        if os['tag'] == 2050 and          \
           os['major_version'] >= 1 and   \
           os['minor_version'] >= 1:
    ...
    def synchronize_clock(self, force_timestamp=None):
        ...
        os = self._hw.controller().sensor_graph().os_info() #FIXME: there must be a simpler way to check. 
        if os['tag'] != 2050 or          \
           not (os['major_version'] >= 1 and os['minor_version'] >= 1):
            raise HardwareError("Firmware does not support this function.", os_tag=os['tag_string'], requires="2050 (^1.1)")
    ...
    def _build_utc_assigner(self, reports):
        ...
        assigner.anchor_stream(START_STREAM)
        assigner.anchor_stream(END_STREAM)
