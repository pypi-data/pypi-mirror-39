...
def main():
	...
	if not glob.glob(os.path.join(os.path.abspath(resultDir),'*.tab')):
		log("No hits found in %s . Quitting." % resultDir)
		# Remove temp directory
		if not args.keeptemp:
			shutil.rmtree(tempDir)
		sys.exit(1)
	...
	if args.nopairing:
		...
		log("Pairing is off. Reporting hits only.")
		# Remove temp directory
		if not args.keeptemp:
			shutil.rmtree(tempDir)
		sys.exit(1)
	...
	if args.reportTIR in ['all','paired']:
		tirmite.writePairedTIRs(outDir=outDir, paired=paired, hitIndex=hitIndex, genome=genome, prefix=args.prefix)
	...
	if args.gffOut:
		...
		if args.reportTIR in ['all','unpaired']:
			...
			unpairedTIRs = None
	# Write gff3
		tirmite.gffWrite(outpath=os.path.join(outDir,args.gffOut), featureList=pairedEles, writeTIRs=args.reportTIR, unpaired=unpairedTIRs,prefix=args.prefix)
