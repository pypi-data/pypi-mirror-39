...
from .create_data import create_computationaltool, create_computationaltoolcwlimport
from ..cwl_utils import insert_computationaltool_inputs
...
class ViewTestCase(TestCase):
    def setUp(self):
        ...
        comptool = create_computationaltool(
            name='STAR',
            version='2.17',
            description='Aligner',
            url='https://github.com/alexdobin/STAR',
            cwl=cwl_location)
        cwl_location = 'file://' + os.getcwd() + '/dcomputationaltool/tests/cwl/tools/star.yml'
    ...
    def test_computationaltool(self):
        ...
        self.assertEqual(len(comptool['imports']), 1)
        self.assertEqual(len(comptool['inputs']), 25)

    def test_computationaltool_imports(self):
        self.client.force_authenticate(self.user)
        response = self.client.get(
            reverse('dcomputationaltool:computationaltool-list'), format='json')
        self.assertEqual(len(response.json()), 1)
        comptool = response.json()[0]['imports'][0]
        self.assertEqual(comptool['name'], 'Docker YML')
