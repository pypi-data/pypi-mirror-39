...
def back_test_core(X):
    n,price,sus,ind,ind_weight,factor,factor_name = X
    ...
    return portfolio_value.unstack().rename(factor_name),pd.concat(weight_list,axis=1).rename_axis('dt',axis=1).stack().rename(factor_name)
...
def output_weight(Weight,weight_path,ind,ind_weight,sus,fac):
    ...
    for f in Weight.columns:
        ...
        Fac=fac[f].stack().rename(f).reindex(weight_df.index)
        ...
        pd.concat([Fac,Industry,Sus,weight_df],axis=1)\
                ...
                .to_csv(os.path.join(weight_path,f+'.csv'),index=False,float_format='%.3f')
    return


def run_back_test(data_box,n=5,back_end=None,n_jobs=-1,factor_pool=None,weight_path=None,**kwargs):
    '''
    ...
    if back_end is None:
        ...
        for f in data_box.Factor.columns.levels[0]:
            print('Process factor {} with signle processor'.format(f))
            X=(n,data_box.Price,data_box.Sus,data_box.Ind,data_box.Ind_weight,data_box.Factor.xs(f,axis=1),f)
            BT_value,BT_weight=back_test_core(X)
        ...
        if back_end is 'loky':
            mul_res=Parallel(n_jobs=n_jobs,**kwargs)(
                    delayed(back_test_core)( (n,data_box.Price,data_box.Sus,data_box.Ind,data_box.Ind_weight,data_box.Factor.xs(f,axis=1),f)
                                            ) for f in data_box.Factor.columns.levels[0])
        elif back_end is 'multiprocessing':
            X=[(n,data_box.Price,data_box.Sus,data_box.Ind,data_box.Ind_weight,data_box.Factor.xs(f,axis=1),f) for f in data_box.Factor.columns.levels[0]]
