...
class TestFrame(TestCase):
    ...
    def test_bins_bet_lines(self):
        ...
        binwidth = 0.01
        bidir = False
        bins = SignalValidator(
            dict_of_data=self.dict_of_data,
            line_freq=line_freq,
            binwidth=binwidth,
            bidir=bidir,
        )._SignalValidator__bins_bet_lines()
    ...
    def test_extrapolate_without_zero(self):
        sigval = SignalValidator(
            self.dict_of_data, line_freq=0.5, binwidth=1.0, delay_between_frames=0.0
        )
    ...
    def test_extrapolate_from_zero(self):
        sigval = SignalValidator(
            self.dict_of_data, line_freq=1.0, binwidth=1.0, delay_between_frames=0.0
        )
    ...
    def test_pipeline_single_frame_si(self):
        ...
        dict_of_data = dict(
            PMT1=pd.DataFrame(
                [1, 10, 20, 30, 100], columns=["abs_time"], dtype=np.uint64
            ),
            Lines=pd.DataFrame(lines, columns=["abs_time"], dtype=np.uint64),
        )
        binwidth = 1.0
        line_freq = 0.2
        delay_bet_frames = 0.0
        num_of_lines = 7
        sigval = SignalValidator(
            dict_of_data=dict_of_data,
            binwidth=binwidth,
            line_freq=line_freq,
            delay_between_frames=delay_bet_frames,
            num_of_lines=num_of_lines,
            image_soft=ImagingSoftware.SCANIMAGE.value,
        )
        sigval.run()
        self.assertSequenceEqual(
            sigval.dict_of_data["Lines"].abs_time.tolist(), lines[:-2]
        )
    ...
    def test_pipeline_few_frames_si(self):
        ...
        self.assertSequenceEqual(
            sigval.dict_of_data["Frames"].abs_time.tolist(), [0, 45, 90]
        )
        self.assertEqual(sigval.line_delta, 5)
    ...
    def test_pipeline_single_frame_mscan(self):
        ...
        sigval = SignalValidator(
            ...
            delay_between_frames=delay_bet_frames,
            num_of_lines=num_of_lines,
            image_soft=ImagingSoftware.MSCAN.value,
        )
        sigval.run()
        self.assertSequenceEqual(
            sigval.dict_of_data["Lines"].abs_time.tolist(), lines[:-2]
    ...
    def test_pipeline_few_frames_mscan(self):
        ...
        dict_of_data = dict(
            PMT1=pd.DataFrame(
                [1, 10, 20, 30, 100, 200], columns=["abs_time"], dtype=np.uint64
            ),
            Lines=pd.DataFrame(lines, columns=["abs_time"], dtype=np.uint64),
        )
        binwidth = 1.0
        ...
        sigval.run()
        self.assertSequenceEqual(
            sigval.dict_of_data["Lines"].abs_time.tolist(), lines[:-1]
        )
        self.assertSequenceEqual(
            sigval.dict_of_data["Frames"].abs_time.tolist(), [0, 15, 30, 45]
        )
        self.assertEqual(sigval.line_delta, 5)
