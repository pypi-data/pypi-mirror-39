...
def get_user_wxacode(request):
    ...
    if hasattr(user, "wxa_code"):
        return Response({
            "result": "success", 
            "data": UserWxaCodeSerializer(user.wxa_code, context={"request": request}).data
        })
    wxacode = get_wxa_code_for_user(user)
    return Response({"result": "success", "data": UserWxaCodeSerializer(wxacode, context={"request": request}).data})
