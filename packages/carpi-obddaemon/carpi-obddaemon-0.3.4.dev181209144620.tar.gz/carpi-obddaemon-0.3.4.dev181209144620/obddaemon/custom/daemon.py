...
from obddaemon.custom.Obd2DataParser import parse_value, check_value
        ...
        while retries > 0:
            try:
                ...
                with Serial(device,
                    ...
                    try:
                        while True:
                            state = ObdKeys.STATE_OK
                            d = dict()
                            for c in SerialObdDaemon.FETCH_SEQUENCE:
                                v = self.send_and_wait(ser, c)
                                res = check_value(val)

                                if res != ObdKeys.STATE_OK:
                                    log.warning("Detected error {} when checking data".format(ObdKeys.state_str(res)))
                                    state = res
                                    break

                                val = parse_value(c, v)
                                if val is not None:
                                    bus.publish(SerialObdDaemon.OBD_MAPPING[c],
                                                val[0] if type(val) is tuple else val)

                            if self._get_config_bool('Console', 'DoPprint', False):
                                pprint(d)

                            bus.publish(ObdKeys.KEY_STATE_INFO, state)

                            if res == ObdKeys.STATE_NO_DATA_ERROR:
                                log.info("Waiting 2 seconds and retrying ...")
                                sleep(1.5)
                            elif res == ObdKeys.STATE_SERIAL_OBD_ERROR:
                                log.warning("Connection issue, restarting connection ...")
                                raise SerialException("OBD Connection failed")
