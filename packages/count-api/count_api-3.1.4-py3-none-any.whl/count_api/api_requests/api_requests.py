...
class APIRequests:
    ...
    def tables_upload_file(self, path, name, overwrite_key=None, append_key=None, column_types=None, column_names=None):
        ...
        with requests.Session() as s:
            r = s.post(self.base_url+'tables.upload', files=files, data=data, headers=self.headers)
        file_handler.close()
    ...
    def tables_upload_data(self, data, name, overwrite_key = None, append_key=None, column_types=None, column_names=None):
        ...
        r = None
        with requests.Session() as s:
            r = s.post(self.base_url+'tables.upload', files=files, data=data_dict, headers=self.headers)
    ...
    def tables_delete(self, key): 
        ...
        r = None
        with requests.Session() as s:
            r = s.post(self.base_url+'tables.delete', json=payload, headers=self.headers)
        if r.status_code == 200:
            return
        else :
    ...
    def tables_getmetadata(self, key):
        ...
        r = None
        with requests.Session() as s:
            r = s.post(self.base_url+'tables.getMetadata', json=payload, headers=self.headers)
        if r.status_code == 200:
    ...
    def tables_getcolumns(self, key):
        ...
        r = None
        with requests.Session() as s:
            r = s.post(self.base_url+'tables.getColumns', json=payload, headers=self.headers)
        if r.status_code == 200:
    ...
    def tables_addnotebook(self, key, query):
        ...
        r = None
        with requests.Session() as s:
            r = s.post(self.base_url+'tables.addNotebook', json = payload, headers=self.headers)
        if r.status_code == 200:
    ...
    def tables_getnotebook_metadata(self, notebook_key):
        ...
        r = None
        with requests.Session() as s:
            r = s.post(self.base_url+'notebooks.getMetadata', json=payload, headers=self.headers)
        if r.status_code == 200:
    ...
    def tables_getdata(self, key, query, offset=None, limit=None):
        ...
        r = None
        with requests.Session() as s:
            r = s.post(self.base_url+'tables.getData', json=payload, headers=self.headers)
        if r.status_code == 200:
            return r.content
        else :
    ...
    def auth_getstate(self, visual_key, workspace_id):
        ...
        r = None
        with requests.Session() as s:
            r = s.post(self.base_url+'auth.getState', json=payload, headers=self.headers)
        if r.status_code == 200:
            return json.loads(r.content.decode('utf8'))
        else :
    ...
    def users_getprofile(self):
        ...
        r = None
        with requests.Session() as s:
            r = s.post(self.base_url+'users.getProfile', json=payload, headers=self.headers)
        if r.status_code == 200:
            return json.loads(r.content.decode('utf8'))
        else :
    ...
    def notebooks_downloadpreview(self, key, notebook_key, query, height, width, path, dtype):
        ...
        r = None
        with requests.Session() as s:
            r = s.post('%snotebooks.downloadPreview/%s?%s' % (self.base_url,key,'&'.join(query)), json=payload, headers=self.headers, stream=True)
        if r.status_code == 200:
            # stream into file
    ...
    def notebooks_download(self, key, path):
        ...
        r = None
        with requests.Session() as s:
            r = s.post('%snotebooks.download' % (self.base_url), json=payload, headers=self.headers, stream=True)
        if r.status_code == 200:
            # stream into file
            with open(path, 'wb') as f:
                r.raw.decode_content = True
