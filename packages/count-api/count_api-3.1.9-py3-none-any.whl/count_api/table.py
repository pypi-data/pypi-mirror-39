...
class Table:
    ...
    @try_catch
    def head(self, rows=10, columns=10):
        """`head(n=10)`:
            - Prints first n rows of table
            - `rows`: Integer number of rows requested. Max 100. Default 10.
            - `columns`: Integer number of columns requested. Max 20. Default 10.
        """
        return self._head(rows, columns)

    @try_catch
    def tail(self, rows=10, columns=10):
        """`tail(n=10)`:
            - Prints last n rows of table
            - `rows`: Integer number of rows requested. Max 100. Default 10.
            - `columns`: Integer number of columns requested. Max 20. Default 10.
        """
        end_at = self.size()[1]
        return self._head(rows, columns, end_at)

    def _head(self, rows, columns, end_at = None):
        limit = min(rows,100)
