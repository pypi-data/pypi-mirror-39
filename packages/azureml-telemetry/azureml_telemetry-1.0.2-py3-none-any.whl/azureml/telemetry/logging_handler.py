...
# ---------------------------------------------------------
"""logging_handler.py, A file for appinsights telemetry classes"""
...
class AppInsightsLoggingHandler(logging.Handler):
    ...
    def emit(self, record):
        ...
        try:
            ...
            if record.exc_info and \
                ...
                self._synchronous_client.track_trace(format_exc(), properties=properties)
                return
    ...
    def _create_synchronous_channel(self, context):
        """
        creates synchronous app insight channel
    ...
    def _create_asynchronous_channel(self, context):
        """
        creates async app insight channel
        :param context: App insight context
        :return: TelemetryChannel
        """
...
class _RetrySynchronousSender(SynchronousSender):
    """This class represents Synchronous sender with limited  retry. SenderBase does infinite retry,
        this class avoids that.
     """
...
class _RetryAsynchronousSender(AsynchronousSender):
    """This class represents Asynchronous sender with limited retry. SenderBase does infinite retry,
        this class avoids that.
     """
...
def _http_send(logger, data_to_send, service_endpoint_uri, send_timeout=10):
    """
    This function replicates appinsights SenderBase send method.
    :param logger: logger
