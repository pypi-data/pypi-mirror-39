...
def get_matchup(use_2018, user_mods):
    ...
    sc = scall.loc[(scall.date >= specs["start_date"][0]) & (scall.date < specs["end_date"][0])]
    del scall
    ...
    pitcher, batters = specs["pitcher"][0], specs["batter"]
...
def parse_inputs(use_2018, inputs):
    defaults = get_inputs()
    for param_name, data in defaults["matchup"].items():
        if inputs.get(param_name, None) is not None:
            if data["number_dims"] == 0 and isinstance(inputs[item], list):
                    inputs[item] = inputs[item][0]
