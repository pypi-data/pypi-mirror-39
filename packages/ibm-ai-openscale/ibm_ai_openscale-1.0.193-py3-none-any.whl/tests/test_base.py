...
class TestBase(unittest.TestCase):
    ...
    def publish_model(self, model):
        ...
        TestBase.published_model_details = published_model_details
    ...
    def list_deployments(self):
        TestBase.subscription.list_deployments()

    def get_payload_logging_details(self):
        payload_logging_details = TestBase.subscription.payload_logging.get_details()
    ...
    def get_performance_monitoring_details(self):
        performance_monitoring_details = TestBase.subscription.performance_monitoring.get_details()
    ...
    def stats_on_payload_logging_table(self):
        ...
        TestBase.subscription.payload_logging.print_table_schema()
        print("Show table:")
        TestBase.subscription.payload_logging.show_table()

        print("Describe table description:")
        table_description = TestBase.subscription.payload_logging.describe_table()
        print("Table description:\n{}".format(table_description))

        table_content = TestBase.subscription.payload_logging.get_table_content()
        print("Table content:\n{}".format(table_content))
        print(type(table_content))

        python_table_content = TestBase.subscription.payload_logging.get_table_content(format='python')
        print("Python Table content:\n{}".format(python_table_content))

        self.assertIsNotNone(python_table_content)

        if TestBase.scoring_result is not None and 'fields' in TestBase.scoring_result.keys():
            if 'prediction' in TestBase.scoring_result['fields']:
                self.assertIn('prediction', python_table_content['fields'])

            if 'probability' in TestBase.scoring_result['fields']:
    ...
    def stats_on_performance_monitoring_table(self):
        ...
        TestBase.subscription.performance_monitoring.print_table_schema()
        TestBase.subscription.performance_monitoring.show_table()
        TestBase.subscription.performance_monitoring.describe_table()
        TestBase.subscription.performance_monitoring.get_table_content()
        performance_metrics = TestBase.subscription.performance_monitoring.get_table_content(format='python')
    ...
    def get_metrics(self):
        deployment_metrics = TestBase.ai_client.data_mart.get_deployment_metrics()
        deployment_metrics_deployment_uid = TestBase.ai_client.data_mart.get_deployment_metrics(deployment_uid=TestBase.deployment_uid)
        deployment_metrics_subscription_uid = TestBase.ai_client.data_mart.get_deployment_metrics(subscription_uid=TestBase.subscription.uid)
        deployment_metrics_asset_uid = TestBase.ai_client.data_mart.get_deployment_metrics(asset_uid=TestBase.subscription.source_uid)
    ...
    def get_performance_metrics(self):
        self.get_metrics()
        performance_metrics_asset_uid = TestBase.ai_client.data_mart.get_deployment_metrics(asset_uid=TestBase.subscription.source_uid, metric_type='performance')
        performance_metrics_deployment_uid = TestBase.subscription.performance_monitoring.get_metrics(deployment_uid=TestBase.deployment_uid)
    ...
    def unsubscribe(self):
        print("Subscription: {}".format(TestBase.subscription))
        TestBase.ai_client.data_mart.subscriptions.delete(TestBase.subscription.uid)

    def unbind(self):
        TestBase.ai_client.data_mart.bindings.delete(TestBase.binding_uid)
...
class TestBaseDB2(TestBase):
    ...
    def stats_on_feedback_table_from_db2(self, model):
        feedback_data, fields = model.get_feedback_data()

        tables = list_db2_tables(self.database_credentials, self.schema_name)
        print(tables)
        feedback_table = None
        for table in tables:
        ...
        print("Feedback table: {}".format(feedback_table))
        table_content = execute_sql_query(""" SELECT * FROM "{}" """.format(feedback_table), db2_credentials=self.database_credentials)
        print(table_content)

        self.assertEqual(len(feedback_data), len(table_content))

    def stats_on_payload_table_from_db2(self, model, scoring_number):
        payload_scoring = model.get_scoring_payload()
        records_number = len(payload_scoring['values']) * scoring_number
        ...
        table_content = execute_sql_query(""" SELECT * FROM "{}" """.format(payload_table), db2_credentials=self.database_credentials)
        print(table_content)

        self.assertEqual(records_number, len(table_content))
