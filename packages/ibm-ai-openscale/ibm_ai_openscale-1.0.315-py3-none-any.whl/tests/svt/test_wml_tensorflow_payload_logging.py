...
class TestAIOpenScaleClient(unittest.TestCase):
    ...
    def test_01_setup_data_mart(self):
        if 'db2' in self.database_credentials:
            db_creds = DB2(self.database_credentials)
        else:
            db_creds = PostgreSQL(self.database_credentials)

        TestAIOpenScaleClient.ai_client.data_mart.setup(db_credentials=db_creds, schema=self.schema)
