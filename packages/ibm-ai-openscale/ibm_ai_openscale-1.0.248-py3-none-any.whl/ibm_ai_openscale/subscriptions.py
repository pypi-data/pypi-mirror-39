...
class Subscriptions:
    ...
    def get_details(self, subscription_uid=None, binding_uid=None):
        ...
        if subscription_uid is None:
            if binding_uid is not None:
                ...
                sc = p.map(lambda binding_uid: self.get_details(binding_uid=binding_uid)['subscriptions'], binding_uids)
                result = {'subscriptions': [sub_info for details in sc for sub_info in details]}
    ...
    def add(self, asset, add_all_deployments=True, deployment_uids=None):
        """
             ...
             >>>
             >>> subscription_1 = client.data_mart.subscriptions.add(WatsonMachineLearningAsset(source_uid=model_uid)) # all deployments will be added as by default add_all_deployments=True
