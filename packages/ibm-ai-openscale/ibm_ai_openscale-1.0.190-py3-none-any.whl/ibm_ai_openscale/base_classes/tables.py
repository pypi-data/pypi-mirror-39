from ibm_ai_openscale.supporting_classes.enums import Choose
import random
...
class Table:
    ...
    def _print_text_table(header, values, title):
        ...
        maximum_content_width = columns - 4 - 3*(len(header) -1)
        ...
        for row in values:
            for index, v in enumerate(row):
                if columns_width[index] < len(v):
                    columns_width[index] = len(v)
        ...
        if actual_content_width > maximum_content_width:
            ...
            values = [
                [
                    values[i][j][:final_column_width[j] - 3] + '...'
                    if (len(values[i][j]) > final_column_width[j])
...
class RefreshingTable(Table):
    def __init__(self, header, get_records, list_columns=None, date_field_name='created'):
        Table.__init__(self, header, [], list_columns, date_field_name)
    ...
    def get_record(self, choose=None, **kwargs):
        try:
            return Table.get_record(self, choose, **kwargs)
        except:
            self.update()
            return Table.get_record(self, choose, **kwargs)

    def list(self, column_list=None, limit=None, default_limit=None, sort_by='created', sort_reverse=True,
             limit_fields_length=36, filter_setup={}, title=None):
        self.update()
        Table.list(self, column_list, limit, default_limit, sort_by, sort_reverse, filter_setup, title)