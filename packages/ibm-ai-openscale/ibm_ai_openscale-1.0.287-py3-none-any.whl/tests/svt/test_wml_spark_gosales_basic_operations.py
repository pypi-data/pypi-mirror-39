...
class TestAIOpenScaleClient(unittest.TestCase):
    ...
    def test_02_setup_again_data_mart(self):
        TestAIOpenScaleClient.ai_client.data_mart.setup(db_credentials=self.database_credentials, schema=self.schema)
    ...
    def test_05_bind_again_wml_instance_and_get_wml_client(self):
        if "ICP" in get_env():
            TestAIOpenScaleClient.binding_uid = self.ai_client.data_mart.bindings.add("WML instance on ICP", WatsonMachineLearningInstance4ICP())
        else:
            TestAIOpenScaleClient.binding_uid = self.ai_client.data_mart.bindings.add("WML instance on Cloud", WatsonMachineLearningInstance(self.wml_credentials))
    ...
    def test_08_get_uids(self):
        print("Bindings: {}".format(TestAIOpenScaleClient.ai_client.data_mart.bindings.get_uids()))
    ...
    def test_10_prepare_deployment(self):
        ...
        if self.model_uid is None:
            ...
            published_model_details = self.wml_client.repository.store_model(model=model_data['model'], meta_props=model_props, training_data=model_data['training_data'], pipeline=model_data['pipeline'])
    ...
    def test_13_subscribe_again(self):
        TestAIOpenScaleClient.ai_client.data_mart.subscriptions.add(WatsonMachineLearningAsset(TestAIOpenScaleClient.model_uid))
    ...
    def test_15_check_if_deployments_were_added(self):
    ...
    def test_18_select_asset_and_get_details(self):
        ...
        TestAIOpenScaleClient.subscription = TestAIOpenScaleClient.ai_client.data_mart.subscriptions.get(name="AIOS Spark GoSales model", choose='random')
        TestAIOpenScaleClient.subscription = TestAIOpenScaleClient.ai_client.data_mart.subscriptions.get(name="AIOS Spark GoSales model", choose='first')
        TestAIOpenScaleClient.subscription = TestAIOpenScaleClient.ai_client.data_mart.subscriptions.get(name="AIOS Spark GoSales model", choose='last')
        TestAIOpenScaleClient.subscription.get_details()

    def test_19_list_deployments(self):
