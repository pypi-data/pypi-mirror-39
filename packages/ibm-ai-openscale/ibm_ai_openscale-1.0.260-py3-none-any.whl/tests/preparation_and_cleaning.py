...
def clean_db2_icp_schema(db2_credentials, schema_name):
    schema = str(schema_name).upper()

    print("Cleaning db2 on ICP schema.")
    result = execute_sql_query(""" select 'DROP TABLE "'||rtrim(tabschema)||'"."'||rtrim(tabname)||'"' from syscat.tables where OWNER = 'DB2INST1' and TABSCHEMA = '{}' and type = 'T' """.format(schema), db2_credentials=db2_credentials)
    for query in result:
