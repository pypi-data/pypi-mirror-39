...
class ConfigurationParserTests(unittest.TestCase):
    def test_will_return_null_when_no_config(self):
        error_reporter = Mock()
        ...
        self.assertIsNone(cp.parse(None, None))
        self.assertIsNotNone(cfi.event)
        self.assertEqual(FetcherError.EMPTY_JSON, cfi.event.error_details)

    def test_will_return_null_when_config_with_no_data(self):
        error_reporter = Mock()
        signature_verifier = Mock()
        signature_verifier.verify.return_value = True
        cfi = TestConfigurationFetchedInvoker()
        cp = ConfigurationParser(signature_verifier, error_reporter, cfi)
        config_fetch_result = ConfigurationFetchResult('', ConfigurationSource.CDN)

        self.assertIsNone(cp.parse(config_fetch_result, None))
        self.assertIsNotNone(cfi.event)
        self.assertEqual(FetcherError.EMPTY_JSON, cfi.event.error_details)
    ...
    def test_will_return_null_when_invalid_json(self):
        json = """
        {
            "data"("sss"
        }
        ...
        self.assertIsNone(conf)
        self.assertIsNotNone(cfi.event)
        self.assertEqual(FetcherError.CORRUPTED_JSON, cfi.event.error_details)
