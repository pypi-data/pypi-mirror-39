...
class ImpressionInvoker:
    ...
    def invoke(self, reporting_value, client_experiment, context):
        try:
            ...
            if internal_experiment and client_experiment is not None and not self.is_roxy:
                prop = self.custom_property_repository.get_custom_property('rox.' + property_type.DISTINCT_ID.name)
                ...
                self.analytics_client.track({
                    ...
                    'experimentId': client_experiment.identifier,
                    'experimentVersion': '0',
                    'type': 'IMPRESSION',
                    'time': event_time,
                })
