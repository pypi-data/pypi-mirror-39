...
def _wrap_azure_call(callback, bucket_name, key, *args, **kwargs):
    ...
    try:
        ...
        raise NotFound('container %s not found' % bucket_name)
...
def __az_create_blob_service(bucket_name, azure_config):
    ...
    if len(bucket_name_parts) == 1:
        ...
        account_name = bucket_name_parts[0]
        actual_bucket_name = bucket_name_parts[1]

    storage_key = azure_config.storage_key

    if not storage_key:
        storage_key = get_access_key_using_msi(account_name)
        azure_config.set_storage_key_env_var(storage_key)

    blob_service = BlockBlobService(account_name=account_name, account_key=storage_key)
...
def _az_download(bucket_name, object_name, azure_config=None):
    ...
    def actual_download():
        result = block_blob_service.get_blob_to_bytes(actual_bucket_name, object_name)
