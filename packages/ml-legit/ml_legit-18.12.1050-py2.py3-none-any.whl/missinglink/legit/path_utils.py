...
class StorageUtils(object):
    @classmethod
    def _split_bucket_prefix(cls, bucket):
        try:
...
class LocalEnumerator(StorageUtils):
    ...
    def _file_to_info(cls, path, st, fields):
        ...
        if fields is None or 'sha' in fields:
            ...
            if attr_mtime != str(st.st_mtime):
                ...
                cls._set_xattr(attr, st.st_mtime, params['sha'])
            else:
                params['sha'] = attr_sha
    ...
    @classmethod
    def _recursive_scan(cls, root_path, current_path, fields):
        try:
        ...
        try:
            for entry in scandir(current_path):
                if entry.is_dir():
                    for info in cls._recursive_scan(root_path, entry.path, fields):
    ...
    @classmethod
    def enumerate_path(cls, path, fields=None, root_path=None):
        def get_root_path():
        ...
        for info in cls._recursive_scan(root_path, path, fields):
            yield info
...
class S3Enumerator(LocalEnumerator):
    ...
    def enumerate_path(cls, path, fields=None, root_path=None):
        ...
        bucket, prefix = cls._split_bucket_prefix(path)
...
def enumerate_path_with_info(path, fields=None, root_path=None):
    if path.startswith(s3_moniker):
        enumerate_class = S3Enumerator
    else:
        enumerate_class = LocalEnumerator

    for file_info in enumerate_class.enumerate_path(path, fields, root_path):
...
def enumerate_paths_with_info(paths, fields=None, root_path=None):
    if isinstance(paths, six.string_types):
        paths = [paths]

    for path in paths:
        for file_info in enumerate_path_with_info(path, fields=fields, root_path=root_path):
            yield file_info
