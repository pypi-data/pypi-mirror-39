...
class PipelineClient:
    ...
    def _reportvalidator(self, report):
        """
        Checks whether the current status of a job is in error, if so it raises exceptions as appropriate
        :param report: report originally returned by run_job
        :type report: requests.Response
        """
        pipeline_logger.info("Entering report validator")
        try:
            report_json = report.json()
            if not isinstance(report_json['job_id'], str):
            ...
            updated_report = self.check_status(report).json()
            if updated_report['status'] == "ERROR":
                raise PipelineClientException("Execution error - {}. "
                                              "Check report associated with job_id {} for more details."
                                              .format(updated_report["reason"], updated_report["job_id"]))

        except (KeyError, AttributeError) as e:
            raise PipelineClientException('Response object did not contain expected key/attribute: {}'.format(repr(e)))
        pipeline_logger.info("Validator affirms report validity")
    ...
    def run_job(self):
        ...
        self._reportvalidator(response)
    ...
    def run_job_synchronously(self, update_policy=False, delete_policy_after_job_run=True):
        ...
        pipeline_logger.info("Cleaning file %s",
                             self.data['source']['s3_url'])
        while self.check_status(response).json()['status'] != 'COMPLETED':
            self._reportvalidator(report=response)
