...
class FileValidationTest(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.s3loc = S3Location("s3://{}/test_data/file_validation/".format(bucket_only))
        cls.relpath_test_data = os.path.join("test_data", "file_validation")
        cls.smaller_than_10b_s3 = [os.path.join(cls.relpath_test_data, file) for file in cls.smaller_than_10b]
        cls.between_1kb_10b_s3 = [os.path.join(cls.relpath_test_data, file) for file in cls.between_1kb_10b]
        cls.larger_than_1kb_s3 = [os.path.join(cls.relpath_test_data, file) for file in cls.larger_than_1kb]

        cls.abs_path_test_data = os.path.join(os.path.dirname(os.path.abspath(__file__)), cls.relpath_test_data)
    ...
    def _create_and_upload_files(list_of_files, s3loc, desired_size=0):
        ...
        print(return_list[0].time_creation)
        for file in return_list:
            s3c.s3_client.upload_file(
                Filename=file.file_path, Bucket=s3loc.bucket,
                Key=s3loc.path + file.file_name)
        return return_list
    ...
    def test_min_and_max_file_size_s3(self):
        ...
        self._run_and_consider_output_s3(incorrect=self.larger_than_1kb_s3 + self.smaller_than_10b_s3,
                                         correct=self.between_1kb_10b_s3)

    def test_non_existent_bucket(self):
        obj = FileValidation()
        with self.assertRaises(FileValidationException):
            obj.run_validator(path="s3://bucket-that-shouldnt-exist/path-that-also-shouldnt-exist", glob_pattern="*")
    ...
    def test_time_min_and_max_s3(self):
        ...
        self._run_and_consider_output_s3(correct=self.smaller_than_10b_s3 + self.between_1kb_10b_s3,
                                         incorrect=self.larger_than_1kb_s3)

    def test_time_min_and_max_ints_s3(self):
        self.object = FileValidation(timestamp=(str(self.time_first.timestamp()),
                                                str(self.time_third.timestamp())),
                                     timestamp_format="%s")
        self._run_and_consider_output_s3(correct=self.smaller_than_10b_s3 + self.between_1kb_10b_s3,
                                         incorrect=self.larger_than_1kb_s3)
    ...
    def test_time_max_only(self):
        self.object = FileValidation(timestamp=(None, self.time_third.strftime(self.timestamp_format)))
        self._run_and_consider_output(correct=self.local_smaller_than_10b + self.local_between_1kb_10b,
                                      incorrect=self.local_larger_than_1kb)

        self.object = FileValidation(timestamp=(None, str(self.time_third.timestamp())),
                                     timestamp_format="%s")
        self._run_and_consider_output(correct=self.local_smaller_than_10b + self.local_between_1kb_10b,
                                      incorrect=self.local_larger_than_1kb)

        self.object = FileValidation(timestamp=(None, self.time_third))
        self._run_and_consider_output(correct=self.local_smaller_than_10b + self.local_between_1kb_10b,
                                      incorrect=self.local_larger_than_1kb)
