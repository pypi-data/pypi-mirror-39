...
class FileValidation(object):
    ...
    def __init__(self, timestamp=(), file_size=(), file_count=(), timestamp_format=""):
        """
        Class to allow easy validation of files present in a given path based on various properties
        ...
        logger.info("Timestamps can be parsed with following formats: {}".format(self._allowed_timestamp_formats))
        ...
        logger.info("Initialising file validation with the following limits - timestamps: {},"
                    " file size ranges: {}, and file count range: {}."
                    .format(*[x if x else "not checked" for x in [self._timestamp, self._file_size, self._file_count]]))
    ...
    def run_validator(self, path, glob_pattern="*", consider_correctly_formed_files=True, raise_exceptions=False):
        ...
        :param path: path to the files - can either be a path for S3Location or an absolute local file path
        :type path str
        :param glob_pattern: pattern of glob files to be examined
        ...
        """
        logger.info("File validation with path and glob pattern: {} {}".format(path, glob_pattern))
        ...
        logger.info(f"File validation carried out with the following files: {acceptable_files}")
        excluded_files, excl_for_size, excl_for_time = [], {}, {}
        if self._file_size:
    ...
    def _find_file_paths_and_bucket(self, path, glob_pattern):
        ...
        """
        try:
            s3loc = S3Location(path)
            files_with_path = self._s3fs.glob(os.path.join(s3loc.s3_url, glob_pattern))
            # remove bucket name from start of each path
            files_with_path = [path.split("/", 1)[1] for path in files_with_path]
            bucket = s3loc.bucket

        except (S3ClientError, FileValidationException) as e:
            logger.info("Unable to treat path ({}) as S3 path (due to {}). Attempting as local file path.".
                        format(path, repr(e)))
            files_with_path = glob.glob(os.path.join(path, glob_pattern))
            bucket = None

        if len(files_with_path) == 0:
            raise FileValidationException("s3fs.glob and glob found no files! Check access permissions and content of "
                                          "path/folder. Path and pattern passed to S3FileSystem.glob and glob.glob "
