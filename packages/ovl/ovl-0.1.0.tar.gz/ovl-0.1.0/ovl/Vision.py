...
from socket import *
from numpy import copy, vstack, hstack
...
class Vision(object):
    ...
    def __init__(self, filters=None, directions_function=None, target_amount=1, width=320, height=240,
                 connection_dst='NOCONNECTION', port=9212, **kwargs):
        """
        ...
        if connection_dst == 'NOCONNECTION':
            ...
            try:
                NetworkTables.initialize(connection_dst)
    ...
    def send_to_destination(self, data):
        ...
        if self.connection_type == Vision.NT_CONNECTION:
            self.socket.putText(self.network_port, data)
    ...
    def apply_filter(self, filter_function, apply_results=False, parameters=None):
        ...
        if self.__params is not None or parameters is not None:
            ...
            for parameter in parameters_list:
                params += repr(parameter) + ', '
                filter_function(self.contours, )
            function_call = numerical_replace('output, ratio = 0(c, p)', filter_function.__name__)
            o = []
            exec_dict = {filter_function.__name__: filter_function,
                         'c': self.contours,
                         'p': params,
                         'array': ndarray,
                         'output': o}
            exec (function_call, exec_dict)
    ...
    def start(self, **kwargs):
        """
        Action: Sets up the camera and activates the loop
        :param kwargs:
        :return: None
        """
        if "camera" in kwargs:
    ...
    def json_serialize(self, pass_camera=False, pass_network=False, pass_calibration=False):
        ...
        if pass_network:
            ...
            translation['roborio_name'] = self.roborio_name
    ...
    def display_contours(self, img, amount=1, color=None, save_path=None):
        """
    ...
    def photo_array(self, amount, delay=3, name_convention=NC_TIME, return_images=False, path=False):
        ...
        if self.camera is None:
            self.camera_setup()
        images = []
        for image_num in xrange(amount):
            time.sleep(delay)
