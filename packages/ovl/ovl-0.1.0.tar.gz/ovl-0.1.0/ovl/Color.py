...
class Color(object):
    ...
    def set_low(self, new_range):
        point = self.copy()
        point.apply_vector_low(self.low_bound, False)
        point.apply_vector_low(new_range)

    def set_high(self, new_range):
        point = self.copy()
        point.apply_vector_high(self.high_bound, False)
        point.apply_vector_high(new_range)
    ...
    def __init__(self, low, high):
        ...
        if type(high) is tuple:
            high = list(high)
        self.__low_bound = low
        self.__high_bound = high
    ...
    def high(self):
        ...
        :rtype: uint8 numpy array
        """
        return array(self.__high_bound)
    ...
    def light(self, amount=50):
        ...
        """
        point = self.copy()
        point.apply_vector_low([0, -amount, 0])
...
class MultiColor(object):
    ...
    def light(self, amount=50):
        for color_range in self.colors:
            color_range.light(amount)

    def dark(self, amount):
        for color_range in self.colors:
            color_range.dark(amount)
...
class BuiltInColors(General.ToolClass):
    ...
    """
    yellow_hsv = Color([0, 100, 100], [55, 255, 255])
    blue_hsv = Color([100, 100, 100], [135, 255, 255])
    green_hsv = Color([35, 100, 100], [75, 255, 255])
    purple_hsv = Color([140, 100, 100], [160, 255, 255])
    ...
    help_text = "The BuiltInColor class is a resource class with a bunch of built-in ready to use\n" \
