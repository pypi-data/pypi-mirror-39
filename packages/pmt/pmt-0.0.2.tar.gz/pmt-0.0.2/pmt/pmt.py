...
def demo_thread_main(
        ...
        thread_param,
        #第四个参数必须是list,main传给thread_main的结果数组,以备thread_main把结果写到这个数组里在最后让main总结
        thread_results):
    for job in thread_jobs:
        print("t"+str(thread_id)+"  "+job["name"]+" "+str(thread_param))
...
def main():
    ...
    THREAD_N = 4
    thread_results = domt(demo_thread_main,THREAD_N,com_list,{"线程参数1":"参数值1"})
