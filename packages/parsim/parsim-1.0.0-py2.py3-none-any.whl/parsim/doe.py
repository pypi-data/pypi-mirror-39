...
schemes = ['mc', 'ff2n', 'fullfact', 'fracfact', 'ccdesign', 'lhs', 'pb']
    ...
    def _level_sampling(self):
        ...
        for i in range(self.get('npar')):
            if self.get('mapping') == 'int':
                high, low = self.distr_dict[self.parameters[i]].interval(self.get('beta'))
            else:
                raise NotImplementedError
...
class fullfact(FactorLevelScheme):
    ...
    def _init(self):
        super(fullfact, self)._init()
        self.add_valid_args(['levels'])
        self.set('levels', 2)

    def _check_args(self):
        super(fullfact, self)._check_args()
...
class fracfact(FactorLevelScheme):
    ...
    defined by the user by supplying a so-called generator expression with the "gen" keyword.
    ...
    Keyword arguments:
        ...
        fold (str): Specify optional folding of design. "all" to fold all colummns, or a comma-delimited list of
            columns to fold (starting with 1 for first column). For example, fold='all', or fold=1,3,5.
    """

    def _init(self):
        super(fracfact, self)._init()
        self.add_required_args(['gen'])
        self.add_valid_args(['fold'])
        self.set('fold', None)

    def _check_args(self):
        super(fracfact, self)._check_args()
        # fold
...
class ccdesign(FactorLevelScheme):
    ...
    def _init(self):
        super(ccdesign, self)._init()
        self.add_valid_args(['face', 'alpha'])
        self.set('face', 'cci')
        self.set('alpha', 'o')

    def _check_args(self):
        super(ccdesign, self)._check_args()
        # face
