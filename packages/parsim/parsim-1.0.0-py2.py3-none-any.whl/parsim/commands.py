...
"""
import parsim
from parsim.core import Project, Case, Study, ParsimError
...
def _process_parameter_file_option(project, parser, param_option, doe=False):
    """
    ...
    if param_option:
        ...
        if os.path.isfile(param_file):
            if doe:
                param_file_dict, distr_dict = project.parse_parameter_file(param_file, doe=True)
            else:
                param_file_dict = project.parse_parameter_file(param_file)
        else:
            parser.error('The specified parameter file "%s" is not found' % param_file)
    else:
        param_file = None
        param_file_dict = {}
        distr_dict = {}
...
def _process_doe_args(parser, args):
    ...
    for a in args:
        ...
        try:
            ...
            parser.error('Error processing DOE argument: "%s". Expects key-value pair.' % a)
        ...
        try:
            ...
            parser.error('Error processing DOE argument: "%s"' % a)
...
def config(argv=None):
    ...
    if project.exists:
        ...
        parser.error('There is no parsim project in this directory')
...
def case(argv=None):
    ...
    template_path = project.get_template_path(args.template)
    if not template_path:
        parser.error("Can't find a model template to use (default invalid, or not defined)")

    # Create parameter dict from parameter file
    param_dict, param_file = _process_parameter_file_option(project, parser, args.parameters)
    ...
    case_id = args.name
    case = Case(project, case_id)
    case.create(template_path, user_parameters=param_dict, description=args.description)

    logger.debug('Case: %s %s %s %s' % (template_path, param_file, case_id, case.path))
...
def study(argv=None):
    ...
    if not os.path.isfile(caselist_file):
        parser.error('The specified caselist file "%s" is not found' % caselist_file)

    if args.name:
        study_name = args.name
    else:
        basename = os.path.basename(caselist_file)
        study_name, ext = os.path.splitext(basename)

    path = project.get_study_path(study_name)

    study = Study(project, study_name)
    study.create(template_path, caselist_file=caselist_file, user_parameters=param_dict, description=args.description)

    logger.debug('Study: %s %s %s %s %s' % (template_path, param_file, caselist_file, study_name, path))
...
def doe(argv=None):
    ...
    # Create parameter dict from parameter file # -- Or should we send only file name to Case/Study?
    param_dict, distr_dict, param_file = _process_parameter_file_option(project, parser, args.parameters, doe=True)
    ...
    study = Study(project, study_name)
    study.create(template_path, doe_scheme=args.doe_scheme, doe_args=doe_args, user_parameters=param_dict,
                 distr_dict=distr_dict, description=args.description)

    logger.debug('Study: %s %s %s %s' % (template_path, param_file, study_name, path))
...
def dakota(argv=None):
    ...
    if not os.path.isfile(dakota_file):
        parser.error('The specified Dakota input file "%s" is not found' % dakota_file)
    ...
    if restart:
        ...
        study.create(template_path, user_parameters=param_dict, description=args.description)
...
def collect(argv=None):
    ...
    if project.exists:
        ...
        if type == 'study':
            study = Study(project, args.study)
            if not study.exists:
                parser.error('Cannot find study on disk (bug?)')
        else:
            parser.error('Cannot find a study from the given name (%s)' % args.study)
    else:
        parser.error('There is no parsim project in this directory')
