...
def viewkeys(obj, **kwargs):
    ...
    behaviour on Py2.7 as on Py3. Copy from future.utils.
...
def list_or_dict_unicode_to_str(d):
    """
    Recursively convert all unicode string values in a dict or list to ascii.

    Unicode string elements will be converted to ascii. The same function will
    be called recursively for dict or list elements.
    
    Args:
        d: Dict or list
    """
    # Exit if Python 3 (or higher)
    ...
    if isinstance(d, list):
        for i, v in enumerate(d):
            if isinstance(v, unicode):
                d.pop(i)
                try:
                    d.insert(i, v.encode('ascii'))
                except UnicodeEncodeError:
                    d.insert(i, v)
            else:
                list_or_dict_unicode_to_str(v)
    elif isinstance(d, dict):
        for k in d:
            if isinstance(d[k], unicode):
                try:
                    d[k] = d[k].encode('ascii')
                except UnicodeEncodeError:
                    pass
            else:
                list_or_dict_unicode_to_str(d[k])
            if isinstance(k, unicode):
                try:
                    ascii_key = k.encode('ascii')
                    d[ascii_key] = d.pop(k)
                except UnicodeEncodeError:
                    pass
...
def create_caselist_file(output_path, case_names, column_dict=None, case_dicts=None, parameter_names=None, csv=None):
    ...
    if column_dict:
        ...
        for row_dict in caselist_rows:
            missing = column_key_set - viewkeys(row_dict)
            if missing:
...
class ParsimObject:
    ...
    Attributes:
        ...
        parent_object_path (str): Path to parent object (`Study` or `Project`).
        parent_object_type (str): Type of parent object
        psm_path (str): Path to object storage subdirectory (inside object directory).
        data_file (str): Name of data file for object data storage.
        logger (logging.Logger): Logger instance of this object.
        logger_name (str): Name of object logger instance.
        logger_file (str): File used by logger file handler.
    ...
    def __init__(self, path=None, name=None, registry=None):
        # Make sure the object has a valid path now!
        if path:
            self.path = path
        assert isinstance(self.path, str) or isinstance(self.path, basestring), \
            'ParsimObject should have a path set at this point (self.path missing)'
        self.parent_object_path, self.parent_object_type = self.find_parent_object_path()
        logger.debug('myself: %s, %s' % (self.path, self.type))
        logger.debug('parent_object: %s, %s' % (self.parent_object_path, self.parent_object_type))
        self.data_file = self.get_data_file_path(self.path)
        self.psm_path = os.path.dirname(self.data_file)
        # Empty logger variables
        self.logger = None
        self.logger_name = None
        self.logger_file = None
        # Check if object exists already, or if it's new
        self.exists = os.path.isfile(self.data_file)
        # Registry for instance data structures to store on disk (self.data_file)
        ...
        self.data = {}
        if name:
            self.set('name', self.validated_name(name))
        self.open = False
    ...
    def create(self, name=None, description=None, noSave=False, onlySave=False):
        ...
        `ParsimObject` creation is made in two steps. The first step initializes
        the object, but does not save the object to disk. The second step only 
        saves the object to disk. Derived classes will usually do their class-specific 
        initialization in between these steps, calling the baseclass method twice 
        with either of `noSave` or `onlySave` arguments set to True.
        
        Args:
            name (str): Sets `name` value in dict attribute `data`, if provided.
            description (str): Optional description of the object. 
            noSave (bool): Set to True to do only initialization, but skipping save to disk.
            onlySave (bool): Set to True to do only saving of object to disk (assuming 
                initialization is already done).
        """
        if not onlySave:
            logger.debug('ParsimObject.create: type=%s path=%s' % (self.type, self.path))
            if name:
                self.set('name', self.validated_name(name))
            if description:
            ...
            self.set('parsim_version', parsim.__version__)
            date = datetime.datetime.today()
            self.set('creation_date', date.strftime(TIMESTAMP_FORMAT))
            if self.exists or os.path.exists(self.data_file):
                raise ParsimError('%s already exists. Abort create.' % self.type)
            if self.type in ['case', 'study']:
                if os.path.exists(self.path):
                    raise ParsimError('%s path already exists: %s. Abort create.' % (self.type, self.path))
                else:
                    os.mkdir(self.path)
            if not os.path.exists(self.psm_path):
                os.mkdir(self.psm_path)
            self._init_logger()
        if not noSave:
            self.save()
            self.exists = True
            self.logger.info('Parsim %s "%s" successfully created' % (self.type, self.get('name')))
    ...
    def load(self):
        ...
        """
        if not self.exists:
            raise ValueError('No %s exists, cannot load...' % self.type)
    ...
    def _init_logger(self):
        ...
        assert os.path.exists(self.psm_path)
        # Construct logger name for this object
        if self.type == 'project':
            self.logger_name = 'parsim.project'
        elif self.type == 'study':
            self.logger_name = 'parsim.project.study_' + self.get('name')
        else: # case
            if self.study:
                self.logger_name = 'parsim.project.study_' + self.study.get('name') + '.case_' + self.get('name')
            else:
                self.logger_name = 'parsim.project.case_' + self.get('name')
        self.logger = logging.getLogger(self.logger_name)
        self.logger_file = os.path.join(self.psm_path, 'log')
        h = logging.FileHandler(self.logger_file)
        ...
        h.addFilter(parsim.loggers.ParsimLogFilter(self.logger_name, scope=1))
        self.logger.addHandler(h)
        self.logger_fileHandler = h
        self.open = True
        return
    ...
    def save(self):
        ...
        for name in self.registry:
            ...
            if hasattr(attr, "__getstate__"):
                ...
                state[name] = attr
        with open(self.data_file, 'w') as f:  # should check if file exists?
    ...
    def delete(self, force=False, logging=True):
        ...
        If the object's `type` attribute is "project", the delete is aborted (projects
        ...
        if not force:
            prompt = '-- Are you SURE you want to delete the %s named %s? (no/[yes]' % (self.type, name)
            try:
                # Python 2
    ...
    def info(self):
        ...
        list_or_dict_unicode_to_str(self.data)
        template = """\
            Project name        : %(name)s
    ...
    def validated_name(self, name):
        ...
        """
        if re.match(ParsimObject.valid_name_pattern, name):
            return name
        else:
            raise ValueError("Invalid %s name. Names may only contain letters, numbers, spaces and hyphens" % self.type)
    ...
    def add_comment(self, msg):
        ...
        """
        self.logger.log(parsim.loggers.USER_LOG_LEVEL, msg)
    ...
    def get_data_file_path(cls, path):
        ...
        """
        return os.path.join(path, DEFAULT_PARSIM_DIRECTORY, DEFAULT_DATA_FILE[cls.type])
...
class Project(ParsimObject):
    ...
    def __init__(self, path=None):
        ...
        if existing_path:
            ...
            self.path = p
        self.config = {}
        ParsimObject.__init__(self, registry=['config'])
    ...
    def create(self, name, **config_dict):
        ...
        Args:
            name (name): Mandatory name of project. 
        ...
        ParsimObject.create(self, name=name, description=description)
    ...
    def info(self):
        ...
        """
        list_or_dict_unicode_to_str(self.config)
        template = """\
            Project name        : %(name)s
    ...
    def get_case_path(self, case_id, study=None, only_existing=False):
        ...
        if only_existing and not os.path.isfile(Case.get_data_file_path(path)):
            return None
        else:
            return path
    ...
    def get_study_path(self, name, only_existing=False):
        ...
        if only_existing and not os.path.isfile(Study.get_data_file_path(path)):
            return None
        else:
            return path
    ...
    def find_target(self, target_arg):
        ...
        if self.exists:
            if target_arg:
                ...
                if type == 'case':
                    target = Case(self, main_target)
                elif type == 'study':
                    study = Study(self, main_target)
                    if study.exists:
                        if sub_target:
                            target = Case(self, sub_target, study=study)
    ...
    def parse_parameter_file(self, path, get_comments=False, doe=False):
        """
            ...
            for rawline in f:
                ...
                if len(line) > 0 and not (line[0] in '#*;'):
                    ...
                    if m:
                        ...
                        if self.get('paramlist_upper_names'):
                        ...
                        if par_value[0] in r'{\"\'':
                            ...
                            try:
                                ...
                                if doe and re.match('(\w+)\(.+\)', par_value):
...
class Study(ParsimObject):
    ...
    def __init__(self, project, name='', path=None):
        ...
        if not path:
            path = project.get_study_path(name)
        ParsimObject.__init__(self, path, name=name)

    def create(self, template_path, user_parameters=None, caselist_file=None, doe_scheme=None, doe_args=None,
               distr_dict=None, description=None):
        """
        ...
        Args:
            template_path (str): Path to model template used for Cases of the Study.
        ...
        def create_study_and_cases():
            ...
            # Save to object store (to avoid error if it's missing when we create cases...)
            self.save()
            ...
            ParsimObject.create(self, onlySave=True)
        ...
        if not user_parameters:
            user_parameters = {}
        ParsimObject.create(self, noSave=True, description=description)
        try:
            create_study_and_cases()
        except:
    ...
    def _create_cases(self, template_path, user_parameters, cases):
        ...
        try:
            default_parameters = self.project.parse_parameter_file(default_parameter_path)
        ...
        for case_id, case_dict in cases:
            ...
            case = Case(self.project, case_id, study=self)
            case.create(template_path, user_parameters=user_parameters, caselist_parameters=case_dict,
                        default_parameters=default_parameters, description=case_description)
    ...
    def __next__(self):
        if self._iter_index == self._iter_len:
            raise StopIteration
        case = Case(self.project, self._iter_ids[self._iter_index], study=self)
    ...
    def run(self, executable, **kwargs):
        ...
        """
        self.logger.info('Starts RUN operation on cases (executable: %s)...' % executable)
        failed_cases = []
        for case in self:
            try:
        ...
        if failed_cases:
            self.logger.warning('Finished RUN operation: Failure for %d out of %d cases.' %
                                (len(failed_cases), len(self.get('case_id_list'))))
        else:
            self.logger.info('Successfully finished RUN operation!')
    ...
    def run_dakota(self, dakota_file=None, executable=None, pre_run=False,
        ...
        try:
            ...
            if err.returncode < 0:
                self.logger.warning('Dakota terminated with signal %s'
                                    % str(-err.returncode))
            else:
                self.logger.error('Dakota crashed: %s' % err)
                raise ParsimError(err)
        except KeyboardInterrupt:
            raise ParsimError('*** Interrupted by user ***')
        else:
            run_time = datetime.datetime.now() - start_time
            self.logger.info('Dakota finished successfully (runtime: %s)' % run_time)
        finally:
            try:
                ferr.close()
                fout.close()
    ...
    def collect(self, **kwargs):
        ...
        self.logger.info('Start collecting case results...\n'
        ...
        for case in self:
            ...
            try:
                ...
                if not output_keys_set: # First case is reference...
                    ...
                    missing = output_keys_set - viewkeys(case_data)
                    if missing:
                        case.logger.error('Missing output fields: %s' % missing)
                    extra = viewkeys(case_data) - output_keys_set
                    if extra:
                        case.logger.error('More output fields than the previous cases: %s' % extra)
                    if missing or extra:
        ...
        try:
            ...
            self.logger.error('Failed writing output file (with collected results)\n%s' % err)
            err.handled = True
            raise

        if failed_cases:
            self.logger.warning('Finished collecing results: Failure for %d out of %d cases.' %
                                (len(failed_cases), len(self.get('case_id_list'))))
        else:
            self.logger.info('Successfully finished collecting case results!')
...
class Case(ParsimObject):
    ...
    def __init__(self, project, name, study=None):
        self.project = project
        self.study = study
        path = project.get_case_path(name, study=study)
        logger.debug('Case.create: path=%s' % path)
        ParsimObject.__init__(self, path, name=name)

    def load(self):
        ParsimObject.load(self)
        # Load parent study, if not already available
        if self.parent_object_type == 'study' and not self.study:
            if os.path.basename(self.get('study_path')) != os.path.basename(self.parent_object_path):
                raise ParsimError('Mismatch between stored study_path (%s) and parent_object_path (%s)' %
                                  (os.path.basename(self.get('study_path')), os.path.basename(self.parent_object_path)))
            logger.debug('Case.load() loads study: study_path (%s) and parent_object_path (%s)' \
                         % (os.path.basename(self.get('study_path')), os.path.basename(self.parent_object_path)))
            self.study = Study(self.project, path=self.parent_object_path)

    def create(self, template_path, user_parameters=None, caselist_parameters=None, default_parameters=None,
               description=None):
        """
        ...
        if self.parent_object_type == 'case':
        ...
        # Base-class will abort if exists...
        ParsimObject.create(self, noSave=True, description=description)
        self.set('project_path', self.project.path)
        self.set('project_name', self.project.get('name'))
        if self.study:
        ...
        self.set('template_path', template_path)
        # Todo: Check existence of template path? In case/study command and/or in Case/Study object creation?
        # Create missing parameter dicts
        if not user_parameters:
            user_parameters = {}
        ...
        default_view = viewkeys(default_parameters)
        user_view = viewkeys(user_parameters)
        caselist_view = viewkeys(caselist_parameters)
        ...
        self.set('create_log', os.path.join(self.psm_path, 'create.log'))
    ...
    def run(self, executable, args='', sub_dir=None, out=None, err=None, shell=False):
        ...
        if os.path.isabs(executable):
            if os.path.isfile(executable):
                ...
                raise ParsimError('Error: Cannot find executable by absolute path (%s)' % executable)
        else:
        ...
        # Start log message:
        self.logger.info('Executing command/script/executable: %s %s' % (tail, ' '.join(args)))
        ...
        with open(outfile, 'w') as fout, open(errfile, 'w') as ferr:
            ...
            try:
                ...
                if err.returncode < 0:
                    self.logger.warning('Executable was terminated with signal %s \n%s'
                                        % (str(-err.returncode), loginfo))
                else:
                    self.logger.error('Executable crashed: %s\n%s' % (err, loginfo))
                    raise ParsimCaseError(err)
            except KeyboardInterrupt:
                raise ParsimError('*** Interrupted by user ***')
            else:
                run_time = datetime.datetime.now() - start_time
                self.logger.info('Executable finished successfully (runtime: %s)\n%s' % (run_time, loginfo))
    ...
    def collect(self, input=None, parameters=None):
        ...
        for filename in input_files:
            ...
            try:
                ...
                self.logger.error('Failure reading JSON results file (%s)\n%s' % (filename, err))
