...
class DAU(object):
	...
	def getNewShowMax(self):
		...
		return self.getDayCount(zeroTime - dayTime) * (1 + rate / 5.0) * 0.01
...
class WorksPersonas(object):
	...
	def __init__(self):
		...
		for hit in data:
			self.behaviors[hit['_id']] = hit['_source']

	def saveData(self):
		if es.updateStatisticsData('works_personas', self.behaviors):
			with open('lastPersonasTime.ini', 'w') as f:
				f.write(str(self.lastPersonasTime))

	def updateLastPersonasTime(self, newTime):
		newTime = esTimeToTime(newTime)
	...
	def createBehavior(self, bid, uid, vid):
		return {'bid': bid, 'uid': uid, 'vid': vid, 'play': 0, 'like': False, 'shareF': 0, 'shareI': 0, 'maxPlayTime': 0, 'playTime': 0, 'timeCount': 0, 'show': 0, 'showPlay': 0, 'subAdd': 0}
	...
	def actionOpen(self, uid, page, args):
		if page == 'pages/common/videoplay/videoplay' and len(args) > 2 and args[0] == '1' and args[2] in [1, 2, 3, 4]:
			self.getBehavior(uid, args[1])['showPlay'] += 1

	def actionShow(self, uid, page, args):
		if args[1] in [0, 1, 20, 3]:
			self.getBehavior(uid, args[0])['show'] += 1
