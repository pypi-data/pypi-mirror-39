...
def getDurationSectionsAvg(worksDict):
	...
	for index in range(length):
		durationSectionsInfo.append([0, 0])
...
def addBehaviors(subjectDict, worksDict, worksPersonas):
	...
	durationSectionsAvg = getDurationSectionsAvg(worksDict)
...
def addBehavior(subject, works, behavior):
	...
	works['showPlay'] += behavior['showPlay']
	subject['showPlay'] += behavior['showPlay']
...
def createDict(works, aids):
	...
	for hit in works:
		...
		if not worksInfo.has_key('publishtime'):
			continue
		publishTime = int(time.time() + 28800 - es.esTimeToTime(worksInfo['publishtime'], True)) / 3600
		...
		for durationIndex, value in enumerate(DurationSections):
			if duration < value:
				break
		worksDict[worksInfo['vid']] = {'title': worksInfo['title'], 'aid': aid, 'publishTime': publishTime, 'playTime': 0, 'timeCount': 0, 'show': 0, 'showPlay': 0, 'recShow': 0, 'recShowPlay': 0, 'durationIndex': durationIndex, 'statis': [0, 0, 0, 0, 0, 0, 0, 0, 0, duration, size]}
