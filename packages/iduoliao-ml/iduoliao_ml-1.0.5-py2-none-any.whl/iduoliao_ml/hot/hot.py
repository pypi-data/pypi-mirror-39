...
def updateWeight(weight):
	global SubjectTotalWeight, SubjectWeight, WorksWeight, TimeDecay
	SubjectTotalWeight = weight['subjectTotal']
	SubjectWeight = weight['subject']
	WorksWeight = weight['works']
	TimeDecay = weight['timeDecay']

def calHot(dauFactor, subjectDict, worksDict):
	subjectAids, subjectHotMat = calSubjectHot(subjectDict)
	worksVids, worksAids, worksHotMat, worksRate = calWorksHot(worksDict, dauFactor)

	shArray = []
	for index, aid in enumerate(worksAids):
		shArray.append([subjectHotMat[subjectAids.index(aid), 0] * worksRate[index, 0]])

	shm = imatrix.inOne(mat(shArray))
	whm = imatrix.inOne(worksHotMat)
	...
	for index, vid in enumerate(worksVids):
		...
		works['hot'] = int(works['totalHot'] * 100000) + calLowerBound(works['statis'][WorksIndexs['like']], works['statis'][WorksIndexs['play']])

		works['isNew'] = works['publishTime'] < 72 and works['show'] < 50
...
def calWorksHot(worksDict, dauFactor):
	worksVids = []
	worksAids = []
	worksArgs = []
	...
	worksForceMat = multiply(multiply(worksBaseMat[:, WorksIndexs['longTime']] * WorksWeight['longTime'] + worksBaseMat[:, WorksIndexs['subAdd']] * WorksWeight['subAdd'], worksMat[:, WorksIndexs['playTimeAvg']]), worksMat[:, WorksIndexs['conv']])
