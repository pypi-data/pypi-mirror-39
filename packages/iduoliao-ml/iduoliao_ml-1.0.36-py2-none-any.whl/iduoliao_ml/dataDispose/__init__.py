...
def readWorks():
	...
	for hit in es.scrollSearch('works_video_read', 'video', {"size":1000}):
		...
		if worksInfo.has_key('vid') and worksInfo.has_key('aid') and worksInfo.has_key('publishtime'):
			...
			worksActions[vid] = {'time': es.timeToEsLocalTime(es.esTimeToTime(worksInfo['publishtime'], True)), 'vid': vid, 'aid': worksInfo['aid'], 'duration': worksInfo['duration'], 'action': 4, 'uid': worksInfo['publisheruid'], 'value': 1}
	es.updateStatisticsData('works_actions', worksActions)
	return worksDict
...
def addWorksAction(worksDict, worksActions, _id, actionId, _time, uid, items):
	...
	if actionId == '6' and (value >= 60 or value >= 0.9 * worksInfo['duration']):
		worksActions[_id + '$1'] = {'time': _time, 'vid': vid, 'aid': worksInfo['aid'], 'duration': worksInfo['duration'], 'action': 2, 'uid': uid, 'value': 1}
