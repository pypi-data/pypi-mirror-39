...
class Widgetize(widgets.VBox):
    def __init__(self, pipeline_object, dataset):
        '''
        Docstring
        :params:
        :param pipeline_object: the vaex pipeline element object
        :param dataset: a vaex dataset on which the pipeline object will operate
        '''
        ...
        for trait_name in pipeline_object.trait_names():
            ...
            if 'ui' in metadata:
                ...
                if isinstance(widget, widgets.SelectMultiple):
                    widget.options = self.dataset.get_column_names(virtual=True, strings=True)
                    traitlets.link((self.pipeline_object, trait_name), (widget, 'value'))
                else:
                    traitlets.link((self.pipeline_object, trait_name), (widget, 'value'))

        fit_button = widgets.Button(description='Fit', button_style='info')
        fit_button.on_click(lambda button: self.pipeline_object.fit(self.dataset))
        # add it to the 'form'.
