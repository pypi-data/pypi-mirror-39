...
class ANNOYModel(state.HasState):
    ...
    predcition_name = prediction_name = traitlets.Unicode(default_value='annoy_prediction', help='Output column name for the neighbours when transforming a dataset')
    ...
    def fit(self, dataset):
        ...
        for i in range(len(dataset)):
            self.index_builder.add_item(i, dataset[self.features][i])
    ...
    def predict(self, dataset, n_neighbours=None, search_k=None):
        search_k = search_k or self.search_k
        n_neighbours = n_neighbours or self.n_neighbours
        result = np.zeros((len(dataset), n_neighbours), dtype=np.int)
        for i in range(len(dataset)):
            result[i] = self.index_builder.get_nns_by_vector(n=n_neighbours, search_k=search_k, vector=dataset[self.features][i])
        return result
...
try:
    from .autogen import annoy as _
    del _
except ImportError:
    pass
