...
class SyllableCounter:
    ...
    def count_syllables_word(self, word, count=0):
        ...
        if word in self.OUTLIERS:
            return self.OUTLIERS[word]
        ...
        with open(OUTLIERS_FILENAME, newline='') as f:
            ...
            with open(OUTLIERS_FILENAME, newline='', mode='w') as w:
                ...
                for row in sorted_csv:
                    ...
                    self.OUTLIERS[row[0]] = int(row[1])

    def load_custom_outliers(self, file_path):
        """
        Allows user to extend the custom outliers with own words. File must be
        a .csv file where each line is A_WORD, A_NUMBER.
