...
class ParagraphFormatter (argparse.ArgumentDefaultsHelpFormatter):
    """argparse formatter to maintain paragraph breaks in wrapped text.
    ...
    def _para_reformat(self, text, width, indent='', multiline=False):
        ...
        def blocker(text):
            ...
            the next block of text from 'text'.

            A block is either a single line, or a group of contiguous
            lines. The former is returned when not in multiline mode,
            the text in the line was indented beyond the indentation
            of the first line, or it was a blank line (the latter two
            jointly referred to as "no-wrap" lines). A block of
            concatenated text lines up to the next no-wrap line is
            returned when in multiline mode. The boolean value
            indicates whether text wrapping should be done on the
            returned text.
