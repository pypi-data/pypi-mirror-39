...
def compare(dataset_dict, ic="waic", model_names=None, method="BB-pseudo-BMA", b_samples=1000, alpha=1, seed=None):
    ...
    model_names : list[str], optional
        List with names for the models in the list of data. Defaults to None
    method : str
    ...
    if model_names is None:
        model_names = np.arange(len(dataset_dict))
    print(len(dataset_dict), model_names)

    if ic == "waic":
        ic_func = waic
        df_comp = pd.DataFrame(
            index=model_names, columns=["waic", "pwaic", "dwaic", "weight", "se", "dse", "warning"]
        )

    elif ic == "loo":
        ic_func = loo
        df_comp = pd.DataFrame(
            index=model_names, columns=["loo", "ploo", "dloo", "weight", "se", "dse", "warning"]
        )
    ...
    if method == "stacking":
        ...
        ses = pd.Series(z_bs.std(axis=0), index=model_names)
