...
class FlaskApi(AbstractAPI):
    ...
    @classmethod
    def _get_response_with_request_id_header_internal(cls, response, request):
        response.headers['X-Request-Id'] = request.headers['X-Request-Id'] if request \
            and 'X-Request-Id' not in response.headers and 'X-Request-Id' in request.headers else ''
        return response
    ...
    def get_response(cls, response, mimetype=None, request=None):
        ...
        return cls._get_response_with_request_id_header_internal(flask_response, request)
    ...
    @classmethod
    def get_connexion_response(cls, response, mimetype=None, request=None):
        if isinstance(response, ConnexionResponse):
            return cls._get_response_with_request_id_header_internal(response, request)

        if not isinstance(response, flask.current_app.response_class):
            response = cls.get_response(response, mimetype, request)

        response.headers['X-Request-Id'] = request.headers['X-Request-Id'] if request \
            and 'X-Request-Id' not in response.headers and 'X-Request-Id' in request.headers else ''

        return cls._get_response_with_request_id_header_internal(ConnexionResponse(
            status_code=response.status_code,
