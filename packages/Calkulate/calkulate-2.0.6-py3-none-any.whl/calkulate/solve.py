...
def Gran_EMF0(EMF,Tk,Msamp,Macid,Cacid,AT,HSO4,HF):
    # DAA03 Eq. (11)
...
def Gran(Macid,EMF,Tk,Msamp,Cacid,
    ...
    for i in range(Gran_reps):
        ...
        if i == 0:
            Gran_G_vec[i] = F1(EMF,Tk,Msamp,Macid)
        ...
        Gran_E0_vec[i,L2] = Gran_EMF0(EMF[L2],Tk[L2],
            Msamp,Macid[L2],Cacid,Gran_AT[i],
            Gran_HSO4[L2],Gran_HF[L2])
    ...
    return Gran_AT_final, Gran_E0_final, i, Gran_AT
...
def Gran_VINDTA(datfile,Msamp,Cacid,S,CT,PT,Tk_force=None):
    ...
    XT = conc.XT(S,CT,PT)
    KX = dissoc.KX_F(Tk[0],S,XT[3],XT[4])
