...
class TestContactValidation( unittest.TestCase ) :
    ...
    def testMissingGroupsRaises( self ) :
        contactsUnderTest = ContactData()
        contactsUnderTest.clusters = {
            'one' : [ 'a', 'b' ],
            'two' : [ 'c', 'd' ],
        }

        with self.assertRaisesRegex( ContactsError, '^Group\(s\) in cluster not defined' ) :
            contactsUnderTest.validate()
