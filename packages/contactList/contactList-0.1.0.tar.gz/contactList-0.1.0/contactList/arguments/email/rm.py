...
"""
Subcommand graph for `email rm`.
"""

EMAIL_ARGUMENT = {
    'names' : [ 'email', ],
    'options' : {
        'help' : 'Email address(es) to remove',
        'nargs' : '+',
        'type' : str,
    },
}
...
GROUP_ARGUMENT = {
    ...
    'options' : {
        'default' : list(),
        'dest' : 'groups',
        'help' : 'Group(s) from which to remove the email address (optional, default all)',
        'nargs' : '+',
        'type' : str,
    },
}
...
RM_EMAIL_SUBCOMMAND_GRAPH = {
    ...
    'options' : {
        'dest': 'remove',
        'help' : 'Remove an email from a selected group or groups, or from all groups.',
    },
    'specification' : {
        'subcommands' : list(),
        'arguments' : [
