...
class cylog:
    ...
    def fit(self): 
        ...
        self.event=pd.to_numeric(df['event'].loc[self.BEG:self.END-1])
        self.prob=pd.to_numeric(df['prob1'].loc[self.BEG:self.END-1])
        ...
        self.acc = accuracy_score(pd.to_numeric(self.event),pd.to_numeric(self.pred))
