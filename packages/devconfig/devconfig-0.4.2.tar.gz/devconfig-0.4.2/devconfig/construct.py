...
def fstring(loader, node):
    _f = loader.construct_scalar(node)
    def _fstringer(**ctx):
        retname = ''.join(sample(ascii_lowercase, 16))
        l = {}
        exec(f'{retname}=f"""{_f}"""', ctx, l)
        return l[retname]
    return _fstringer


def attrib(loader, node):
    return attr.ib(**loader.construct_mapping(node))
attrib.__doc__ = attr.ib.__doc__


def attrs(loader, node):
    return attr.s(**loader.construct_mapping(node))
