...
class UserQuestionAnswerRecordDetail(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        res_queryset=UserQuestionAnswerRecord.objects.filter(question_id=question_id).values('answer','answer__option').annotate(answer_count=Count('answer')).order_by().order_by("-answer_count")
...
def weixin_user_course_index(request):
    ...
    return JsonResponse(data={
        ...
        'section_detail': serializer(ucs.section, output_type='json', datetime_format='string', exclude_attr=['image']),
