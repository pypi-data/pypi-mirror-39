...
date: 15/12/2017 (updated to 31/10/2018)
...
3. model_cnn: Convolutional Neural Networks for images
4. model_pretrained: some famous pretrained nets such as VGG, ResNet50, Incep,...
5. fc_model: Linear Regression, with one fully connected layer
6. model_lstm: LSTM - Long Short Term Memory nets
7. model_mlp: Multilayer Perceptron
8. model_vgglike: simple CNN based on VGG
9. model_cnn1d: Convolutional Neural Networks for 1D data

and others:
10. call_model: select model to learn
11. grid_search_output: Fine-tuned, grid-search for finding the best models
12. cv_run_search: cross-validation for grid-search (called in grid_search_output)
13. grid_search_coef: grid-search to find the best coefficient
14. grid_search_model: Fine-tuned, grid-search for finding the best models (long code)
"""
...
def rf_model (rf_n_estimators=500,rf_max_depth=None,rf_min_samples_split=2):
    """ Random Forest
    Args:
    ...
    Returns:
        model
    """
    return RandomForestClassifier(n_estimators=rf_n_estimators, max_depth=rf_max_depth, min_samples_split=rf_min_samples_split)

def gbc_model (rf_n_estimators=500,rf_max_depth=None,rf_min_samples_split=2):
    """ GradientBoosting
    Args:
        rf_n_estimators (int): number of tree (default python n_estimators=10)
        rf_max_depth: The maximum depth of the tree (default=None)
    ...
    Returns:
        model
    """
    return GradientBoostingClassifier(n_estimators=rf_n_estimators,  max_depth=rf_max_depth, min_samples_split=rf_min_samples_split)
...
def model_cnn(input_reshape=(32,32,3),num_classes=2,optimizers_func='Adam',numfilter=20, 
        ...
        dropout_cnn=0, dropout_fc=0,lr_rate=0.0005,lr_decay=0, loss_func='binary_crossentropy', padded='y' ) :
    ...
    if name_model == 'resnet50':
        ...
        if pretrained_file[len(pretrained_file)-2]+pretrained_file[len(pretrained_file)-1] in ['ml']:
            ...
            if os.path.isfile(pretrained_file_ml):
                ...
                print 'the pre-trained file you provided does not exists!!'
                exit()
...
def model_cnn1d(input_reshape,num_classes=1, optimizers_func ='rmsprop', 
        ...
        dropout_cnn=0, dropout_fc=0,lr_rate=0.0005,lr_decay=0, loss_func='binary_crossentropy' ):
...
def call_model (type_model,m_input_shape, m_num_classes, m_optimizer,m_learning_rate, m_learning_decay,m_loss_func,
            ...
            svm_c, svm_kernel, rf_n_estimators, m_pretrained_file):
    ...
    if type_model== "fc_model":
        ...
        model=rf_model(rf_n_estimators=rf_n_estimators)
    elif type_model== "gbc_model":
        model=gbc_model(rf_n_estimators=rf_n_estimators)
        ...
        model=model_cnn1d(m_input_shape,m_num_classes,
            ...
            nummaxpool=mc_nummaxpool,
            maxpoolsize=mc_poolsize,
            dropout_cnn=mc_dropout_rate_cnn, dropout_fc=ml_dropout_rate_fc) 
