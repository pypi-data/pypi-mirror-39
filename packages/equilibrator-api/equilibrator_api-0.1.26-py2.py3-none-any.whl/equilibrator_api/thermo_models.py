...
class PathwayThermoModel(object):
    ...
    def __init__(self, S, fluxes, dG0_r_prime, cids,
        ...
        assert self.dG0_r_prime.shape == (self.Nr, ), 'dG0 required for all reactions'
        assert self.dG0_r_std.shape == (self.Nr, self.Nr), 'dG0 required for all reactions'

        self.fluxes = fluxes
        if self.fluxes is None:
            self.fluxes = ones(self.Nr)
        self.fluxes = array(self.fluxes)
        assert self.fluxes.shape == (self.Nr, ), 'Fluxes required for all reactions'

        _signs = list(map(sign, self.fluxes.flat))
        self.I_dir = array(diag(_signs))
    ...
    def _MakeDrivingForceConstraints(self, ln_conc_lb, ln_conc_ub):
        ...
        b = hstack([b1, b2, b2, ln_conc_ub, -ln_conc_lb])
    ...
    def _GetTotalEnergyProblem(self, min_driving_force=0.0, direction='min'):
        ...
        for j in range(A.shape[0]):
            ...
            constraints.append(Constraint(sum(row), ub=b[j], name='row_%d' % j))

        total_g0 = float(self.fluxes @ self.dG0_r_prime)
        total_reaction = self.S @ self.fluxes
        row = [total_reaction[i] * x[i] for i in range(self.Nc)]
    ...
    def _MakeMDFProblem(self):
        ...
        for j in range(A.shape[0]):
            row = [A[j, i] * x[i] for i in range(A.shape[1])]
            constraints.append(Constraint(sum(row), ub=b[j],
                                          name=cnstr_names[j]))
    ...
    def FindMDF(self, calculate_totals=True):
        ...
        def get_primal_array(l):
            return array([v.primal for v in l])
