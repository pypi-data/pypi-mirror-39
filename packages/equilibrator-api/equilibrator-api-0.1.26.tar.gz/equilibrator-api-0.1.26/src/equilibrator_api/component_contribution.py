...
from numpy import array, load, sqrt, zeros
...
class ComponentContribution(object):
    ...
    def reactions_to_matrices(self, reactions):
        ...
        """
        X = zeros((self.Nc, len(reactions)))
        G = zeros((self.Ng, len(reactions)))
    ...
    def dG0_prime_multi(self, reactions):
        ...
        dG0_r_primes = map(lambda r: r.dG0_prime(
                pH=self.pH, pMg=self.pMg, ionic_strength=self.ionic_strength),
                          reactions)
        dG0_r_primes = array(list(dG0_r_primes))
