...
def translate_to(throw, catch=Exception):
    ...
    with translate_to(Error):
        # Inside this 'with' block, any Exception will be re-raised as the
    ...
    'catch' is either an exception class, or a tuple of exception classes. If
    omitted, it defaults to Exception. Any exception matching 'catch' will be
    re-raised as 'throw'.
    """
    try:
        yield
        ...
        if not isinstance(throw, Exception):
            throw = throw(describe(err))
        # We don't need to call raise_from() since Python 2 doesn't support
        # exception chaining, but Python 3 implicitly chains 'err' to any
        # exception raised by this 'except' block.
        raise throw
