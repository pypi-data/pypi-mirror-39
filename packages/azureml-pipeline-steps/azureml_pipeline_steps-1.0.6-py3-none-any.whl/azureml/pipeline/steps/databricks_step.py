...
from azureml.pipeline.databricks import AutoScale
...
class DatabricksStep(PipelineStep):
    ...
    :type node_type: str
    :param num_workers: The number of workers for the Databricks run cluster
    :type num_workers: int
    :param autoscale: The autoscale configuration for the Databricks run cluster
    :type autoscale: azureml.pipeline.steps.databricks.AutoScale
    ...
    def __init__(self, name, inputs=None, outputs=None, existing_cluster_id=None, spark_version=None, node_type=None,
                 num_workers=None, autoscale=None, spark_env_variables=None, spark_conf=None, notebook_path=None,
                 notebook_params=None, python_script_path=None, python_script_params=None, main_class_name=None,
                 jar_params=None, python_script_name=None, source_directory=None, hash_paths=None,
                 run_name=None, timeout_seconds=None, runconfig=None, maven_libraries=None, pypi_libraries=None,
                 egg_libraries=None, jar_libraries=None, rcran_libraries=None, compute_target=None,
        ...
        if self._runconfig_cluster_present(runconfig):
            ...
            if runconfig.environment.databricks.cluster.min_workers is not None:
                autoscale = AutoScale(runconfig.environment.databricks.cluster.min_workers,
                                      runconfig.environment.databricks.cluster.max_workers)
        ...
        if existing_cluster_id is None:
            ...
            if num_workers is None and autoscale is None:
                raise ValueError("one of num_workers or AutoScale is required")
            if num_workers is not None and autoscale is not None:
                raise ValueError("exactly one of num_workers or AutoScale is required")
            if num_workers is not None and not isinstance(num_workers, int):
                raise ValueError("num_workers must be an int")
            if autoscale is not None and not isinstance(autoscale, AutoScale):
                raise ValueError("autoscale must be an AutoScale")
        ...
        if existing_cluster_id is None:
            ...
            if num_workers is not None:
                ...
                self._params["max_workers"] = autoscale.max_workers
                self._params["min_workers"] = autoscale.min_workers
