...
import time
import logging
from ibm_ai_openscale_cli.utils import executeCommand, executeCommandWithResult
...
def getCFInstanceCredentials(instance_params):
    ...
    logger.info('\t - Looking for existing instances of service "{}"'.format(service_name))
    ...
    for instance in existingInstances:
        if instance['service'] == service_name:
            logger.info('\t - Found existing instance "{}" of "{}"'.format(instance['name'], service_name))
            if existingInstanceName is None:
                existingInstanceName = instance['name']
    if existingInstanceName is not None:
        logger.info('\t - Using instance {}'.format(existingInstanceName))
        instance_name = existingInstanceName
    else:
        logger.info('\t - No existing instance of "{}" found.'.format(service_name))
        logger.info('\t - Creating a new instance "{}" for service "{}"'.format(instance_name, service_name))
        instanceList = createCFServiceInstance(
            service_name, service_plan_name, instance_name)
        for instance in instanceList:
            logger.info('\t - Created instance "{}" of "{}"'.format(instance['name'], service_name))
            existingInstanceName = instance['name']

    logger.info('\t - Looking for existing credentials with name "{}"'.format(key_name))
    credentials = getCFServiceKey(instance_name, key_name)

    if credentials is None:
        logger.info('\t - Existing credentials with name "{}" not found'.format(key_name))
        logger.info('\t - Generating credentials with name "{}"'.format(key_name))
        createCFServiceKey(instance_name, key_name)
        credentials = getCFServiceKey(instance_name, key_name)
        logger.info('\t - Generated "{}" for "{}"'.format(key_name, instance_name))
    else:
        logger.info('\t - Existing credentials with name "{}" for instance "{}" found, reusing'.format(key_name, instance_name))
    ...
    if metadataMapList is None:
        for _ in range(30):
            ...
            if metadataMapList is not None:
                break
            logger.info('\t\tWaiting for provision request to finish ...')
    return metadataMapList
