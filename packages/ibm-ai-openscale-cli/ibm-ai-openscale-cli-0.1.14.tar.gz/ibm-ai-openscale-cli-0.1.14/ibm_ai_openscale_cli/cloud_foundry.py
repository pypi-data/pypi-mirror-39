...
from ibm_ai_openscale_cli.utils import get_error_message
...
class CloudFoundry(object):
    ...
    def get_user(self):
        ...
        logger.debug('CloudFoundry.get_user()')
    ...
    def list_spaces(self, user_id):
        ...
        logger.debug('CloudFoundry.list_spaces()')
        response = self._request(
            method='GET',
    ...
    def list_organizations(self, user_id):
        ...
        logger.debug('CloudFoundry.list_organizations()')
        response = self._request(
            method='GET',
    ...
    def get_service_by_name(self, name):
        logger.debug('CloudFoundry.get_service_by_name()')
        response = self._request(
            method='GET',
            url='/v2/services',
    ...
    def list_plans_by_service(self, service_guid):
        logger.debug('CloudFoundry.list_plans_by_service()')
        response = self._request(
            method='GET',
    ...
    def list_instances_by_plan(self, plan, spaces=None):
        logger.debug('CloudFoundry.list_instances_by_plan()')
        if not spaces:
            user = self.get_user()
    ...
    def list_instances(self, service_name, spaces=None):
        logger.debug('CloudFoundry.list_instances()')
    ...
    def create_instance(self, name, service_plan_guid, space_guid):
        logger.debug('CloudFoundry.create_instance()')
        data = {
            'name': name,
    ...
    def delete_instance(self, guid):
        logger.debug('CloudFoundry.delete_instance()')
        response = self._request(
            method='DELETE',
            accept_json=False,
    ...
    def create_instance_key(self, service_instance_guid):
        logger.debug('CloudFoundry.create_instance_key()')
        data = {
    ...
    def list_instance_keys(self, guid):
        logger.debug('CloudFoundry.list_instance_keys()')
        response = self._request(
            method='GET',
    ...
    def delete_instance_key(self, guid):
        logger.debug('CloudFoundry.delete_instance_key()')
        response = self._request(
            method='DELETE',
    ...
    def get_or_create_instance(self, service_name, service_instance_name=None, service_plan_guid=None, organization_name=None, space_name=None):
        ...
        """
        logger.debug('CloudFoundry.get_or_create_instance(): service name: %s, plan: %s', service_name, service_plan_guid)
        ...
        if not instance:
            name = service_instance_name if service_instance_name else 'aios-{0}-{1}'.format(service_name, time.time())
