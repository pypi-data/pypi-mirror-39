...
import time
import logging
from ibm_ai_openscale_cli.utils import executeCommandWithResult, executeCommand
...
def getRCInstanceCredentials(instance_params):
    ...
    if metadataMapList is not None:
        for metadataMap in metadataMapList:
            logger.info('\tFound existing instance "{}" of "{}"'.format(metadataMap['Name'], service_name))
        logger.info('\tUsing instance {}'.format(metadataMap['Name']))
        ...
        metadataMap = metadataMapList[0]
        logger.info('\tCreated instance "{}" of "{}"'.format(metadataMap['Name'], service_name))
