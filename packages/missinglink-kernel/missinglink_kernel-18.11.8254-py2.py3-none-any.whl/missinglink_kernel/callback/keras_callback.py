...
class KerasCallback(BaseCallback, ModelHashInterface, GradCAMInterface, VisualBackPropInterface):
    ...
    def __ml_test_loop_wrap(self, is_method_bounded, call_test_loop):
        def _ml_test_loop(model, f, ins, batch_size=None, verbose=0, steps=None):
            ...
            return call_test_loop(model, f, ins, batch_size, verbose, steps)

        def _ml_test_loop_bounded(f, ins, batch_size=None, verbose=0, steps=None):
            temp_steps = self.__calc_steps_if_possible(ins, batch_size, steps)
            self._on_test_begin(temp_steps)

            return call_test_loop(f, ins, batch_size, verbose, steps)
    ...
    def _patch_test_loop(self, call_test_loop=None):
        training_model = self._get_training_model()
        ...
        def patch_if_possible(obj, name):
            ...
            ml_test_loop = self.__ml_test_loop_wrap(is_method_bounded, call_test_loop or prev_test_loop)
