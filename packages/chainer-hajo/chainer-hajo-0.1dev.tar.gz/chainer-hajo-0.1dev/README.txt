...
* ``PrintNoLog`` will log a set of observations,
...
* ``SecondTrigger`` is a trigger object which will fire
   ...
   This trigger avoids such problems
   by triggering based on the wall clock time.

* ``LogHyperparameter`` is a trainer extension which will copy
   the current value of a hyperparameter over into an reporting observation.
...
* ``SynthAndGate`` will convolve the input signal and then calculate
...
* ``ConvAndGate`` provides a similar functionality to ``SynthAndGate``
   but without the limiting by tanh.
