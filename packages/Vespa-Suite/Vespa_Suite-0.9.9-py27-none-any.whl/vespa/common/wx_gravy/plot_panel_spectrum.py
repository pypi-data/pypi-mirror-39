...
class PlotPanelSpectrum(wx.Panel):
    ...
    def __init__(self, parent, naxes=2,
        ...
        for axis in self.all_axes:
            axis.set_axis_bgcolor(self.prefs.bgcolor)
        ...
        for i, item in enumerate(data):
            for j, dat in enumerate(item):
                if isinstance(dat, dict):
                    if 'line_color_real' not in dat.keys():
                        dat['line_color_real'] = self.prefs.line_color_real
                    if 'line_color_imaginary' not in dat.keys():
                        dat['line_color_imaginary'] = self.prefs.line_color_imaginary
                    if 'line_color_magnitude' not in dat.keys():
    ...
    def format_axes(self):
        ...
        if self.prefs.xaxis_ppm:
            self.all_axes[self.naxes-1].set_xlabel('Frequency [ppm]')
        elif self.prefs.xaxis_hertz:
            self.all_axes[self.naxes-1].set_xlabel('Frequency [Hz]')
        elif xaxis_sec:
            self.all_axes[self.naxes-1].set_xlabel('Time [ms]')
...
class MyFrame(wx.Frame):
    ...
    def make_data(self, lines=8, points=2048):
        ...
        for i, vals in enumerate(zip(amp0, ppm0, pha0)):
            amp, ppm, pha = vals
            lines[i,:] = util_spectral.calculate_spectrum([amp], [ppm], [pha], self, ta=ta, tb=tb)
