...
# Our modules
import pane_metabolite_info
import auto_gui.manage_metabolites as gui_manage_metabolites
    ...
    def on_key_down(self, event):
        if wx_util.is_select_all(event):
            wx_util.select_list_ctrl_items(self.ListMetabolites, 
                                   range(self.ListMetabolites.GetItemCount()))
...
Would you like to see the import log?"""            
    ...
    def populate_metabolites(self):
        ...
        for i, metabolite in enumerate(self.metabolites):
            ...
            self.ListMetabolites.InsertStringItem(i, name)

            # Mark the public column if necessary
            public = "x" if metabolite.is_public else " "
            self.ListMetabolites.SetStringItem(i, 1, public)
            ...
            self.ListMetabolites.SetStringItem(i, 2, usage_count)
