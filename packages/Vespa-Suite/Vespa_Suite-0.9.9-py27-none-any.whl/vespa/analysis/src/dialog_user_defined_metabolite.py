...
import auto_gui.dialog_user_defined_metabolite as dialog_user_defined_metabolite

from vespa.common.util.generic_spectral import calculate_spectrum
from plot_panel_user_defined_metabolite import PlotPanelUserDefinedMetabolite
...
class DialogUserDefinedMetabolite(dialog_user_defined_metabolite.MyDialog):
    ...
    def __init__(self, parent, dataset, metabolite=None, unique_names=[]):
        ...
        class _UserDefinedMetabolitePrefs(prefs_module.AnalysisPrefs):
            def __init__(self):
                self.bgcolor = "white"
        ...
        self.prefs = _UserDefinedMetabolitePrefs()
        ...
        # Plotting is disabled during init because not all value are finalized
        # However, some controls (e.g. dynamic list) fire their change events 
        # which triggers a call to plot(). This is a Windows-specific bug.
        ...
        # Sash position is normally stored in self.prefs (see above); here
        # we just hardcode it.
        wx.CallAfter(self.WindowSplitter.SetSashPosition, 450, True)
    ...
    def initialize_controls(self):
        ...
        sw      = self.dataset.sw
        maxppm  = util_ppm.pts2ppm(0, self.dataset)
        minppm  = util_ppm.pts2ppm(dim0-1, self.dataset)
        ...
        for heading in headings:
            if heading:
                ...
                self.GridSizerMetabolite.AddSpacer( (1, 1) )
    ...
    def update_spectrum(self):
        ...
        if areas:
            for i in range(len(areas)):
                ...
                spectrum = calculate_spectrum( areas[i],ppms[i],phases[i],dataset,10000.0,apod)
...
class DynamicList1(object):
    ...
    def add_row(self, row_vals, update=False):
        ...
        '''
        maxppm = util_ppm.pts2ppm(0, self.dataset)
        minppm = util_ppm.pts2ppm(self.dataset.spectral_dims[0]-1, self.dataset)
