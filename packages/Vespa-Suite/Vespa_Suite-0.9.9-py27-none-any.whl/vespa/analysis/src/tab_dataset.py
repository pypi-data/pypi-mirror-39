...
import constants
import tab_raw
import tab_prep_fidsum
import tab_prep_timeseries
import tab_prep_wbnaa
import tab_spectral
import tab_voigt
import tab_giso
import tab_watref
import dialog_user_prior
import dialog_user_metabolite_info
import auto_gui.dataset as dataset_module
...
class TabDataset(dataset_module.DatasetUI):
    ...
    def __init__(self, _outer_notebook, top, name):
        ...
        style = wx.aui.AUI_NB_SCROLL_BUTTONS  |  \
                wx.aui.AUI_NB_BOTTOM          |  \
                wx.NO_BORDER

        self.NotebookDataset = notebooks.VespaAuiNotebook(self, style)
        sizer.Add(self.NotebookDataset, 1, wx.EXPAND, 0)

        self.Bind(wx.aui.EVT_AUINOTEBOOK_PAGE_CHANGED, self.on_tab_changed, self.NotebookDataset)
    ...
    @property
    def voxel(self):
        return [self.SpinX.GetValue() - 1, self.SpinY.GetValue() - 1,
    ...
    def populate_controls(self, preset=False):
        ...
        for block in dataset.blocks.itervalues():
...
#                 tab.process()
    ...
    def on_voxel_change(self, event=None):
        ...
        self.TextData.ChangeValue(raw.get_data_source(self.voxel[0]))
