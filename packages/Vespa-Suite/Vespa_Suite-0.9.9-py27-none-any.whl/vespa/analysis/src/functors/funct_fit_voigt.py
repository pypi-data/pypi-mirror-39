...
import vespa.analysis.src.lowess as lowess
import vespa.analysis.src.splines as splines
import vespa.analysis.src.util_initial_values as util_initial_values
...
from vespa.analysis.src.constants import FitInitialSmallPeakFreqs
from vespa.analysis.src.constants import FitInitialSmallPeakAreas
from vespa.analysis.src.constants import FitInitialBaselineMethod
from vespa.analysis.src.constants import FitInitialPhaseMethod
from vespa.analysis.src.constants import FitOptimizeWeightsMethod
from vespa.analysis.src.constants import FitInitialB0ShiftMethod
from vespa.analysis.src.constants import FitInitialLinewidthMethod
from vespa.analysis.src.constants import FitMacromoleculeMethod

from vespa.analysis.src.constrained_levenberg_marquardt import constrained_levenberg_marquardt
...
#    self.prior_area_scale = np.array(set.prior_area_scale)
#    self.prior_peak_ppm   = np.array(set.prior_peak_ppm)
#    self.prior_peak_ppm   = np.round(util_ppm.ppm2pts(self.prior_peak_ppm, chain))
#    self.prior_search_ppm = np.array(set.prior_search_ppm)
#    self.prior_search_ppm = util_ppm.ppm2pts(self.prior_search_ppm, chain, rel=True)
#    self.prior_search_ph0 = np.array(set.prior_search_ph0)
...
#     a = np.array(a)
    ...
    # PPM constraints
    fredel  = set.optimize_limits_range_ppm * 2.0 * np.pi       # convert Hz to radians here
    fremin  = chain.init_freq - fredel                          # in radians here
    fremax  = chain.init_freq + fredel                          # in radians here

    # Linewidth constraints - start with VOIGT assumptions
    lwAmin = set.optimize_limits_min_linewidth
    lwAmax = set.optimize_limits_max_linewidth
    lwBmin = set.optimize_limits_min_linewidth
    lwBmax = set.optimize_limits_max_linewidth
    ...
    if chain._block.set.macromol_model == FitMacromoleculeMethod.SINGLE_BASIS_DATASET:
        ...
        mmol_area_max = mmol_area * (1.0 + set.optimize_limits_mmol_range_area/100.0)
        mmol_area_min = mmol_area * 0.01
        mmol_fre_max  = mmol_fre + set.optimize_limits_mmol_range_ppm     # in Hz
        mmol_fre_min  = mmol_fre - set.optimize_limits_mmol_range_ppm     # FIXME bjs - does this need to be radians???
    ...
    if set.baseline_method:
        ...
        if set.baseline_method == constants.FitBaselineMethod.BSPLINE_VARIABLE_KNOT:
            ...
            if constants.PYWAVELETS_AVAILABLE:
                ...
                import vespa.analysis.src.wavelet_filter as wavelet_filter
...
#         nparam += 2 
    ...
    nstr = round(util_ppm.ppm2pts(set.cramer_rao_ppm_end, chain._dataset))
    nstr = int(np.where(nstr > 0, nstr, 0))
    nend = round(long(util_ppm.ppm2pts(set.cramer_rao_ppm_start, chain._dataset)))
...
def do_processing_full_fit(chain):
    ...
    for i in range(niter):
        chain.iteration += 1
        if chain.statusbar: chain.statusbar.SetStatusText(' Functor = baseline_model (Loop %d/%d) ' % (i,niter), 0)
        baseline_model(chain)
        if chain.statusbar: chain.statusbar.SetStatusText(' Functor = optimize_model (Loop %d/%d) ' % (i,niter), 0)
