...
import auto_gui.dialog_experiment_indices as dialog_experiment_indices
...
class DialogExperimentIndices(dialog_experiment_indices.MyDialog):
    ...
    def __init__(self, parent, db, experiment_id, dim_ids):
        ...
        self.dim_ids = dict((value, key) for key, value in dim_ids.iteritems())

        # Reduce the dims to a list of tuples containing the unique values
        # for each dim.
        self.dims = self.dim_ids.keys()
        self.dims = zip(*self.dims)
    ...
    def _autosize_listctrls(self):
        ...
        for i, dim in enumerate(self.dims):
            ...
            if listctrl.IsShown():
                ...
                listctrl_width, _ = listctrl.GetSizeTuple()
        ...
        for i, dim in enumerate(self.dims):
            ...
            if dim == mrs_experiment.DEFAULT_LOOP:
                ...
                for i, value in enumerate(dim):
                    listctrl.InsertStringItem(i, str(i + 1).rjust(index_digits))

                    listctrl.SetStringItem(i, 1, (formatter % value).rjust(value_digits))
