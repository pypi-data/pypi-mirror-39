...
import prefs
import util_menu
import util_pulse_config
import plot_panel_transform as pp_transform
import plot_panel_transform_contour as pp_transform_contour
...
class TabTransform(panel_tab_transform.PanelTabTransform):
    ...
    def __init__(self, inner_notebook, left_neighbor, 
        ...
        if self.is_create_modify:
            ...
            # we this here as there are always default results to plot
...
# bjs-end - temporary comment out to do a release without Grad_Refocus
    ...
    def get_raw_gui_data(self, return_list=False):
        ...
        if not kernel.hide_file1:
            ...
            d['file1'] = [value, u'(File)']
            l.append([u'file1', value, u'(File)'])

        if not kernel.hide_file2:
            value = self.panel_kernel_globals.TextFile2.GetValue().strip()
            d['file2'] = [value, u'(File)']
            l.append([u'file2', value, u'(File)'])
    ...
    def validate_gui(self):
        ...
        if not msg:
            if "duration" in d.keys() and "time_steps" in d.keys():
                duration = pulse_funcs_util.check_and_suggest_duration(
                            d["time_steps"], d["duration"],
                            self._pulse_design.machine_specs.min_dwell_time,
                            self._pulse_design.machine_specs.dwell_time_increment,
                            )
...
#             last_control = self.TextBandwidth
        ...
        width, height = self.parent.GetSizeTuple()
...
#             last_control = self.TextBandwidth
    ...
    def initialize_display_controls(self):
        ...
        # trouble finding one.
        x, y = self.ComboBlochRangeUnits.GetSizeTuple()
        ...
        for axes in self.view_1d.axes:
            ...
            self.view_1d.figure.subplots_adjust(left=0.12,right=0.98,
    ...
    def run(self):
        ...
        if successful: 
            ...
            if 'plot_method' in outputs.keys() and 'update_method' in outputs.keys():
    ...
    def plot_1d(self, outputs):             
        ...
        '''
        update_profiles = outputs["update_profiles"] if "update_profiles" in outputs.keys() else False
        relim_flag      = outputs["relim_flag"]      if "relim_flag"      in outputs.keys() else False
        update_refocus  = outputs["update_refocus"]  if "update_refocus"  in outputs.keys() else False
