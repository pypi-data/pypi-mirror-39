...
# Our modules
import dialog_machine_specs
import auto_gui.manage_machine_specs as manage_machine_specs
...
class DialogManageMachineSpecs(manage_machine_specs.MyDialog):
    ...
    def populate_machine_specs(self):
        ...
        for i, template in enumerate(self.templates):
            self.ListMachineSpecs.InsertStringItem(i, template.name)
            default = "x" if template.is_default else " "
            self.ListMachineSpecs.SetStringItem(i, 1, default)
