...
# Our modules
import auto_gui.manage_transform_kernels as manage_transform_kernels
import dialog_view_transform_kernel
...
from vespa.rfpulse.src import dialog_editor_transform
...
class DialogManageTransformKernels(manage_transform_kernels.MyDialog):
    ...
    def on_new(self, event):
        ...
        # this is launched from the application rather than from a given
        # pulse_project, so most times we will use default settings for
        # these two global objects
        machine_settings = None
        master_parameters = None
        
        dialog = dialog_editor_transform.DialogEditorTransform(self, self.all_names, 
                                                                     self.all_labels)
    ...
    def on_edit(self, event):
        ...
        if len(transform_kernels):
            ...
            dialog = dialog_editor_transform.DialogEditorTransform(self, all_names, 
                                                                   all_labels, kernel)
            if dialog.ShowModal() == wx.ID_OK:
    ...
    def on_import(self, event):
        ...
        if filename:
            ...
            if msg:
                ...
                msg = """RFPulse imported %d of %d transform_kernel(s).
    ...
    def get_selected_open_intersection(self):
        ...
        empty.
        """
        open_transform_kernels = wx.GetApp().vespa.get_open_projects()
    ...
    def populate_transform_kernels(self):
        ...
        for i, transform_kernel in enumerate(self.transform_kernels):
            self.ListTransformKernels.InsertStringItem(i, transform_kernel.name)
            self.ListTransformKernels.SetStringItem(i, 1, transform_kernel.menu_label)

            # Mark the public column if necessary
            public = "x" if transform_kernel.is_public else " "
            self.ListTransformKernels.SetStringItem(i, 2, public)

#             # Display referrer count if non-zero
#             referrers = len(transform_kernel.referrers)
#             referrers = (str(referrers) if referrers else "")
#             self.ListTransformKernels.SetStringItem(i, 3, referrers)
