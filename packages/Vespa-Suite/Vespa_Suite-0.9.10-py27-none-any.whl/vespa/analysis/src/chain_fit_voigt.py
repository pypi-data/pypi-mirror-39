...
import functors.funct_fit_voigt      as funct_fit_voigt
...
class ChainFitVoigt(chain_fit_identity.ChainFitIdentity):
    ...
    def __init__(self, dataset, block):
        ...
        (Dataset) objects.  These are referenced here as:
        
        self._dataset = dataset
        self._block   = block
        self.data = [ ]

        We also set local values for data acquisiton parameters. These do not 
        change over time, so we can set them here once and make it easy access
        ...
        self.functor_initial        = funct_fit_voigt.do_processing_initial
        self.functor_full_fit       = funct_fit_voigt.do_processing_full_fit
        self.functor_plot_refresh   = funct_fit_voigt.do_processing_plot_refresh
        self.functor_output_refresh = funct_fit_voigt.do_processing_output_refresh
        self.functor_voxel_change   = funct_fit_voigt.do_processing_voxel_change
    ...
    def nmet(self):
        """
        Number of metabolites to be fitted.
        """
    ...
    def reset_results_arrays(self):
        ...
        if self._block.set.macromol_model == FitMacromoleculeMethod.SINGLE_BASIS_DATASET:
            nmmol = 2 
        nparam = self._block.calc_nparameters()
    ...
    def run_global_init(self):
        ...
        self.peakpts    = util_ppm.ppm2pts(np.array(ppms), self._dataset)    # for weight array calc
    ...
    def run(self, voxels, entry='initial_only', statusbar=None, do_init=True):
        ...
        # NB. spectral chain needs update for this line to be valid    
        ...
        if entry == 'initial_only':
            self.functor_initial(self)
        elif entry == 'full_fit' or entry == 'all':
            self.functor_full_fit(self)
        elif entry == 'plot_refresh':
            self.functor_plot_refresh(self)
        elif entry == 'output_refresh':
            self.functor_output_refresh(self)
            ...
            self.functor_voxel_change(self, flag_auto_initvals=flag_auto_initvals)
        else:
        ...
        block.initial_values[:,voxel[0],voxel[1],voxel[2]] = voigt_checkin(self.nmet, self.initial_values, dataset)
        block.fit_results[   :,voxel[0],voxel[1],voxel[2]] = voigt_checkin(self.nmet, self.fit_results, dataset)
    ...
    def lorgauss_internal(self, a, pderflg=True, 
        ...
        **finalwflg:** [keyword][float][default=False] xxxx
        ...
        fre  = a[nmet:nmet*2] - util_ppm.ppm2hz(peaks,self._dataset)*2.0*np.pi    # in Radians here
...
def voigt_checkin(nmet, source, dataset):
    ...
    """
    dest = source.copy()
    dest[nmet:nmet*2] = util_ppm.hz2ppm(dest[nmet:nmet*2]/(2.0*np.pi), dataset, acq=False)
...
def voigt_checkout(nmet, source, dataset):
    ...
    """
    dest = source.copy()
    dest[nmet:nmet*2] = util_ppm.ppm2hz(dest[nmet:nmet*2], dataset, acq=False)*2.0*np.pi
