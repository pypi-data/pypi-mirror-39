...
import plot_panel_spectral
import plot_panel_svd_filter
import auto_gui.spectral as spectral
import functors.funct_ecc as funct_ecc
import functors.funct_water_filter as funct_watfilt
...
#        self.SliderSingularValues.SetRange(1, hlsvdpro.MAX_SINGULAR_VALUES)
    ...
    def on_check_datasetb_status(self, label):
        ...
        # check if datasetB is the label sent
    ...
    def on_user_function(self, event):
        ...
        if label == constants.UserButton.AUTOPHASE:
            ...
            if filename:
                ...
                if not dfname:
                    dfname = self.dataset.blocks['raw'].data_source
    ...
    def svd_checklist_update(self):
        ...
        for i in range(nsvd):
            ...
            index = self.list_svd_results.InsertStringItem(sys.maxint, ' '+str(res[i][0])+' ')
            self.list_svd_results.SetItemImage(index, in_model[i])
            self.list_svd_results.SetStringItem(index, 1, '%.2f'%(res[i][1]))
            self.list_svd_results.SetStringItem(index, 2, '%.1f'%(res[i][2]))
            self.list_svd_results.SetStringItem(index, 3, '%.1f'%(res[i][3]))
            self.list_svd_results.SetStringItem(index, 4, '%.1f'%(res[i][4]))
            self.list_svd_results.SetStringItem(index, 5, '%.1f'%(res[i][5]))
