...
import util_menu
import util_import
import plot_panel_voigt  
import util_analysis_config
import dialog_dataset_browser
import dialog_experiment_indices
import dynamic_list_voigt_metabolite
import figure_layouts 
import auto_gui.voigt as voigt

# TODO - move these into COMMON
import vespa.simulation.src.dialog_mixed_metabolite_output as dialog_mixout
from vespa.simulation.src.constants import ThirdPartyExportTypes
...
def _configure_combo(control, choices, selection=''):
        lines = [choices[key] for key in choices.keys()]
        control.Clear()        
...
#            else:
#                self.MacromoleculeGridSizer.AddSpacer( (1, 1) )        
    ...
    def on_menu_view_results(self, event):
        ...
        if flavor == 'text':
            ...
            if layout == 'lcm':
                ...
                fig_call = figure_layouts.analysis_brp1024
                nobase = True
                fixphase = True
                dpi = 64    
            if filename:
...
#        self.block.set.prior_ignore_mask = val
    ...
    def on_b0_shift_method(self, event): 
        ...
        value = constants.FitInitialB0ShiftMethod.choices.keys()[index]
...
#        self.block.set.initial_b0_shift_method = key
    ...
    def on_initial_baseline_method(self, event): 
        index = event.GetEventObject().GetSelection()
        value = constants.FitInitialBaselineMethod.choices.keys()[index]
    ...
    def on_initial_small_peak_areas(self, event):
        index = event.GetEventObject().GetSelection()
        value = constants.FitInitialSmallPeakAreas.choices.keys()[index]
        self.block.set.initial_small_peak_areas = value
        self.process_and_plot()

    def on_initial_small_peak_freqs(self, event):
        index = event.GetEventObject().GetSelection()
        value = constants.FitInitialSmallPeakFreqs.choices.keys()[index]
        self.block.set.initial_small_peak_freqs = value
        self.process_and_plot()

    def on_initial_linewidth_method(self, event): 
        index = event.GetEventObject().GetSelection()
        value = constants.FitInitialLinewidthMethod.choices.keys()[index]
    ...
    def on_initial_phase_method(self, event): 
        index = event.GetEventObject().GetSelection()
        value = constants.FitInitialPhaseMethod.choices.keys()[index]
    ...
    def on_lineshape_model(self, event): 
        index = event.GetEventObject().GetSelection()
        value = FitLineshapeModel.choices.keys()[index]
...
#        self.dynamic_macromolecule_list.reset_to_default()
    ...
    def on_baseline_method(self, event=None):
        ...
        method = constants.FitBaselineMethod.choices.keys()[index]
    ...
    def on_mmol_model(self, event):
        ...
        method = constants.FitMacromoleculeMethod.choices.keys()[index]
    ...
    def on_optimize_method(self, event): 
        index = event.GetEventObject().GetSelection()
        value = constants.FitOptimizeMethod.choices.keys()[index]
    ...
    def on_optimize_weights_method(self, event): 
        index = event.GetEventObject().GetSelection()
        value = constants.FitOptimizeWeightsMethod.choices.keys()[index]
    ...
    def on_output_results_to_html(self, event):
        ...
        if filename:
            ...
            data_source = raw.get_data_source(voxel[0])
    ...
    def on_dynamic_metabolite_list(self, event=None):
        ...
        width, height = self.PanelMetaboliteLines.GetSizeTuple()
...
#        self.list_lines = []
#
#        self.maxppm = util_ppm.pts2ppm(0, self.dataset)
#        self.minppm = util_ppm.pts2ppm(self._dataset.dims[0]-1, self.dataset)
#
...
#         pivot  = util_ppm.ppm2pts(data.tdfd.set.phase_1_pivot, data)
...
#     td      = 1.0/sw
#     piv     = util_ppm.ppm2pts(dataset.phase_1_pivot, dataset, acq=True)
...
#         frmm = a[na+1] - util_ppm.ppm2hz(mmpkppm[0],fitt_data)*2.0*np.pi
...
#         frmm = a[na+nmm:na+nmm*2] - util_ppm.ppm2hz(mmpkppm,fitt_data)*2.0*np.pi
...
#         fre = fr - util_ppm.ppm2hz(mpeaks,fitt_data)*2.0*np.pi    # in Radians here
