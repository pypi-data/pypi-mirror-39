...
class PlotXIds(common_menu.IdContainer):
    ...
    def enumerate_booleans(klass, menu):
        ...
        # I need to know the type of wx ids. It's probably int, but I don't
        # want to assume. I get it by checking the type of a random menu item.
        # Strangely, menus themselves don't seem to have ids.
        wx_id_type = type(menu.FindItemByPosition(0).Id)

        for attribute_name, value in klass.__dict__.iteritems():
            # Make sure this is one of my attributes.
            if attribute_name.isupper() and util_misc.is_iterable(value) and \
               isinstance(value[0], wx_id_type):
    ...
    def init_ids(klass):
        ...
        for key, value in klass.__dict__.iteritems():
            if key.isupper() and (value == "replace me"):
                ...
                value = tuple([wx.NewId() for i in range(4)])
...
class AnalysisMenuBar(common_menu.VespaMenuBar):
    ...
    def __init__(self, main, import_items):
        ...
        for module_name in import_items.keys():
            ...
            if 'separator' in module_name.lower():
                ...
                self.file_import_menu.AppendItem(menu_item)
...
#            ("Add Fitting Tab - Giso",  main.on_add_giso_tab, wx.ITEM_NORMAL),
    ...
    help = [
            ...
            ("&Analysis Help Online",   main.on_analysis_help_online),
