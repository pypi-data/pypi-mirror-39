...
class _Settings(object):
    ...
    def __init__(self,  attributes=None):
        ...
        self.optimize_limits_mmol_range_area    = 50.0          # % from start values
        self.optimize_limits_mmol_range_ppm     = 8.0           # Hz
...
class BlockFitVoigt(block_fit_identity.BlockFitIdentity):
    ...
    This object represents the settings and results involved in analyzing
    spectral data by fitting it to a data model which contains metabolite
    and baseline signal estimations.
    ...
    def __init__(self, attributes=None):
        ...
        """        
        block_fit_identity.BlockFitIdentity.__init__(self, attributes)
    ...
    @property
    def dims(self):
        """This is dimensions for fitting results, not data size. It's read only."""
        # Note that self.data.shape is a tuple. Dims must be a list.
        if self.fit_results is not None:
            return list(self.fit_results.shape)
        return None
    ...
    def __unicode__(self):
        lines = [ ]
        id_ = ("(%s)" % self.id) if self.id else ""
        lines.append("----------- BlockFitVoigt %s --------------" % id_)
    ...
    def set_dims(self, dataset):
        ...
        if self.dims is None or self.fit_baseline is None:
            ...
            result_dims[0] = self.calc_nparameters()
    ...
    def calc_nparameters(self):
        """ 
        Centralized calculation of number of parameters used in the fitting
        model. This keeps us consistent across use areas:
          
          nparam = area(nmet)+ppm(nmet)+phase0+phase1+ta+tb
          
        """
        nparam = len(self.set.prior_list)*2+4 
    ...
    def check_parameter_dimensions(self, dataset):
        ...
        """
        nparam = self.calc_nparameters() 
            
        if self.fit_results.shape[0] != nparam:
            self._reset_dimensional_data(dataset)
...
#     def attach_dataset_mmol(self, dataset):
#         ''' attaches the provided dataset as the input to ecc algorithm'''
# 
#         self.set.macromol_model                   = FitMacromoleculeMethod.SINGLE_BASIS_DATASET
#         self.set.macromol_single_basis_dataset    = dataset
#         self.set.macromol_single_basis_dataset_id = dataset.id  
#         self.set.macromol_single_basis_dataset_fname =     
        

    def deflate(self, flavor=Deflate.ETREE):
        if flavor == Deflate.ETREE:
            e = ElementTree.Element("block_fit_voigt", 
                                      { "id" : self.id,
    ...
    def results_as_csv(self, voxel, lw=0.0, lwmin=0.0, lwmax=0.0, source="", dsetname="", nzfill=2, decor1=False):
        """
        Given a voxel, linewidth params, and a data source (often a filename), 
    ...
    def results_in_table(self, voxel, lw=0.0, lwmin=0.0, lwmax=0.0, nozeros=False, noppm=False, 
                               fixphase=False, no_conf=False, places=5, pad=2, short_form=False):
        """
        Given a voxel, linewidth params, and a data source (often a filename), 
