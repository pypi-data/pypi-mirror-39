...
import dialog_mixed_metabolite_designer
import auto_gui.mixed_metabolite_output as mixed_metabolite_output
...
def _make_basis(vals, metabs_dict, npts, sw, apod, broad, field, 
               resppm, midppm, singlet_flag=False):
...
class DialogMixedMetaboliteOutput(mixed_metabolite_output.MyDialog):
    ...
    def __init__(self, parent, experiment, local=None, format=None):
        ...
        if not local:
            ...
            self.local.midppm        = local.midppm
    ...
    def on_ok(self,event):
        ...
        if msg:
            ...
            cmt.append("-" * 75)
            cmt += (unicode(self.experiment)).split('\n')
            cmt.append("")
        ...
        for i, dim in enumerate(self.experiment.dims):
            ...
            if dim == mrs_experiment.DEFAULT_LOOP:
                ...
                for j, value in enumerate(dim):
                    listctrl.InsertStringItem(j, str(j + 1).rjust(index_digits))

                    listctrl.SetStringItem(j, 1, (formatter % value).rjust(value_digits))
...
#            self.panel_output_location.Hide()
    ...
    def _do_output_lcmodel(self, all_lines, output_list):
        ...
        if self.experiment.isotope == "1H":
            ...
            midppm = common_constants.DEFAULT_PROTON_CENTER_PPM   
        else:
            # should be 0.0
            resppm = common_constants.DEFAULT_XNUCLEI_CENTER_PPM   
            midppm = common_constants.DEFAULT_XNUCLEI_CENTER_PPM           
        ...
        for vals in output_list:
            ...
            time = _make_basis(vals, metabs_dict, npts, sw, apod, 
                               broad, b0, resppm, midppm, singlet_flag)
    ...
    def _do_output_jmruitext(self, all_lines, output_list):
        ...
        for vals in output_list:
            ...
            time = _make_basis(vals, metabs_dict, npts, sw, apod, 
                               broad, b0/1e6, resppm, midppm, False)
...
class DynamicOutputList(object):
    ...
    def __init__(self, parent, metabolite_grid_sizer, metabolites, local):
        ...
        self.maxppm = util_ppm.pts2ppm(0, self.local)
#        self.minppm = util_ppm.pts2ppm(self.local.dims[0]-1, self.local)
        self.minppm = util_ppm.pts2ppm(self.local.spectral_dims[0]-1, self.local)
