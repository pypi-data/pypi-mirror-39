...
if wx_util.is_wx_floatspin_ok():
    from wx.lib.agw.floatspin import FS_LEFT, FS_RIGHT, FS_CENTRE, FS_READONLY
else:
    from vespa.common.wx_gravy.widgets.floatspin.floatspin import FS_LEFT, FS_RIGHT, FS_CENTRE, FS_READONLY
from vespa.common.wx_gravy.widgets.floatspin_multiplier.floatspin_multiplier import FloatSpinMultiplier


class EditFloatSpinMultiplier(edit_widget.EditWidget):
    ...
    def __init__(self, name, parent, id, label, sizer, pos,
        ...
        edit_widget.EditWidget.__init__(self, name, 'FloatSpinMultiplier', parent, id,
                                        label, sizer, pos,
    ...
    def create_widget(self):
        self.widget = FloatSpinMultiplier(self.parent.widget, self.id, style=self.style, agwStyle=self.extrastyle)
...
def builder(parent, sizer, pos, number=[1]):
    ...
    """
    label = edit_widget.increment_label('floatspin_multiplier', number)
    floatSpinWin = EditFloatSpinMultiplier(label, parent, wx.NewId(), misc._encode(label), sizer, pos, common.property_panel)
...
def xml_builder(attrs, parent, sizer, sizeritem, pos=None):
    ...
    floatSpinWin = EditFloatSpinMultiplier(label, parent, wx.NewId(), misc._encode(label), sizer, pos, common.property_panel, show=False)
...
def initialize():
    ...
    return edit_widget.initialize(
        'EditFloatSpinMultiplier', builder, xml_builder,
