...
class DistanceRuler(InteractiveWidget):
    ...
    def _update_text(self):
        ...
        self._text_visual.text = "{:.2f}".format(distance)
...
class AngleRuler(InteractiveWidget):
    ...
    def on_mouse_press(self, event, view):
        ...
        if set(event.modifiers) == {vispy.util.keys.CONTROL}:
            if event.button == 1:
                # Set start
                start = position - self.center
                scale = numpy.linalg.norm(start)
                if scale != 0:
                    self._start = start / scale
    ...
    def on_mouse_move(self, event, view):
        ...
        if set(event.modifiers) == {vispy.util.keys.CONTROL}:
            if event.button == 1:
                end = position - self.center
                scale = numpy.linalg.norm(end)
                if scale != 0:
                    self._end = end / scale
                self.update()
                return True
    ...
    def _update_text(self):
        angle = numpy.arccos(numpy.dot(self._start, self._end))
...
def get_selected_point(line, positions, threshold=0.1):
    n = positions.shape[0]
    if n == 0:
        return None
...
class Renderer(cepton_util.common.OptionsMixin):
    ...
    def on_key_press(self, event):
        if event.key is None:
            return False
        elif event.key == vispy.util.keys.SPACE:
            if self.is_running:
                self.pause_animation()
            else:
            ...
            pprint.pprint(self.get_camera_options())
            return True
        return False
