...
class Viewer(MainWindow):
    def __init__(self, renderer=None):
        ...
        self.window.setWindowTitle("Cepton Viewer")
    ...
    def create_toolbox_debug(self, parent_layout):
        ...
        # Angle ruler
        angle_ruler = QPushButton("Angle Ruler")
        layout.addRow(angle_ruler)
        angle_ruler.setCheckable(True)

        def on_angle_ruler(*args):
            self.renderer.angle_ruler_enabled = angle_ruler.isChecked()
            self.refresh()
        angle_ruler.clicked.connect(on_angle_ruler)

        # Distance ruler
        distance_ruler = QPushButton("Distance Ruler")
        layout.addRow(distance_ruler)
        distance_ruler.setCheckable(True)

        def on_distance_ruler(*args):
            self.renderer.distance_ruler_enabled = distance_ruler.isChecked()
            self.refresh()
        distance_ruler.clicked.connect(on_distance_ruler)
        ...
        def refresh():
            angle_ruler.setChecked(self.renderer.angle_ruler_enabled)
            distance_ruler.setChecked(self.renderer.distance_ruler_enabled)
