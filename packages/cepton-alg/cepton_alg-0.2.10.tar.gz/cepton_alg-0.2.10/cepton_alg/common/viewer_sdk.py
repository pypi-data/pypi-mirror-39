...
class Viewer(MainWindow):
    ...
    def load_player_settings(self, path):
        with open(path, "r") as f:
            player_settings = json.load(f)
        self.renderer.set_options(**player_settings.get("renderer", {}))
        self.refresh()

    def save_player_settings(self, path):
        player_settings = {
            "renderer": self.renderer.get_options(),
        }
        with open(path, "w") as f:
            json.dump(player_settings, f, indent=4, sort_keys=True)

    def open_replay(self, path):
        self.load_settings(os.path.dirname(path))
        player_settings_path = self.settings_dir.player_settings_path
        if player_settings_path is not None:
            self.load_player_settings(player_settings_path)
    ...
    def create_menu_file(self, menu):
        ...
        load_player_settings = menu.addAction("Load Player Settings")

        def on_load_player_settings():
            options = {
                "caption": "Load Player Settings",
                "filter": "JSON (*.json)",
            }
            path = QFileDialog.getOpenFileName(self.window, **options)[0]
            if not path:
                return
            self.load_player_settings(path)
        load_player_settings.triggered.connect(on_load_player_settings)

        # Save player settings
        save_player_settings = menu.addAction("Save Player Settings")

        def on_save_player_settings():
            if not self.settings_dir:
                warnings.warn("No settings directory!")
                return
            self.save_player_settings(
                self.settings_dir.default_player_settings_path)
        save_player_settings.triggered.connect(on_save_player_settings)

        # Save player settings as...
        save_player_settings_as = menu.addAction("Save Player Settings As...")

        def on_save_player_settings_as():
            options = {
                "caption": "Save Player Settings",
            ...
            self.save_player_settings(path)
        save_player_settings_as.triggered.connect(on_save_player_settings_as)

        def update():
            save_player_settings.setEnabled(bool(self.settings_dir))
    ...
    def create_toolbox(self):
        ...
        # TODO
        # if _enable_capture and sys.platform.startswith("linux"):
