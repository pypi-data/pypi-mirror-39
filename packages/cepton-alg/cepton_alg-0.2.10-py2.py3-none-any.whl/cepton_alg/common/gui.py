...
def wrap_widget(cls):
    class Wrapped(cls):
        dragEnter = pyqtSignal(QEvent)
        dragMove = pyqtSignal(QEvent)
        drop = pyqtSignal(QEvent)
...
class MainWindow(WindowBase):
    def __init__(self, **kwargs):
        self.window = wrap_widget(QMainWindow)()
        ...
        f11 = QShortcut(QKeySequence(Qt.Key_F11), self.window)
        f11.activated.connect(toggle_fullscreen)

        self.window.setAcceptDrops(True)
        self.window.dragEnter.connect(self.on_drag)
        self.window.dragMove.connect(self.on_drag)
        self.window.drop.connect(self.on_drop)
...
class Window(WindowBase):
    def __init__(self, **kwargs):
        self.window = wrap_widget(QWidget)()
