...
class RequestCurvesPrices(object):
    ...
    def __init__(self, chunk_increment=CHUNK_INCREMENT):
        ...
        self.df_top = None
        self.df_leg = None
        self.all_combination = True
        self.outputFormat = 'GroupByDate'
    ...
    def expand(self):
        ...
        for key in change_key:
            dic_in[change_key[key]] = dic_in[key]
            dic_in.pop(key)
        ...
        assert isinstance(self.all_combination, bool), """ all_combination should be a boolean """

        if self.all_combination:
            items = [(k, v)
        ...
        self.li_dic_api = self._split_input_by_dates(li_leg)
    ...
    def _chunk_dates(self, start_date, end_date):
        ...
        chunks_str = [(dt.datetime.strftime(start, fmt), dt.datetime.strftime(end, fmt))
    ...
    def call_api(self, api, debug=False):
        ...
        for k, dic_api in enumerate(self.li_dic_api):
            raw_response = api.post(self.url, payload=dic_api)
            if debug:
        ...
        response = ResponseCurvesPrice(li_raw_data=li_raw_response,
                                       obj_req=self)
