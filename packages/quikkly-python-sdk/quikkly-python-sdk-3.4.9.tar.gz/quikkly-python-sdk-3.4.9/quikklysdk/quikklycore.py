...
LIB_VERSION_MIN = (3, 4, 9)  # Minimum native libs version for this SDK
...
def init(native_libs_dir=None, blueprint_file=None):
    ...
    global PIPELINE
    global BLUEPRINT
    if FUNCS is not None and PIPELINE is not None or BLUEPRINT is not None:
        return
    ...
    try:
        BLUEPRINT = json.loads(blueprint.decode('utf-8'))
...
def release():
    ...
    global PIPELINE
    global BLUEPRINT
    if FUNCS is None or PIPELINE is None or BLUEPRINT is None:
...
def max_data_value(template_id):
    init()
    max_data = FUNCS['quikklycore.qc_max_data_value'](PIPELINE, template_id.encode('utf-8'))
    if 0 <= max_data < (2**64):
        return max_data
    else:
        raise QuikklyError('Template %s has invalid max_data_value %s' % (template_id, max_data))


def get_template_info():
    init()
    templates = copy.deepcopy(BLUEPRINT['templates'])
    for template in templates:
        template['max_data_value'] = max_data_value(template['identifier'])
        template['max_codes'] = template['max_data_value'] + 1
        template['data_bits'] = int(math.log(template['max_codes'], 2))  # Note that this rounds down, in case the template has non-integer data bits available.
    return templates
