...
def settle(world,obj,
	...
	Arguments:
	- world: a RobotModel describing the world
	- obj: a RigidObjectModel, RobotModelLink, or floating-base Robot that will
	  be settled.
	- forcedir: a vector parallel to the direction of force whose magnitude is the
	  maximum distance this procedure will try to move the object.
	- forcept: local coordinates of the center of force application.
	- settletol: the simulation will stop when two subsequent transforms lie within
	  this tolerance.
	- orientationDamping: a virtual spring will attempt to keep the initial orientation
	  with this torsional spring constant 
	- perturb: if nonzero, the application force will be perturbed at random by this
	  amount every step.  If equal to 1, this means the force is sampled from a 45
	  degree cone in the direction forcedir.
	- margin: the collision detection margin used in simulation.  If None, uses the
	  Simulator default.  Otherwise, overrides the default.  Must be at least settletol.
	- debug: if True, uses the visualization to debug the settling process

	Return value: (transform,touched)
	- transform: The resulting se3 transform of the object, or None if the object
	  didn't hit anything by the time it translated by ||forcedir|| units
	- touched: a dictionary whose keys are object IDs touched by the object at the final
	  transform, and whose values are lists of ContactPoints (see klampt.model.contact)
