...
def _supportsNameLabels():
    return _checkVersion('10.2a13')


def _supportsLineFeeding():
    return _checkVersion('10.2a65')
...
def oeTurbine(BladeIndex, ArcLength=0.0):
    result = oeLine(LinePoint=ptArcLength, ArcLength=ArcLength)
    result.BladeIndex = BladeIndex
    return result


def oeTurbineEndA(BladeIndex):
    result = oeLine(LinePoint=ptEndA)
    result.BladeIndex = BladeIndex
    return result


def oeTurbineEndB(BladeIndex):
    result = oeLine(LinePoint=ptEndB)
    result.BladeIndex = BladeIndex
    return result
...
class VesselTypeDataImportSpecification(PackedStructure):
    ...
    _fields_ = [
        ...
        ('ImportMassInertia', ctypes.wintypes.BOOL),
...
class HelperMethods(object):
    ...
    @staticmethod
    def GetBooleanModelProperty(handle, propertyId):
...
class OrcaFlexObject(DataObject):
    ...
    def isBuiltInAttribute(self, name):
        return name in ('modelHandle', 'type', 'groupParent') or DataObject.isBuiltInAttribute(self, name)
    ...
    def SaveSummaryResults(self, filename):
        HelperMethods.SaveSpreadsheet(self.handle, sptSummaryResults, filename, None)
...
class OrcaFlexLineObject(OrcaFlexObject):
    ...
    def RangeGraphXaxis(self, varName, arclengthRange=None, period=None):
        ...
        if arclengthRange is None:  # support 9.1 and earlier
            pointCount = _GetRangeGraphNumOfPoints(self.handle, varID, status)
        else:
            pointCount = _GetRangeGraphNumOfPoints2(self.handle, arclengthRange, varID, status)
        _CheckStatus(status)
        x = _allocateArray(pointCount)

        if period is None:
            if HelperMethods.IsLinePayoutNonZero(self.handle):
                raise ValueError('must specify period when line payout is non-zero')
            if HelperMethods.GetModelState(self.modelHandle) == ModelState.InStaticState.Value \
                    or HelperMethods.IsFrequencyDomainDynamics(self.modelHandle):
                period = Period(pnStaticState)
            else:
                simStart = HelperMethods.GetSimulationStartTime(self.modelHandle)
...
class FatigueAnalysis(DataObject):
    ...
    def isBuiltInAttribute(self, name):
        return name in ('progressHandler', 'threadCount') or DataObject.isBuiltInAttribute(self, name)
