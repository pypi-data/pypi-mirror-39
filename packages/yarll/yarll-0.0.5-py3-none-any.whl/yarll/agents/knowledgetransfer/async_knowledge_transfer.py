...
class AKTThread(Thread):
    ...
    def __init__(self, master, env, task_id, n_iter, start_at_iter=0):
        ...
        self.writer = tf.summary.FileWriter(os.path.join(self.master.monitor_path, "task" + str(self.task_id)),
    ...
    def build_networks(self):
        with tf.variable_scope("task{}".format(self.task_id)):
            ...
            self.probs = tf.nn.softmax(tf.matmul(self.master.L1,
    ...
    def learn_Karpathy(self):
        ...
        iteration = self.start_at_iter
        # Keep executing episodes until the master requests a stop (e.g. using SIGINT)
        while iteration < self.n_iter and not self.master.stop_requested:
            iteration += 1
    ...
    def build_networks(self):
        with tf.variable_scope("shared"):
            ...
            if self.config["feature_extraction"]:
                ...
                self.L1 = self.states
            self.knowledge_base = tf.Variable(
                tf.truncated_normal(
