...
def filter_offline_shows(queryset, is_logged_in):
    ...
    # Non-Admin users may only see objects with publish_status == 0 whose live_as_of date exists and is in the past
    if not is_logged_in:
...
def filter_offline_seasons(queryset, is_logged_in):
    ...
    (Unless you're logged into the Admin)
    """
    # Rule 1: both my parental Season and my grand-parental Show may not have publish_status < 0
