...
class AzureContext(BackendContext, CloudConnector):
    ...
    def _authorize_app(self, service_principal):
        ...
        role_defs = list(auth_client.role_definitions.list(
            scope,
            filter='roleName eq \'MissingLinkRM\''
        ))
        if not role_defs:
            role_id = uuid.uuid4()
        else:
        ...
        self._create_role_if_missing(auth_client, role_definition_id, roles, service_principal, scope)
        click.echo('Done')

    def _create_role_if_missing(self, auth_client, role_id, roles, service_principal, scope):
        role = [r for r in roles if r.role_definition_id == role_id]
        if not role:
            auth_client.role_assignments.create(
                scope,
                uuid.uuid4(),
                RoleAssignmentCreateParameters(role_definition_id=role_id, principal_id=service_principal.object_id)
            )
