...
class DockerTools(object):
    ...
    def validate_local_config(self, org, force, ssh_key_path, token, capacity):
        ...
        self.setup_rms_volume(ssh_key=ssh_key, token=token, ml_data=ml_data, prefix=prefix, force=True, capacity=capacity)
    ...
    def _apply_config_to_volume(self, ssh_key=None, token=None, prefix=None, ml_data=None, env=None, capacity=None):
    ...
    def setup_rms_volume(self, ssh_key=None, token=None, prefix=None, ml_data=None, env=None, force=False, capacity=1):
        click.echo('building volume')
        self.ensure_rms_volume(force)
        self._apply_config_to_volume(ssh_key, token, prefix=prefix, ml_data=ml_data, env=env, capacity=capacity)
    ...
    def _get_gpus(self):
        ...
        self.pull_image_from_image(self.CUDA_BASE_IAMGE, 'Validating GPU configuration')
        ...
        gpus = len(smi_output.decode('utf-8').split('\n'))
        return gpus
