...
class InfrastructureFactory(Base):
    ...
    def get_infrastructure(self, hostname: str, infrastructure_name: str):
        ...
        if not hostname:
            hostname = "none"
        try:
            return self.local_storage.infra[hostname][infrastructure_name]
        except KeyError:
            pass

        infra = self.get_infra_from_infrastructure_factory(
            hostname=hostname, infrastructure_name=infrastructure_name
        )
        if hostname in self.local_storage.infra:
            self.local_storage.infra[hostname][infrastructure_name] = infra
        else:
            self.local_storage.infra[hostname] = {infrastructure_name: infra}
        return infra

    def get_infra_from_infrastructure_factory(
        self, hostname: str, infrastructure_name: str
    ):
        """Retrieve an infrastructure instance for the selected instance.
    ...
    def flush_local_storage(self):
        ...
        """
        for hostname in self.local_storage.infra:
            for (infrastrucuture_name, infra) in self.local_storage.infra[
                hostname
            ].items():
                try:
                    self.registered_infrastructure[
                        infrastrucuture_name
                    ].clean_up(infra)
