...
def jcode_of(model_name, model_dict):
    ...
    for name, value in model_dict.items():
        if name == '//':
            model_comment = value
            continue
        assert is_comment_format(value), "invalid value: " + value
...
def jcontent_of(model_name, model_dict):
    """
    >>> output_dict = {'//': 'comment', 'aa': '+bool=1//', 'bb': 'datetime="true"//n#abc', 'cc': 'long=22 //hehe'}
    >>> print(jcontent_of('User', output_dict))
    @ParsecNotDBColumn
    Boolean aa;  //
    ...
    return jclass_tpl.format(sql_head=sql_head, model_name=model_name, jcode=jcode, model_comment=model_comment
                             ).replace('<(', '{').replace(')>', '}')


def process(infile, outpath, replace=True):
    ...
    for model_name, model_dict in model_data.items():
        ...
        if replace:
            if os.path.isfile(filename):
                ...
                with open(filename, 'w') as f:
                    f.write(jcontent_of(model_name, model_dict))
        else:
            if os.path.isfile(filename):
                head, jcode, tail = split_jclass(open(filename).read())
