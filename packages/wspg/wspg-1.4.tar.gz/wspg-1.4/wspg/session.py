...
class Session:
    ...
    async def _execute_sql(self, sql: psycopg2.sql.Composable, fetch_result: bool = True) -> Optional[JsonLiteral]:
        try:
            ...
            raise ClientLeakableError(db_msg, {'postgresql': diagnostics}) from e
...
class SessionLoggerAdapter(logging.LoggerAdapter):
    def process(self, msg, kwargs):  # type: ignore
        ...
        websocket = self.extra['session'].websocket
        if len(websocket.remote_address) == 4:
            # IPv6
            remote_address = "[{0}]:{1}".format(*websocket.remote_address)
        else:
            remote_address = "{0}:{1}".format(*websocket.remote_address)

        xff = websocket.request_headers.get("X-Forwarded-For")
        if xff:
            remote_address = "{xff} via {remote_address}".format(xff=xff, remote_address=remote_address)
