 ...
 * "schema" is optional - the default value is no schema
...
A consistent result ordering is required when sliced selects are used, so if
the id column isn't specified in the "order" list, it will be appended
ascending.  Because of this, if further slices are expected to be loaded, the
initial select must specify a slice with no (or null) "relative-to".
