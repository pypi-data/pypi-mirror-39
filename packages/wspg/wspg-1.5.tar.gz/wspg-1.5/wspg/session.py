...
class Session:
    ...
    async def _select(self, relation_sql: psycopg2.sql.Composable, query_rules: Optional[QueryRulesRuleSetParser], returning_columns: Optional[List[str]], order: Optional[List[Dict[str, str]]], limit: Optional[int], _slice: Optional[Dict]) -> Optional[JsonLiteral]:
        ...
        if _slice:
            ...
            if 'id' not in [item['column'] for item in order]:
                order.append({'column': "id", 'direction': "ascending"})
    ...
    async def handle_get_canonical_representations(self, wspg_msg: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async def _get_canonical_representations(self, types, value):
        ...
        async with self.endpoint.db_pool.acquire() as connection:
            async with connection.cursor() as cursor:
                for _type in types:
                    type_literal = psycopg2.sql.Literal(_type)
                    type_name_query = psycopg2.sql.SQL("SELECT CAST({} AS regtype)").format(type_literal)

                    try:
                        await cursor.execute(type_name_query)
                    except psycopg2.Error as e:
                        # These errors are things like 'unknown type' which it
                        # seems sensible to report to the caller.
                        raise client_leakable_error_from_psycopg2_error(e) from e

                    type_name_sql = psycopg2.sql.SQL((await cursor.fetchone())[0])
                    cast_query = psycopg2.sql.SQL("SELECT CAST({} AS {})").format(value_literal, type_name_sql)

                    try:
                        await cursor.execute(cast_query)
                    except psycopg2.DataError as e:
                        # These errors are 'invalid input format' which is fine
                        # to ignore
                        continue
                    except psycopg2.Error as e:
                        raise client_leakable_error_from_psycopg2_error(e) from e

                    result[_type] = (await cursor.fetchone())[0]
...
def verify_schema_is_acceptable(schema, endpoint):
    if endpoint.acceptable_schemas:
