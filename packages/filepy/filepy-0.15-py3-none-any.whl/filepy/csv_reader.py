    ...
    def _analyse(self):
        for index, line in enumerate(open(self._path_to_file)):
            if index == 0 and self._first_line_column_names:
                self._column_names = re.compile(self._delimiter).split(line.strip())
                continue
            if self._skip_first_column:
                self._data.append(re.compile(self._delimiter).split(line.strip())[1:])
            else:
                self._data.append(re.compile(self._delimiter).split(line.strip()))
