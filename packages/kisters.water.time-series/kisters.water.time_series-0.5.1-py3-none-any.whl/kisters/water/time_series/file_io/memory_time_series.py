...
class MemoryTimeSeries(TimeSeriesCutRangeMixin, TimeSeriesAttributesMixin, TimeSeriesItemMixin):
    ...
    def __validate_instance(cls, metadata: Mapping[str, Any], data: pd.DataFrame, comments: pd.DataFrame) -> str:
        if ('tsPath' not in metadata or 'id' not in metadata
                or 'name' not in metadata or 'shortName' not in metadata):
            return "Metadata provided doesn't have one of the following keys: 'tsPath', 'id', 'name' or 'shortName'"
