...
def ScreenNonlinearity( x, gamma='sRGB' ):
	...
	"""
	cls = float if isinstance( x, ( int, float ) ) else None
	if numpy: value = numpy.array( x, dtype='float64', copy=True )
	elif isinstance( x, ( tuple, list ) ): return [ ScreenNonlinearity( xi, gamma=gamma ) for xi in x ]
	else: value = float( x )
...
def Linearize( Y, gamma='sRGB' ):
	...
	"""
	cls = float if isinstance( Y, ( int, float ) ) else None
	if numpy: value = numpy.array( Y, dtype='float64', copy=True )
	elif isinstance( Y, ( tuple, list ) ): return [ Linearize( Yi, gamma=gamma ) for Yi in Y ]
	else: value = float( Y )
	if gamma is 'sRGB' or gamma <= 0:  # NB: looks very similar to a gamma of 2.2, even though the exponent it uses is 2.4
		if numpy:
...
def LoadLUT( source, DACbits=8 ):
	...
	dtype = 'uint8' if DACbits <= 8 else 'uint16'
