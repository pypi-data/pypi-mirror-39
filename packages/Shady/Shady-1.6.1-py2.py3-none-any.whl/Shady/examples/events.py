...
if __name__ == '__main__':
	...
	* ShaDyLib does not issue an event of type 'text' when you
	...
	try: _SHADY_CONSOLE_INTERACT
		...
		"""
		As a final note before we turn on the flood of event
		information: if you ever want to remove a handler from
		a particular slot, you just set the handler to `None`
		in that slot:
		"""#:
	...
	handler, in slot -3 so that it runs before the others.
	
	All the new handler really does is print the event instance.
	However, we'll add some custom spacing to the output to make it
	easier to read. We'll also add a new field to the event that
	gives the current frame number in Shady.
	
	Then you can manipulate the mouse and keyboard, and observe what
	happens: 
	"""#:
