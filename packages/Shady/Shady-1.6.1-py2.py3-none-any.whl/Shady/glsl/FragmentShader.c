...
vec4 Texture_Pixels_XY( sampler2D texture, vec2 textureSize, int textureChannels, vec2 coords ) // [0,0] is bottom left;  [1, 0] is next pixel horizontally
{
	...
	vec4 pixel = texture2D( texture, coords / textureSize );
...
vec4 Texture_Pixels_IJ( sampler2D texture, vec2 textureSize, int textureChannels, vec2 coords ) // [0,0] is top left; [0, 1] is next pixel horizontally
{
	coords = mod( coords.yx, textureSize ); // mod necessary because of the different coordinate convention
	vec4 pixel = texture2D( texture, ( coords + vec2( 0.5, 0.5 ) ) / textureSize ); // offset of 0.5 to reach the pixel center, because texture2D thinks of whole-number coordinates as matching gridlines between pixels
	if(      textureChannels == 1 ) { pixel.rgb = pixel.rrr; pixel.a = 1.0; } // because GL_LUMINANCE is deprecated (and causes float texture pixel values to be stored as ints, when used)
	else if( textureChannels == 2 ) { pixel.rgba = pixel.rrrg; }        // because GL_LUMINANCE_ALPHA is deprecated (and causes float texture pixel values to be stored as ints, when used)
	return pixel;
}
...
float SinewaveSignal( vec2 coord )
{
	...
	return uSignalParameters[ 0 ] * sinusoid( coord, uSignalParameters[ 1 ], uSignalParameters[ 2 ], uSignalParameters[ 3 ] );
}
...
void main(void)
{
	vec2 coord2D = ( uCarrierTransformation * vFragCoordinateInEnvelope ).xy;
...
//#SIGNAL_FUNCTION_CALLS
	...
	vec2 v = 2.0 * vFragCoordinateInEnvelope.xy - uEnvelopeSize;
...
//#MODULATION_FUNCTION_CALLS
	...
	bool useNoisyBit =  uDitheringDenominator > 0.0  && !useLUT;
	...
	const float up = 1.0 + 1.97e-3; // OpenGL is about to round everything to 8-bit RGB anyway, so it makes no difference
	                                // if we're off by anything up to half a grey-level (0.5/255 = 1.96e-3)
