...
from . import Console; from .Console import GetIPython, ThreadedShell, Interactive
...
def AdjustGamma( world, finish=None, target=None, xBlue=True ):
	...
	def gamma_adjuster( self, event ):
		...
		elif event.type == 'key_press' and event.key in [ 'down', 'up', 'left', 'right' ]:
			delta = 0.01
		...
		# old style: absolute position; changes regardless of mouse button 
		#if event.type == 'mouse_motion':
		#	x, y = RelativeLocation( event, target, anchor=0, normalized=True )
		#	if abs( x ) <= 1 and abs( y ) <= 1:
		#		# x and y range from -1 to +1
		#		target.gamma = 1.5 + y
		#		if xBlue: target.bluegamma *= 1.0 + 0.5 * x   
...
def PixelRuler( base, steps=((10,(0,0,1)), (100,(1,0,0)), (1000,(0,1,0))) ):
	"""
	...
	Draw grid lines over the base: the 1st pixel (pixel 0) is not colored.
	The 10th, 20th, 30th,... pixels (i.e. pixels 9, 19, 29) are blue.
	Similarly every 100th pixel is red, and every 1000th pixel is green.
	
	If `base` is a `World` instance, render the pattern at a depth of `z=0.9`
	and return the corresponding `Stimulus` instance. Otherwise just return
	the texture as a `numpy` array.
	"""
	world = base if hasattr( base, '_isShadyObject' ) else None
	if world: base = world.size
	try: AsSize( base )
	except: img = numpy.array( base, copy=True )
	else: img = CheckPattern( base, 0.9, meanZero=False ) # TODO
	...
	for step, color in sorted( dict( steps ).items() ):
		for pos in range( step - 1, img.shape[ 0 ], step ):
			...
			img[ pos, :, 3:4 ] = 1.0 # alpha
		for pos in range( step - 1, img.shape[ 1 ], step ):
			for channel, value in enumerate( color ): img[ :, pos, channel ] = value
			img[ :, pos, 3:4 ] = 1.0 # alpha
	if world: return world.Stimulus( img, name='pixelruler', z=0.9, anchor=world.anchor )
	else: return img
...
def FrameIntervalGauge( world, corner=( -1, +1 ), thickness=49, variable='width', color=( 0, 0, 0 ) ):
	"""
	...
	def Length( arg=None ):
		...
		if msec is None: return 1
		fr = world.framesCompleted
		latest = msec[ fr % msec.size ]
		previous = msec[ ( fr - 1 ) % msec.size ]
		value = ( latest - previous )
		#if arg == 'ceil': value = math.ceil( value / 40.0 ) * 40.0
		return value * 10.0 if value == value else 1
	dims = [ 1000, thickness ]
	if variable == 'height': dims = dims[ ::-1 ]
	fgColor = color
	bgColor = [ 1, 1, 1 ]
	base = numpy.ones( dims[ ::-1 ] + [ 3 ], dtype=float )
	bgTexture = PixelRuler( base * [ [ bgColor ] ], steps={ 10:[0,0,1], 100:[1,0,0], 1000:[0,1,0] } )
	fgTexture = PixelRuler( base * [ [ fgColor ] ], steps={ 10:[0,0,1], 100:[1,0,0], 1000:[0,1,0] } )
	def Make():
		ruler = world.Stimulus( bgTexture, name='gauge%02d', debugTiming=False ).Set(
			position = world.Place( corner ),
			anchor = corner,
			#alpha = 0.5,
			z = -0.999,
			**{ variable : 500 }
			#**{ variable : lambda t: Length( 'ceil' ) }
		)
		bar = world.Stimulus( fgTexture, name='gauge%02d', debugTiming=False ).Set(
			alpha=1.0, position=ruler, anchor=ruler, z=ruler, visible=ruler,
			**{ variable : Length }
		)
		return ruler, bar
	return world.RunDeferred( Make )
...
def Loupe( target, update_period=1.0, **kwargs ):
	...
	*unlinearized*.  At extreme values, color-contrast-enhancement may
		...
		if triggered:
			...
			if target:
				if self.textureSlotNumber < 0 or self.textureID < 0 or list( target.size ) != list( self.textureSize ):
					self.LoadTexture( target.Capture() ) # bit of a hack but by far the easiest way of initializing everything right
				target.CaptureToTexture( self.textureID )
				if target.lut: self.bg = ApplyLUT( target.bgred, target.lut.values ) / enhanced.dacMax
	...
	enhanced.DeferredUpdate = lambda: setattr( enhanced, 'update_now', 'defer' ) # TODO: circular self-reference -> immortality
	return enhanced
...
def AutoFinish( world, shell=False, prefer_ipython=True, plot='auto' ):
	...
	if world._thread_hijacker:
		queue = world._thread_hijacker.Queue
		if plot: queue( PlottingPayload, wait='block' )
	else:
...
def RunShadyScript( *argv, **kwargs ):
	...
	Options are as follows:
		...
		`--script` (or positional argument 0)
			The name of a Python script to run. If your script argument does not is
			not a valid (absolute or relative) path to an existing file, Shady will
			make a second attempt to find the named script inside the `examples`
	...
	from .Console import SHELL_THREAD, ThreadedShell, RunScript, MainLoop
	...
	interaction = cmdline.Option( 'console', 'IPython', type=str, values=( 'None', 'Python', 'IPython' ) ) 
