...
class ClassWithManagedProperties( object ):
	...
	def SetDynamic( self, name, func, order=-1, canonicalized=False ):
		...
		if not canonicalized:
			try: name = self.GetPropertyDescriptor( name ).names[ 0 ]
			except: pass
		if func:
			if dynamics is None: dynamics = self._dynamics = {}
			dynamics[ name ] = ( order, name, func )
	...
	def ShareProperties( self, *pargs, **kwargs ):
		...
		others = [ arg3        for arg1 in pargs for arg2 in ( arg1 if isseq( arg1 ) else [ arg1 ] ) for arg3 in ( arg2.replace( ',', ' ' ).split() if hasattr( arg2, 'split' ) else [ arg2 ] ) if not isinstance( arg3, str ) and arg3 is not self ]
		props  = { arg3 : None for arg1 in pargs for arg2 in ( arg1 if isseq( arg1 ) else [ arg1 ] ) for arg3 in ( arg2.replace( ',', ' ' ).split() if hasattr( arg2, 'split' ) else [ arg2 ] ) if     isinstance( arg3, str ) }
		props.update( kwargs )
