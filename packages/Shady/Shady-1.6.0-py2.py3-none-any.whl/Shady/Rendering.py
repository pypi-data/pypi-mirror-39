...
__all__ = [
	...
	# constants
	'SINEWAVE_SIG', 'SINEWAVE_MOD', 'HANN_WIN',  # legacy names
	'SIGFUNC', 'MODFUNC', 'WINFUNC', # namespaces that contain new versions of the above
	'DRAWMODE',
	'LOCATION',
]
...
# legacy names
SINEWAVE_SIG = 1  # index of the one built-in signal function in the shader (SinewaveSignal)
SINEWAVE_MOD = 1  # index of the one built-in modulation function in the shader (SinewaveModulation)
HANN_WIN     = 1  # index of the one built-in windowing function in the shader (Hann)
...
class LinkGL( ClassWithManagedProperties ):
	...
	def SetAnimationCallback( self, callback ):
		...
		return self
	SetAnimateMethod = SetAnimationCallback # legacy name
...
class World( LinkGL ):
	...
	_doc_cut = """
		openglContextVersion:
			Supply 330 to get an OpenGL 3.3 context (supported by the windowing
			backend in the accelerator, but not yet by the rest of the Shady
			infrastructure). Leave at 0 to fall back to the default legacy context
			version (OpenGL 1.3, GLSL 1.2).
		backend:
	...
	def __init__( self, width=None, height=None, left=None, top=None, screen=None, threaded=True, canvas=False, frame=False, fullScreenMode=None, visible=True, openglContextVersion=0, backend=None, acceleration=None, debugTiming=False, profile=False, logfile=None, window=None, **kwargs ):
	...
	def _Construct( self, width, height, window, left, top, screen, canvas, frame, fullScreenMode, visible, openglContextVersion, debugTiming, profile, **kwargs ):
		...
		if ShaDyLib:
			...
			self.ditheringDenominator = dacMax = PyEngine.QueryDACMax()
			for prop in [ World.ditheringDenominator, Stimulus.ditheringDenominator ]:
				if not prop.default_changed: prop.SetDefault( dacMax )
		...
		if self.logger: self.logger.Log( world_size=tuple( self.size ) )
	...
	lookupTableTextureID = slot = ManagedProperty( -1 )            #
	...
	ditheringDenominator = ManagedProperty(
		default = 0.0,
	...
	def Capture( self, pil=False, fullscreen=False, saveas='', size=None, origin=None ):
		...
		if fullscreen:
			...
			else: Announce( 'numpy unavailable - returning PIL image' )
		else:
			...
			if not pil and not numpy:
				if Image: Announce( 'numpy unavailable - returning PIL image' ); pil = True
				else:     Announce( 'numpy unavailable - returning raw bytes' )
			if pil or saveas:
...
class Stimulus( LinkGL ):
	...
	penThickness = ManagedProperty(
		default = 5.0,
		doc = """
			This is a scalar floating-point value that determines the thickness, in pixels,
			lines drawn when you set `.drawMode` to `DRAWMODE.LINES` or `DRAWMODE.LINE_STRIP`.
		""",
	)
	smoothing = ManagedProperty(
		default = 1,
		doc = """
			This is a boolean value that determines whether lines, dots and polygons should
			be drawn with or without smoothing, when `.drawMode` is set to something other
			than the default `DRAWMODE.QUAD`.  The results are unfortunately unpredictable
	...
	def _DrawShapes( self, mode ):  # NB: never called if there's a DLL
		...
		PyEngine.DrawShapes( mode_lookup[ mode ], self.envelopeSize, self.penThickness, self.smoothing, self.nPoints, self.points )
	...
	def _DoAnchorTranslation( self, xp, yp ):  # NB: never called if there's a DLL
		...
		PyEngine.GL.glTranslatef( PartialTranslation( -width, xp ) + ox, PartialTranslation( -height, yp ) + oy, -self.z - oz )
	...
	def Capture( self, pil=False, saveas='' ):
		...
		bounding box.  Therefore, two caveats:  (1) if other stimuli overlap this one,
		the capture will contain image data composited from all of them; (2) the bounding-
		box method is smart enough to compensate for `.envelopeTranslation` and
		`.envelopeScaling`, but not `.envelopeRotation`.
