...
# 
# Copyright (c) 2017-2018 Jeremy Hill, Scott Mooney
# 
...
class Renderer( object ):
	def __init__( self, shaderProgram=None, ptr=None ):
		self._ptr = ptr
		self.__needs_cleanup = False
		if ptr is None and shaderProgram is not None:
			self._ptr = dll.ShaDyLib_Renderer_New( shaderProgram )
...
class Window( object ):
	def __init__( self, width=None, height=None, left=0, top=0, screen=None, frame=False, fullScreenMode=False, visible=True, openglContextVersion=0, glslDirectory="", substitutions="", ptr=None ):
		...
		if ptr is None:
			...
			self._ptr = dll.ShaDyLib_Window_New( width, height, left, top, int( screen ), frame, fullScreenMode, visible, openglContextVersion, IfStringThenRawString( glslDirectory ), IfStringThenRawString( substitutions ) )
			err = dll.ShaDyLib_ClearError()
			if err: raise( ShaDyLibException( IfStringThenNormalString( err ) ) )
			self.__needs_cleanup = True
	def __del__( self ):
...
def LoadSharedLibrary( dllname = None ):
	...
	if dllname is None:
		...
		if platform.architecture()[ 0 ].startswith( '32' ): arch = 'i386'  # NB: we override platform.machine() here because for example, on 32-bit Python for 64-bit Windows, platform.machine() can return 'AMD64'
		if arch == 'AMD64': arch = 'x86_64'
		if   arch == 'i386':   arch = '32bit'
		elif arch == 'x86_64': arch = '64bit'
