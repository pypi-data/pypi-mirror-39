    ...
    def loadDll():
        ...
        if os.name == 'posix':
            if sys.platform == 'mac' or sys.platform == 'darwin':
                ...
                cDLLName = 'libISAgentSDKC.dylib'
            else:
                cDLLPath = os.path.join(cDLLPath,'lib')
                cDLLName = 'libISAgentSDKC.so'
        else:
            if sys.maxsize > 2**32:
                cDLLPath = os.path.join(cDLLPath,'lib\\x64')
            else:
                cDLLPath = os.path.join(cDLLPath,'lib\\win32')
            cDLLName = 'ISAgentSDKC.dll'
        ...
        if os.path.isfile(cDLLFilePath):
            ...
            for d in sys.path:
                ...
                if os.path.isfile(cDLLFilePath):
                    ...
                    break
        if not cLib:
            raise Exception('Could not find the native C Ionic library, please verify your Ionic Python module installation.')
        return cLib

# load our Ionic SDK C library
cLib = IonicDll.loadDll()
