...
def exec_scl(pyspectre_obj, cmd, block=True):
    if pyspectre_obj.spectre is None or pyspectre_obj.fresh:
        ...
        try:
            ...
            if isinstance(pyspectre_obj.spectre, REPLWrapper):
                result = pyspectre_obj.spectre.run_command(cmd).strip('\r\"')
            elif isinstance(pyspectre_obj.spectre, REPLWrapper_client):
                result = pyspectre_obj.spectre.run_command(cmd, block=block).strip('\r\"')
...
def setattribute(pyspectre_obj, obj, attrname, attrval, block=True):
    ...
    if attrval == 'nil' or attrval == 't':
        result = exec_scl(pyspectre_obj,'sclSetAttribute( \"{}\" \"{}\" {} )'.format(obj, attrname, attrval), block=block)
    else:
        result = exec_scl(pyspectre_obj,'sclSetAttribute( \"{}\" \"{}\" \"{}\" )'.format(obj, attrname, attrval), block=block)
    if result != "t":
        raise LookupError('failed to set attribute: {}::{}::{}::{} \n'
                          '     make sure it appears in netlist file!'.format(pyspectre_obj.analysis_name,obj, attrname, attrval))
    return
