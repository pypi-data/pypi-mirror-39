...
class AnalysisContext:
    ...
    def set_qc_status(self, status=QCStatus.PASS, comments=''):
        """
        ...
        self.__comm.send_request("projectset_manager/set_qa_status", data_to_send)
    ...
    def get_files(
            self, input_id, modality=None, tags=None, reg_expression=None,
            file_filter_condition_name=None, subject_container_id=None
    ):
        """
        ...
        if not have_container_data:
            ...
            res = self.__comm.send_request("file_manager/get_container_files", {
                    ...
                    if find_file_data:
                        ...
                        if _should_include_file(
                            ...
                            selected_files.append(File(
                                container_id, fname, fmetadata, ftags, self.__comm
                            ))
    ...
    def new_scan_session(self, ssid=None, date_at_scan=None, age_at_scan=None, backup_age=None,
    ...
    def upload_file(
            ...
            protocol=None, container_id=None
    ):
        """
        ...
        if total_bytes > 0:
            logger.debug('Uploading file {} with size {} bytes and session ID {}'.format(
                source_file_path, total_bytes, session_id
            ))
        else:
            logger.warning('Skipping empty file {} upload'.format(
                source_file_path
            ))
            return
    ...
    def set_metadata_value(self, key, value, title=None, readonly=False):
        ...
        if not self._get_metadata_value(key):
            ...
            allowed_types = {
                int: "integer", list: "list", str: "string", float: "decimal"
            }
...
class Container:
    ...
    def _set_metadata_value(self, key, value):
        return self.__context._set_metadata_value(key, value,
