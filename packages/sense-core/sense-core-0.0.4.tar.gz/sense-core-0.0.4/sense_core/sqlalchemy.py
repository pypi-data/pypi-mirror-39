...
def get_sqlalchemy_session(label, database):
    global __session_map
    label1 = label + database
    if label1 in __session_map:
        return __session_map[label1]
...
def close_sqlalchemy_session(label,database):
    label1 = label + database
    global __session_map
    if label in __session_map:
        try:
            __session_map[label1].close()
        finally:
            del __session_map[label1]
