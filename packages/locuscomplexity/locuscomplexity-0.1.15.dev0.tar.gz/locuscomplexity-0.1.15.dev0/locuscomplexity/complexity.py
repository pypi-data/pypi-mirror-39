...
def build_m(df_data, dimension1, year, binary=True):
    """
    ...
    df_cp = df_nat.groupby(by=[dimension1, 'YEAR']).sum()
    ...
    #Compute RCA
    df = df_num.div(df_denum).reset_index().drop('YEAR', 1).set_index(dimension1)
...
def complexity_indices(df_data, dimension1, year):
    """
    ...
    """
    m = build_m(df_data, dimension1,year)
    Mc, Mp = m_dn_un(m)
