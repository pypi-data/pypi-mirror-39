...
def iter_fitness(n, m):
    ...
    :return: (Series) Fitness vector where [i] is the fitness of instance i
    """
    if n == 0:
...
def iter_complexity(n, m):
    ...
    :param n: (int) number of iteration
    :param m: (Dataframe) Matrix m
    :return: (Series) Complexity vector where [i] is the complexity of function i
    """
    if n == 0:
...
def fitness(m, dimension1, n=N_ITERATION):
    ...
    :return: (Dataframe) Fitness vector with <dimension1> and 'f' as columns
    """
...
def outlook(m, df_complexity):
    ...
    :return: (Dataframe) Outlook vector with <dimension1> as index and 'opportunity_value' column
    """
    m = m.loc[~(m == 0).all(axis=1)]
...
def gain(m, df_complexity):
    ...
    :param m: (Dataframe) M binary RCA matrix
    :param df_complexity: (Dataframe) Functional Complexity vector, returned by complexity
    :return: (Dataframe) Opportunity gain matrix with <function_name> as index and <dimension1> as columns
    """
    m = m.loc[~(m == 0).all(axis=1)]
