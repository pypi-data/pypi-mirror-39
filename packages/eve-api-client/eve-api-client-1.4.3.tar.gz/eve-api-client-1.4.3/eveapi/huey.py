...
def log_data_age(r):
    ...
    if 'Last-Modified' in r.headers:
        time = datetime.datetime.now() - datetime.datetime.strptime(r.headers['Last-Modified'], '%a, %d %b %Y %H:%M:%S %Z')
...
def esi(method, url, headers, params, data, fetch_pages, auth_callback, cache, page=False):
    """
    ESI call method.
    ...
    if delay and delay > 0:
        ...
        return error_limit_retry(auth_callback, data, fetch_pages, headers, method, page, params, url, delay=delay)
    ...
    if 200 <= r.status_code < 300:  # is 2xx
        ...
        if fetch_pages and total_pages > 1:
            ...
            for page_request in pages:
                data = page_request.get(blocking=True)  # block to get result
                response_data.extend(data)  # assume both are lists
        ...
        if r.json()['error'] == 'expired':
            return esi_task(method, url, headers, params, data, fetch_pages, auth_callback, cache, page=page) \
                .get(blocking=True)
