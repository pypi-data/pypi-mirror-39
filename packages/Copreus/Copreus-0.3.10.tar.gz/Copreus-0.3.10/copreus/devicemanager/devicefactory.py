...
class DeviceFactory:
    ...
    def create_device(config, mqtt_client=None, logger=None, spi_lock=None, i2c_lock=None):
        ...
        try:
            ...
            if "ASPI" in driver["bases"]:
                result = klass(config, mqtt_client, logger, spi_lock=spi_lock)
            elif "AI2C" in driver["bases"]:
                result = klass(config, mqtt_client, logger, i2c_lock=i2c_lock)
            else:
    ...
    def old_create(config, mqtt_client, logger, spi_lock=None, i2c_lock=None):
        ...
        if type_name == "ADC":
            ...
            result = BME_280(config, mqtt_client, logger, i2c_lock)
        elif type_name == "DHT":
