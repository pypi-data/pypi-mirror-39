...
from .acq import explore, exploit
...
class BoMain:
    ...
    def get_y(self):
        ...
        if self.model == None:
            ...
            return self.model.Y + self.normcnst
    ...
    def get_mu(self, x):
        ...
        if self.model == None:
            ...
            return self.model.mu(x) + self.normcnst
    ...
    def get_grad(self, x):
        ...
        """
        return self.model.predictive_gradients(x)
...
#            self.model.add_data(xnew, ynew - self.normcnst)
            X = np.vstack([self.get_x(), np.atleast_2d(xnew)])
            Y = np.vstack([self.get_y(), ynew])
            self.normcnst = np.mean(Y)
            self.est_yrange = np.max(Y)-np.min(Y)
            self.model.redefine_data(X, Y - self.normcnst)
    ...
    def _init_model(self):
        ...
        """
        self.normcnst = np.mean(self.initY)
        self.est_yrange = np.max(self.initY)-np.min(self.initY)
        self.model = Model(
            self.initX,
            self.initY - self.normcnst,
            self.bounds,
            self.min_dist_acqs,
            self.minzacc,
            self.kerntype,
    ...
    def run_optimization(self):
        ...
        number of initialization points and BO points have been acquired or a convergence criterion is met.
        """
        ...
        for i_iter in range(self.initpts + self.iterpts):
            ...
            # evaluation
            #self.mainOutput.progress_msg("Evaluating", 1)
            ...
            #self.mainOutput.progress_msg("Updating GP model", 2)
            ...
            if self._should_optimize(i_iter):
                #self.mainOutput.progress_msg("Optimizing the GP model", 2)
            ...
            #self.mainOutput.progress_msg("Calculating minimum prediction and "
            #                           + "other iteration quantities", 1)
    ...
    def _acqnext(self, i_iter):
        ...
        """
        xnext = self._minimize_acqfn(self.acqfn)

        # check if xnext indicates we should trigger pure exploration
        if self._location_overconfident(xnext):
            xnext = self._minimize_acqfn(explore)
            return xnext, explore
        else:
            return xnext, self.acqfn
    ...
    def _minimize_acqfn(self, acqfn):
        ...
        n_data = self.get_y().size
        gmin = Minimization.minimize_from_random(
                    acqfn, self.bounds, num_pts=round(n_data*1.3),
    ...
    def _evaluate_xnew(self, xnew):
        ...
        ynew = np.atleast_1d(self.f(xnew))
        os.chdir(self.dir)
        self.mainOutput.progress_msg(
            "Objective function evaluated," + 
            " time [s] %s" % (local_timer.str_lapTime()), 1)

        # here use symmetry operations (if such are defined) to return
        # multiple xnew with the same ynew - this is not implemented yet
        # NOTE: this will currently break rst-functionality
    ...
    def _optimize_model(self, i):
        ...
        if np.any(theta == None):
            ...
            # the latter is required so that the model properly updates
    ...
    def _add_convergence(self):
        ...
        if self.model == None:
            ...
            besty = besty + self.normcnst
            muhat = muhat + self.normcnst
