...
from .main_output import MainOutput
from ..bo.acq import elcb, lcb, explore, exploit, ei
...
class Settings:
    ...
    def __init__(self, ipfile, timer):
        ...
        self.thetapriorpar = self._setopt(
                                    'thetapriorparam', None, 'floatlist'
                                         )
        ...
        self.pp_local_mins = self._setopt('pp_local_minima', None, int)

        # hard coded - related to minimizing the model in search of xhat
        self.minzacc = 15
        self.min_dist_acqs = 0.02*min(self.periods)
        ...
        if self.thetapriorpar is None and self.thetaprior is not None:
            ...
            if self.thetaprior == "gamma":
                shape, rate = Distributions.gammaparams(0.1*diff, 2*diff,
                                                        0.005, 0.995)
            ...
            for i in range(self.dim):
                ...
                elif self.thetaprior == "normal":
                    mean, std = self.thetainit[i+1], self.thetainit[i+1]*0.3
                    self.thetapriorpar.append([mean, std])
                else:
