...
class Minimization:
    ...
    def _run_minimizer(func, x0, bounds, args, finite_grads=False):
        ...
        """
        if finite_grads:
            return fmin_l_bfgs_b(func, x0, bounds=bounds, args=args,
                                      factr=1e8, pgtol=1e-4, maxfun=1e4,
                                      maxiter=1e4, maxls=15, approx_grad=True)
        else:
            m = fmin_l_bfgs_b(func, x0, bounds=bounds, args=args,
                                      factr=1e8, pgtol=1e-4, maxfun=1e4,
                                      maxiter=1e4, maxls=15)
            return(m)

    @staticmethod
    def minimize(func, bounds, acqs, min_dist_acqs, accuracy=30, args=(),
                 lowest_min_only=True):
        """
        Tries to find global minimum of func by starting minimizers from
        accuracy percentage of lowest acquisitions. func has to return both
        ...
        X, Y = zip(*points)
        ind_last = min(len(Y)-1, max(0, round((accuracy/100.)*(len(Y)-1))))
        X = X[:ind_last+1]
        X = np.array([X[i] for i in range(len(X))])

        minima = []
        for acq in X:
            x, f, d = Minimization._run_minimizer(func, acq, bounds, args)
            if d['warnflag'] == 0:
    ...
    @staticmethod
    def minimize_from_sobol(func, bounds, num_pts, shift, args=(),
                 lowest_min_only=True):
        """
        Tries to find global minimum of func by starting minimizers from
        ...
        """
        sobs = np.transpose(
            sobol_seq.i4_sobol_generate(len(bounds), num_pts, 1))
        ...
        minima = []
        for pnt in points:
            x, f, d = Minimization._run_minimizer(func, pnt, bounds, args)
            if d['warnflag'] == 0:
                minima.append([list(x), float(f)])
        if len(minima) == 0:
            err_min = [list(points[-1]),
                       float(func(points[-1], args[0], args[1])[0])]
    ...
    @staticmethod
    def minimize_from_random(func, bounds, num_pts, shift, args=(),
                 lowest_min_only=True):
        """
        Tries to find global minimum of func by starting minimizers from
    ...
    def _remove_duplicates(original, min_distance):
        ...
        while True:
            next = []
            ref = current[0][0]; ref_f = current[0][1]
            if ref == firstRef:
