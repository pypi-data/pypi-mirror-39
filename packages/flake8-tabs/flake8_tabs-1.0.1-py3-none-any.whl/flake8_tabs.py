	...
	def __init__(self, logical_line, indent_char, line_number, noqa, tokens):
		...
		for token_set in self._group_tokens_by_physical_line(tokens):
			...
			for token in token_set:
				...
				if token.type == tokenize.OP:
					...
					if last_braces_count < braces_count:
						...
						brace_offset = token.end[1] - line_indent.tabs - line_indent.spaces
						...
						# Add up all the removed indentation that happened before any other
						# indentation to find the proper level of outdenting
			...
			if indent_style not in ("indent", "align"):
				...
				indent_style = "align" if not brace_at_end else "indent"
			...
			if braces_count > prev_braces_count:
				if indent_style == "indent":
					...
					current_indent_delta += (0, brace_offset)
			...
			# Expect the closing braces to be on the new level already if it is the first
			# content on the this line
			if braces_count < prev_braces_count:
