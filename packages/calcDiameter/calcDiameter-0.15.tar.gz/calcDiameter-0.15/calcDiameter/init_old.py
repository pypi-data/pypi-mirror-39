...
def maxDistXY(coordinates,residues, bin_ = 5):
    """
...
def testRotations(coordinates, range_1, range_2, residues, bin_=5, test=False):
    """
    ...
    if test:
        ...
        for point in coordinates2:
            if (point[0] - x_mean)**2 + (point[1] - y_mean)**2 < test**2:
                print ("centre is %f,%f" % (x_mean, y_mean))
                return False
    return i_j[id_], max_diameter, coordinates2, p1, p2, label1, label2

def findDiameter(coordinates,name,residues, plot3d=False, plot=True, no_rotation = False, bin_=5, test=5):
    ...
    try:  
        ij, max_diameter, rotated, p1, p2, label1, label2 = testRotations(coordinates, range_1, range_2, residues, bin_=bin_, test=5)
        success = True
    except:
...
def calcDiameter(pdb, plot=True, plot3d=False, no_rotation=False, bin_=5):
    """
...
def getDiametersCn(n, res_min = 0.0, res_max = 3.0, plot=True, bin_=5):
    """
    ...
    for pdbcode in pdbs:
        ...
        name = pdbcode
        try:
            diameter, label1, label2 = findDiameter(coords, name, residues, plot=plot, bin_=bin_)
            print(("%s max diameter between %s & %s is %f" % (pdbcode, label1, label2, diameter)))
            data[pdbcode] = [diameter, label1, label2, n]
        except:
            print(("%s failed. It is probably a transmembrane alpha-helix bundle of C%s symmetry" % (pdbcode, n)))
            continue
    return pd.DataFrame.from_dict(data, orient='index',columns=['Diameter', 'label1', 'label2', 'Cn'])
