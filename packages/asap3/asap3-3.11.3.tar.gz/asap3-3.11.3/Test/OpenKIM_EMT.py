...
openkimmodel = 'EMT_Asap_Standard_Jacobsen_Stoltze_Norskov_%s'
...
for element in elements:
    for i in (0, 1):
        ...
        try:
            kim = OpenKIMcalculator(openkimmodel % element)
        except AsapError as oops:
           if oops.args[0].startswith('Failed to initialize OpenKIM model'):
               print("OpenKIM model {} not installed - skipping test.".format(openkimmodel % element))
               continue
           else:
