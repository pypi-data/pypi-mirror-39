...
class Download(object):
    help_menu = 'upload a build to Koji'
    _help = """
Upload a build's artifacts to Koji.

Positional Arguments:

[build]  The name of the build to upload, eg. "ceph_10.2.0-2redhat1trusty"
"""
    name = 'upload'
        ...
        for arch, binaries in six.iteritems(payload):
            for binary in binaries:
                if os.path.isfile(binary):
                    # TODO: check the sha256sum of the already-uploaded file
                    # here?
                    log.info('skipping %s' % binary)
                    continue
                log.info('uploading %s' % binary)
