...
class BulkProcesses(object):
    ...
    def upload_document_photo(self, excelDataFileToRead,groupid=None,categoryid=None,subcategoryid=None,aslink=False):
        ...
        :param excelDataFileToRead:
            ...
            Asset Id | Document/Photos Complete File Path |
        ...
        if excelDataFileToRead != None and excelDataFileToRead.strip("") != "" \
           and os.path.exists(excelDataFileToRead) and \
           excelDataFileToRead.lower().split('.')[1] in \
           validDocumentToImportExtensions:
            try:
                wb = openpyxl.load_workbook(excelDataFileToRead)
                sheet = wb.active
                sheet['E' + str(1)] = 'Migrated Document Id'
                sheet['F' + str(1)] = 'Operation Performed On'
                sheet['G' + str(1)] = 'Status'
                ...
                for row in range(2, sheet.max_row + 1):
                    ...
                    if external_id and len(external_id) > 200:
                        ...
                        msg = "Truncating external_id for {0}".format(assetId)
                        self.logger.warning(msg)
                    self.logger.info(
                        'Processing {2}/{3}: Complex Asset Id: [{0}]; '
                        'File name: [{1}]; External ID: [{4}]'.format(
                            assetId, fileNameToUpload, row, sheet.max_row
                            , external_id))
                    if fileNameToUpload is not None and \
                            not fileNameToUpload.isspace() \
                            and os.path.exists(fileNameToUpload):
                        documentGroupId = groupid
                        if os.path.splitext(fileNameToUpload.lower())[1][1:] \
                                in validImageExtensions:
                            if documentGroupId is None:
                                documentGroupId = 7
                        else:
                            isKeyPhoto = False
                            if documentGroupId is None:
                        ...
                        document.parent_identifier = assetId
                        document.label = 'Importing complex asset document/photo - [' + fileToUploadName + '] on ' + time.strftime("%d/%m/%Y %H:%M:%S")
                        ...
                        if aslink == False:
                            ...
                            document.label = fileNameToUpload
                            document.is_key_photo = False
                        try:
                            doc = self.docapi.document_post(document)
                        except ApiException as e:
                            self.logger.error('Error occurred while '\
                                'uploading document/photo: [{0}]; '\
                                'Error status {1}, Reason: {2} {3}\n'.format(
                                    fileToUploadName, e.status, e.body, e.reason))
                        else:
                            docid = doc[0].get('Id')
                            self.logger.info('UPLOADED document/photo: '\
                                '[{1}]; Response: [{0}]\n'.format(
                                docid, fileToUploadName))

                            isDocumentUploadSuccessful = True if docid is not None and not docid.isspace() else False
                            if isDocumentUploadSuccessful:
                                sheet['E' + str(row)] = docid
                                sheet['F' + str(row)] = time.strftime("%c")
                                sheet['G' + str(row)] = 'SUCCESS'
                            else:
                                sheet['E' + str(row)] = 'N/A'
                                sheet['F' + str(row)] = time.strftime("%c")
                                sheet['G' + str(row)] = 'Error; Please check integration application logs for details'
                    else:
                        self.logger.warning('[{1}/{2}]: File: [{0}] is non-existent\n'.format(fileNameToUpload, row, sheet.max_row))
                        sheet['E' + str(row)] = 'N/A'
                        sheet['F' + str(row)] = time.strftime("%c")
                        sheet['G' + str(row)] = 'Document/Photo file is non-existent'
            ...
            if len(excelfile) > 0 and excelfile.strip("") != "":
                if os.path.isfile(excelfile) == False:
                    ...
                    self.upload_document_photo(excelfile, groupid, \
                        doccategoryid, docsubcategoryid, aslink)
                    break
            else:
                break
        return 0
