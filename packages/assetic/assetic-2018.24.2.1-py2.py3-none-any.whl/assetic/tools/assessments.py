...
class AssessmentHelper(object):
    ...
    def create_form_from_csv(self,form_repr,csvfile,structure=None):
        ...
        if tab == None:
            ##error encountered
            self.logger.debug("Tab could not be created")
            return 1

        ##get the structure from csv
        rows = self.read_csv_file_into_dict(csvfile,structure)
        if len(rows) == 0:
            self.logger.debug("No rows returned by csv reader")
            return 1

        ##Add the controls
        for row in rows:
            self.logger.debug("Process row label '{0}'".format(row["Label"]))
            self.process_csv_dict_row(row,tab,widget_labels,label_formatter)
    ...
    def process_csv_dict_row(self,field_def,tab,widget_labels,label_formatter):
        """
        ...
        required = False
        if "Required" in field_def and field_def["Required"] != None:
        ...
        if control != None:
            ...
            msg = "No contol created for field definition {0}".format(field_def)
