...
def save_model_with_run_history(run_history, origin, container, model, model_name):
    ...
    artifact_map, common_artifact_id_prefix = _get_artifact_info(upload_results)
...
def _path_to_name(path):
    return os.sep.join(path.split(os.sep)[2:])  # removes MODELS_DIR/<container_id>
...
def save_model_to_local(container, model, model_name):
    ...
    modeldata, modeldata_path = modeldata_handler.save(model, os.path.join(model_dir, model_name))
    ...
    if model_handler is not None:
        saved_model_path = model_handler.save(model, os.path.join(model_dir, model_name))
...
def _get_artifact_info(upload_results):
    ...
    common_artifact_id_prefix = common_artifact_id_prefix.replace(os.path.sep, "/")
...
def load_model_from_local(model_name):
    ...
    modeldata_path = os.path.join(model_dir, "{}{}".format(model_name, AML_MODELDATA_EXTENSION))
...
def load_model(model_name, version=None, workspace=None):
    ...
    """
    model_path = Model.get_model_path(model_name, version, _workspace=workspace)
...
def _load_model_with_metadata(model_path, modeldata):
    ...
    if handler is None:
        raise Exception("Load handler not found for metadata: {}".format(modeldata.handler_name))
