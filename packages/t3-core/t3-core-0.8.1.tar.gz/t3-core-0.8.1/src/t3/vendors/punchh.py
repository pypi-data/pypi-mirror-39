...
class PunchhAuthentication(authentication.BaseAuthentication):
    ...
    def authenticate(self, request):
        ...
        if not jwt_token:
            raise AuthenticationFailed('No token in request header')
        ...
        if not all([x in payload for x in ['jti', 'sub', 'iat', 'v1_token', 'v2_token']]):
            raise AuthenticationFailed('Invalid Punchh JWT used')
        ...
        if settings.PUNCHH_AUTH_SERVER_VERIFY:
            ...
            if response.status_code != 200:
                raise AuthenticationFailed('Punchh servers rejected token')
