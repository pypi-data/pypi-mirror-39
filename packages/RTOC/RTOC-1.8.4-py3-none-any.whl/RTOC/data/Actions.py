...
class Actions:
    ...
    def checkUpdates(self):
        ...
        if current != None:
            text = 'Installed version: '+str(current)
            if not available:
                info = "Sorry. Couldn't get version information from PyPi. Please visit 'https://pypi.org/project/RTOC/'"
            else:
                text += ', Newest version: '+str(available[0])
                if current == available[0]:
                    info = 'RTOC is up to date.'
                else:
                    info = 'New version available! Please update with:\n\n"pip3 install RTOC --upgrade"\n'
        else:
            text = 'RTOC was not installed with PyPi.'
            info = 'To enable version-checking, please install it with "pip3 install RTOC"'

        pyqtlib.info_message('Version info', text, info)
