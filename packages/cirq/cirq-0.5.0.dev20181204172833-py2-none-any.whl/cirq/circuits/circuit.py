...
class Circuit(object):
    ...
    def insert(
        ...
        u""" Inserts a moment or operations into the circuit.
    ...
    def to_text_diagram_drawer(
        ...
        if 'qubit_name_suffix' in _3to2kwargs: qubit_name_suffix = _3to2kwargs['qubit_name_suffix']; del _3to2kwargs['qubit_name_suffix']
        else: qubit_name_suffix =  u''
        if 'use_unicode_characters' in _3to2kwargs: use_unicode_characters = _3to2kwargs['use_unicode_characters']; del _3to2kwargs['use_unicode_characters']
        else: use_unicode_characters =  True
...
def _draw_moment_groups_in_diagram(moment_groups,
    ...
    top_left = u'┌' if use_unicode_characters else u'/'
    top_right = u'┐' if use_unicode_characters else u'\\'
    bottom_left = u'└' if use_unicode_characters else u'\\'
    bottom_right = u'┘' if use_unicode_characters else u'/'
...
def _decompose_measurement_inversions(op):
    if ops.MeasurementGate.is_measurement(op):
        ...
        return [ops.X(q)
                for q, b in izip(op.qubits, gate.invert_mask)
                if b]
