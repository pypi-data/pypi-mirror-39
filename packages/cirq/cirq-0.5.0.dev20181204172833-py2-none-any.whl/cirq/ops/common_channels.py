...
class AsymmetricDepolarizingChannel(raw_types.Gate):
    ...
    def __init__(self, p_x, p_y, p_z):
        ...
        self._p_x = validate_probability(p_x, u'p_x')
        self._p_y = validate_probability(p_y, u'p_y')
        self._p_z = validate_probability(p_z, u'p_z')
        self._p_i = 1 - validate_probability(p_x + p_y + p_z, u'p_x + p_y + p_z')
...
class GeneralizedAmplitudeDampingChannel(raw_types.Gate):
    ...
    def __init__(self, p, gamma):
        ...
        self._gamma = validate_probability(gamma, u'gamma')
        self._p = validate_probability(p, u'p')
...
class RotationErrorChannel(raw_types.Gate):
    u"""Channel to introduce rotation error in X, Y, Z."""

    def __init__(self, eps_x, eps_y, eps_z):
        ur"""The rotation error channel.
    ...
    def __repr__(self):
        return u'cirq.rotation_error(eps_x={!r},eps_y={!r},eps_z={!r})'.format(
    ...
    def __unicode__(self):
        return u'rotation_error(eps_x={!r},eps_y={!r},eps_z={!r})'.format(
            self._eps_x, self._eps_y, self._eps_z
        )
    ...
    def _circuit_diagram_info_(
        ...
        return u'RE({!r},{!r},{!r})'.format(
            self._eps_x, self._eps_y, self._eps_z
        )


RotationErrorChannel = value.value_equality(RotationErrorChannel)
