...
@pytest.mark.parametrize(u'phase_exponent', [
    -0.5, 0, 0.1, 0.25, 0.5, 1, cirq.Symbol(u'p')
    ]
)
...
def test_no_symbolic_qasm_but_fails_gracefully():
    ...
    v = cirq.PhasedXPowGate(phase_exponent=cirq.Symbol(u'p')).on(q)
...
def test_eq():
    ...
    eq.make_equality_group(
        lambda: cirq.PhasedXPowGate(exponent=cirq.Symbol(u'a'),
                                    phase_exponent=0))
    eq.add_equality_group(
        cirq.PhasedXPowGate(exponent=cirq.Symbol(u'a'),
                            phase_exponent=0.25))
    eq.add_equality_group(cirq.PhasedXPowGate(exponent=0, phase_exponent=0))
    eq.add_equality_group(
        cirq.PhasedXPowGate(exponent=0, phase_exponent=cirq.Symbol(u'a')))
    eq.add_equality_group(cirq.PhasedXPowGate(exponent=0, phase_exponent=0.5))
    eq.add_equality_group(cirq.PhasedXPowGate(
        exponent=cirq.Symbol(u'ab'),
        phase_exponent=cirq.Symbol(u'xy')))
...
def test_trace_bound():
    ...
    assert cirq.trace_distance_bound(cirq.PhasedXPowGate(
        phase_exponent=0.25, exponent=cirq.Symbol(u'a'))) >= 1


def test_diagram():
    q = cirq.NamedQubit(u'q')
    c = cirq.Circuit.from_ops(
        cirq.PhasedXPowGate(phase_exponent=cirq.Symbol(u'a'),
                            exponent=cirq.Symbol(u'b')).on(q),
        cirq.PhasedXPowGate(phase_exponent=0.25,
                            exponent=1).on(q)
    )
