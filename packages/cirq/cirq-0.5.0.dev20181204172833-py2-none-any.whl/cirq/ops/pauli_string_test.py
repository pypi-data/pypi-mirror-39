...
@pytest.mark.parametrize(u'pauli', cirq.Pauli.XYZ)
...
def test_repr():
    ...
    assert (repr(pauli_string) ==
            u"cirq.PauliString({cirq.NamedQubit('q0'): cirq.Pauli.Z, "
            u"cirq.NamedQubit('q1'): cirq.Pauli.Y, cirq.NamedQubit('q2'): "
            u"cirq.Pauli.X}, False)")
...
def test_str():
    ...
    pauli_string = cirq.PauliString({q2: cirq.Pauli.X, q1: cirq.Pauli.Y,
                                     q0: cirq.Pauli.Z})
    assert unicode(pauli_string) == u'{+, q0:Z, q1:Y, q2:X}'
    assert unicode(pauli_string.negate()) == u'{-, q0:Z, q1:Y, q2:X}'
...
@pytest.mark.parametrize(u'shift,t_or_f',
        itertools.product(xrange(3), (True, False)))
...
def test_with_qubits():
    ...
    qubit_pauli_map = dict((q, cirq.Pauli.XYZ[q.x % 3]) for q in old_qubits)
