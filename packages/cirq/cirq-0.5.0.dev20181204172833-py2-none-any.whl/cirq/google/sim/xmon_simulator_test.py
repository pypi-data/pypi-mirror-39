...
def large_circuit():
    np.random.seed(0)
    qubits = [cirq.GridQubit(i, 0) for i in xrange(10)]
    sqrt_x = cirq.PhasedXPowGate(exponent=0.5, phase_exponent=0.0)
    circuit = cirq.Circuit()
    for _ in xrange(11):
        circuit.append(
    ...
    circuit.append([cirq.MeasurementGate(key=u'meas')(*qubits)])
    return circuit
...
def test_run(scheduler):
    ...
    circuit.append(
        [cirq.MeasurementGate(key=u'a')(Q1),
         cirq.MeasurementGate(key=u'b')(Q2)])
...
def test_param_resolver_exp_w_axis_half_turns():
    exp_w = cirq.PhasedXPowGate(
        exponent=1.0, phase_exponent=cirq.Symbol(u'a'))
    circuit = cirq.Circuit()
    circuit.append(exp_w(Q1))
    resolver = cirq.ParamResolver({u'a': 0.5})
    result = compute_gate(circuit, resolver)
...
def test_param_resolver_exp_w_multiple_params():
    exp_w = cirq.PhasedXPowGate(
        exponent=cirq.Symbol(u'a'),
        phase_exponent=cirq.Symbol(u'b'))
    circuit = cirq.Circuit()
    circuit.append(exp_w(Q1))
...
def test_param_resolver_exp_11_half_turns():
    circuit = cirq.Circuit()
    circuit.append(cirq.CZ(Q1, Q2)**cirq.Symbol(u'a'))
    resolver = cirq.ParamResolver({u'a': 0.5})
...
def test_run_circuit_sweep():
    circuit = cirq.Circuit.from_ops(
        cirq.X(Q1)**cirq.Symbol(u'a'),
        cirq.MeasurementGate(u'm').on(Q1),
    )
...
def test_unsupported_gate(scheduler):
    ...
    simulator = cg.XmonSimulator()
    with pytest.raises(TypeError, message=u"UnsupportedGate"):
...
def test_measurement_qubit_order(scheduler):
    circuit = cirq.Circuit()
    meas = cirq.MeasurementGate(key=u'')
...
def test_inverted_measurement(scheduler):
    circuit = cirq.Circuit.from_ops(
        cirq.MeasurementGate(u'a', invert_mask=(False,))(Q1),
        cirq.X(Q1),
        cirq.MeasurementGate(u'b', invert_mask=(False,))(Q1),
        cirq.MeasurementGate(u'c', invert_mask=(True,))(Q1),
        cirq.X(Q1),
        cirq.MeasurementGate(u'd', invert_mask=(True,))(Q1))
    simulator = cg.XmonSimulator()
    result = run(simulator, circuit, scheduler)
    np.testing.assert_equal(result.measurements,
...
def test_inverted_measurement_multiple_qubits(scheduler):
    circuit = cirq.Circuit.from_ops(
        cirq.MeasurementGate(u'a', invert_mask=(False, True))(Q1, Q2),
        cirq.MeasurementGate(u'b', invert_mask=(True, False))(Q1, Q2),
        cirq.MeasurementGate(u'c', invert_mask=(True, False))(Q2, Q1))
    simulator = cg.XmonSimulator()
    result = run(simulator, circuit, scheduler)
...
def test_measurement_multiple_measurements(scheduler):
    circuit = cirq.Circuit()
    measure_a = cirq.MeasurementGate(u'a')
    measure_b = cirq.MeasurementGate(u'b')
    circuit.append(cirq.X(Q1))
...
def test_handedness_of_basic_gates():
    circuit = cirq.Circuit.from_ops(
        ...
        cirq.Z(Q1)**-0.5,
        cirq.Y(Q1)**0.5,
        cirq.MeasurementGate(key=u'').on(Q1),
    )
...
def bit_flip_circuit(flip0, flip1):
    ...
    m1, m2 = cirq.MeasurementGate(u'q1')(q1), cirq.MeasurementGate(u'q2')(q2)
    circuit = cirq.Circuit()
...
def test_circuit_parameters():
    sim = cg.XmonSimulator()
    circuit = bit_flip_circuit(cirq.Symbol(u'a'), cirq.Symbol(u'b'))
