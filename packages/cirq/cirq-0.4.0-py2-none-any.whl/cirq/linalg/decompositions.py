...
def _perp_eigendecompose(matrix, tolerance
                         ):
    ...
    for i in xrange(len(vecs)):
        for j in xrange(i + 1, len(vecs)):
            if not tolerance.all_near_zero(np.dot(np.conj(vecs[i].T), vecs[j])):
                raise ArithmeticError(u'Eigenvectors overlap.')
...
def kron_factor_4x4_to_2x2s(
    ...
    if np.any(np.isnan(restored)) or not tolerance.all_close(restored, matrix):
        raise ValueError(u"Can't factor into kronecker product.")
...
def so4_to_magic_su2s(
    ...
    if not tolerance.all_close(reconstructed, mat):
        raise ArithmeticError(u'Failed to decompose to desired tolerance.')
...
def _numpy_array_repr(arr):
    return u'np.array({!r})'.format(arr.tolist())
