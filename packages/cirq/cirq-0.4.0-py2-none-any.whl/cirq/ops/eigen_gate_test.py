...
def test_inverse():
    ...
    with pytest.raises(TypeError):
        _ = cirq.inverse(CExpZinGate(cirq.Symbol(u'a')))


def test_trace_distance_bound():
    assert cirq.trace_distance_bound(CExpZinGate(0.001)) < 0.01
    assert cirq.trace_distance_bound(CExpZinGate(cirq.Symbol(u'a'))) >= 1
...
def test_matrix():
    ...
    assert not cirq.has_unitary(CExpZinGate(cirq.Symbol(u'a')))
    assert cirq.unitary(CExpZinGate(cirq.Symbol(u'a')), None) is None
...
def test_is_parameterized():
    ...
    assert cirq.is_parameterized(CExpZinGate(cirq.Symbol(u'a')))


def test_resolve_parameters():
    assert cirq.resolve_parameters(CExpZinGate(cirq.Symbol(u'a')),
