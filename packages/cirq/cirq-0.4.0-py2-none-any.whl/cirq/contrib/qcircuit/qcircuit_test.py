...
def test_qcircuit_qubit():
    ...
    qy = _QCircuitQubit(cirq.NamedQubit(u'y'))
    assert repr(qx) == u'_QCircuitQubit({!r})'.format(x)
    assert x != qx
    assert qx != 0
    assert qx == qx2
    assert qx != qy
...
def test_text_to_qcircuit_diagrammable():
    ...
    expected_info = cirq.CircuitDiagramInfo(
            (u'\multigate{1}' + name, u'\ghost' + name),
            exponent=0.5,
            connected=False)
    ...
    actual_info = f.qcircuit_diagram_info(
            cirq.CircuitDiagramInfoArgs(known_qubits=qubits,
                                        ...
                                        qubit_map=qubit_map))
    expected_info = cirq.CircuitDiagramInfo(
            (u'\ghost{\\text{SWAP}}', u'\multigate{1}{\\text{SWAP}}'),
            connected=False)
    assert actual_info == expected_info
    ...
    actual_info = f.qcircuit_diagram_info(
            cirq.CircuitDiagramInfoArgs(known_qubits=qubits,
                                        ...
                                        precision=3,
                                        qubit_map=qubit_map))
    expected_info = cirq.CircuitDiagramInfo((u'\\gate{\\text{×}}',) * 2)
    assert actual_info == expected_info
