...
u"""Error simulator that adds randomly activated error gates after every moment.
"""
...
class DepolarizerChannel(object):
    u"""Depolarizing Channel Transformer for error simulation.
    ...
    _parameter_name = u'error_parameter'
    ...
    def transform_job(self, job):
        u"""Creates a new job object with depolarizing channel.
        ...
        for moment in circuit:
            ...
            for gate in self.depolarizing_gates:
                ...
                for q in qubit_list:
                    ...
                    if any(errors):
                        key = self._parameter_name + unicode(error_number)
