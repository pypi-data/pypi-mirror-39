...
def test_executor_explicit():
    ...
    gates = {(i, j): ExampleGate([unicode(k) for k in ij])
             for ij in combinations(xrange(n_qubits), 2)
             for i, j in (ij, ij[::-1])}
...
class DiagonalGate(cirq.Gate):
    def __init__(self, n_qubits, diagonal):
        dimension = 2 ** n_qubits
        if (diagonal.shape != (dimension,) or not
            np.allclose(
                np.absolute(diagonal), np.ones(dimension))):
            raise ValueError(u'Diagonal must be an (2**n_qubits)-dimensional '
...
def random_diagonal_gates(n_qubits,
                 acquaintance_size
                 ):
...
@pytest.mark.parametrize(u'n_qubits, acquaintance_size, gates',
...
def test_executor_random(n_qubits,
                         acquaintance_size,
                         gates):
    qubits = cirq.LineQubit.range(n_qubits)
