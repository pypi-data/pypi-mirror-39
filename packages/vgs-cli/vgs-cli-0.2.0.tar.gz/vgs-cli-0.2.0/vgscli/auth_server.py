...
class AuthServer:
    ...
    def authenticate(self, environment):
        ...
        if not webbrowser.open(url, new=1, autoraise=True):
            print("Can not open your browser, please open this URL in your browser: {}".format(url))

        while self.token_handler.get_code() is None:
            time.sleep(1)
        self.retrieve_access_token()
        return self.token_util.get_access_token().password

    def refresh_authentication(self):
        self.token_util.process_token_response(refresh_token(self.auth_api).body)
    ...
    def retrieve_access_token(self):
        ...
        self.token_util.process_token_response(response.body)
