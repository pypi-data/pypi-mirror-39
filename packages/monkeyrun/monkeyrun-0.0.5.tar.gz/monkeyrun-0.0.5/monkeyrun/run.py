...
try:
    ...
    from .monkeyLib import get_crash_log
except Exception:
    from base import MonkeyRunner, ApkInstall
...
class DbCreateDelete(object):
    ...
    def transfer_performence(self):
        ...
        try:
            ...
            if res.text == 0:
                logger.info("status: %d, return: %s" % (res.status_code, res.text))
                logger.info("调用技术检查项--成功--，----END----")
            else:
                logger.error("调用技术检测项接口返回不为0，return: %s" % res.text)
        except Exception as e:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        with self.db:
            ...
            # self.db.update("drop table %s" % self.tname)
            # self.db.fetch_all('VACUUM;')
            # logger.info("删除表，并清除占用空间")
...
def run(device="", apk_url="", scheme="", activity="", d=0, h=0, m=5, s=0, dbname="monkey.db", tname="", tid=0,
    ...
    if device == "" or apk_url == "" or scheme == "" or activity == "":
        ...
        with begin as b:
            if b:
                with dbcd:
                    run = MonkeyRunner(devices=device, fpath=url, activity=activity, schame=scheme, dbname=dbname,
                                       tname=tname)
