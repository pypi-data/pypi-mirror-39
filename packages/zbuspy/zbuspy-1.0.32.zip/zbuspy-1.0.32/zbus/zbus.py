...
class CaseInsensitiveDict(dict):   
    def __init__(self, data=None):
        ...
        self._proxy = dict((k.lower(), k) for k in data)
        for k in data:
            self[k] = data[k]

    def __contains__(self, k):
        if k is None: return False
        return k.lower() in self._proxy
    ...
    def __setitem__(self, k, v):
        ...
        self._proxy[k.lower()] = k
        
    def _key(self, k):
        key = self._proxy.get(k.lower())
        return key
...
class Message(Dict): 
    ...
    def replace(self, data):
        ...
        name = 'headers'
        if name not in self:
            self[name] = CaseInsensitiveDict()
        if self[name] is not CaseInsensitiveDict:
            self[name] = CaseInsensitiveDict(self[name]) 
...
class MqClient(WebsocketClient):
    ...
    def __init__(self, address):  
        ...
        def onmessage(msg):
            req = json.loads(msg)  
            req = Message(data=req)
            req.headers = CaseInsensitiveDict(req.headers)
