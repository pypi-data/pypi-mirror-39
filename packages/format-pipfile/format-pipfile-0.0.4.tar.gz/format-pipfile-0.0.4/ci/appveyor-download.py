...
APPVEYOR_BASE_URL = os.environ.get("APPVEYOR_BASE_URL", "https://ci.appveyor.com")
...
def make_auth_headers():
    ...
    if not os.path.exists(path):
        raise RuntimeError(
            "Please create a file named ``.appveyor.token`` in your home directory. You "
            "can get the token from {}/api-token".format(APPVEYOR_BASE_URL)
        )
...
def download_latest_artifacts(account_project, build_id):
    ...
    if build_id is None:
        url = "{}/api/projects/{}".format(APPVEYOR_BASE_URL, account_project)
    else:
        url = "{}/api/projects/{}/build/{}".format(
            APPVEYOR_BASE_URL, account_project, build_id
        )
...
parser.add_argument(
    "--id",
    metavar="PROJECT_ID",
    default="demosdemon/format-pipfile",
