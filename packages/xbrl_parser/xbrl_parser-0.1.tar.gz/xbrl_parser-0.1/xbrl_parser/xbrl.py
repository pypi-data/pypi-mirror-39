...
class XBRLParser(object):
    ...
    def parseGAAP(self,
        ...
        options_avg_exercise = \
            xbrl.find_all(name=re.compile("(us-gaap:)[^s]*(ShareBasedCompensationArrangementByShareBasedPaymentAwardOptionsVestedAndExpectedToVestOutstandingWeightedAverageExercisePrice)",
                          re.IGNORECASE | re.MULTILINE))
        gaap_obj.options_avg_exercise = self.data_processing(options_avg_exercise,
            xbrl, ignore_errors, logger, context_ids)

        options_outstanding = \
            xbrl.find_all(name=re.compile("(us-gaap:)[^s]*(ShareBasedCompensationArrangementByShareBasedPaymentAwardOptionsOutstandingNumber)",
                          re.IGNORECASE | re.MULTILINE))
        gaap_obj.options_outstanding = self.data_processing(options_outstanding,
            xbrl, ignore_errors, logger, context_ids)
