...
class CryptoPostSignum(BaseCryptoService):
    ...
    def sign(self, alias, fingerprint, password=None, hash_alg=None, sign_alg=None):
        ...
        if password is not None:
            ...
            if (reader[0]['pin'] is not None) and (reader[0]['pin'] == encoded_password):
                logger.error("Blocked repeated use of incorrect PIN to reader %s"
                             % reader[0]['reader'])
                pin_ok = False
            else:
                # select FILE
                self.server.cmd('00A408000614009001200200', reader[0]['reader'])
                # enter PIN  e.g. 8 digit 12345678: 00200010083132333435363738
                response_all = self.server.cmd('00200010%02X%s' % (len(password), encoded_password),
                                               reader[0]['reader'])
        ...
        if not pin_ok:
            ...
            self.server.cmd('0022F303', reader[0]['reader'])
            # set security context
            response_all = self.server.cmd('0022F1B80383010E', reader[0]['reader'])
            ...
            if response_all[0][-4:] == "9000":
                ...
                self.server.cmd('102A8086%s00' % padding1, reader[0]['reader'])
                # second half of the data
                response_all = self.server.cmd('102A8086%s00' % padding2, reader[0]['reader'])
    ...
    def apdu(self, token, command):
        ...
        response_all = self.server.cmd(command, token)
    ...
    def reset(self, token):
        response_all = self.server.cmd(None, token, 1)
        if len(response_all) > 0:
            response_data = response_all[0]
        else:
            response_data = BaseCryptoService.CMD_INTERNAL_ERROR
    ...
    def _read_certificates(self):
        ...
        for reader in self.storage.certificates.keys():
            ...
            ##########################
            error = False
            response_all = self.server.cmd('00A4040010A0000000770103000610000000000002', reader, 0)
            if len(response_all) < 1 or response_all[0][-4:] != "9000":
                error = True
            response_all = self.server.cmd('00A408000410001003', reader, 0)
            if len(response_all) < 1 or response_all[0][-4:] != "9000":
                error = True
            response_all = self.server.cmd('00B0000010', reader, 0)  # some kind of card ID
            if len(response_all) < 1 or response_all[0][-4:] != "9000":
                error = True

            # select certificate store
            response_all = self.server.cmd('00A408000614009001200200', reader, 0)
            if len(response_all) < 1 or response_all[0][-4:] != "9000":
                error = True
            ...
            response_all = self.server.cmd('00B0000004', reader)
            if len(response_all) < 1 or response_all[0][-4:] != "9000":
                error = True
            ...
            while length_read < length and not error:
                ...
                if to_read > 100:
                    to_read = 100
                response_all = self.server.cmd('00B0%04X%02X' % (length_read, to_read), reader)  # a folder
            ...
            self.server.cmd('00A40004023F0000', reader, 0)
