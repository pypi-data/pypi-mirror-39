...
from cloudfoxy import CloudFoxyConfig, CloudFoxy
from linuxshell import LinuxShellConfig, LinuxShell
from pythoncard import PythonCard, PythonCardConfig
from globalplatform import GlobalPlatformProConfig, GlobalPlatformPro
...
class RegisterUpstream(object):
    ...
    def get_service(upstream, token=None, signer=None):
        ...
        if (len(upstream_address) < 2) and (upstream_address[0] != 'python'):
            logger.error("Upstream signer is not in the correct format")
            return None
        ...
        if upstream_address[0].lower() == "file":
            ...
            if file_extension.lower() == 'jar':
                if token is None:
                    ...
                    logger.info('GPPro signer doesn\'t support authorization tokens')
                    server_config = GlobalPlatformProConfig(upstream_address[1])
            ...
            # anything else is assumed to be a shell command - without need for any interpreter
                ...
                if token is None:
                    ...
                    logger.info('Linux shell script doesn\'t support authorization tokens')
