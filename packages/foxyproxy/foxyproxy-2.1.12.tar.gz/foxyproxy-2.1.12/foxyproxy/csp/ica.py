...
class CryptoICA(BaseCryptoService):
    ...
    def _read_certificates(self):
        ...
        if total_cards < 1:
            logger.error("%d chips found")
        else:
            logger.error("%d chips found - we inspect presence of certificates now" % total_cards)
        ...
        for reader in self.storage.certificates.keys():
            ...
            while new_item:
                ...
                if self.check_response(response_all, reader, "00B2%02X0400 error" % certificate_id) is not None:
                    break
