...
class CryptoPGP(BaseCryptoService):
    ...
    def sign(self, alias, fingerprint, password=None, hash_alg=None, sign_alg=None):
        ...
        if password is not None:
            ...
            if (reader[0]['pin'] is not None) and (reader[0]['pin'] == encoded_password):
                ...
                response_all = self.server.cmd('00200081%02X%s' % (len(password), encoded_password),
                                               reader[0]['reader'])
    ...
    def _read_certificates(self):
        ...
        for reader in response_data:
            ...
            response_all = self.server.cmd(['00A4040006D2760001240100', '00ca004f00'], reader)
            if (len(response_all) == 2) and (response_all[0][-4:] == "9000") and (response_all[1][-4:] == "9000"):
                ...
                response_all = self.server.cmd(['00A4040006D2760001240100', '0047810002b60000'], reader)
                if (len(response_all) == 2) and (response_all[0][-4:] == "9000") and (response_all[1][-4:] == "9000") \
                    ...
                    self.storage.certificates[reader] = {'token': token_sn,
                                                         'key': public_key,
                                                         'certs': None,
                                                         'chain': None}  # we need to get the certificates from files
                else:
        ...
        for reader in self.storage.certificates.keys():
            ...
            ##########################
            error = False
            response_all = self.server.cmd('00A4040006D2760001240100', reader)
            if len(response_all) < 1 or response_all[0][-4:] != "9000":
                error = True
            response_all = self.server.cmd('00A4010C023F50', reader)
            if len(response_all) < 1 or response_all[0][-4:] != "9000":
                error = True
            response_all = self.server.cmd('00A4010C023F10', reader, 0)
            if len(response_all) < 1 or response_all[0][-4:] != "9000":
                error = True
            response_all = self.server.cmd('00A4020C025660', reader, 0)
            if len(response_all) < 1 or response_all[0][-4:] != "9000":
                error = True
            ...
            subject_cn = None
            while new_item:
                self.server.cmd('00A4000C023F00', reader)  # switch to the app
                self.server.cmd('00A4010C023F50', reader)  # select files with objects
                self.server.cmd('00A4010C023F10', reader)  # we need a directory structure
