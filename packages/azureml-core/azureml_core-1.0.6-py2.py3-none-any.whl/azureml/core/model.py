...
class Model(object):
    """
    Class for AzureML models. Model constructor is used to retrieve a cloud representation of a Model object
    associated with the provided workspace. Must provide either name or ID.
    ...
    def download(self, target_dir=".", exist_ok=False, exists_ok=None):
        """
        Downloads model to target_dir of local file system
    ...
    def deserialize(workspace, model_payload):
        """
        Convert a json object into a Model object. Will fail if the provided workspace is not the workspace the model
        is registered under.
