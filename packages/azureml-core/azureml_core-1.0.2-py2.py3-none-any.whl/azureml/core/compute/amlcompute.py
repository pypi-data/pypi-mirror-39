...
class AmlCompute(ComputeTarget):
    ...
    def _initialize(self, workspace, obj_dict):
        ...
        status = AmlComputeStatus.deserialize(obj_dict['properties']) \
            if provisioning_state == "Succeeded" else None
    ...
    def provisioning_configuration(vm_size='', vm_priority="dedicated", min_nodes=0,
                                   max_nodes=1, idle_seconds_before_scaledown=None,
    ...
    def serialize(self):
        ...
        cluster_properties = {'computeType': 'AmlCompute', 'computeLocation': self.cluster_location,
    ...
    def update(self, min_nodes=None, max_nodes=None, idle_seconds_before_scaledown=None):
        ...
        """
        if not any([min_nodes, max_nodes, idle_seconds_before_scaledown]):
            return
        else:
            self.refresh_state()
            if not min_nodes:
            ...
            if not idle_seconds_before_scaledown:
