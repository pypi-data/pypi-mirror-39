        ...
        try:
            ...
            if operation_state != 'Succeeded':
                if error and 'statusCode' in error and 'message' in error:
                    print('Service creation polling reached terminal state with\n'
                          'StatusCode: {}\n'
                          'Message: {}'.format(error['statusCode'], error['message']))
                elif error:
                    print('Service creation polling reached terminal state, unexpected error response:\n'
                          '{}'.format(error))
                else:
                    try:
                        error_details = self.error['details']
                        print('Service creation polling reached terminal state, current service state: {}\n'
                              'Details: {}'.format(self.state, error_details))
                    except Exception:
                        print('Service creation polling reached terminal state, current service state: '
                              '{}\n'.format(self.state))
