...
class ServiceContext(ChainedIdentity):
    ...
    def _get_endpoint_from_environment(self, key):
    ...
    def _fetch_endpoints(self):
        ...
        for discovery_key in _ServiceKeys.get_discovery_keys():
            try:
                ...
                has_aml_token_auth = isinstance(self.get_auth(), AzureMlTokenAuthentication)
                if has_aml_token_auth and http_error.response.status_code in [401, 409]:
                    self._logger.debug("Service discovery is unsupported for auth: {} and workspace uri: {}".format(
                        type(self.get_auth()), self._get_workspace_scope()))
                else:
                    raise
            else:
    ...
    def _get_endpoint(self, key):
        ...
        :rtype: str
        """
        developer_endpoint = self._get_endpoint_from_environment(key)
        if developer_endpoint is not None:
    ...
    def _get_assets_url(self):
        ...
        try:
            ...
            self._logger.debug("Failed to load mms url from service discovery, "
                               "attempting to construct mms url from run history url")

            #  Execution does not set mms url in remote context
            #  Therefore the mms url is created from the run history url
            url = self._get_run_history_url()
