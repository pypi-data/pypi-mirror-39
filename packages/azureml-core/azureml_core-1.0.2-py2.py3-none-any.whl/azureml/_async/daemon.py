...
class Daemon(ChainedIdentity):
    ...
    def _do_work(self):
        if self.current_timer:
            # Not first call
            self._logger.debug("Triggering do_work on timer {0}".format(self.current_timer.ident))
    ...
    def start(self):
        ...
        if self.current_timer:
            # self._logger.debug("Called start() on existing Timer")
    ...
    def _reset_timer(self, start=False, interval_sec=None):
        ...
        if interval_sec is not None:
            self.interval = interval_sec
        self._logger.debug("Setting timer for {0}s".format(self.interval))
    ...
    def __str__(self):
        return 'Daemon("{0}", {1}s, work: {2})'.format(self.identity, self.interval, self.work)
