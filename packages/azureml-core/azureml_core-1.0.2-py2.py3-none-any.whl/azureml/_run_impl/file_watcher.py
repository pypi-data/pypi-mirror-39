...
class FileWatcher(Thread):
    def __init__(self, logspath, origin, container, artifacts_client, logger,
                 parallelism=None):
    ...
    def create_blobs(self, artifact_uris, files_watched, upload_info, full_paths):
        ...
        try:
            for artifact_uri, fullpath in zip(artifact_uris, full_paths):
                ...
                self.logger.debug("uploading data to container: " + container_name + " blob: " + blob_name)
    ...
    def walk_files(self, files_watched, files_uploading, upload_info, tq, current_stat):
        ...
        if new_files:
            ...
            (artifact_uris, success) = self.create_artifacts(new_files)
            ...
            full_paths = [fullpath for (leaf_file, fullpath) in new_files]
