...
class RunHistoryFacade(ChainedIdentity):
    ...
    def __init__(self, experiment, run_id, origin, _cluster_url=None,
        ...
        self.run_history = RunHistoryClient.create(experiment.workspace, experiment.name, _host=_cluster_url,
                                                   **base_kwargs)
        self._cluster = self.run_history._host

        self.assets = AssetsClient.create(self._experiment.workspace, host=self._cluster, **base_kwargs)

        self.artifacts = ArtifactsClient.create(self._experiment.workspace, host=self._cluster, **base_kwargs)

        self.metrics = MetricsClient.create(self._experiment.workspace, self._experiment.name, self._run_id,
                                            host=self._cluster, use_batch=_batch_upload_metrics,
                                            **base_kwargs)

    @property
    def _client(self):
        self._logger.warn("Deprecated, use Client.run_history instead.")
        return self.run_history

    @property
    def assets_client(self):
        self._logger.warn("Deprecated, use RunHistoryFacade.assets instead.")
        return self.assets
    ...
    def get_run(self):
        dto = self.run_history.get_run(self._run_id)
        return dto

    def get_runstatus(self):
        return self.run_history.get_runstatus(self._run_id)

    def get_status(self):
        run_dto = self.run_history.get_run(self._run_id)
    ...
    def set_tags(self, tags):
        ...
        self.run_history.patch_run(self._run_id, create_run_dto)
    ...
    def log_to_artifact(self, name, value):
        ...
        self.artifacts.upload_artifact(stream, self._origin, self._run_id, name)

    def _build_artifact_uri(self, name):
        return 'aml://artifactId/{}/{}/outputs/{}'.format(self._origin, self._run_id, name)
    ...
    def get_descendants(self, root_run_id, recursive, **kwargs):
        ...
        # subtree
        children = self.run_history.get_child_runs(self._run_id, root_run_id, recursive=recursive, **kwargs)
    ...
    def register_model(self, model_name, model_path=None, tags=None, properties=None, asset_id=None):
        ...
        metadata_dict = None
        if asset_id is None:
            res = self.assets_client.create_asset(model_name,
                                                  artifacts,
            ...
            asset = self.assets_client.get_asset_by_id(asset_id)
    ...
    def create_child_run(self, name, target, child_name=None, run_id=None):
        ...
        child = self.run_history.create_child_run(self._run_id,
                                                  child_run_id,
                                                  target=target,
                                                  run_name=child_name)
        return child
    ...
    def start(self):
        ...
        """
        self.run_history.post_event_run_start(self._run_id)
    ...
    def complete(self):
        ...
        """
        self.flush()
        self.run_history.post_event_run_completed(self._run_id)
    ...
    def fail(self):
        ...
        """
        self.flush()
        self.run_history.post_event_run_failed(self._run_id)
