...
class RunWatcher(object):
    ...
    def __init__(self, run, local_root, remote_root, executor, event, session):
        ...
        self._client = ArtifactsClient.create(self._run_object.experiment.workspace)
    ...
    def get_tb_artifacts(self, client):
        return filter(lambda a: a.path.startswith(self._remote_root),
                      client.get_artifact_by_run_id(RUN_ORIGIN, self._run_object.id))
    ...
    def refresh_log_artifacts(self):
        for artifact in self.get_tb_artifacts(self._client):
            ...
            uri = self._client.get_file_uri(RUN_ORIGIN, self._run_object.id, remote_path)
        ...
        return self._run_object.get_status() in ["Completed", "Failed", "Canceled"]
