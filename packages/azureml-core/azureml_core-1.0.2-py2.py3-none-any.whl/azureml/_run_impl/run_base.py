...
class _RunBase(ChainedIdentity, HasRunPortal):
    ...
    def __init__(self, experiment, run_id, outputs=None,
                 _run_dto=None, _cluster_url=None, logs=None,
                 _worker_pool=None, _user_agent=None, _ident=None,
                 _batch_upload_metrics=True, **kwargs):
        """
        ...
        self._client = RunHistoryFacade(self._experiment, self._run_id, RUN_ORIGIN,
                                        worker_pool=_worker_pool, _cluster_url=_cluster_url,
                                        user_agent=user_agent, _parent_logger=self._logger,
                                        _batch_upload_metrics=_batch_upload_metrics)

        self._internal_run_dto = _run_dto if _run_dto else self._client.get_run()
        self._root_run_id = self._run_dto["root_run_id"]
        self._outputs = outputs
        self._run_number = _string_to_int(self._run_dto["run_number"], "run number")
        self._run_source = self._run_dto.get("properties", {}).get("azureml.runsource", None)
        self._runtype = self._run_dto.get("run_type", self._run_source)
        self._latest_status = self._run_dto.get("status", None)
        self._logger.debug("Constructing run from dto. type: %s, source: %s, props: %s",
                           self._runtype,
                           self._run_source,
                           self._run_dto.get("properties", {}))
        self._context_manager = RunContextManager(self, outputs=outputs, logs=logs,
                                                  heartbeat_enabled=self._run_dto.get("heartbeat_enabled", False),
    ...
    def _run_dto(self):
        ...
        run_dto = self._internal_run_dto
    ...
    def _repr_html_(self):
        info = self._get_base_info_dict()
        info.update([
            ('Details Page', make_link(self.get_portal_url(), "Link to Azure Portal")),
    ...
    def _heartbeat(self):
        self._client.run_history.post_event_run_heartbeat(self._run_id, HEARTBEAT_INTERVAL)
