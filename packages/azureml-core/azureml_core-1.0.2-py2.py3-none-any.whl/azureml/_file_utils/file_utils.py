...
def download_file(source_uri, path=None, max_retries=0, stream=True, protocol="https"):
    """
    ...
    with Session() as session:
        session.mount('{}://'.format(protocol), HTTPAdapter(max_retries=max_retries))
        ...
        if path is not None:
            ...
            module_logger.debug("Invalid output file path, the file was not downloaded")
        return response


def download_file_stream(source_uri, session, encoding="utf-8", max_retries=0, stream=True, protocol="https"):
    """
    Downloads the file from source_uri. Saves the content to the path if set.
    Returns the response of the get request"
    :param source_uri: The name of the file to normalize (may or may not contain the file extension).
    ...
    :rtype: requests.Response
    """
    module_logger.debug("downloading file with max_retries: {max_retries}, "
                        "stream: {stream}, and protocol: {protocol}".format(max_retries=max_retries,
                                                                            stream=stream,
                                                                            protocol=protocol))
    buffer = []
    response = session.get(source_uri, stream=stream)
