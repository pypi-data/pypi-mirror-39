...
def loadconfig(path, key=None):
    config = {}
    config_path = f"{path}/elcato.yaml"
...
def update_relative_to_document(paths, relative_path):
    paths.file_absolute = os.path.abspath(paths.rootdocument_location + relative_path),
    return paths
...
def images(source, path, destination, doc):
    ...
    for item in image_list:
        ...
        shutil.copy(
            paths.source_absolute, paths.destination_absolute + paths.filename
        )
...
def image_size_thumbnail(path):
    ...
    thumbnail.save(f"{path.destination_absolute}{path.filename}.png")
    return filename + ".png"
