...
u"""A variational ansatz based on a linear swap network Trotter step."""
...
class SwapNetworkTrotterAnsatz(VariationalAnsatz):
    u"""An ansatz based on the fermionic swap network.
    ...
    def __init__(self,
                 hamiltonian,
                 iterations=1,
                 include_all_xxyy=False,
                 include_all_yxxy=False,
                 include_all_cz=False,
                 include_all_z=False,
        ...
        super(SwapNetworkTrotterAnsatz, self).__init__(qubits)

    def params(self):
        u"""The parameters of the ansatz."""
        for i in xrange(self.iterations):
            for p in xrange(len(self.qubits)):
                if (self.include_all_z or not
                        numpy.isclose(self.hamiltonian.one_body[p, p], 0)):
                    yield LetterWithSubscripts(u'U', p, i)
            for p, q in itertools.combinations(xrange(len(self.qubits)), 2):
                if (self.include_all_xxyy or not
                        numpy.isclose(self.hamiltonian.one_body[p, q].real, 0)):
                    yield LetterWithSubscripts(u'T', p, q, i)
                if (self.include_all_yxxy or not
                        numpy.isclose(self.hamiltonian.one_body[p, q].imag, 0)):
                    yield LetterWithSubscripts(u'W', p, q, i)
                if (self.include_all_cz or not
                        numpy.isclose(self.hamiltonian.two_body[p, q], 0)):
                    yield LetterWithSubscripts(u'V', p, q, i)

    def param_bounds(self):
        u"""Bounds on the parameters."""
        bounds = []
        for param in self.params():
            if param.letter == u'U' or param.letter == u'V':
                bounds.append((-1.0, 1.0))
            elif param.letter == u'T' or param.letter == u'W':
    ...
    def operations(self, qubits):
        ...
        for i in xrange(self.iterations):
            ...
            def one_and_two_body_interaction(p, q, a, b):
                t_symbol = LetterWithSubscripts(u'T', p, q, i)
                w_symbol = LetterWithSubscripts(u'W', p, q, i)
                v_symbol = LetterWithSubscripts(u'V', p, q, i)
            ...
            for p in xrange(len(qubits)):
                u_symbol = LetterWithSubscripts(u'U', p, i)
    ...
    def default_initial_params(self):
        u"""Approximate evolution by H(t) = T + (t/A)V.
        ...
        for param in self.params():
            if param.letter == u'U':
                ...
                elif param.letter == u'W':
                    params.append(_canonicalize_exponent(
                        hamiltonian.one_body[p, q].imag *
                        step_time / numpy.pi, 4))
                elif param.letter == u'V':
                    params.append(_canonicalize_exponent(
...
def _canonicalize_exponent(exponent, period):
    # Shift into [-p/2, +p/2).
    exponent += period / 2
    exponent %= period
    exponent -= period / 2
