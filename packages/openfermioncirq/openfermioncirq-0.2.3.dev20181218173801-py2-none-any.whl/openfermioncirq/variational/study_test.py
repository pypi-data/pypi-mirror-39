...
def test_variational_study_circuit():
    assert (test_study.circuit.to_text_diagram().strip() == u"""
...
def test_variational_study_optimize_and_extend_and_summary():
    ...
    study = VariationalStudy(
            u'study', test_ansatz, test_objective,
    ...
    study.extend_result(u'run1',
                        repetitions=2)
    assert study.trial_results[u'run1'].repetitions == 3
    ...
    assert result.repetitions == 1
    assert all(
            result.data_frame[u'optimal_parameters'].apply(
                lambda x: XmonSimulateVariationalBlackBox(
                    test_ansatz, test_objective).evaluate(x))
            == result.data_frame[u'optimal_value'])
    ...
    assert isinstance(study.trial_results[u'test'].params.algorithm,
                      ExampleAlgorithm)
    assert isinstance(study.trial_results[u'lazy'].params.algorithm,
                      LazyAlgorithm)

    # Try extending non-existent run
    with pytest.raises(KeyError):
        study.extend_result(u'run100')

    # Check that getting a summary works
    assert unicode(study).startswith(u'This study contains')
...
def test_variational_study_save_load():
    datadir = u'tmp_yulXPXnMBrxeUVt7kYVw'
    study_name = u'test_study'
    ...
    assert unicode(loaded_study.circuit) == unicode(study.circuit)
    ...
    result = loaded_study.trial_results[u'example']
    assert isinstance(result, OptimizationTrialResult)
    assert result.repetitions == 1
    assert isinstance(result.params.algorithm, ScipyOptimizationAlgorithm)
    assert result.params.algorithm.kwargs == {u'method': u'COBYLA'}
    assert result.params.algorithm.options == {u'maxiter': 2}
    assert result.params.cost_of_evaluate == 1.0

    loaded_study = VariationalStudy.load(u'{}.study'.format(study_name),
                                         datadir=datadir)

    assert loaded_study.name == study.name

    # Clean up
    os.remove(os.path.join(datadir, u'{}.study'.format(study_name)))
    os.rmdir(datadir)
