...
class LazyAlgorithm(OptimizationAlgorithm):
    u"""Just returns the initial guess, or the zeros vector."""
...
class ExampleAnsatz(VariationalAnsatz):
    ...
    def operations(self, qubits):
        a, b = qubits
        yield cirq.XPowGate(exponent=cirq.Symbol(u'theta0')).on(a)
        yield cirq.XPowGate(exponent=cirq.Symbol(u'theta1')).on(b)
        yield cirq.CZ(a, b)
        yield cirq.XPowGate(exponent=cirq.Symbol(u'theta0')).on(a)
        yield cirq.XPowGate(exponent=cirq.Symbol(u'theta1')).on(b)
