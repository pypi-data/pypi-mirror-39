...
def rot111(rads):
    u"""Phases the |111> state of three qubits by e^{i rads}."""
...
class CXXYYPowGate(cirq.EigenGate,
    ...
    def __repr__(self):
        if self.exponent == 1:
            return u'CXXYY'
        return u'CXXYY**{!r}'.format(self.exponent)
...
class CYXXYPowGate(cirq.EigenGate,
    ...
    def __repr__(self):
        if self.exponent == 1:
            return u'CYXXY'
        return u'CYXXY**{!r}'.format(self.exponent)


def CRxxyy(rads):
    u"""Controlled version of ofc.Rxxyy"""
    return CXXYYPowGate(exponent=2 * rads / np.pi)


def CRyxxy(rads):
    u"""Controlled version of ofc.Ryxxy"""
