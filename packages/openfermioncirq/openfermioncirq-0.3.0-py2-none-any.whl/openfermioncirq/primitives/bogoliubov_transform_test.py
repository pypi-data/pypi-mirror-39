...
def fourier_transform_matrix(n_modes):
    ...
    return numpy.array([[root_of_unity ** (j * k) for k in xrange(n_modes)]
                        for j in xrange(n_modes)]) / numpy.sqrt(n_modes)


@pytest.mark.parametrize(
        u'transformation_matrix, initial_state, correct_state',
...
@pytest.mark.parametrize(
        u'n_spatial_orbitals, conserves_particle_number',
        [(4, True), (5, True)])
...
@pytest.mark.parametrize(
        u'n_qubits, conserves_particle_number',
...
def test_bogoliubov_transform_quadratic_hamiltonian(n_qubits,
    ...
    subsets = [numpy.random.choice(xrange(n_qubits),
                                   numpy.random.randint(1, n_qubits + 1),
                                   False)
               for _ in xrange(n_eigenstates)]
...
@pytest.mark.parametrize(u'n_qubits, atol', [
    (4, 1e-7),
    (5, 1e-7),
])
...
@pytest.mark.parametrize(u'n_qubits, real, particle_conserving, atol', [
    (5, True, True, 1e-7),
    (5, False, True, 1e-7),
    (5, True, False, 1e-7),
    (5, False, False, 1e-7),
...
def test_bogoliubov_transform_bad_shape_raises_error():
    with pytest.raises(ValueError):
        _ = bogoliubov_transform(cirq.LineQubit.range(4),
                                      numpy.zeros((4, 7))).next()
