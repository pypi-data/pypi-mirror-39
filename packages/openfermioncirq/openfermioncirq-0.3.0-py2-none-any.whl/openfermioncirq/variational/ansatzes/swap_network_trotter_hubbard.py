...
class SwapNetworkTrotterHubbardAnsatz(VariationalAnsatz):
    u"""A Hubbard model ansatz based on the fermionic swap network Trotter step.
    ...
    def __init__(self,
        ...
        super(SwapNetworkTrotterHubbardAnsatz, self).__init__(qubits)

    def params(self):
        u"""The parameters of the ansatz."""
        for i in xrange(self.iterations):
            if self.x_dim > 1:
                yield LetterWithSubscripts(u'Th', i)
            if self.y_dim > 1:
                yield LetterWithSubscripts(u'Tv', i)
            yield LetterWithSubscripts(u'V', i)

    def param_bounds(self):
        u"""Bounds on the parameters."""
        bounds = []
        for param in self.params():
            s = 1.0 if param.letter == u'V' else 2.0
            bounds.append((-s, s))
        return bounds
    ...
    def operations(self, qubits):
        ...
        for i in xrange(self.iterations):
            ...
            def one_and_two_body_interaction(p, q, a, b):
                th_symbol = LetterWithSubscripts(u'Th', i)
                tv_symbol = LetterWithSubscripts(u'Tv', i)
                v_symbol = LetterWithSubscripts(u'V', i)
    ...
    def default_initial_params(self):
        ...
        params = []
        for param, scale_factor in izip(self.params(),
                                       self.param_scale_factors()):
            if param.letter == u'Th' or param.letter == u'Tv':
