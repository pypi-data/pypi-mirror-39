...
def test_cxxyy_repr():
    assert repr(ofc.CXXYY) == u'CXXYY'
    assert repr(ofc.CXXYY**0.5) == u'CXXYY**0.5'
...
def test_cyxxy_repr():
    assert repr(ofc.CYXXYPowGate(exponent=1)) == u'CYXXY'
    assert repr(ofc.CYXXYPowGate(exponent=0.5)) == u'CYXXY**0.5'


@pytest.mark.parametrize(
        u'gate, initial_state, correct_state', [
            (ofc.CXXYY,
...
def test_three_qubit_rotation_gates_on_simulator(gate,
                                                 initial_state,
                                                 correct_state):
