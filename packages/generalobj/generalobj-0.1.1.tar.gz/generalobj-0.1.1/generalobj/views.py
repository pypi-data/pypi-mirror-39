...
from sccdbauth.forms import SCCDBGroupForm, SCCDBUserForm
from sccdbauth.models import SCCDBGroup, SCCDBUser
from cmdb.models import Landscape, Rack, ProductCapability
from cmdb.forms import LandscapeForm, RackForm, ProductCapabilityForm
...
@login_required
def general_obj_new(request, name, callback_prevalidation=False, \
    ...
    try:
        Obj = eval(name)
        ObjForm = eval('%sForm' % name)
    except NameError:
        return -1
    ret_dic = {}
    ret_dic['edit'] = False
    ret_dic['class_name'] = name
...
#            'You got redirected to somewhere')
            if cancel_url:
                ...
                if return_url:
                    ...
                    return HttpResponseRedirect(reverse("%ss" % name.lower()))
        ...
        if form.is_valid():
            if callback_postvalidation:
                ...
                if not form.is_valid():
                    ...
                    return render(request, 'common/templates/generalobj_new.html', \
                           ret_dic)
            ...
            if return_url:
                return HttpResponseRedirect(return_url)
            return HttpResponseRedirect(reverse(name.lower(), args=(obj.id,)))
        ...
        return render(request, 'common/templates/generalobj_new.html', ret_dic)

    return HttpResponse('qqq')


@login_required
def general_obj_edit(request, name, general_obj_id, querysets=[], \
    ...
    ret_dic['delete_url'] = os.path.join(reverse(name.lower(), args=(obj.id,)), 'delete/')
    print(ret_dic)
    ret_dic['obj'] = obj
    ret_dic['class_name'] = obj.__class__.__name__
    if request.method == 'POST':
        cancel = request.POST.get('cancel', '')
...
@login_required
def general_objs(request, name, fields=(), query_dict={}, query_term='', \
    ...
    ret_dic['obj_name'] = name
    fields = []
    obj_name_html = False
    obj_name_html_path = os.path.join(Obj._meta.app_config.path, 'templates', 'blocks', '%s_name.html' % name.lower())
    ...
    for obj in objs:
        if no_id_in_fields:
            ...
            if obj_name_html:
                ...
                field = ["<A href='%s'>%s</A>" % (reverse(name.lower(), args=(obj.id,)), obj.id)]
    ...
    if not no_id_in_fields:
        ret_dic['url_new'] = reverse('%s_new' % name.lower())
    ret_dic['url_tag'] = name.lower()
    ...
    if 'pass_to_template' in kwargs:
        for name, value in kwargs['pass_to_template'].items():
            ret_dic[name] = value
    return render(request, 'common/templates/generalobjs.html', ret_dic)


def general_obj(request, name, general_obj_id, fields=(), template=False, \
        *args, **kwargs):
    ...
    try:
        ...
        pass
    if not template:
        template = 'common/templates/generalobj.html'
    ret_dic = {}
    obj = get_object_or_404(Obj, id=general_obj_id)
    ret_dic['obj'] = obj
    if not fields:
    ...
    ret_dic['url_edit'] = reverse('%s_edit' % name.lower(), args=(obj.id,))
