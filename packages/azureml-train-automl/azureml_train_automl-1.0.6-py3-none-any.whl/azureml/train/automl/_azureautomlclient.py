...
class AzureAutoMLClient(object):
    ...
    def _init_adb_driver_run(self,
        ...
        try:
            ...
            aml_token = self.experiment_client._client.run.\
    ...
    def _repopulate_transformed_context(self, transformed_data_context):
        """
        Read and recover the transformed data from the cache store. If the read from cache is
        not successful, then populate a dictionary so that data transformation can happen again.
    ...
    def _fit_local(self, X=None, y=None, sample_weight=None, X_valid=None, y_valid=None, sample_weight_valid=None,
        ...
        if self.automl_settings.experiment_timeout_minutes:
            def experiment_expired():
                ...
                self.cancel()
            Timer(self.automl_settings.experiment_timeout_minutes * 60, experiment_expired).start()
    ...
    def _modify_run_configuration(self, run_config):
        ...
        # ignore version if it is a dev package
        if "dev" in core_version or core_version == "0.1.0.0":
            core_version = None
        if "dev" in automl_version or automl_version == "0.1.0.0":
            automl_version = None
