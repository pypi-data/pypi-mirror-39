...
# ---------------------------------------------------------
"""
Module to wrap models that don't accept parameters such as
...
class XGBoostClassifier(ClassifierMixin, _AbstractModelWrapper):
    ...
    def get_params(self, deep=True):
        ...
        :param deep: If True, will return the parameters for this estimator \
                 and contained subobjects that are estimators.
        :type deep: boolean
...
class NBWrapper(BaseEstimator, ClassifierMixin, _AbstractModelWrapper):
    """
    Naive Bayes Wrapper for conditional probabilities using either
    Bernoulli or Multinomial models.
    ...
    def __init__(self, model, **kwargs):
        """
        Initialize Naive Bayes Wrapper class with either
        Bernoulli or Multinomial models.
...
class TruncatedSVDWrapper(
        BaseEstimator, TransformerMixin, _AbstractModelWrapper):
    """
    Wrapper around Truncated SVD so that we only have to pass a fraction of
    dimensions. Read more at:
    http://scikit-learn.org/stable/modules/generated/sklearn.decomposition.TruncatedSVD.html.  # noqa
...
class SVCWrapper(BaseEstimator, ClassifierMixin, _AbstractModelWrapper):
    ...
    def predict(self, X):
        """
        Prediction function for svm.SVC Wrapper Model.
...
class NuSVCWrapper(BaseEstimator, ClassifierMixin, _AbstractModelWrapper):
    ...
    def predict(self, X):
        """
        Prediction function for svm.NuSVC Wrapper Model.
        Perform classification on samples in X.
...
class EnsembleWrapper(BaseEstimator, ClassifierMixin):
    ...
    def get_ensemble_predictions(preds):
        ...
        to a single array of probabilities of shape
    ...
    def compute_valid_predictions(models, X):
        """Returns an array of probabilities of shape
        [num_samples, num_classes, num_models]."""
        preds0 = None
        for m in models:
            if m is not None:
...
class LinearSVMWrapper(BaseEstimator, ClassifierMixin, _AbstractModelWrapper):
    """
    Wrapper around linear svm to support predict_proba on sklearn's
    liblinear wrapper.
...
class PipelineWithYTransformations(sklearn.pipeline.Pipeline):
    ...
    def fit(self, X, y, y_min=None):
        """
        Fit function for pipeline transform. Perform the fit_transform of
        y_transformer, then fit into the sklearn.pipeline.Pipeline.
    ...
    def predict(self, X):
        ...
        Perform the prediction of sklearn.pipeline.Pipeline, then do the
        inverse transform from y_transformer.
...
class QuantileTransformerWrapper(BaseEstimator, TransformerMixin):
    ...
    def inverse_transform(self, y):
        """
        Inverse transform function for Quantile transform.
