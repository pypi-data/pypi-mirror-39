...
# ---------------------------------------------------------
"""Global methods used during an AutoML fit method for pre-processing raw
...
def function_debug_log_wrapped(f):
    """
    Decorator that adds logs around transformer class function.
    """
...
class BinTransformer(BaseEstimator, TransformerMixin, TransformerLogger):
    """
    Wrapper over pandas.cut for binning the train data into intervals and
    then applying them to test data.
    ...
    def fit(self, x, y=None):
        """
        Identify the distribution of values with repect to the number of
        specified bins.
...
class NaiveBayes(BaseEstimator, TransformerMixin, TransformerLogger):
    ...
    def fit(self, x, y=None):
        """
        Fit function for Naive Bayes transform to learn conditional
        probablities for textual data.
...
class CatImputer(BaseEstimator, TransformerMixin, TransformerLogger):
    ...
    def fit(self, x, y=None):
        """
        Transforms the data to mark the missing values and identify the most
        frequent category.
    ...
    def transform(self, x):
        """
        Transforms data x by adding the missing values with the most frequent
        categories.
...
class DateTimeFeaturesTransformer(BaseEstimator,
    ...
    """
    Expands datetime features from input into sub features like year, month,
    day, day of the week, day of the year, quarter, week of the month, hour,
    minute and second.
    """
...
class HashOneHotVectorizerTransformer(BaseEstimator,
    ...
    def __init__(self, hashing_seed_val, num_cols=8096, logger=None):
        """
        Initialize for hashing one hot encoder transform with a seed value
        and maximum number of expanded columns.
...
class StringCastTransformer(BaseEstimator,
                            ...
                            TransformerLogger):
    """
    Cast input to string. The input and output type is same for this
    transformer.
    """
...
class LambdaTransformer(BaseEstimator, TransformerMixin, TransformerLogger):
    ...
    def transform(self, x):
        """
        Transform function for lambda transform which calls the lambda
...
class LabelEncoderTransformer(BaseEstimator,
    ...
    def transform(self, x):
        """
        Transform function for label encoding transform to convert
...
class _PreprocessingStatistics:
    ...
    def update_raw_feature_stats(self, feature_type):
        """
        This function increments the counters for different types of features
        """
    ...
    def get_raw_data_stats(self):
        """
        This function returns the string for overall raw feature stats
        """
...
class DataTransformer(BaseEstimator, TransformerMixin, TransformerLogger):
    ...
    def _learn_transformations(self, df, y=None):
        """
        Function to learn data transformation to be done on the raw data
    ...
    def fit_transform_with_logger(self, X, y=None, logger=None, **fit_params):
        """
        Wrapper function around the fit_transform function for the Data
        transformer class.
    ...
    def fit(self, df, y=None):
        """
        Perform the raw data validation and identify the transformations to
        apply.
    ...
    def transform(self, df):
        """
        Transform the input raw data with the transformations idetified in
        fit stage.
    ...
    def get_engineered_feature_names(self):
        """
        Return the list of engineered feature names as string after data
        transformations on the raw data have been finished.
    ...
    def _detect_feature_type(self, column, df):
        """
        Calculate the stats on the raw column and decide the data type of the
        input column.
...
class LaggingTransformer(BaseEstimator, TransformerMixin, TransformerLogger):
    ...
    def transform(self, x):
        """
        Transform function for lagging transform. The function appends
        columns with previous seen rows \
        to the input dataframe or sparse matrix depending on the specified
        lagging length.
