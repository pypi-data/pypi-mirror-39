...
from azureml._history.service_context import ServiceContext
...
from automl.client.core.common.exceptions import (DataException,
                                                  ServiceException)
from automl.client.core.common.utilities import (_log_traceback, _y_nan_check,
...
from ._logging import cleanup_log_map, get_logger, _blacklist_logging_keys
from ._systemusage_telemetry import SystemResourceUsageTelemetryFactory
from .run import AutoMLRun
from .utilities import (_auto_blacklist, _format_training_data,
                        _set_task_parameters, _validate_training_data_dict,
                        friendly_http_exception, _validate_data_splits)
...
class AzureAutoMLClient(object):
    ...
    def __init__(self,
        ...
        self.run_history_client = RunHistoryClient.create(self.experiment.workspace,
                                                          experiment.name,
                                                          _host=self.automl_settings.service_url)
    ...
    def _init_adb_driver_run(self,
        ...
        self._console_logger.write(
            'Running on ADB cluster experiment {0}.'.format(self.automl_settings.name))
        ...
        try:
            ...
            aml_token = self.run_history_client._client.run.\
                get_token(experiment_name=self.automl_settings.name,
                          ...
                          run_id=self.current_run.run_id).token
    ...
    def _create_parent_run(self, X=None, y=None, sample_weight=None, X_valid=None, y_valid=None,
        ...
        if self.automl_settings.exclude_nan_labels:
            self.input_data = _y_nan_check(self.input_data)
        _set_task_parameters(y=self.input_data.get(
            'y'), automl_settings=self.automl_settings)
        ...
        if self.user_script:
            self.logger.info(
                "[ParentRunID:{}] Local run using user script.".format(self.parent_run_id))
        else:
            self.logger.info(
                "[ParentRunID:{}] Local run using input X and y.".format(self.parent_run_id))
        ...
        self._log_data_stat(self.input_data.get(
            "X"), 'X', prefix="[ParentRunId:{}]".format(self.parent_run_id))
        self._log_data_stat(self.input_data.get(
            "y"), 'y', prefix="[ParentRunId:{}]".format(self.parent_run_id))
    ...
    def _fit_local(self, X=None, y=None, sample_weight=None, X_valid=None, y_valid=None, sample_weight_valid=None,
        ...
        if self.automl_settings.experiment_timeout_minutes:
            Timer(self.automl_settings.experiment_timeout_minutes * 60, self.cancel).start()
            ...
            while self.current_iter < self.automl_settings.iterations:
                ...
                if self._status == constants.Status.Terminated:
                    ...
                    self.logger.info("Stopping criteria reached at iteration {0}. "
                                     "Ending experiment.".format(self.current_iter - 1))
                    self._jasmine_client.set_parent_run_status(
    ...
    def _fit_iteration(self, ci, transformed_data_context=None):
        ...
        train_frac = (pipeline_dto.training_percent or 100) / 100
        ...
        ci.print_pipeline(preprocessor, model_name)
    ...
    def _modify_run_configuration(self, run_config):
        ...
        """
        try:
            import azureml.core.VERSION as core_version
            import azureml.train.automl.VERSION as automl_version
        except Exception:
            core_version = None
            automl_version = None
        ...
        if self.automl_settings.sdk_url is not None:
            dependencies.set_pip_option(
                "--extra-index-url " + self.automl_settings.sdk_url)
