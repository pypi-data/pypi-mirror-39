...
# ---------------------------------------------------------
"""Classes and methods for generating engineered feature names
for features extracted using pre-processing"""
import copy
import re
...
class _GenerateEngineeredFeatureNames:
    ...
    The following schema design is followed for storing the engineered
    names:-
    ...
    def __init__(self):
        ...
        # json objects
        self.alias_raw_feature_name_transformation_mapping = dict()
    ...
    def are_engineered_feature_names_available(self):
        """
        This function return 'True' if engineered feature names have already
    ...
    def get_raw_feature_alias_name(self, transformation_json_obj):
        """
        This function takes a list of transformations that need to be done
        on a raw feature and returns the alias name for the raw feature.
    ...
    def parse_raw_feature_names(self, transformed_raw_feature_names):
        """
        Given the transformed raw feature names, compose engineered
        feature names and store the JSON object for these engineered
        JSON objects in a disctionary
        : param transformed_raw_feature_names: A list of string which
                                               are the transformed feature
                                               names from sklearn
                                               transformations
        """
    ...
    def get_json_object_for_engineered_feature_name(self,
        ...
        """
        :param engineered_feature_name: Engineered feature name for whom
...
class FeatureNamesHelper:
    ...
    def get_regular_exp_for_parsing_raw_feature_names(cls):
        """
        This function returns the regular expression required for
        parsing the transformed feature names
        """
        return r"(\d+)(_*)(.*)"
    ...
    def get_transformer_name(cls, transformer_number):
        """
        This function returns a string for transformer name which is added
        in the json representation of the transformations
        """
...
class _FeatureTransformers:
    ...
    def encode_transformations_from_list(self):
        """
...
class _FeatureTransformersAsJSONObject:
    ...
    def __init__(self):
        ...
        self._transformer_as_json = dict()
    ...
    def __str__(self):
        """
        Returns an engineered feature name string for the feature
        """
        return self.get_engineered_feature_name_from_json()

    def get_engineered_feature_name_from_json(self):
        """
        This function takes the json format for an engineered feature
        and returns the '_' separated string representation of the
        engineered feature
    ...
    def _get_engineered_feature_name_from_json_internal(
        ...
        """
        This function takes the json format for an engineered
        feature and forms the '_' separated string representation
        of the engineered feature recursively
        :param json_data: json object for the engineered feature name
    ...
    def get_raw_feature_type(self):
        """
        The function takes a json object for the transformations and returns
        a string for the type of raw feature inside the JSON object
        :param transformation_json_data: JSON object for the transformations
        :return: A string having the raw feature name (numeric,
                 categorical etc.)
        """
        ...
        # the raw feature
        for transformer_key in \
            self._entire_transformation_json_data[
                _FeatureNameJSONTag.Transformations]:
            transformer = self._entire_transformation_json_data[
                _FeatureNameJSONTag.Transformations][transformer_key]
...
class DateTimeHelper:
    ...
    def get_datetime_tranformation_name(cls, index):
        """
        This function returns the date time sub-feature given an index value
