    ...
    for name in metrics:
        ...
        if name in [constants.Metric.AccuracyTable,
            ...
            try:
                ...
                metric = metric_class(y_test, y_pred_probs_full)
...
def compute_metrics_regression(y_pred, y_test,
                               metrics,
                               y_max, y_min, y_std,
                               sample_weight=None,
                               bin_info=None):
    """Computes the metrics for a regression task
    :param metrics: list of metric names from constants.py
    :param y_pred: predicted value
    :param y_test: target value
    :param metrics: metric/metrics to compute
    :param y_max: max target value
    :param y_min: min target value
    :param y_std: standard deviation of targets value
    :param sample_weight:
    :param bin_info: binning information for true values
...
class AccuracyTableMetric(ClassificationMetric):
    ...
    def _build_tables(self, class_labels):
        ...
        y_indices = [y_label_map[v] for v in self._y.tolist()]
        ...
        prob_thresholds = np.linspace(0, 1, num_points).tolist()
        percs = [pt * 100 for pt in prob_thresholds]

        for c_y_bin, c_y_pred in data:
            perc_thresholds = np.percentile(c_y_pred, percs).tolist()
    ...
    def _include_missing_labels(self, class_labels, prob_tables,
        ...
        for class_label in class_labels:
            if class_label in y_label_map:
                ...
                empty_table = np.zeros((len(self._y), 4), dtype=int).tolist()
    ...
    def _create_class_samples(self, class_y_bin, class_y_pred, thresholds):
        ...
        for threshold in thresholds:
            ...
            conf_values = np.array([tp, fp, tn, fn], dtype=int).tolist()
    ...
    def compute(self, sample_weights=None, class_labels=None):
        ...
        return AccuracyTableMetric._data_to_dict(self._data)
    ...
    def aggregate(scores):
        ...
        data_agg = {
            AccuracyTableMetric.PROB_TABLES: (
                np.sum(prob_tables, axis=0).tolist()),
            AccuracyTableMetric.PERC_TABLES: (
                np.sum(perc_tables, axis=0).tolist()),
        ...
        return AccuracyTableMetric._data_to_dict(data_agg)
...
class ConfusionMatrixMetric(ClassificationMetric):
    ...
    def _compute_matrix(self, class_labels, sample_weights=None):
        ...
        str_c_labels = class_labels.astype(str)
        y_pred_str_labels = str_c_labels[y_pred_indexes]
        y_true_str = self._y.astype(str)
        return sklearn.metrics.confusion_matrix(y_true=y_true_str,
                                                y_pred=y_pred_str_labels,
                                                sample_weight=sample_weights,
                                                labels=str_c_labels).tolist()
    ...
    def compute(self, sample_weights=None, class_labels=None):
        ...
        return ConfusionMatrixMetric._data_to_dict(self._data)
    ...
    def aggregate(scores):
        ...
        data_agg = {
            ...
            ConfusionMatrixMetric.MATRIX: matrix_sum.tolist()
        }
        return ConfusionMatrixMetric._data_to_dict(data_agg)
...
class ResidualsMetric(RegressionMetric):
    ...
    def compute(self, sample_weights=None, bin_info=None,
        ...
        self._data[ResidualsMetric.EDGES] = edges.tolist()
        self._data[ResidualsMetric.COUNTS] = counts.tolist()
        return ResidualsMetric._data_to_dict(self._data)

    def _hist_by_bound(self, values, bound, num_bins):
        bound = int(abs(bound))
        bins = np.r_[-np.inf, np.linspace(-bound, bound, num_bins), np.inf]
    ...
    def aggregate(scores):
        ...
        agg_counts = np.sum(counts, axis=0).tolist()

        data_agg = {
            ResidualsMetric.EDGES: agg_edges,
            ResidualsMetric.COUNTS: agg_counts
        }
        return ResidualsMetric._data_to_dict(data_agg)
...
class PredictedTrueMetric(RegressionMetric):
    ...
    def compute(self, sample_weights=None, bin_info=None,
        ...
        self._data = new_data
        return PredictedTrueMetric._data_to_dict(self._data)
    ...
    def _legacy_format_to_data(d):
        bin_starts = list(d[constants.Preds_Reg.TRUE_VALS_BINS_START])
        bin_ends = list(d[constants.Preds_Reg.TRUE_VALS_BINS_END])
        bin_averages = list(d[constants.Preds_Reg.AVG_PRED_VALS_PER_BIN])
        bin_errors = list(d[constants.Preds_Reg.PRED_ERROR_PER_BIN])
        bin_counts = list(d[constants.Preds_Reg.TRUE_VALS_COUNT_PER_BIN])
        bin_edges = bin_starts
        bin_edges.append(bin_ends[-1])

        bin_averages = np.array(bin_averages, dtype=float).tolist()
        bin_errors = np.array(bin_errors, dtype=float).tolist()
        bin_counts = np.array(bin_counts, dtype=int).tolist()
        bin_edges = np.array(bin_edges, dtype=float).tolist()
    ...
    def aggregate(scores):
        ...
        return PredictedTrueMetric._data_to_dict(data_agg)
