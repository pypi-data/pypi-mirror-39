...
def _get_column_data_type_as_str(array):
    """
    This function gets the type of ndarray by looking into the
    ndarray contents.
    :param array: ndarray
...
def _all_dependencies():
    """
    This method retrieves the packages from the site-packages folder by using
    pkg_resources
...
def _is_sdk_package(name):
    """
    This method checks if a package is in sdk by checking the whether the
    package startswith('azureml').
    """
...
def stratified_shuffle(indices, y, random_state):
    """
    shuffles an index in a way such that the first 1%, 2%, 4% etc.
    are all stratified samples.
