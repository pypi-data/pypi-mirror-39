    ...
    def __init__(self,
        ...
        if task == constants.Tasks.CLASSIFICATION:
            ...
            if not self.user_settings.get('enable_tf'):
                bl = [constants.SupportedAlgorithms.TensorFlowDNNClassifier,
            ...
            if not self.user_settings.get('enable_tf'):
                bl = [constants.SupportedAlgorithms.TensorFlowDNNRegressor,
        ...
        if (not enable_tf or blacklist_models is not None) and whitelist_models is not None:
            if blacklist_models is not None:
                bl.extend(blacklist_models)
            if set(whitelist_models).issubset(set(bl)):
                raise ValueError("Can not find models to train, all whitelisted models are also in blacklist")

        if enable_cache and spark_context:
            logging.warning("enable_cache is not supported under spark")
            enable_cache = False
        ...
        self.user_settings["enable_subsampling"] = enable_subsampling
        self.user_settings["subsample_seed"] = subsample_seed
