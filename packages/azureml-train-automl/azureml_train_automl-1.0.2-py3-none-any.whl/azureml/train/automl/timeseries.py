...
class MissingDummiesTransformer(BaseEstimator, TransformerMixin, TransformerLogger):
    ...
    def __init__(self, numerical_columns, logger=None):
        """
        Constructor for MissingDummiesTransformer
...
class NumericalizeTransformer(BaseEstimator, TransformerMixin, TransformerLogger):
    ...
    def __init__(self, logger=None):
        """
        Constructor for NumericalizeTransformer
...
class TimeSeriesTransformer(BaseEstimator, TransformerMixin, TransformerLogger):
    ...
    def __init__(self, param_dict, logger=None):
        ...
        self.columns_to_drop = param_dict.get(TimeSeries.DROP_COLUMN_NAMES)
    ...
    def _construct_pre_processing_pipeline(self, tsdf, columns_to_drop):
        ...
        if len(columns_to_drop) > 0:
            default_pipeline.add_pipeline_step('drop_irrelevant_columns', DropColumns(columns_to_drop),
                                               prepend=True)
    ...
    def fit(self, df, y=None):
        ...
        all_columns_to_drop = [x for x in tsdf.columns if
                               np.sum(tsdf[x].notnull()) == 0]
        if isinstance(self.columns_to_drop, str):
            all_columns_to_drop.extend([self.columns_to_drop])
        elif self.columns_to_drop is not None:
            all_columns_to_drop.extend(self.columns_to_drop)

        self.freq = tsdf.infer_freq(True)
        self.pipeline = self._construct_pre_processing_pipeline(tsdf, all_columns_to_drop)
