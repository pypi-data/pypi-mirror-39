...
def _get_model(file_path, just_return_model=False):
    ...
    model = None
    try:
        remaining_modules = {k:v for k,v in sys.modules.items() if k not in existing_modules}
        print("*"*200)
        for k,v in remaining_modules.items():
            try:
                if not os.path.isabs( v.__file__) :
                    print(v.__file__, 11111)
                    print(v)
            except:
                pass
