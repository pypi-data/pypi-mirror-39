...
class BaseFunc(object):
    ...
    def AfterDaysLocalTime(self, days, ntype):
        ...
        """
        if ntype == '1':
            return (datetime.datetime.now() + datetime.timedelta(days=int(days))).strftime("%Y-%m-%d %H:%M")
        elif ntype == '2':
            return (datetime.datetime.now() + datetime.timedelta(days=int(days))).strftime("%Y年%m月%d日")
        else:
