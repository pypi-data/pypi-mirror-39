...
class Component(object):
    ...
    stat_data : dict, str, or None
        Statistical data used for load variance. Set to None to remove noise.
        Set to 'auto' to automatically make noise based on dataset. Pass a
        dict with statistical parameters to manually adjust noise.
    is_re : bool
    ...
    def __init__(
        ...
        life, fail_prob, stat_data, is_re, **kwargs
    ):
        ...
        self.stat_data = stat_data  # statistical parameters
    ...
    def noise_calc(self, hr):
        ...
        if self.stat_data is None:
            ...
            fail = np.random.rand(self.num_case) > self.fail_prob
            noise = self._noise_calc(hr)
            noise = noise*fail
    ...
    def update_init(self):
        ...
        if (self.num_case is not None and self.size is not None):
            ...
            if self.stat_data is None:
                ...
                self.stat_dict = self._stat_derive()
            elif isinstance(self.stat_data, dict):
                for v in self.stat_data:
                    if np.atleast_1d(self.stat_data[v]).size == 1:
                        self.stat_dict[v] = self.stat_data[v]*np.ones(8760)
                    else:
                        self.stat_dict[v] = self.stat_data[v]
