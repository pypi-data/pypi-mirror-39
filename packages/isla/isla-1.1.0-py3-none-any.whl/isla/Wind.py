...
class Wind(IntermittentComponent):
    ...
    def __init__(
        ...
        life=20.0, fail_prob=0.01, stat_data=None, **kwargs
    ):
        ...
        super().__init__(
            ...
            life, fail_prob, stat_data, True, **kwargs
        )
    ...
    def _stat_derive(self):
        ...
        return {'beta': np.array(beta_list)}
    ...
    def _noise_calc(self, hr):
        ...
        """
        beta = self.stat_dict['beta']
        if self.stat_data == 'auto':
            noise = np.random.weibull(
                beta[hr % 24], self.num_case
            )
        else:
            noise = np.random.weibull(beta, self.num_case)
