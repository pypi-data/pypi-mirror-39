...
def txn_replay(session_filename, txn, proxy, result_queue, h2conn, request_IDs):
    ...
    try:
        ...
        #                            headers=txn_req_headers,stream=False) # making stream=False raises contentdecoding exception? kill me
        method = req.getMethod()
        response = None
        mbody = None
        #txn_req_headers['Host'] = "localhost"
        ...
        if 'Connection' in txn_req_headers:
            del txn_req_headers['Connection']
        ...
        if method == 'GET':
            responseID = h2conn.request('GET', url=extractHeader.extract_GET_path(
                txn_req_headers), headers=txn_req_headers, body=mbody)
            ...
            responseID = h2conn.request('HEAD', url=extractHeader.extract_GET_path(
                txn_req_headers), headers=txn_req_headers)
            print("get response", responseID)
            return responseID
    ...
    while bSTOP == False:
        for session in iter(input.get, 'STOP'):
            ...
            if session == 'STOP':
                ...
                bSTOP = True
                break
            txn = session.getFirstTransaction()
            req = txn.getClientRequest()
            # Construct HTTP request & fire it off
            txn_req_headers = req.getHeaders()

            with h2ATS(txn_req_headers['Host'], secure=True, proxy_host=glb.proxy_host, proxy_port=glb.proxy_ssl_port) as h2conn:
                request_IDs = []
                respList = []
                for txn in session.getTransactionIter():
                    try:
                        ret = txn_replay(session._filename, txn,
                                         proxy, result_queue, h2conn, request_IDs)
                        respList.append(txn.getProxyResponse())
                ...
                for id in request_IDs:
                    ...
                    if glb.verify:
                        for field, value in response.headers.items():
                            response_dict[field.decode(
                                'utf-8')] = value.decode('utf-8')

                        r = result.Result(
                            "", expectedH.getStatus(), response.status, response.read())
                        b_res, res = r.getResult(
                            response_dict, expectedH.getHeaders(), colorize=glb.colorize)
                        print(res)
