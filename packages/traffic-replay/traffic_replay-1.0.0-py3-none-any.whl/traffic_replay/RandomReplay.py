...
# import h2Replay commented out because h2 will be implemented at a later date
bSTOP = False
            ...
            with requests.Session() as request_session:
                ...
                for txn in session.getTransactionIter():
                    ...
                    try:
                        if type % 3 == 0:
                            NonSSL.txn_replay(
                                session._filename, txn, proxy, result_queue, request_session)
                        elif type % 3 == 1:
                            SSLReplay.txn_replay(
                                session._filename, txn, proxy, result_queue, request_session)
                        # elif type % 3 == 2:
                        #     h2Replay.txn_replay(
                        #         session._filename, txn, proxy, result_queue, request_session)
...
def client_replay(input, proxy, result_queue, nThread):
    ...
    for i in range(nThread):
        ...
        t2 = Thread(target=SSLReplay.session_replay,
                    args=[input, proxy, result_queue])
        t = Thread(target=NonSSL.session_replay,
                   args=[input, proxy, result_queue])
        # t1 = Thread(target=h2Replay.session_replay,
        #             args=[input, proxy, result_queue])
        t2.start()
        t.start()
        # t1.start()
