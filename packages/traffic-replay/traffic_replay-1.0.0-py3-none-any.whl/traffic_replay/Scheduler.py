...
from trlib import SessionValidator as sv

import traffic_replay.WorkerTask as WorkerTask
...
def LaunchWorkers(path, nProcess, proxy, replay_type, nThread):
    ...
    # sessions.sort(key=lambda session: session.getTimestamp())
    Processes = []
    ...
    for session in sessions:
        if replay_type == 'mixed':
            ...
            if "https" == session.getFirstTransaction().getClientRequest().getScheme():
    ...
    for i in range(nProcess):
        ...
        if replay_type == 'mixed':
            if i & 1:  # odd/SSL
                p = Process(target=WorkerTask.worker, args=[
                            QList[i], OutputQ, proxy, 'ssl', nThread])
            else:  # even/nonSSL
                p = Process(target=WorkerTask.worker, args=[
                            QList[i], OutputQ, proxy, 'nossl', nThread])
        else:
            p = Process(target=WorkerTask.worker, args=[
                        QList[i], OutputQ, proxy, replay_type, nThread])
