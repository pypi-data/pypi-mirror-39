...
class ExploreApi(object):
    ...
    def aggregate(self, collection, agg, **kwargs):
        ...
        :param list[str] pwithin: Any element having its centroid contained within the given bbox : 'top, left, bottom, right'. 
        ...
        :param list[str] notpwithin: Any element having its centroid outside the given bbox : 'top, left, bottom, right'.
    ...
    def aggregate_with_http_info(self, collection, agg, **kwargs):
        ...
        all_params = ['collection', 'agg', 'f', 'q', 'pwithin', 'gwithin', 'gintersect', 'notpwithin', 'notgwithin', 'notgintersect', 'pretty', 'max_age_cache']
    ...
    def count_with_http_info(self, collection, **kwargs):
        ...
        all_params = ['collection', 'f', 'q', 'pwithin', 'gwithin', 'gintersect', 'notpwithin', 'notgwithin', 'notgintersect', 'pretty', 'max_age_cache']
        all_params.append('callback')
        all_params.append('_return_http_data_only')
        all_params.append('_preload_content')
        all_params.append('_request_timeout')
    ...
    def geoaggregate(self, collection, agg, **kwargs):
        ...
        :param callback function: The callback function
            for asynchronous request. (optional)
        :param str collection: collection (required)
        :param list[str] agg: - The agg parameter should be given in the following formats:            {type}:{field}:interval-{interval}:format-{format}:collect_field-{collect_field}:collect_fct-{function}:order-{order}:on-{on}:size-{size}:fetcbGeometry-{fetchGeometry values}   Where :      - **{type}:{field}** part is mandatory.       - **interval** must be specified only when aggregation type is datehistogram, histogram and geohash.      - **format** is optional for datehistogram, and must not be specified for the other types.      - (**collect_field**,**collect_fct**) couple is optional for all aggregation types.      - (**order**,**on**) couple is optional for all aggregation types.      - **size** is optional for term and geohash, and must not be specified for the other types.      - **include** is optional for term, and must not be specified for the other types.   - {type} possible values are :           geohash, datehistogram, histogram and term. geohash must be the main aggregation.   - {interval} possible values depends on {type}.           If {type} = datehistogram, then {interval} = {size}(year,quarter,month,week,day,hour,minute,second). Size value must be equal to 1 for year,quarter,month and week unities.           If {type} = histogram, then {interval} = {size}.           If {type} = geohash, then {interval} = {size}. It's an integer between 1 and 12. Lower the length, greater is the surface of aggregation.           If {type} = term, then interval-{interval} is not needed.    - format-{format} is the date format for key aggregation. The default value is yyyy-MM-dd-hh:mm:ss.   - {collect_fct} is the aggregation function to apply to collections on the specified {collect_field}.      {collect_fct} possible values are :           avg,cardinality,max,min,sum,geobbox,geocentroid   - {fetchGeometry} is to be specified for `geohash` and `term` aggregations only   - {fetchGeometry} : When it's centroid : the geoaggregation geometry is the centroid of each bucket.   - {fetchGeometry} : When it's bbox: the geoaggregation geometry is the data extend (bbox) in each bucket.   - {fetchGeometry} : When it's geohash: the geoaggregation geometry is the geohash extend of each bucket. It's applied only for Geohash aggregation tye. It is not supported for term aggregation type.    - {fetchGeometry} : When it's first: the geoaggregation geometry is the geometry of the first document in each bucket (chronogically)   - {fetchGeometry} : When it's last: the geoaggregation geometry is the geometry of the last document in each bucket (chronogically)   - {fetchGeometry} : When it's {field}-first: the geoaggregation geometry is the geometry of the first document in each bucket (ordered on the {field})   - {fetchGeometry} : When it's {field}-last: the geoaggregation geometry is the geometry of the last document in each bucket (ordered on the {field})   - (collect_field,collect_fct) should both be specified, except when collect_fct = `geobbox` or `geocentroid`, it could be specified alone. The metrics `geobbox` and `geocentroid` are returned as features collections.   - {order} is set to sort the aggregation buckets on the field name, on the count of the buckets or on the the result of a metric sub-aggregation. Its values are 'asc' or 'desc'.    - {on} is set to specify whether the {order} is on the field name, on the count of the aggregation or on the result of a metric sub-aggregation. Its values are 'field', 'count' or 'result'.    - When {on} = `result`, then (collect_field,collect_fct) should be specified. Except when {collect_fct} = `geobbox` or `geocentroid`, then {on}=`result` is prohibited   - {size} Defines how many buckets should be returned.    - {include} Specifies the values for which buckets will be created. This values are comma separated. If one value is specified then regular expressions can be used (only in this case) and buckets matching them will be created. If more than one value are specified then only buckets matching the exact values will be created.   If {fetchGeometry} is specified, the returned geometry is the one used in the geojson.   If **fetchGeometry-centroid** and **collect_fct**=`geocentroid` are both set, the centroid of each bucket is only returned as the geo-aggregation geometry and not in the metrics. Same for **fetchGeometry-bbox** and **collect_fct**=`geobbox`   **agg** parameter is multiple. The first (main) aggregation must be geohash. Every agg parameter specified is a subaggregation of the previous one : order matters.    For more details, check https://github.com/gisaia/ARLAS-server/blob/master/docs/arlas-api-exploration.md  (required)
        :param list[str] f: - A triplet for filtering the result. Multiple filter can be provided in distinct parameters (AND operator is applied) or in the same parameter separated by semi-colons (OR operator is applied). The order does not matter.    - A triplet is composed of a field name, a comparison operator and a value.      The possible values of the comparison operator are :           Operator |                   Description                    | value type          :eq:     | {fieldName} equals {comma separated values}. **OR** operation is applied for the specified values | numeric or strings           :ne:     | {fieldName} must not equal {comma separated values }. **AND** operation is applied for the specified values | numeric or strings           :like:   | {fieldName}  is like {value}                     | numeric or strings           :gte:    | {fieldName} is greater than or equal to  {value} | numeric           :gt:     | {fieldName} is greater than {value}              | numeric           :lte:    | {fieldName} is less than or equal to {value}     | numeric           :lt:     | {fieldName}  is less than {value}                | numeric           :range:  | {fieldName} is between `{comma separated [min<max] values}`. **OR** operation is applied for the specified ranges | numeric or strings. If the field's type is date, then min & max should be timestamps in millisecond or a Date expression
        :param list[str] q: A full text search. Optionally, it's possible to search on a field using this syntax: {fieldname}:{text}
        :param list[str] pwithin: Any element having its centroid contained within the given bbox : 'top, left, bottom, right'. 
        :param list[str] gwithin: Any element having its geometry contained within the given geometry (WKT)
        ...
        :param list[str] notgintersect: Any element having its geometry not intersecting the given geometry (WKT)
        :param bool pretty: Pretty print
        :param bool flat: Flat the property map: only key/value on one level
        :param int max_age_cache: max-age-cache
    ...
    def geoaggregate_with_http_info(self, collection, agg, **kwargs):
        ...
        all_params = ['collection', 'agg', 'f', 'q', 'pwithin', 'gwithin', 'gintersect', 'notpwithin', 'notgwithin', 'notgintersect', 'pretty', 'flat', 'max_age_cache']
        all_params.append('callback')
        all_params.append('_return_http_data_only')
        all_params.append('_preload_content')
        all_params.append('_request_timeout')
    ...
    def geohashgeoaggregate_with_http_info(self, collection, geohash, **kwargs):
        ...
        all_params = ['collection', 'geohash', 'agg', 'f', 'q', 'pwithin', 'gwithin', 'gintersect', 'notpwithin', 'notgwithin', 'notgintersect', 'pretty', 'flat', 'max_age_cache']
        all_params.append('callback')
        all_params.append('_return_http_data_only')
        all_params.append('_preload_content')
        all_params.append('_request_timeout')
    ...
    def geosearch(self, collection, **kwargs):
        ...
        :param bool pretty: Pretty print
        :param bool flat: Flat the property map: only key/value on one level
        :param list[str] include: List the name patterns of the field to be included in the result. Seperate patterns with a comma.
        :param list[str] exclude: List the name patterns of the field to be excluded in the result. Seperate patterns with a comma.
        ...
        :param list[str] sort: - Sort the result on the given fields ascending or descending.    - Fields can be provided several times by separating them with a comma. The order matters.    - For a descending sort, precede the field with '-'. The sort will be ascending otherwise.   - For a geodistance sort, specify the point, from which the distances are calculated, as follow : 'geodistance:lat lon'   
        :param int max_age_cache: max-age-cache
        :return: FeatureCollection
                 If the method is called asynchronously,
                 returns the request thread.
    ...
    def geosearch_with_http_info(self, collection, **kwargs):
        ...
        all_params = ['collection', 'f', 'q', 'pwithin', 'gwithin', 'gintersect', 'notpwithin', 'notgwithin', 'notgintersect', 'pretty', 'flat', 'include', 'exclude', 'size', '_from', 'sort', 'max_age_cache']
        all_params.append('callback')
        all_params.append('_return_http_data_only')
        all_params.append('_preload_content')
        all_params.append('_request_timeout')
    ...
    def get_arlas_hit_with_http_info(self, collection, identifier, **kwargs):
        ...
        all_params = ['collection', 'identifier', 'pretty', 'max_age_cache']
        all_params.append('callback')
        all_params.append('_return_http_data_only')
        all_params.append('_preload_content')
        all_params.append('_request_timeout')
    ...
    def range_with_http_info(self, collection, field, **kwargs):
        ...
        all_params = ['collection', 'field', 'f', 'q', 'pwithin', 'gwithin', 'gintersect', 'notpwithin', 'notgwithin', 'notgintersect', 'pretty', 'max_age_cache']
        all_params.append('callback')
        all_params.append('_return_http_data_only')
        all_params.append('_preload_content')
        all_params.append('_request_timeout')
    ...
    def search(self, collection, **kwargs):
        ...
        :param int size: The maximum number of entries or sub-entries to be returned. The default value is 10
        :param int _from: From index to start the search from. Defaults to 0.
        :param str sort: - Sort the result on the given fields ascending or descending.    - Fields can be provided several times by separating them with a comma. The order matters.    - For a descending sort, precede the field with '-'. The sort will be ascending otherwise.   - For a geodistance sort, specify the point, from which the distances are calculated, as follow : 'geodistance:lat lon'   
        :param int max_age_cache: max-age-cache
        :return: Hits
                 If the method is called asynchronously,
                 returns the request thread.
    ...
    def search_with_http_info(self, collection, **kwargs):
        ...
        all_params = ['collection', 'f', 'q', 'pwithin', 'gwithin', 'gintersect', 'notpwithin', 'notgwithin', 'notgintersect', 'pretty', 'include', 'exclude', 'size', '_from', 'sort', 'max_age_cache']
        all_params.append('callback')
        all_params.append('_return_http_data_only')
        all_params.append('_preload_content')
        all_params.append('_request_timeout')
    ...
    def tiledgeosearch_with_http_info(self, collection, x, y, z, **kwargs):
        ...
        all_params = ['collection', 'x', 'y', 'z', 'f', 'q', 'pwithin', 'gwithin', 'gintersect', 'notpwithin', 'notgwithin', 'notgintersect', 'pretty', 'flat', 'include', 'exclude', 'size', '_from', 'sort', 'max_age_cache']
        all_params.append('callback')
        all_params.append('_return_http_data_only')
        all_params.append('_preload_content')
        all_params.append('_request_timeout')
    ...
    def tiledgeosearch1_with_http_info(self, collection, x, y, z, **kwargs):
        ...
        all_params = ['collection', 'x', 'y', 'z', 'f', 'q', 'pwithin', 'gwithin', 'gintersect', 'notpwithin', 'notgwithin', 'notgintersect', 'size', '_from', 'sort', 'sampling', 'coverage', 'max_age_cache']
        all_params.append('callback')
        all_params.append('_return_http_data_only')
        all_params.append('_preload_content')
        all_params.append('_request_timeout')
