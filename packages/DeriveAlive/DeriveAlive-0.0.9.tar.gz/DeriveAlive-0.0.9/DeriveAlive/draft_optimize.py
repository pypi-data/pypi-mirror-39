...
def _BFGSVector(f, var_list, iters=10000, tol=1e-10, eta=0.01, valid_data=False):
    ...
    inv_hessian = np.eye(2)

    for i in range(iters):
        step = -1 * np.matmul(inv_hessian, g.der)
        values_flat_update = values_flat + step

        y = f(values_flat_update).der - f(values_flat).der
        identity = np.eye(2)
        rho = 1 / np.matmul(np.transpose(y), step)

        A = identity - np.matmul(np.vstack(step), rho * y.reshape(1, 2))
        B = identity - np.matmul(rho * np.vstack(y), s.reshape(1, 2))
        C = rho * np.matmul(np.vstakc(step), s.reshape(1, 2))
...
def BFGS(f, x, iters=10000, tol=1e-10, eta=0.01, data=[]):
    """ Run the gradient descent algorithm to minimize a function f.
    ...
    if isinstance(x, list):
        ...
        if m > 1:
            for i in range(len(x)):
                if not isinstance(x[i], da.Var):
                    x[i] = da.Var(x[i], _get_unit_vec(m, i))
            return _BFGSVector(f, x, iters=iters, tol=tol, eta=eta, valid_data=valid_data)
        x = x[0]
    if not isinstance(x, da.Var):
        x = da.Var(x)
    ...
    for i in range(iters):
        # Take step in direction of steepest descent
        step = da.Var(-1 * np.matmul(inv_hessian, g.der), None)
        x_update = x + step
        y = f(xUpdate).der - f(x).der

        identity = np.eye(2)

        rho * np.reshape(y.val, [-1, 1])

        A = identity - step.val * rho * np.reshape(y.val, [-1, 1])
        B = identity - rho * np.reshape(y.val, [-1, 1]) * step.val
        C = rho * np.reshape(y.val, [-1, 1]) * step.val

        delta_hessian = np.matmul(np.matmul(A, inv_hessian), B) + C

        g = f(xUpdate)
        var_path.append(xUpdate.val)
        g_path.append(g.val)
