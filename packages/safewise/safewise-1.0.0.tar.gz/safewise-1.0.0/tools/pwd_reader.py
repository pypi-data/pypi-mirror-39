...
def getMasterKey(client):
    ...
    ENC_KEY = 'Activate SafeWISE Password Manager?'
...
def getDecryptedNonce(client, entry):
    print()
    print('Waiting for SafeWISE input ...')
    print()
    if 'item' in entry:
        item = entry['item']
    else:
...
def main():
    ...
    print()
    print('Confirm operation on SafeWISE')
    print()
    ...
    path = os.path.expanduser('~/Dropbox/Apps/SafeWISE Password Manager/')
