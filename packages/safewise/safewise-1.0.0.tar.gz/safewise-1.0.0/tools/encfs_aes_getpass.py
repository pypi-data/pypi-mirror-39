...
'''
Use SafeWISE as a hardware key for opening EncFS filesystem!
...
def wait_for_devices():
    ...
    while not len(devices):
        sys.stderr.write("Please connect SafeWISE to computer and press Enter...")
        input()
        devices = enumerate_devices()
...
def choose_device(devices):
    if not len(devices):
        raise RuntimeError("No SafeWISE connected!")
...
def main():
    ...
    if not os.path.exists(passw_file):
        ...
        sys.stderr.write('Computer asked SafeWISE for new strong password.\n')
        sys.stderr.write('Please confirm action on your device.\n')

        # 32 bytes, good for AES
        safewise_entropy = client.get_entropy(32)
        urandom_entropy = os.urandom(32)
        passw = hashlib.sha256(safewise_entropy + urandom_entropy).digest()
