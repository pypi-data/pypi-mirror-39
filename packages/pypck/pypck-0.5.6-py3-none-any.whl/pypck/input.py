...
class Input(object):
    ...
    An implementation of :class:`~pypck.inputs.Input` has to provide easy accessible attributes
    and/or methods to expose the PCK command properties to the user.
    Each Input object provides an implementation of :func:`~pypck.inputs.Input.try_parse`
    static method, to parse the given plain text PCK command. If the command can be parsed by
    the Input object, a list of instances of :class:`~pypck.inputs.Input` is returned. 
    If parsing is successful, the :func:`~pypck.inputs.Input.process` method can be called
    ...
    def try_parse(input):
        ...
        :type     input:     :class:`~pypck.inputs.Input`
        :return:             The parsed Inputs (never null)
        :rtype:              List with instances of :class:`~pypck.inputs.Input`
        """
        raise NotImplementedError

    def process(self, conn):
        """
        Processes the :class:`~pypck.inputs.Input` instance and triggers further actions.
...
class ModInput(Input):
    ...
    The class in inherited from :class:`~pypck.inputs.Input`
    """
