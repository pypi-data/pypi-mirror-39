...
def read_cluster_data(basepath, date):
    ...
    pbdata.ParseFromString(read_bytes(__get_full_path(os.path.join(basepath, Source.CLUSTERS.name), date)))
    samples = []
...
def read_cp_data(basepath, sourcepath, date, fullpath=None):
    pbdata = gv_pb_data.CpData()
    if fullpath is None:
        fullpath = __get_full_path(os.path.join(basepath, sourcepath), date)
    pbdata.ParseFromString(read_bytes(fullpath))
    samples = []
    for pbsample in pbdata.sample:
        samples.append({
...
def read_section_data(basepath, date):
    ...
    pbdata.ParseFromString(read_bytes(__get_full_path(os.path.join(basepath, Source.SECTIONS.name), date)))
    samples = []
    for pbsample in pbdata.sample:
        samples.append({
...
def read_zone_data(basepath, date):
    ...
    pbdata.ParseFromString(read_bytes(__get_full_path(os.path.join(basepath, Source.ZONES.name), date)))
    samples = []
    for pbsample in pbdata.sample:
        samples.append({
...
def __get_full_path(basepath, date):
