...
class Synset(object):
    ...
    def get_relations(self, type: str) -> Generator['Relation', None, Iterable['Relation']]:
        if type in Relation.types[self.pos]:
            ...
            raise ValueError(f"No relation of type '{type}' for '{self.pos}'!")
...
class Morpho(object):
    ...
    @property
    def stem(self) -> str:
        groups = re.search(r'^[\S][\S][\S][\S][\S][\S][\S][\S][\S]([ir-])$', self.miscellanea)
        return groups.group(1) if groups else ''
            ...
            if not result or (pos in ('n', 'v', 'a', 'r') and not result[0]):
                ...
                if pos == '*':
                    ...
                    if len(resolved_pos) > 1:
                        ...
                        pos = resolved_pos
                _is_phrase = False
                language_lemma = db(language, "lemma")
                if language_lemma:
                    ...
                    language_index = db(language, "synset")
                    if language_index:
...
class WordNet(object):
    ...
    def get(self, lemma, pos='*', strict=True) -> List[object]:
        try:
            ...
            if results:
                ...
                _list = None
        return list(_list) if _list else []
            ...
            try:
                ...
                if results:
                    for result in results:
                        ...
                        yield semfield
        yield from (semfield for semfield in self._semfields)
    ...
    def lemmas(self) -> Generator[object, None, Iterable[object]]:
        if self._lemmas is None:
            try:
                ...
                temp = list()
                pos = ['*', 'n', 'v', 'a', 'r']
                for result in language_index:
                    for i in range(1, 5):
                        if result[i]:
                            try:
                                _lemma = Lemma(result[0], pos[i], self.language)
                            except TypeError:
                                _lemma = Phrase(result[0], pos[i], self.language)
                            else:
                                temp.append(_lemma)
                                yield _lemma
                    self._lemmas = temp
    ...
    def relations(self) -> Generator['Relation', None, Iterable['Relation']]:
        ...
        yield from (relation for relation in self._relations)
    ...
    def get_relations(self, *, source: Synset=None, target: Synset=None, w_source: Lemma=None, w_target: Lemma=None, type=None, lexical=False) -> Generator['Relation', None, Iterable['Relation']]:
        if not lexical and (w_source or w_target):
            raise ValueError("Source or target lemma given for non-lexical relation")
        elif lexical and (not w_source and not w_target):
            raise ValueError('Lexical relations must have a source and target lemma')
        else:
            temp = list()
            qr = ''
            if lexical:
                if w_source:
                    qr = f" WHERE w_source='{w_source.lemma}'"
                if w_target:
                    if w_source:
                        qr += f" AND w_target='{w_target.lemma}'"
                    else:
                        qr = f" WHERE w_target='{w_target.lemma}'"
            ...
            try:
                ...
                if results:
                    for result in results:
                        relation = Relation(*result, self.language)
                        temp.append(relation)
            yield from (relation for relation in temp)
