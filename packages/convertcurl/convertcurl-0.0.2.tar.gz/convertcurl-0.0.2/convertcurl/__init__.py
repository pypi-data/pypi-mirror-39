...
def url(args, kwargs, express) -> str:
    if '?' not in args.curl[1] or args.params:
        return args.curl[1]
    url, string = args.curl[1].split('?', 1)
...
def cookie(string) -> dict:
    return dict(i.split('=', 1) for i in string.split('; '))
...
def main():
    ...
    parser.add_argument('curl', nargs='+', help='curl command string')
    ...
    print()
    text = f'url = "{url(args, kwargs, express)}"\n'
