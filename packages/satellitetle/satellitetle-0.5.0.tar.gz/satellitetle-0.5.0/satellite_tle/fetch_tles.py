...
def fetch_tles(requested_norad_ids):
    ...
    for source, url in sources:
        ...
        try:
            ...
            logging.error('Failed to download from {}.'.format(source))
            continue
        except ValueError:
            logging.error('Failed to parse catalog from {}.'.format(source))
            continue
    ...
    for norad_id in missing_norad_ids:
        try:
            ...
            logging.error('Fetch {} from Celestrak (satcat) failed!'.format(norad_id))
            continue
