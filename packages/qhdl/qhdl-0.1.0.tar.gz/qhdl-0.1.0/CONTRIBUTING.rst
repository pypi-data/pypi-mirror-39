...
Report bugs at https://github.com/mabuchilab/qhdl/issues.
...
The best way to send feedback is to file an issue at https://github.com/mabuchilab/qhdl/issues.
...
3. Check https://travis-ci.org/mabuchilab/qhdl/pull_requests
...
Ready to contribute? Follow `Aaron Meurer's Git Workflow Notes`_ (with ``mabuchilab/qhdl`` instead of ``sympy/sympy``)

In short,

1. Clone the repository from ``git@github.com:mabuchilab/qhdl.git``
...
Python versions.
You still can (and should) look at https://travis-ci.org/mabuchilab/qhdl/ to check that your commits pass all tests.
...
QHDL uses the `git-flow`_ branching model. That is, the ``develop`` branch takes the role of ``master`` in the `Git Workflow Notes`_.

In order to create topic branches with ``git flow``, after cloning the  ``qhdl`` repository, you should initialize it as follows:
...
QHDL includes a full test-suite using pytest_.
We strive for a `test coverage`_ above 90%.


From a checkout of the ``qhdl`` repository, assuming conda_ is installed, you can use
...
.. _test coverage: https://coveralls.io/github/mabuchilab/qhdl?branch=develop
...
QHDL could always use more documentation, whether
...
At any point, from a checkout of the ``qhdl`` repository (and
...
``qhdl``, and that you have successfully run ``make test`` (which creates
...
To run e.g. only the tests defined in ``tests/test_qhdl.py``, use::

    $ ./.venv/py37/bin/pytest tests/test_qhdl.py
