...
@click.pass_context
def ps(ctx, raw, trunc, workspace, verbose):
    """
    ...
    with api_client_exception_handler():
        ...
        if workspace is None:
            ...
            workspaces = client.get_workspaces_by_name(workspace)
            if len(workspaces) == 0:
                logger.error("No workspace found named %s", workspace)
                return
            runs = client.list_runs([str(ws.id) for ws in workspaces])
    ...
    for run in runs:
        ...
        run.ws_name = run.workspace.name if run.workspace else ""
