...
from smart_contract.origin_v1 import ProducedEnergy, ConsumedEnergy
from smart_contract.origin.producer_v1 import contract as producer_v1
from core.output import GeneralSmartContractClient
...
class OriginProducer(OriginV1):
    ...
    def mint(self, produced_energy: ProducedEnergy) -> dict:
        """
        Source:
        ...
        """
        if not isinstance(produced_energy.value, int):
        ...
        receipt = self.send_raw('producer', 'saveSmartMeterRead', self.asset_id, produced_energy.value,
...
class OriginConsumer(OriginV1):
    ...
    def mint(self, consumed_energy: ConsumedEnergy) -> dict:
        """
        Source:
        ...
        """
        if not isinstance(consumed_energy.value, int):
            raise ValueError('No Produced energy present or in wrong format.')
        if not isinstance(consumed_energy.is_meter_down, bool):
            raise ValueError('No Produced energy status present or in wrong format.')
        if not isinstance(consumed_energy.previous_hash, str):
            raise ValueError('No Produced hash of last file present or in wrong format.')
        receipt = self.send_raw('consumer', 'saveSmartMeterRead', self.asset_id, consumed_energy.value,
