...
class Xanity:
    ...
    def savemetadata(self):
        ...
        for anal in self.avail_analyses.values():
            del anal.module
        if PY_SYSVER == 3:
            pickle.dump(self, open(path.join(self.exp_data_dir, 'xanity_metadata.dill'), mode='wb'), pickle.HIGHEST_PROTOCOL)
        elif PY_SERVER ==2:
            pickle.dump(self, open(path.join(self.exp_data_dir, 'xanity_metadata.dill'), mode='w'), pickle.HIGHEST_PROTOCOL)
    ...
    def run_one_exp(self, experiment, index):
        ...
        os.makedirs(experiment.subexp_dirs[index], exist_ok=False) if PY_SYSVER == 3 else os.makedirs(experiment.subexp_dirs[index])
...
########################################################
    ...
    def save_variable(self, value):
        ...
        if PY_SYSVER == 3:
            pickle.dump(value, open(path.join(datapath, name + '.dill'), 'wb'), pickle.HIGHEST_PROTOCOL)
        elif PY_SYSVER ==2:
            pickle.dump(value, open(path.join(datapath, name + '.dill'), 'w'), pickle.HIGHEST_PROTOCOL)
    ...
    def load_variable(self, name):
        ...
        for dpath in datapaths:
            if PY_SYSVER == 3:
                vals.append(pickle.load(open(path.join(dpath, name + '.dill'), 'rb'), pickle.HIGHEST_PROTOCOL))
            elif PY_SYSVER == 2:
                vals.append(pickle.load(open(path.join(dpath, name + '.dill'), 'r'), pickle.HIGHEST_PROTOCOL))
    ...
    def persistent(self, name, value=None):
        ...
        if value and not path.isfile(filename):
            ...
            pickle.dump(value, open(filename, 'wb'), pickle.HIGHEST_PROTOCOL) if PY_SYSVER == 3 \
                else pickle.dump(value, open(filename, 'w'), pickle.HIGHEST_PROTOCOL)
            return value
        
        # load the saved value and return it:
        if path.isfile(filename):
            return pickle.load(open(filename, 'rb'), pickle.HIGHEST_PROTOCOL) if PY_SYSVER == 3 \
                else pickle.load(open(filename, 'r'), pickle.HIGHEST_PROTOCOL)
        else:
            return None
    ...
    def freeze_conda(self):
        if PY_SYSVER == 3:
            pickle.dump(self.hash_conda_env_file(), open(self.conf_files.conda_hash, 'wb'), pickle.HIGHEST_PROTOCOL)
            pickle.dump(self.hash_conda_env(), open(self.conf_files.env_hash, 'wb'), pickle.HIGHEST_PROTOCOL)
        elif PY_SYSVER == 2:
            pickle.dump(self.hash_conda_env_file(), open(self.conf_files.conda_hash, 'w'), pickle.HIGHEST_PROTOCOL)
            pickle.dump(self.hash_conda_env(), open(self.conf_files.env_hash, 'w'), pickle.HIGHEST_PROTOCOL)
    ...
    def check_conda_env(self):
        ...
        saved_hash = pickle.load(open(self.conf_files.env_hash, 'rb'), pickle.HIGHEST_PROTOCOL) if PY_SYSVER ==3 \
            else pickle.load(open(self.conf_files.env_hash, 'r'), pickle.HIGHEST_PROTOCOL)
    ...
    def check_conda_file(self):
        ...
        saved_hash = pickle.load(open(self.conf_files.conda_hash, 'rb'), pickle.HIGHEST_PROTOCOL) if PY_SYSVER == 3 \
            else pickle.load(open(self.conf_files.conda_hash, 'r'), pickle.HIGHEST_PROTOCOL)
    ...
    def hash_conda_env(self):
        ...
        conda_env_contents = subprocess.check_output([
    ...
    def _get_live_module(self, module_path):
        ...
        module_name = path.split(module_path)[-1].split('.py')[0]
        if PY_SYSVER == 2:
