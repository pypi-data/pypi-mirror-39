...
def parser(xanity, clargs=None):
    ...
    if clargs:
        list_to_parse = list(clargs)
    else:
        list_to_parse = sys.argv
    ...
    action_arg, remaining_args = parser.parse_known_args(list_to_parse)

    if xanity.called_from_hook:
    ...
    xanity.args, xanity.unknownargs = parser.parse_known_args(list_to_parse)
