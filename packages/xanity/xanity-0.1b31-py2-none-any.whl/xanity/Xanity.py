...
from .common import digest_file, digest_string, pickle_dump, pickle_load
...
class Xanity:
    ...
    def __init__(self, clargs=None):
        ...
        if self.project_root:
            ...
            self.uuid = open(self.conf_files.uuid, mode='r').read()
        self._parse_args()

    def _orient(self):
        self.status.activity = ACTIVITIES.ORIENT
        if self.action in COMMANDS.RUN + COMMANDS.ANAL:
    ...
    def _parse_args(self):
        ...
        if self.clargs:
            self.args, self.unknownargs = self.parser.parse_known_args(self.clargs)
        else:
            self.args, self.unknownargs = self.parser.parse_known_args()

        if self.project_root and (self.paths.experiments in self.command or self.paths.experiments in self.command):
    ...
    def _resolve_requests(self):
        ...
        # if analyzing define anals to run
            ...
            if hasattr(self.args, 'analyses') and self.args.analyses:
            ...
            if hasattr(self.args, 'runid') and self.args.runid:
...
########################################################
    ...
    def hash_conda_env(self):
        ...
        try:
            ...
            if 'EnvironmentLocationNotFound' in repr(e):
                return None
            else:
                raise e
