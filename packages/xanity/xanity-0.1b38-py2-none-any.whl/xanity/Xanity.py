...
class Xanity(object):
    ...
    def _parse_args(self, clargs=None):
        ...
        if hasattr(args, 'action'):
            if args.action in COMMANDS:
                ...
                self.action = getattr(self, args.action)
    ...
    def _orient(self, clargs=None):
        ...
        # sensitive ordering
        if not self.project_root:
            self._populate_root_info(self._resolve_xanity_root())
    ...
    @ContextDecorator(ACTIVITIES.INTERNAL)
    def _fint_recent_run_path(self):
        expnames = set()
        for anal in self.analyses.values():
    ...
    @ContextDecorator(ACTIVITIES.INTERNAL)
    def _resolve_xanity_root(self, hint=None):
        """ set xanity_root """
    ...
    def run(self, clargs=None):
        ...
        self._absolute_trigger_()

    @ContextDecorator(ACTIVITIES.EXTERNAL)
    def find_recent_data(self):
        return self._fint_recent_run_path()
