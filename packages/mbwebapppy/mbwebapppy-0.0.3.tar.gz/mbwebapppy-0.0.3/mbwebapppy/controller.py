...
class Controller(object):
        ...
        with self.__lock:
            if cur_time - self.last_update_time > self.update_interval:
                for car in self.cars:
                    car.odometer = self.get_odometer(car.finorvin)
                    #car.stateofcharge = self.get_state_of_charge(car.id)
                    #car.doors = self.get_doors(car.id)
                    car.tires = self.get_tires(car.finorvin)
    ...
    def get_odometer(self, car_id):
        ...
        api_result = self._retrieve_api_result(car_id, "status")

        odometer = Odometer()

        for odometer_option in ODOMETER_OPTIONS:
            if api_result.get("data") is not None:
                option = api_result.get("data").get(odometer_option)
                option_status = CarAttribute(option.get("value"),
                                             option.get("status"),
                                             option.get("ts"))
                setattr(odometer, odometer_option, option_status)
            else:
                _LOGGER.warn("get_odometer for %s failed", car_id)        
    ...
    def get_tires(self, car_id):
        ...
        for tire_option in TIRE_OPTIONS:
            if api_result.get("data") is not None:
                curr_tire = api_result.get("data").get(tire_option)
    ...
    def _retrieve_json_at_url(self, url):
        ...
        if res.status_code == 403:
            _LOGGER.warn(
                "Session invalid, try relogin with http code %s", res.status_code)
            self.is_valid_session = False
            self.session = None
            self.session = requests.session()
            self._get_session_cookies(self.username, self.password)
            return
        _LOGGER.info("Connect to URL %s Status Code: %s Content: %s", str(url),
