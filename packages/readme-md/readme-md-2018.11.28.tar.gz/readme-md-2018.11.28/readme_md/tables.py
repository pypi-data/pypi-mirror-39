...
def functions(functions):
    ...
    for func in functions:
        ...
        spec = readme_md.get.spec(func)
        module = inspect.getmodule(func)
        fullname = "%s.%s" % (module.__name__, spec)
        doc = readme_md.get.doc(func)
...
def attrs(cls):
    ...
    matrix = []
    for name, value in readme_md.get.attrs(cls):
...
def methods(cls):
    ...
    matrix = []
    for name, method in readme_md.get.methods(cls):
        spec = readme_md.get.spec(method)
        spec = spec.replace("self, ", "").replace("(self)", "()")
        doc = readme_md.get.doc(method)
...
def properties(cls):
    ...
    maxtrix = []
    for name, prop in readme_md.get.properties(cls):
        doc = readme_md.get.doc(prop)
