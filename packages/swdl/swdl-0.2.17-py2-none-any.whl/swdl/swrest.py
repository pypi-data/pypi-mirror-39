...
class DataService:
    ...
    """
    def __init__(self, username=None, password_hashed=None, api_stream_url=None):
        if not username:
            user_file = expanduser("~/.swdlrc")
            if os.path.exists(user_file):
                user_config = json.loads(open(user_file).read())
        ...
        if api_stream_url:
            self.api_stream_url = api_stream_url+self.url_extension
    ...
    def get(self, url):
        ...
        try:
            ...
            if (ret.status_code != 200):
                print(url,ret.status_code, ret.text)
                return None
    ...
    def post(self, url, **kwargs):
        ...
        """
        try:
            ret = requests.post(url, json=kwargs)
            if (ret.status_code != 200):
                print(url,ret.status_code, ret.text)
                return None
            return json.loads(ret.text)
    ...
    def get_matches(self):
        ...
        """
        url = "{}/match/all/{}/{}".format(self.api_stream_url,
                                          self.user_escaped,
                                          self.password)
    ...
    def get_match(self, match_id):
        ...
        """
        url = "{}/match/single/{}/{}/{}".format(self.api_stream_url,
                                                match_id,
                                                self.user_escaped,
                                                self.password)
    ...
    def get_events(self, match_id):
        ...
        """
        url = "{}/tags/{}/{}/{}".format(self.api_stream_url,
                                        match_id,
                                        self.user_escaped,
                                        self.password)
        result = self.get(url)
    ...
    def add_event(self, match_id, event_id, timestamp):
        ...
        self.post(url, eventType=event_id, timestamp=timestamp)
    ...
    def upload_player_positions(self, timestamp, match):
        try:
            ...
            url = "{}/playerPositions/{}/{}/{}/{}".format(self.api_stream_url,
                                                          match_id,
                                                          timestamp,
                                                          self.user_escaped,
                                                          self.password)
    ...
    def get_positions(self,
        ...
        url = "{}/positions/request/{}/{}/{}".format(self.api_stream_url,
                                                     match_id,
                                                     self.user_escaped,
                                                     self.password)
        ret = self.post(url,
                        virtualCameraId=virtual_camera_id,
                        lastModified=last_modified,
                        source=source)
        if "positions" in ret:
            ret = ret["positions"]
        return ret if (ret or type(ret)==list) else []
    ...
    def pull_labels(self, match,
        ...
        # Get positions
        pos_unsorted = self.get_positions(match_id,
                                          time_from,
                                          virtual_camera_id,
                                          source)
        ...
        if pos_unsorted is not None and len(pos_unsorted) > 0:
            ...
            if highest_timestamp + 1 > label.positions.shape[0]:
                label.postions = label.positions.resize((highest_timestamp/label_resolution + 1,
                                                         7), refcheck=False)
            ...
            for p in positions:
                ...
                label.positions[timestamp/label_resolution] = temp
