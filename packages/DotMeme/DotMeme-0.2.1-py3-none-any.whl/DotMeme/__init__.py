...
def meme(memeFilePath, *points, imageName="meme.png", saveMeme=False):
	...
	try:
		...
		memeFilePath = os.path.join(__path__[0], 'Memes', memeFileName)
	try:
	...
	for point in ymlData['text']:
		...
		fontLocation = os.path.join(__path__[0], 'font.ttf')
...
def listMemes():
	...
	"""
	for meme in os.listdir(os.path.join(__path__[0], 'Memes')):
		yield meme
