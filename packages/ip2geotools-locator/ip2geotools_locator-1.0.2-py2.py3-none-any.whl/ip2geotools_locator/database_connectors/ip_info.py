...
class IpInfoDB:
    ...
    def __init__(self, api_key):
        ...
        if api_key == None:
            logger.critical("%s: Database %s needs API key to work!" % (__name__, IpInfo.__name__))
    ...
    def get_location(self, ip):
        ...
        try:
            ...
            self.__db_data = IpInfo.get(ip, self.__api_key)
    ...
    def add_to_map(self):
        ...
        """
        try:
            logger.debug("%s: Calling add_marker method for %s DB" % (__name__, IpInfo.__name__))
