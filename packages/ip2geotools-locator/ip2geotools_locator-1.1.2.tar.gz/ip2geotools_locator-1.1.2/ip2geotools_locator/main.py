...
class Locator:
    ...
    def __init__(self, generate_map = True):
        ...
        logger.debug("%s: Settings parsed for commercial and noncommercial databases." % __name__)
    ...
    def get_locations(self, ip, databases = []):
        ...
        """
        self.ip_address = ip
        logger.info("%s: Gathering location records for IP address %s." % (__name__, ip))

        # Change settings according to provided db list;
        if "noncommercial" in databases:
            logger.debug("%s: Adjusting settings for all noncommercial databases." % (__name__))
            
            # Each noncomercial DB is activated
            for db in self.noncommercial:
                self.noncommercial[db]["active"] = True
            # Each comercial DB is deactivated
            for db in self.commercial:
                self.commercial[db]["active"] = False
        
        elif "commercial" in databases:
            logger.debug("%s: Adjusting settings for all commercial databases." % (__name__))
            
            # Each noncomercial DB is deactivated
            for db in self.noncommercial:
                self.noncommercial[db]["active"] = False
            # Each comercial DB is activated
            for db in self.commercial:
            ...
            # Configure each DB
            for i in databases:
                logger.debug("%s: Adjusting settings for database %s." % (__name__, i))
        ...
        if self.noncommercial["host_ip"]["active"]:
            logger.debug("%s: database HostIP is set as Active in settings. Gathering location." % __name__)
            ...
            if self.noncommercial["host_ip"]["generate_marker"]:
                logger.debug("%s: Folium marker for HostIP database is set as Active in settings. Calling add_to_map method." % __name__)
                host_ip.add_to_map()


        # Json settings parsed for DbIpCity database
        if self.noncommercial["ip_city"]["active"]:
            logger.debug("%s: database DbIpCity is set as Active in settings. Gathering location." % __name__)
            ip_city = IpCityDB()
            self.locations.append(ip_city.get_location(ip))
            
            if self.noncommercial["ip_city"]["generate_marker"]:
                logger.debug("%s: Folium marker for DbIpCity database is set as Active in settings. Calling add_to_map method." % __name__)
                ip_city.add_to_map()


        # Json settings parsed for Ip2location database
        if self.noncommercial["ip2location"]["active"]:
            logger.debug("%s: database Ip2location is set as Active in settings. Gathering location." % __name__)
            ...
            if self.noncommercial["ip2location"]["generate_marker"]:
                logger.debug("%s: Folium marker for Ip2location database is set as Active in settings. Calling add_to_map method." % __name__)
                ip2location.add_to_map()


        # Json settings parsed for Ipstack database
        if self.noncommercial["ipstack"]["active"]:
            logger.debug("%s: database Ipstack is set as Active in settings. Gathering location." % __name__)
            ...
            if self.noncommercial["ipstack"]["generate_marker"]:
                logger.debug("%s: Folium marker for Ipstack database is set as Active in settings. Calling add_to_map method." % __name__)
                ipstack.add_to_map()


        # Json settings parsed for MaxMind GeoLite2City database
        if self.noncommercial["max_mind_lite"]["active"]:
            logger.debug("%s: database MaxMind GeoLite2City is set as Active in settings. Gathering location." % __name__)
            ...
            if self.noncommercial["max_mind_lite"]["generate_marker"]:
                logger.debug("%s: Folium marker for MaxMind GeoLite2City database is set as Active in settings. Calling add_to_map method." % __name__)
        ...
        if self.commercial["eurek"]["active"]:
            logger.debug("%s: database Eurek is set as Active in settings. Gathering location." % __name__)
            eurek = EurekDB()
            self.locations.append(eurek.get_location(ip))
            
            if self.commercial["eurek"]["generate_marker"]:
                logger.debug("%s: Folium marker for Eurek database is set as Active in settings. Calling add_to_map method." % __name__)
                eurek.add_to_map()
        

        # Json settings parsed for GeobytesCityDetails database
        if self.commercial["geobytes_city"]["active"]:
            logger.debug("%s: database GeobytesCityDetails is set as Active in settings. Gathering location." % __name__)
            ...
            if self.commercial["geobytes_city"]["generate_marker"]:
                logger.debug("%s: Folium marker for GeobytesCityDetails database is set as Active in settings. Calling add_to_map method." % __name__)
                geobytes_city.add_to_map()
        

        # Json settings parsed for IP Info database
        if self.commercial["ip_info"]["active"]:
            logger.debug("%s: database IP Info is set as Active in settings. Gathering location." % __name__)
            ...
            if self.commercial["ip_info"]["generate_marker"]:
                logger.debug("%s: Folium marker for IP Info database is set as Active in settings. Calling add_to_map method." % __name__)
                ip_info.add_to_map()
        

        # Json settings parsed for DbIpWeb database
        if self.commercial["ip_web"]["active"]:
            logger.debug("%s: database DbIpWeb is set as Active in settings. Gathering location." % __name__)
            ip_web = IpWebDB()
            self.locations.append(ip_web.get_location(ip))
            
            if self.commercial["ip_web"]["generate_marker"]:
                logger.debug("%s: Folium marker for DbIpWeb database is set as Active in settings. Calling add_to_map method." % __name__)
                ip_web.add_to_map()
        ...
        if self.commercial["max_mind"]["active"]:
            logger.debug("%s: database MaxMindGeoIp2City is set as Active in settings. Gathering location." % __name__)
            max_mind = MaxMindDB()
            self.locations.append(max_mind.get_location(ip))
            
            if self.commercial["max_mind"]["generate_marker"]:
                logger.debug("%s: Folium marker for MaxMindGeoIp2City database is set as Active in settings. Calling add_to_map method." % __name__)
                max_mind.add_to_map()
        

        # Json settings parsed for NeustarWeb database
        if self.commercial["neustar_web"]["active"]:
            logger.debug("%s: database NeustarWeb is set as Active in settings. Gathering location." % __name__)
            neustar_web = NeustarWebDB()
            self.locations.append(neustar_web.get_location(ip))
            
            if self.commercial["neustar_web"]["generate_marker"]:
                logger.debug("%s: Folium marker for NeustarWeb database is set as Active in settings. Calling add_to_map method." % __name__)
                neustar_web.add_to_map()
        

        # Json settings parsed for Skyhook database
        if self.commercial["skyhook"]["active"]:
            logger.debug("%s: database Skyhook is set as Active in settings. Gathering location." % __name__)
            skyhook = SkyhookDB()
            self.locations.append(skyhook.get_location(ip))
            
            if self.commercial["skyhook"]["generate_marker"]:
                logger.debug("%s: Folium marker for Skyhook database is set as Active in settings. Calling add_to_map method." % __name__)
                skyhook.add_to_map()
    ...
    def calculate(self, average = True, clustering = False, interval = False, median = False):
        ...
        if average:        
            logger.debug("%s: Calculation of Averaged location is Active." % __name__)
        ...
        if interval:
            logger.debug("%s: Calculation of Confidence Interval from location is Active." % __name__)
            pass
        
        # Calculate Median from given locations
        if median:
            logger.debug("%s: Calculation of Median from locations is Active." % __name__)
        ...
        logger.info("%s: Returning list of locations: %s" % (__name__, str(calculated_locations)))
