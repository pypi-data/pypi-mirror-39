...
class HyperDriveRun(ScriptRun):
    ...
    def __init__(self, experiment, run_id, hyperdrive_run_config, run_config=None):
        ...
        self.run_history_client = RunHistoryClient.create(
            workspace=experiment.workspace,
            experiment_name=experiment.name,
            user_agent=HYPER_DRIVE_RUN_USER_AGENT)
    ...
    def _get_best_run_id_by_primary_metric(self):
        """Find and return the id of the Run instance that corresponds to the best performing run amongst all the
        completed runs.
    ...
    def get_metrics(self):
        ...
        child_run_ids = [run.id for run in self.get_children()]
        # noinspection PyProtectedMember
        return self.run_history_client._get_metrics_by_run_ids(child_run_ids)
