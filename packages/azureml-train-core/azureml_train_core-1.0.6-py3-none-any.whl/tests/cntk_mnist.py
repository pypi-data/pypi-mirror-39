...
parser.add_argument('--data_dir', type=str, help='input directory of MNIST data')
args = parser.parse_args()


def maybe_download(filename, work_dir, source_url):
    full_path = os.path.join(work_dir, filename)
    print("maybe_download fullpath is {}".format(full_path))
    if not os.path.exists(full_path):
        print("file {} not exists. Start to download ...".format(full_path))
        gzfname, h = urlretrieve(source_url + filename, './delete.me')
        return gzfname
    else:
        return full_path
...
def try_download(data_dir, image_file, label_file, cimg):
    mnist_url = 'http://yann.lecun.com/exdb/mnist/'
    data_gz_file = maybe_download(image_file, data_dir, mnist_url)
    data = loadData(data_gz_file, cimg)
    label_gz_file = maybe_download(label_file, data_dir, mnist_url)
    labels = loadLabels(label_gz_file, cimg)
...
if __name__ == '__main__':
    ...
    train = try_download(data_dir, train_image, train_labels, num_train_samples)

    
    num_test_samples = 10000

    print("Downloading test data")
    test = try_download(data_dir, test_image, test_labels, num_test_samples)
