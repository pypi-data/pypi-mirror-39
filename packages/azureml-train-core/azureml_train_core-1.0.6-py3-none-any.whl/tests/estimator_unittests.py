...
class EstimatorUnitTests(unittest.TestCase):
    ...
    def validate_estimator_config(self, est, script='train.py', args=[], user_deps=False,
        ...
        self.assertEqual(pip_packages_count, sum(1 for _ in est.conda_dependencies.pip_packages))

    def test_estimator_default_env(self):
        est = Estimator(self._project_folder, compute_target=self._mock_compute_target)
    ...
    def test_estimator_additional_pip_pkg(self):
        ...
        # index-url, extra-index-url and azureml-sdk included default
        self.assertEqual(3, sum(1 for _ in est.conda_dependencies.pip_packages))
    ...
    def test_estimator_use_gpu_setter(self):
        est = Estimator(self._project_folder, compute_target=self._mock_compute_target, use_gpu=True)
    ...
    def test_estimator_use_docker_setter(self):
        est = Estimator(self._project_folder, compute_target=self._mock_compute_target, use_docker=False)
    ...
    def test_estimator_pip_env_both(self):
        with self.assertRaises(Exception):
            custom_env = EnvironmentDefinition()
            custom_env.python.user_managed_dependencies = True
            Estimator(self._project_folder, compute_target=self._mock_compute_target,
    ...
    def test_estimator_use_docker_false_custom_image_set(self):
        with self.assertRaises(Exception):
            Estimator(self._project_folder, compute_target=self._mock_compute_target, use_docker=False,
    ...
    def test_datastore_as_project_dir(self):
        es = Estimator(
            self._project_folder, compute_target=self._mock_compute_target,
            source_directory_data_store=self._mock_data_ref_test)
        self.assertEqual(self._mock_data_ref_test.name, es._estimator_config.source_directory_data_store)

        df_keys = list(es.run_config.data_references.keys())
        self.assertIn(self._mock_data_ref_test.name, df_keys)
    ...
    @patch('azureml.train.estimator.MMLBaseEstimator._submit')
    def test_datastore_fit_override_source_data_store(self, mock_fit):
        es = Estimator(
            self._project_folder, compute_target=self._mock_compute_target)
        self.assertEqual(0, len(es.run_config.data_references))
        self.assertIsNone(es.run_config.source_directory_data_store)

        self._experiment.submit(es, source_directory_data_store=self._mock_data_ref_test)

        self.assertEqual(0, len(es.run_config.data_references))
        self.assertIsNone(es.run_config.source_directory_data_store)

        self.assertEqual(1, len(es._last_submitted_runconfig.data_references))
        self.assertEqual(self._mock_data_ref_test.name, es._last_submitted_runconfig.source_directory_data_store)

    def _validate_train_data_ref(self, dref):
        self.assertEqual(EstimatorUnitTests.DATA_STORE_NAME, dref.data_store_name)
        self.assertEqual(EstimatorUnitTests.PATH_ON_DATA_STORE, dref.path_on_data_store)
    ...
    def test_estimator_get_telemetry_values(self):
        ...
        self.assertEqual(False, telemetry_values['addCondaOrPipPackage'])
        self.assertEqual('batchai', telemetry_values['computeTarget'])
        self.assertIsNone(telemetry_values['vmSize'])

    def test_estimator_amlcompute_get_telemetry_values(self):
        est = Estimator(self._project_folder, vm_size="STANDARD_D1_v2",
                        entry_script='myscript.py', script_params={"arg1": 123, "arg2": "value"})
        telemetry_values = est._get_telemetry_values(est._fit)
        self.assertEqual('amlcompute', telemetry_values['computeTarget'])
        self.assertEqual('STANDARD_D1_v2', telemetry_values['vmSize'])

    def test_estimator_localcompute_get_telemetry_values(self):
        est = Estimator(self._project_folder, compute_target="local",
                        entry_script='myscript.py', script_params={"arg1": 123, "arg2": "value"})
        telemetry_values = est._get_telemetry_values(est._fit)
        self.assertEqual('local', telemetry_values['computeTarget'])

    @patch('azureml.train.estimator.MMLBaseEstimator._fit')
    def test_estimator_negative(self, mock_fit):
        try:
            Estimator(self._project_folder, compute_target=self._mock_compute_target_local, node_count=2,
    ...
    def test_custom_distrib_compute(self):
        estimator = Estimator(self._project_folder, compute_target=self._mock_compute_target)
        ...
        estimator = Estimator(self._project_folder, compute_target=self._mock_compute_target, node_count=2,
                              distributed_backend="mpi")
        self.assertEqual("Python", estimator._estimator_config.framework)
    ...
    def test_estimator_auto_create_cluster(self):
        est = Estimator(self._project_folder, vm_size="STANDARD_D1_V2", vm_priority="dedicated")
    ...
    def test_estimator_auto_create_cluster_distributed(self):
        est = Estimator(self._project_folder, vm_size="STANDARD_D1_V2", vm_priority="dedicated",
                        node_count=2, distributed_backend='mpi')
        self.assertEqual(DEFAULT_CPU_IMAGE, est.run_config.environment.docker.base_image)
        self.assertEqual("Python", est.run_config.framework)
        self.assertEqual("IntelMpi", est.run_config.communicator)
        self.assertEqual(est.run_config.target, "amlcompute")
        self.assertEqual(est.run_config.amlcompute.vm_size, "STANDARD_D1_V2")
        self.assertEqual(est.run_config.amlcompute.vm_priority, "dedicated")
    ...
    def test_fit_override_compute(self, mock_run_config, mock_conda_dependencies, mock_submit):
        ...
        estimator = Estimator(self._project_folder, vm_size="STANDARD_D1_V2", vm_priority="dedicated")
        self.assertEqual(DEFAULT_CPU_IMAGE, estimator.run_config.environment.docker.base_image)
    ...
    def test_estimator_no_compute(self):
        with self.assertRaises(Exception):
            Estimator(self._project_folder, pip_packages=['some package'])

    @patch('azureml.train.estimator.MMLBaseEstimator._submit')
    @patch('azureml.core.conda_dependencies.CondaDependencies.save_to_file')
    @patch('azureml.core.runconfig.RunConfiguration.save')
    def test_empty_value_script_params(self, mock_run_config, mock_conda_dependencies, mock_submit):
        # Case1: verbose flag value specified as empty string
        estimator = Estimator(self._project_folder, compute_target=self._mock_compute_target,
                              script_params={"param1": ""})
        self._experiment.submit(estimator)
        self.assertEqual(1, len(estimator._last_submitted_runconfig.arguments))
        self.assertTrue("param1" in estimator._last_submitted_runconfig.arguments)

        # Case2: verbose flag value specified as None
        estimator = Estimator(self._project_folder, compute_target=self._mock_compute_target,
                              script_params={"param1": None})
        self._experiment.submit(estimator)
        self.assertEqual(1, len(estimator._last_submitted_runconfig.arguments))
        self.assertTrue("param1" in estimator._last_submitted_runconfig.arguments)
    ...
    @patch('azureml.core.conda_dependencies.CondaDependencies.save_to_file')
    @patch('azureml.core.runconfig.RunConfiguration.save')
    def test_empty_value_script_params_override(self, mock_run_config, mock_conda_dependencies, mock_submit):
        estimator = Estimator(self._project_folder, compute_target=self._mock_compute_target)
        # Case1: verbose flag value specified as empty string
        self._experiment.submit(estimator, script_params={"param1": ""})
        self.assertEqual(1, len(estimator._last_submitted_runconfig.arguments))
        self.assertTrue("param1" in estimator._last_submitted_runconfig.arguments)

        # Case2: verbose flag value specified as None
        self._experiment.submit(estimator, script_params={"param1": None})
        self.assertEqual(1, len(estimator._last_submitted_runconfig.arguments))
        self.assertTrue("param1" in estimator._last_submitted_runconfig.arguments)
    ...
    @patch('azureml.core.conda_dependencies.CondaDependencies.save_to_file')
    @patch('azureml.core.runconfig.RunConfiguration.save')
    def test_multiple_value_script_params_override(self, mock_run_config, mock_conda_dependencies, mock_submit):
        estimator = Estimator(self._project_folder, compute_target=self._mock_compute_target)
        self._experiment.submit(estimator, script_params={"param1": ["val1", "val2"]})
        self.assertEqual(3, len(estimator._last_submitted_runconfig.arguments))
        self.assertTrue("param1" in estimator._last_submitted_runconfig.arguments)
        self.assertTrue("val1" in estimator._last_submitted_runconfig.arguments)
        self.assertTrue("val2" in estimator._last_submitted_runconfig.arguments)
