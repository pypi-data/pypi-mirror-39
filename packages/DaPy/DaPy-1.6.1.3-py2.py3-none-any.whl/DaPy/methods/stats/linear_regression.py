...
class LinearRegression:
    ...
    def fit(self, X, Y, solve='MLE', F=True, R=True, T=True):
        ...
        if T is True:
            ...
            t = [beta_ / (c_ * sigma_hat) for c_, beta_ in zip(c, betas)]
            try:
                from scipy.stats import t as t_test
                sigs = map(lambda x: 1.0 - t_test.cdf(x, n-p), t)
            except ImportError:
                sigs = ['-'] * len(t)
            ...
            for i, (beta_, t_, sig_) in enumerate(zip(betas[1:], t[1:], sigs[1:]), 1):
                table.append(['X_%d' % i, round(beta_, 4), round(t_, 4), sig_])
