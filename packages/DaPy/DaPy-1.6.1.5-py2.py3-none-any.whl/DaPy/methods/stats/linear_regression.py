...
class LinearRegression:
    ...
    def fit(self, x, y, w=None, solve='MLE', F=True, R=True, T=True):
        '''
        Parameter
        ---------
        X : matrix-like
        ...
        if w is None:
            W = self._engine.diag([1] * X.shape[1])
        else:
            W = self._engine.diag(w)
        self._fit_mle(X, Y, W)
        ...
        if T is True:
            ...
            try:
                ...
                sigs = map(lambda x: round(2 * t_test.cdf(min(x, -x), n-p), 4), t)
            except ImportError:
                sigs = ['-'] * len(t)
    ...
    def _fit_mle(self, X, Y, W):
        self._beta = X.dot(W).T.dot(X).I.dot(X.T).dot(Y)
