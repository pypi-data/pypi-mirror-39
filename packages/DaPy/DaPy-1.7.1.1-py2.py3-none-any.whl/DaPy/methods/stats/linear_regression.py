...
from DaPy.core import DataSet, Frame, SeriesSet
...
class LinearRegression:
    ...
    def _stepwise(self, X, Y, W, enter, drop, verbal=True, **kwrds):
    ...
    def _fit(self, X, Y, W=None, **kwrds):
        ...
        X, Y = mat(X), mat(Y)
        y_, c = self._fit_ls(self._engine.mat(X), self._engine.mat(Y), W, kwrds['variables'])
    ...
    def _Perf(self, R2, n ,p, Cp):
        ...
        RMSE = round(sqrt(self._engine.mean(self._res ** 2)), 6)
    ...
    def _Outliers(self, X, Y_h, n, p):
        ...
        SRE = self._res / (sigma_hat * ((1 - hs) ** 2))
        up, down = n - p - 2, n - p - 1.0
        SRE_del = abs((SRE * ((up / (down - SRE ** 2)) ** 0.5)).tolist())
        CookDis = (self._res ** 2) / ((p + 1) * sigma_hat ** 2) * hs / (1 - hs) ** 2
        mean_h = 3 * (p+1.0) / n
    ...
    def fit(self, X, Y, W=None, method='enter', **kwrds):
        ...
        if method.lower() == 'stepwise':
            ...
            return self._stepwise(X, Y, W, enter, drop, **kwrds)
