...
class BaseSheet(object):
    ...
    def __delitem__(self, key):
        assert isinstance(key, (int, str, list, tuple)), '`%s` should be '%key +\
                                                        'int, str or slice.'
        if isinstance(key, int):
            self.pop(key)
...
class SeriesSet(BaseSheet):
    ...
    def merge(self, other, self_key=0, other_key=0, keep_key=True, keep_same=True):
        ...
        def check_key(key, d, str_):
            ...
            assert isinstance(key, str), 'key shoule be a string or int type'
            assert key in d._columns, '`%s` is not a vaiable in %s dataset'% (key, str_)
            return key
        ...
        assert keep_key in (True, False, 'other', 'self'), '`keep_key` should be '+\
                                                   'in True, False, "other", "self"'
        assert keep_same in (True, False), '`keep_same` should be True or False.'
