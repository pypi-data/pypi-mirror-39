...
def mkdirOutputDir(aAlbumInfo, aTrackInfo = None, aPlaylistInfo = None, aVideoInfo = None):
    ...
    if aAlbumInfo != None:
        # creat outputdir
        title = pathHelper.replaceLimiChar(aAlbumInfo['title'], '-')
    ...
    if aPlaylistInfo != None:
        title = pathHelper.replaceLimiChar(aPlaylistInfo['title'], '-')
...
def downloadAlbum():
    ...
    while True:
        ...
        for item in aAlbumTracks['items']:
            ...
            aAllTask.append(thread.submit(thradfunc_dltrack, paraList))
        # wait all download thread
        wait(aAllTask, return_when=ALL_COMPLETED)
    return
