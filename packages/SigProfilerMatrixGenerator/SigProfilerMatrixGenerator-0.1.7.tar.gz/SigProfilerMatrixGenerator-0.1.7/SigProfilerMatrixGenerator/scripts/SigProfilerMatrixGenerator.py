...
def catalogue_generator_single (vcf_path, vcf_path_original, vcf_files, bed_file_path, chrom_path, project, output_matrix, context, exome, genome, ncbi_chrom, functionFlag, bed, bed_ranges, chrom_based, plot, tsb_ref, transcript_path, gs):
	'''
			...
			for lines in f:
				...
				# Saves all relevant data from the file
					...
					try:
						...
						if ref not in 'ACGT-':
							logging.info("The ref base is not recognized. Skipping this mutation: " + chrom + " " + str(start) + " " + ref + " " + mut)
							skipped_count += 1
							continue
						
						if mut not in 'ACGT-':
							logging.info("The mutation base is not recognized. Skipping this mutation: " + chrom + " " + str(start) + " " + ref + " " + mut)
							skipped_count += 1
							continue
						
						if ref == mut:
							logging.info("The ref base appears to match the mutated base. Skipping this mutation: " + chrom + " " + str(start) + " " + ref + " " + mut)
							skipped_count += 1
							continue

						if line == prev_line:
							logging.info("There appears to be a duplicate single base substitution. Skipping this mutation: " + chrom + " " + str(start) + " " + ref + " " + mut)
							skipped_count += 1
							continue
						...
						if flag:
							...
							try:
								...
								logging.info(chrom_start + " is not supported. You will need to download that chromosome and create the required files. Continuing with the matrix generation...")
								continue
							flag = False
						...
						for i in range (start-3, start+2, 1):
							try:
								...
								logging.info("The position is out of range. Skipping this mutation: " + chrom + " " + str(start) + " " + ref + " " + mut)
								skip_mut = True
								break
						if skip_mut:
							skipped_count += 1
						...
						if char != ref:# and revcompl(char) != ref:
							logging.info("The reference base does not match the reference genome. Skipping this mutation: "+ chrom + " "+ str(start) + " "+ ref + " "+ mut)
							skipped_count += 1
							continue				
						...
						logging.info("There appears to be an error in this line. Skipping this mutation: " + chrom + " " + str(start) + " " + ref + " " + mut)
						skipped_count += 1
	...
	# if skipped_count != 0:
	# 	print(str(skipped_count) + " single base substitution(s) were skipped ")
...
def catalogue_generator_DINUC_single (vcf_path, vcf_path_original, vcf_files, bed_file_path, chrom_path, project, output_matrix, exome, genome, ncbi_chrom, functionFlag, bed, bed_ranges, chrom_based, plot, tsb_ref):
	'''
	...
	# if skipped_count != 0:
	# 	print(str(skipped_count) + " double base substitutions skipped")
...
def catalogue_generator_INDEL_single (vcf_path, vcf_path_original, vcf_files, bed_file_path, chrom_path, project, output_matrix, exome, genome, ncbi_chrom, limited_indel, functionFlag, bed, bed_ranges, chrom_based, plot, tsb_ref, transcript_path, gs):
	'''
	...
	with open (vcf_path + vcf_files[0]) as data:
		...
		for lines in data:
			try:
				...
				for r in ref:
					if r not in 'ACGT-':
						logging.info("The ref contains bases that are not recognized. Skipping this mutation: " + chrom + " " + str(start) + " " + ref + " " + mut)
						skipped_count += 1
						skip = True
						break
				if skip:
				...
				for m in mut:
					if m not in 'ACGT-':
						logging.info("The mutation contains bases that are not recognized. Skipping this mutation: " + chrom + " " + str(start) + " " + ref + " " + mut)
						skipped_count += 1
						skip = True
						break
				if skip:
				...
				if first_flag:
					...
					try:
						...
						logging.info(initial_chrom + " is not supported. You will need to download that chromosome and create the required files. Continuing with the matrix generation...")
						continue
				...
				try:
					...
					logging.info("The start position is out of range. Skipping this mutation: " + chrom + " " + str(start) + " " + ref + " " + mut)
					skipped_count += 1
					continue

				if ref[0] != base and ref[0] != '-':
					logging.info("The reference base does not match the reference chromosome position. Skipping this mutation: " + chrom + " " + str(start) + " " + ref + " " + mut)
					skipped_count += 1
					continue
	...
	# if skipped_count != 0:
	# 	print(str(skipped_count) + " INDEL(s) were skipped ")
...
def panel_check (genome, bed_temp_file, output_matrix, bed_file_path, project):
	...
	#current_dir = os.getcwd()
	#ref_dir = re.sub('\/scripts/sigProfilerMatrixGenerator.py$', '', current_dir)
	#panel_file = ref_dir + "/references/vcf_files/BED/" + project + "/" + genome + ".bed"
	panel_file = bed_file_path
	#panel_output_path = ref_dir + "/references/vcf_files/" + project + "_panel/SNV/"
	panel_output_path = output_matrix + "vcf_files/SNV/"
	panel_output = panel_output_path + project + "_panel.vcf"
	#panel_output = ref_dir + "/references/vcf_files/" + project + "_panel/SNV/" + project + "_panel.vcf"
...
def matrix_generator_INDEL (output_matrix, samples, indel_types, indel_types_tsb, indel_dict, indel_tsb_dict, project, exome, limited_indel, bed, initial_chrom=None, plot=False):
	...
	if limited_indel:
		...
		file_prefix_tsb = project + ".INDEL83"
	else:
