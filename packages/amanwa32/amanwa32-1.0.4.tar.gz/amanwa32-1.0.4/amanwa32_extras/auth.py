...
#:     Default is :class:`amanwa32_extras.appengine.auth.models.User`.
#:
#: session_backend
...
default_config = {
    'user_model':      'amanwa32_extras.appengine.auth.models.User',
...
class AuthStore(object):
    ...
    def __init__(self, app, config=None):
        ...
        :param app:
            A :class:`amanwa32.WSGIApplication` instance.
        :param config:
    ...
    def user_model(self):
        ...
        if isinstance(cls, six.string_types):
            cls = self.config['user_model'] = amanwa32.import_string(cls)
...
_store_registry_key = 'amanwa32_extras.auth.Auth'
#: Key used to store :class:`Auth` in the request registry.
_auth_registry_key = 'amanwa32_extras.auth.Auth'
...
def get_store(factory=AuthStore, key=_store_registry_key, app=None):
    ...
    :param key:
        ...
        if it is not set.
    :param app:
        A :class:`amanwa32.WSGIApplication` instance used to store the instance.
        The active app is used if it is not set.
    """
    app = app or amanwa32.get_app()
...
def set_store(store, key=_store_registry_key, app=None):
    ...
    :param request:
        A :class:`amanwa32.WSGIApplication` instance used to retrieve the
