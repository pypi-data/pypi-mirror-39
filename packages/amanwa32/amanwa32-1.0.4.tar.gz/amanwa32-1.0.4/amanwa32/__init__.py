...
# google.appengine.ext.webapp imports amanwa32 in the
...
if os.environ.get('APPENGINE_RUNTIME') != 'python27':  # pragma: no cover
    try:
        ...
        # Running amanwa32 outside of GAE.
        pass

try:  # pragma: no cover
    # Thread-local variables container.
    from amanwa32_extras import local
    _local = local.Local()
except ImportError:  # pragma: no cover
    logging.warning("amanwa32_extras.local is not available "
                    "so amanwa32 won't be thread-safe!")
...
class RequestHandler(object):
    ...
    def __init__(self, request=None, response=None):
        ...
        Also in webapp dispatching is done by the WSGI app, while amanwa32
...
class Webapp3HandlerAdapter(BaseHandlerAdapter):
    """An adapter to dispatch a ``amanwa32.RequestHandler``.
...
class Router(object):
    ...
    def default_adapter(self, handler):
        ...
        This way amanwa32 can support, for example, a :class:`RequestHandler`
        ...
        if inspect.isclass(handler):
            if _webapp and issubclass(handler, _webapp.RequestHandler):
                ...
                # Default, compatible with amanwa32.RequestHandler.
...
class WSGIApplication(object):
    ...
    def set_globals(self, app=None, request=None):
        ...
        If :mod:`amanwa32_extras.local` is available, the app and request
        ...
        If :mod:`amanwa32_extras.local` is not available, app and request will
    ...
    def get_response(self, *args, **kwargs):
        ...
        the resulting response::
            ...
            class HelloHandler(amanwa32.RequestHandler):
                def get(self):
                    self.response.write('Hello, world!')

            app = amanwa32.WSGIapplication([('/', HelloHandler)])
