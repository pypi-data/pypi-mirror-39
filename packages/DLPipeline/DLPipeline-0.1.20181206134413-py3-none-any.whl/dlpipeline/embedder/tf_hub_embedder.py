...
class TFHubEmbedder(DLPDataIterator):
  ...
  def build(self, C):
    ...
    embedder_input_node = tf.placeholder(tf.float32, [None, height, width, depth], name='embedding_input')
    module = hub.Module(module_specification)
    embedding_output_node = module(embedder_input_node)
    ...
    self.embedder_input_node = embedder_input_node
    self.embedding_output_node = embedding_output_node
