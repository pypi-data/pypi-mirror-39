...
class ConfusionMatrixSaverHook(tf.train.SessionRunHook):
  ...
  def end(self, session):
    cm = tf.get_default_graph().get_tensor_by_name(  self.confusion_matrix_tensor_name + ':0').eval(session=session).astype(int)
...
def plot_confusion_matrix(*, correct_labels=None,
                          predict_labels=None,
                          labels,
  ...
  if normalize:
    conf_mat = conf_mat.astype('float') * 10 / conf_mat.sum(axis=1)[:, np.newaxis]
    conf_mat = np.nan_to_num(conf_mat, copy=True)
