...
class TFHubEmbedder(DLPDataIterator):
  ...
  def build(self, C):
    ...
    module_spec = hub.load_module_spec(C.embedding_module)
    height, width = hub.get_expected_image_size(module_spec)
    depth = 3

    with tf.Graph().as_default() as graph:
      resized_image_node = tf.placeholder(tf.float32, [None, height, width, depth])
      module = hub.Module(module_spec)
      embedding_node = module(resized_image_node)
      wants_quantization = any(node.op in FAKE_QUANT_OPS
                               for node in graph.as_graph_def().node)

    self.hub_module = NamedOrderedDictionary({
      'embedding_node':    embedding_node,
      'resized_image_node':resized_image_node,
      'quantize':          wants_quantization,
      'module_spec':       module_spec
      })

    self.graph = graph
    self.module_spec = module_spec
    self.resized_image_node = resized_image_node
    self.embedding_node = embedding_node
    self.quantize = wants_quantization
    self.input_size = hub.get_expected_image_size(self.hub_module.module_spec)
    self.depth_size = hub.get_num_image_channels(self.hub_module.module_spec)
    self.batch_size, self.embedding_size = self.hub_module.embedding_node.get_shape().as_list()

  def fetch_data(self, transformer, dataset,  *, batch_size=64, C, sub_set='training'):
    '''
    ...
    if True:
      if transformer.will_preprocess:
        (features, label) = self.get_random_distorted_embeddings_online(
            dataset,
            sub_set,
            input_jpeg_tensor=transformer._transformation_nodes.distorted_image_node,
            distorted_image=            transformer._transformation_nodes.distorted_jpeg_node,
            batch_size=C.batch_size,)
      else:
        (features, label, _) = self.get_random_cached_embeddings(
            dataset,
            sub_set,
            jpeg_data_tensor=transformer._transformation_nodes.jpeg_node,
            decoded_image_tensor=transformer._transformation_nodes.decoded_image_node,
  ...
  def create_embedding_file(self,
    ...
    try:
      embedding_values = self.run_embedding_on_image(
  ...
  def get_or_create_embedding(self,
    ...
    if not os.path.exists(embedding_file_path):
      self.create_embedding_file(embedding_file_path,
                                 dataset,
                                 label_name,
                                 index,
                                 category,
  ...
  def run_embedding_on_image(self,
    ...
    with tf.Session(config=config) as sess:
      ...
      embedding_values = sess.run(self.embedding_node,
                                  {self.resized_image_node:resized_input_values})
  ...
  def cache_embeddings(self,
    ...
    Args:
      ...
      images.
      embedding_directory: Folder string holding cached files of embedding values.
      jpeg_data_node: Input tensor for jpeg data from file.
  ...
  def get_random_distorted_embeddings_online(self,
    ...
    with tf.Session(config=config) as sess:
      for unused_i in range(batch_size):
        ...
        embedding_values = sess.run(self.embedding_node,
                                    feed_dict={self.resized_image_node:distorted_image_data})
        embedding_values = np.squeeze(embedding_values)
