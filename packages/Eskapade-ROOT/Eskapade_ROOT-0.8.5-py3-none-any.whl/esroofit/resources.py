...
# Fixtures
_FIXTURE = {_.name: _ for _ in pathlib.Path(resource_filename(esroofit.__name__, 'data')).glob('*')}

# C/C++ libraries that are shipped with eskapade.
if sys.platform == 'darwin':
    _LIBS = {_.name: _ for _ in pathlib.Path(resource_filename(esroofit.__name__, 'lib')).glob('*.dylib')}
else:
    _LIBS = {_.name: _ for _ in pathlib.Path(resource_filename(esroofit.__name__, 'lib')).glob('*.so')}

# Tutorials that are shipped with eskapade.
_TUTORIALS = {_.name: _ for _ in pathlib.Path(resource_filename(esroofit.__name__, 'tutorials')).glob('*.py')}

# Templates that are shipped with eskapade.
_TEMPLATES = {_.name: _ for _ in pathlib.Path(resource_filename(esroofit.__name__, 'templates')).glob('*')}

# Configs that are shipped with eskapade.
_CONFIGS = {_.name: _ for _ in pathlib.Path(resource_filename(esroofit.__name__, 'config')).glob('**/*')}
