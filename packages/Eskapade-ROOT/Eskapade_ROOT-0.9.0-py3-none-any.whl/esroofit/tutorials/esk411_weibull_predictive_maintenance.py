...
from esroofit.links import WsUtils, RooFitPercentileBinning, ConvertRooDataSet2RooDataHist, PrintWs
...
if settings['process']:
    ...
    conv = ConvertRooDataSet2RooDataHist()
    conv.read_key = 'rds'
...
if settings['fit_plot']:
    ...
    wsu = WsUtils(name='fitter', pages_key='weibull_fit_report')
    wsu.add_fit(pdf=fitpdf, data='binnedData', key='fit_result')
    ch.add(wsu)

    # --- 2. plot the fit result
    wsu = WsUtils(name='plotter1', pages_key='weibull_fit_report')
    ...
    wsu = WsUtils(name='plotter2', pages_key='weibull_fit_report')
    ...
    wsu = WsUtils(name='plotter3', pages_key='weibull_fit_report')
