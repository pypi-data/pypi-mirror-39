...
import re
import uuid
from typing import Pattern, Set, Optional, Dict, List, Any, Union
...
class Integer(Validator):
    ...
    def validate(self, raw_value: Union[None, int, str, _MissingType], raw: bool) -> Optional[int]:
...
class Number(Validator):
    ...
    def validate(self, raw_value: Union[None, int, float, str], raw: bool) -> Optional[float]:
        if isinstance(raw_value, str):
            if not raw:
...
class String(Validator):
    ...
    pattern: Optional[Pattern] = attr.attrib(converter=lambda x: re.compile(x) if x else None)

    def validate(self, raw_value: Optional[str], raw: bool) -> Optional[str]:
        if isinstance(raw_value, str):
            value = raw_value
        elif raw_value is None:
            if self.nullable:
        ...
        if self.format not in (StringFormat.Default, StringFormat.Password, StringFormat.Binary):
...
class Boolean(Validator):
    ...
    def validate(self, raw_value: Union[None, bool, str], raw: bool) -> Optional[bool]:
        if isinstance(raw_value, str):
            if not raw:
...
class Array(Validator):
    ...
    def validate(self, raw_value: Union[None, str, List],
                 raw: bool) -> Optional[List]:
        if isinstance(raw_value, str):
            if not raw:
            ...
            try:
                ...
                raise ValidatorError({i: e.error})
...
def schema_to_validator(schema: Dict, components: Dict) -> Validator:
    ...
    if 'oneOf' in schema:
        return OneOf(validators=[schema_to_validator(sch, components) for sch in schema['oneOf']])
    elif 'anyOf' in schema:
        return AnyOf(validators=[schema_to_validator(sch, components) for sch in schema['anyOf']])
    elif 'allOf' in schema:
        return AllOf(validators=[schema_to_validator(sch, components) for sch in schema['allOf']])
    else:
