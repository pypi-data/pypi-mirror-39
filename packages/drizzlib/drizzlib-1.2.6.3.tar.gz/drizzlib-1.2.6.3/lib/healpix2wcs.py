...
def healpix2wcs(
    ...
    if use_bilinear_interpolation:
        ...
        for x in range(x_dim):
            ...
            _log('Processing line {:3d}/{:d} ({:4.1f}%) [{:40s}]'.format(x, x_dim, 100*progress, '#'*int(progress*41)))
        _log('Processed line  {:3d}/{:d} (100%)  [{:40s}]\n'.format(x_dim, x_dim, '#' * 40))

    if wbis!= None:
        if 'CAR' in wbis.wcs.ctype[0] or 'CAR' in wbis.wcs.ctype[1]:
            from reproject import reproject_exact
            data, footprint = reproject_exact((data,w), wbis, shape_out=[int(y_dim/scale_factor),int(x_dim/scale_factor)])
