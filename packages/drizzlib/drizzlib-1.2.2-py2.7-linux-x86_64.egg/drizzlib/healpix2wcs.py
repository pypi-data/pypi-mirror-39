...
def healpix2wcs(
    ...
    if header is not None:
        ...
        if crpix is None:
            crpix = (image_size[1] / 2., image_size[0] / 2.)
    ...
    # Uncomment to use reprojection in CAR
    ...
    #
    #     print('The map will be created in (\''+str(w.wcs.ctype[0])+'\',\''+str(w.wcs.ctype[1])+'\') but will be reprojected at the end.')
    #     print('Make sure that reproject package is install. Type \'pip install reproject\' if not')
    #     # w.wcs.crpix = 2 * w.wcs.crpix
    #     # x_dim = 2 * x_dim
    #     # y_dim = 2 * y_dim
    ...
    m, h = read_map(healpix, h=True, hdu=field, offset=blank_value)
    ...
    if use_bilinear_interpolation:
        ...
        if 'CAR' in w.wcs.ctype[0] or 'CAR' in w.wcs.ctype[1]:
            ...
            if partial:
                ...
                wrap_healpixs = coo_matrix((nonzero[1], (np.zeros(len(nonzero[1])), nonzero[1])), shape=(1, m.shape[1])).tocsr()
                # wrap_healpixs = range(m.shape[1])
        ...
        for x in range(x_dim):
            for y in range(y_dim):
                ...
                # This gives results that are different from IDL
                # todo: why isn't it like this ? I don't get it.
                # wpix_poly = np.array([
                #     [x,     y],
                #     [x,     y + 1],
                #     [x + 1, y + 1],
                ...
                if total != 0:
                    ...
                    _log("Processing pixel lol (%d, %d)." % (x, y))
                else:
                    v_drizzle = np.nan
                    _log("Processing pixel (%d, %d). Warning: Sum of weights is 0." % (x, y))
    ...
    #     data, footprint = reproject_exact((data,w), wbis, shape_out=[x_dim,y_dim])
