...
def get_all_topics(user, select_related=True):
    ...
    if not (user.has_perm('SeuPORT.sft_mgr_forum')):
...
def index(request, template_name="SeuPORT/index.html"):
    ctx = {}
    topics = None
    user = request.user
...
def recent(request, template_name="SeuPORT/recent.html"):
    ctx = {}
    user = request.user
    topics = get_all_topics(user)
    q = request.GET.get('q', '')
...
def forum(
        ...
        template_name="SeuPORT/forum.html"):
...
def topic(request, topic_id, template_name="SeuPORT/topic.html"):
    user = request.user
...
@csrf_exempt
def markitup_preview(request, template_name="SeuPORT/markitup_preview.html"):
...
def new_post(
        ...
        template_name='SeuPORT/post.html'):
    user = request.user
