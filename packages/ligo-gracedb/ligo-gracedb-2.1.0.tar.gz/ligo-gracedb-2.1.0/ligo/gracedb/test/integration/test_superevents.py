...
class TestSuperevents(TestGraceDb):
    ...
    def setUpClass(cls):
        ...
        cls._events = []
        for i in range(8):
            event = cls._gracedb.createEvent("Test", "gstlal",
                eventFile, "LowMass").json()
            cls._events.append(event['graceid'])

        # Create one useful superevent
        superevent = cls._gracedb.createSuperevent(1, 2, 3,
            preferred_event=cls._events[0], category='Test').json()
        cls._superevent = superevent['superevent_id']
    ...
    def test_basic_creation(self):
        ...
        response = self._gracedb.createSuperevent(0, 1, 2,
            preferred_event=self._events[1], category='Test')
        ...
        with self.assertRaises(HTTPError):
            response = self._gracedb.createSuperevent(10, 11, 12,
                preferred_event=self._events[1], category="Test")

    def test_creation_with_events(self):
        """Create superevent with additional events"""
        response = self._gracedb.createSuperevent(0, 1, 2,
            preferred_event=self._events[2], category='Test',
    ...
    def test_creation_with_labels(self):
        ...
        response = self._gracedb.createSuperevent(0, 1, 2,
            preferred_event=self._events[5],
            labels=[label], category="Test")
    ...
    @mock.patch.object(GraceDb, 'allowed_labels', new_callable=mock.PropertyMock)
    def test_creation_with_bad_labels(self, mock_allowed_labels, mock_request):
        ...
        for label in bad_labels:
            with self.assertRaises(NameError):
                r = self._gracedb.createSuperevent(0, 1, 2,
                    preferred_event=self._events[6],
                    labels=label, category='Test')
    ...
    def test_creation_with_bad_categories(self, mock_post):
        ...
        with self.assertRaises(NameError):
            r = self._gracedb.createSuperevent(0, 1, 2,
                preferred_event=self._events[6],
                labels=['BAD_LABEL'], category='BAD_CATEGORY')

    def test_retrieval(self):
        """Retrieve a superevent"""
        response = self._gracedb.superevent(self._superevent)
        data = response.json()
    ...
    def test_update(self):
        ...
        response = self._gracedb.updateSuperevent(self._superevent,
            11, 12, 13, preferred_event=self._events[6])
        data = response.json()
    ...
    def test_event_addition_and_removal(self):
        ...
        initial_superevent_data = self._gracedb.superevent(self._superevent) \
            .json()
        ...
        # Add event
        response = self._gracedb.addEventToSuperevent(self._superevent, event)
        added_event_data = response.json()
        updated_superevent_data = self._gracedb.superevent(self._superevent) \
            .json()
        ...
        with self.assertRaises(HTTPError):
            response = self._gracedb.addEventToSuperevent(self._superevent,
                event)

        # Remove event
        response = self._gracedb.removeEventFromSuperevent(self._superevent,
            event)
        updated_superevent_data = self._gracedb.superevent(self._superevent) \
            .json()
        ...
        with self.assertRaises(HTTPError):
            response = self._gracedb.removeEventFromSuperevent(
                self._superevent, event)

        # Try to remove preferred event (expect error)
        with self.assertRaises(HTTPError):
            response = self._gracedb.removeEventFromSuperevent(
                self._superevent, updated_superevent_data['preferred_event'])
    ...
    def test_confirm_as_gw(self):
        ...
        initial_id = self._superevent

        # Make sure this is a non-GW type
        self.assertFalse(initial_id.startswith('TGW'))
        self.assertNotIn('GW', initial_id[:3])

        # Confirm as gw
        response = self._gracedb.confirm_superevent_as_gw(self._superevent)
        ...
        gw_id = confirmed_data['superevent_id']
        ...
        # Get data "old" (TS-type) and "new" (TGW-type) IDs
        initial_id_data = self._gracedb.superevent(initial_id).json()
        gw_id_data = self._gracedb.superevent(initial_id).json()

        # Compare results
        self.assertEqual(initial_id_data, gw_id_data)
        self.assertEqual(initial_id_data['superevent_id'], gw_id)

    def test_log_creation_and_retrieval(self):
        """Create a log and retrieve it"""
        # Write log
        msg = 'test message'
        response = self._gracedb.writeLog(self._superevent, msg)
        data = response.json()
        ...
        log_N = data['N']
        response = self._gracedb.logs(self._superevent, log_N)
    ...
    def test_log_creation_and_retrieval_with_file(self):
        ...
        # Get files
        response = self._gracedb.files(self._superevent)
    ...
    def test_log_tag_and_untag(self):
        ...
        # Add tag
        tag = 'sky_loc'
        response = self._gracedb.addTag(self._superevent, data['N'], tag)
        ...
        # Get superevent log
        response = self._gracedb.logs(self._superevent, data['N'])
        log_data = response.json()
    ...
    def test_labels(self):
        ...
        response = self._gracedb.writeLabel(self._superevent, label)
        data = response.json()
        ...
        # Remove label
        response = self._gracedb.removeLabel(self._superevent, label)
    ...
    def test_emobservations(self):
        ...
        response = self._gracedb.writeEMObservation(self._superevent, emgroup,
            ra_list, ra_width_list, dec_list, dec_width_list, start_time_list,
            duration_list, comment=comment)
        data = response.json()
        ...
        response = self._gracedb.emobservations(self._superevent)
    ...
    def test_voevents(self):
        ...
        response = self._gracedb.createVOEvent(self._superevent, 'PR',
            **pr_voevent)
        ...
        filename2 = os.path.join(self.TEST_DATA_DIR, "test_file2.txt")
        self._gracedb.writeLog(self._superevent, 'fake skymap',
            filename=filename)
        self._gracedb.writeLog(self._superevent, 'fake skymap image',
            filename=filename2)
        ...
        response = self._gracedb.createVOEvent(self._superevent, 'UP',
            **up_voevent)
        ...
        response = self._gracedb.voevents(self._superevent)
        list_data = response.json()
        ...
        response = self._gracedb.voevents(self._superevent, pr_data['N'])
        ret_data = response.json()
