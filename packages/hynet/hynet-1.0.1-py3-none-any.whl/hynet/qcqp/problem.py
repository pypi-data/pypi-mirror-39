...
class QCQP:
    ...
    **Caution:** If the QCQP object is used in repeated computations with
    ...
    def _vectorize_constraints(self, constraints, pool=None):
        """
        ...
        pool : multiprocessing.Pool, optional
            Pool of worker processes for parallel processing (optional).
        ...
        if pool is not None:
            result_list = pool.starmap(_vectorize_constraint, data)
        else:
            result_list = list(starmap(_vectorize_constraint, data))
    ...
    def get_constraint_vectorization(self):
        ...
        if config.GENERAL['parallelize'] and self.dim_v > 10 and cpu_count() > 1:
            pool = multiprocessing.Pool()
        else:
            pool = None

        A, b = self._vectorize_constraints(self.eq_crt, pool=pool)
        B, d = self._vectorize_constraints(self.ineq_crt, pool=pool)
