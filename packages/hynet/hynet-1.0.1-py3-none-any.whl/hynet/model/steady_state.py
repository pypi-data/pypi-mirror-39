...
class SystemModel:
    ...
    **Custom OPF Formulations:** The system model serves as a builder for the
    ...
    def get_opf_problem(self):
        ...
        # Constraint generation (with parallel processing for larger systems)
        constraint_generators = self._get_constraint_generators()
        num_cpu = cpu_count()
        if config.GENERAL['parallelize'] and self.dim_v > 10 and num_cpu > 1:
            num_workers = min(len(constraint_generators), num_cpu)
            pool = multiprocessing.Pool(processes=num_workers)
            constraints = pool.map(_call_and_concatenate, constraint_generators)
            pool.close()
            # pool.join()
        else:
            constraints = list(map(_call_and_concatenate, constraint_generators))
    ...
    def get_balance_constraints(self):  # pylint: disable=too-many-locals
        ...
        # REMARK: CSC format for efficient column slicing
    ...
    def get_ampacity_constraints(self):
        ...
        # CONSTRAINT SCALING: In the description of the hybrid AC/DC power
        # grid model, it can be observed that the elements of all constraint
        # matrices are either roughly close to 1 or in the range of the series
        # admittance of the AC branches, except for the current constraints,
        # which feature the series admittance squared. As the series impedance
        # may be very small, the corresponding series admittance is large,
        # e.g., in the range of 10^4. Then, the modulus of the constraint
        # matrix elements is in the range between 0 and 10^4, except for the
        # current constraints, which then feature elements in the range of
        # 10^8. We observed that this frequently leads to numerical issues in
        # the employed solver. To reduce the dynamic range observed in the
        # constraint matrices and, therewith, improve the conditioning of the
        # problem, the current constraints are scaled by the minimum branch
        # series impedance.
        if N_E != 0:
            scaling = np.min(np.abs(self._scr.branch['z_bar'].values))
        ...
        for k in range(N_E):
            ...
            I_src_k = self._Y_src[k, :].conj().transpose().dot(self._Y_src[k, :])
            crt_i_max_src[m] = Constraint(name='i_max_src',
                                          table='branch',
                                          id=index[k],
                                          C=(I_src_k * scaling).tocoo(),
                                          c=None,
                                          a=None,
                                          r=None,
                                          b=I_max_squared[k] * scaling,
                                          scaling=scaling)

            # Flow limit at destination bus
            I_dst_k = self._Y_dst[k, :].conj().transpose().dot(self._Y_dst[k, :])
            crt_i_max_dst[m] = Constraint(name='i_max_dst',
                                          table='branch',
                                          id=index[k],
                                          C=(I_dst_k * scaling).tocoo(),
                                          c=None,
                                          a=None,
                                          r=None,
                                          b=I_max_squared[k] * scaling,
