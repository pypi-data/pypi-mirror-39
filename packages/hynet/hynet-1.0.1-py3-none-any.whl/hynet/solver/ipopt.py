...
class IPOPTProblem(ipopt.problem):
    ...
    def __init__(self, qcqp, options):
        ...
        if config.GENERAL['parallelize'] and qcqp.dim_v > 10 and cpu_count() > 1:
        ...
        jac_v_idx, self._jacobian_C = \
            self._get_jacobian_v(constraints, qcqp.dim_v, pool=pool)
    ...
    @staticmethod
    def _get_jacobian_v(constraints, dim_v, pool=None):
        """
        ...
        if pool is not None:
            result_list = pool.map(_get_sparsity_and_row_reduction, constraints)
        else:
            result_list = list(map(_get_sparsity_and_row_reduction, constraints))
        ...
        if pool is not None:  # Stacking is slow: Use multiprocessing to prestack
            list_jacobian_C = pool.map(scipy.sparse.vstack,
                                       partition_iterable(list_jacobian_C, num_cpu))
