...
from udsoncan.Connection import *
import struct
import time
		...
		if payload is not None:
			...
			if req.service == services.DiagnosticSessionControl:
				if req.subfunction == services.DiagnosticSessionControl.Session.extendedDiagnosticSession:
					response = Response(req.service, Response.Code.PositiveResponse, data=b'\x03')
				else:
			...
			## SecurityAccess
				...
				if req.subfunction == 3:
					response = Response(req.service, Response.Code.PositiveResponse, data=b"\x03\x12\x34\x56\x78")
				elif req.subfunction == 4:
					if req.data == b"\xed\xcb\xa9\x87":
						response = Response(req.service, Response.Code.PositiveResponse, data=b'\x04')
					else:
			...
			## Read Data By identifier
				...
				if req.data == b"\x00\x01" :
					response = Response(req.service, Response.Code.PositiveResponse, data=b'\x00\x01\x12\x34')
				elif req.data == b"\x00\x02" :
					response = Response(req.service, Response.Code.PositiveResponse, data=b'\x00\x02\x56\x78')
				elif req.data == b"\x00\x03" :
					response = Response(req.service, Response.Code.PositiveResponse, data=b'\x00\x03\x9a\xbc')
				elif req.data == b"\x00\x01\x00\x02" :
					response = Response(req.service, Response.Code.PositiveResponse, data=b'\x00\x01\x12\x34\x00\x02\x56\x78')
				else :
