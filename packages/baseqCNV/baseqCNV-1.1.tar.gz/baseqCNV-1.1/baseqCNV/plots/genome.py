...
def plot_genome(bincount, cbs_path, figpath):
    """ Plot the copy number of whole genome.
    Usage:
    ::
        from baseqCNV import plot_genome
...
def plot_genome_multiple(bincount, cbs_path, path_out):
    ...
    .. image:: http://p8v379qr8.bkt.clouddn.com/Genome_20.png
    ...
    for x in range(10):
        #scatter
        axes[x].scatter(df.absstart, df.CN_ploidy, edgecolors='dodgerblue', s=1)
        #chr line
        df_chr_pos = df.groupby(by=["chr"]).agg({"absstart" : "first"})
        for idx, row in df_chr_pos.iterrows():
