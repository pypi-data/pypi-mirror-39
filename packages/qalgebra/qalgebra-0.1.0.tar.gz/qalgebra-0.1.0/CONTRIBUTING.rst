...
Report bugs at https://github.com/mabuchilab/qalgebra/issues.
...
The best way to send feedback is to file an issue at https://github.com/mabuchilab/qalgebra/issues.
...
3. Check https://travis-ci.org/mabuchilab/qalgebra/pull_requests
...
Ready to contribute? Follow `Aaron Meurer's Git Workflow Notes`_ (with ``mabuchilab/qalgebra`` instead of ``sympy/sympy``)

In short,

1. Clone the repository from ``git@github.com:mabuchilab/qalgebra.git``
...
Python versions.
You still can (and should) look at https://travis-ci.org/mabuchilab/qalgebra/ to check that your commits pass all tests.
...
QAlgebra uses the `git-flow`_ branching model. That is, the ``develop`` branch takes the role of ``master`` in the `Git Workflow Notes`_.

In order to create topic branches with ``git flow``, after cloning the  ``qalgebra`` repository, you should initialize it as follows:
...
From a checkout of the ``qalgebra`` repository, assuming conda_ is installed, you can use
...
.. _test coverage: https://coveralls.io/github/mabuchilab/qalgebra?branch=develop
...
At any point, from a checkout of the ``qalgebra`` repository (and
...
``qalgebra``, and that you have successfully run ``make test`` (which creates
...
To run e.g. only the tests defined in ``tests/test_qalgebra.py``, use::

    $ ./.venv/py37/bin/pytest tests/test_qalgebra.py
