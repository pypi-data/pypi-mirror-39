...
class ONC:
	...
	def doRequest(self, url, params=None):
		...
		if self.showInfo:
			print('URL: {}?{}'.format(url, ('&'.join(['{}={}'.format(i[0], i[1]) for i in params.items()]))))
	...
	def getFile(self, filename='', decompress=False):
		url = '{}api/archivefiles'.format(self.baseUrl)
		params = {
			'token': self.token,
			'method': 'getFile',
		...
		if status == 200:
			...
			self.printInfo('File downloaded to "{}" (Status 200: OK).'.format(filePath))
		else:
