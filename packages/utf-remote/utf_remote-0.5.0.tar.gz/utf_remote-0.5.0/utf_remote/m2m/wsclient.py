...
class WSClient(threading.Thread):
    ...
    def get_serial(self):
        """Returns raspberry pi's serial number from /proc/cpuinfo"""

        # Extract serial from the cpuinfo file
    ...
    def on_event(self, event):
        ...
        if event.name == 'ready':
            ...
            bd = bencode.encode(event.text)
            fd = bencode.decode(bd.encode('utf-8'))
            self.on_binary(fd)
    ...
    def on_ready(self):
        ...
        if self.identity is None:
            ...
            self.send(PacketType.agent_connected, key=self.get_serial())
        else:
            self.send(PacketType.request_identify, uuid=self.identity)

    def on_binary(self, data):
        """On a WS message."""
        print('On Binary Message:')
        self.last_packet_time = time.time()
        try:
    ...
    def channel_write(self, channel, data):
        ...
        print('Sending Data...')
        print(data)
        self.send(PacketType.request_send, channel=channel, data=data, key=self.get_serial())

    def on_instruction(self, sender, data):
        """Called with an instruction."""
        print('instruction from {%s} %r', sender, data)
    ...
    def handle_ping(self, packet_type, data):
        ...
        self.send('pongsync complete', data=data[:1024])
    ...
    def handle_route(self, packet_type, channel, data):
        ...
        # self.channel_write(1187661,bencode.encode('bilal'))
    ...
    def on_notify_close(self, packet_type, channel_no, key):
    ...
    def on_instruction_packet(self, packet_type, sender, data):
        ...
        if(bool(data['key'] in self.get_serial()) == False):
            self.send('request_identify', json.dumps({'key': data['key'], 'status': 'false'}))
        else:
            try:
