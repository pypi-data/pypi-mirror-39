            ...
            if request["clearbit_reveal"] is True:
                ...
                if streaming is True:
                    resp = requests.get(
                        ...
                        auth=(clearbit.key, ""),
                        headers={
                            "API-Version": clearbit.Reveal.version
                        }
                    )
                else:
                    resp = requests.get(
                        ...
                        params={"ip": request["ip_address"]},
                        auth=(clearbit.key, ""),
                        headers={"API-Version": clearbit.Reveal.version}
                    )
                # DEPRECATED
                ...
                if streaming is True:
                    if enrichment_api == "person":
                        resp = requests.get(
                            ...
                            auth=(clearbit.key, ""),
                            headers={
                                "API-Version": clearbit.Person.version
                            }
                        )
                        ...
                        resp = requests.get(
                            ...
                            auth=(clearbit.key, ""),
                            headers={
                                "API-Version": clearbit.Company.version
                            }
                        )
                else:
                    if enrichment_api == "person":
                ...
                print("status: {}".format(resp))
                # 202 (async lookup; try again momentarily)
                if resp.status_code == 201 or 202:
                    print("request is pending.")
                    time.sleep(SLEEP_LENGTH)
                    continue
                # if "pending" in resp.keys():
