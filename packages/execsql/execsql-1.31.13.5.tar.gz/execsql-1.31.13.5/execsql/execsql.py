	...
	def __init__(self, title, message, button_list, selected_button=0, no_cancel=False, column_headers=None, 
		...
		if rowset and column_headers:
			...
			for i, row in enumerate(rowset):
				if sys.version_info < (3,):
					enc_row = ['' if c is None else u"Binary data (%s bytes)" % len(c) if type(c) in (buffer, bytearray) else type(u"")(c) for c in row]
				else:
					enc_row = ['' if c is None else "Binary data (%s bytes)" % len(c) if type(c) in (type(memoryview(b'')), bytes, bytearray) else type(u"")(c) for c in row]
...
metacommands.append(MetaCommand(
	#ins_table_rxs(r'^\s*PROMPT\s+COMPARE\s+', ins_table_rxs(r'\s+(?P<orient>AND|BESIDE)\s+', r'\s+(?:PK|KEY)\s*\((?P<pks>(("[A-Z_0-9]+")|[A-Z_0-9]+)(\s*,\s*(("[A-Z_0-9]+")|[A-Z_0-9]+))*)\)\s+MESSAGE\s+"(?P<msg>(.|\n)*)"\s*$', suffix='2'), suffix='1'), x_prompt_compare))
...
def x_rm_sub(**kwargs):
	subvarset = subvars if varname[0] != '~' else commandliststack[-1].localvars
	subvarset.remove_substitution(kwargs['match'])
	return None
