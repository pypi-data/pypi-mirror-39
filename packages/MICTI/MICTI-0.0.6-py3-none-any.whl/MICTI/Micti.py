...
class Micti:
    def __init__(self,data,geneNames,cellNames,k=None,cluster_label=None,cluster_assignment=None, th=0,seed=None, ensembel=False, organisum="hsapiens"):
        #_=self.check_data(data,geneNames,cellNames,cluster_assignment)
        self.data=data
        self.k=k
        self.th=th
    ...
    def CF(self, data):
        matrixx=data.transpose()
        totalCells=matrixx.shape[1]
        i,j = matrixx.nonzero()
        noExpGenesOverAllCells=Counter(i)
        idf=[-1*math.log((1+noExpGenesOverAllCells[i])/float(totalCells+1)) for i in list(noExpGenesOverAllCells.keys())]
        icf_matrix=np.multiply(matrixx.T.toarray()[:,list(noExpGenesOverAllCells.keys())], np.array(idf))
        return icf_matrix 
    ...
    def get_gene_over_representationn(self,topn=10):
        enrichmentTable={}    
        for i in range(self.k):
            top10Genes=[] 
