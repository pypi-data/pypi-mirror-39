...
class Micti:
    ...
    def gene_symbol_to_ENSEMBLID(self, symbol, organisum="hsapiens"):
        if(organisum=="hsapiens"):
            genes=pa.read_csv("../mart_export_stable_genes.txt", sep="\t")
        elif(organisum=="mmusculus"):
            genes=pa.read_csv("../mart_export_mouse_stable_gene.txt", sep="\t")
        else:
    ...
    def RPKM_to_TPM(RPKM):
        TPM=RPKM.div(RPKM.sum(axis=0), axis=1)*10e6
        return TPM
    ...
    def get_sig_gene_over_representation(self):
        enrichmentTable={}    
        for i in range(self.k):
            top10Genes=[] 
            print('Cluster {0:s} ({1:d} cells)'.format(str(self.cluster_label[i]), int(np.sum(self.cluster_assignment==i))))
            sigGenes=np.array(self.get_markers_by_Pvalues_and_Zscore(i).index)
           
            if self.ensembl:
                sigGenes=list(self.ENSEMBLID_to_geneSymbol(sigGenes,organisum=self.organsm))
                print(sigGenes)
            else:
                sigGenes=sigGenes
                print(sigGenes)
            enrichment = gprofiler(sigGenes, organism=self.organsm)
    ...
    def cluster_cells(self,numberOfCluster, method="kmeans", maxiter=500):
        ...
        if method=="kmeans":
            ...
            EM_GMM=EM_GM.EM_for_high_dimension(maxiter=maxiter)
