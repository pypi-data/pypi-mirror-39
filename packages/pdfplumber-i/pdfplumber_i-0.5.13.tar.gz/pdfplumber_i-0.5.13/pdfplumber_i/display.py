...
from pdfplumber_i import utils
from pdfplumber_i.table import TableFinder
    ...
    with wand.image.Image(filename=page_path, resolution=resolution) as img:
        ...
        with img.convert("PNG") as png:
            ...
            if "transparency" in im.info:
                converted = im.convert("RGBA").convert("RGB").convert("L").convert("RGB")
            else:
                converted = im.convert("RGB").convert("L").convert("RGB")
            return converted
