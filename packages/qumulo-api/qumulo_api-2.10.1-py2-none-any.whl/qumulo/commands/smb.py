...
class ShareAceTranslator(AceTranslator):
    def parse_trustee(self, trustee, ace):
        ...
        if trustee_type in ("uid", "gid"):
            ace['trustee'] = {trustee_type: int(trustee_id)}
        elif trustee_type == 'auth_id':
            # NB: validate int, and allow hex if desired:
            ace['trustee'] = {trustee_type: str(int(trustee_id, base=0))}
        elif trustee_type in {'sid', 'name'}:
            ace['trustee'] = {trustee_type: trustee_id}
        else:
            raise ValueError('Trustee type must be "name", "uid", "gid", '
                '"sid", or "auth_id"')
