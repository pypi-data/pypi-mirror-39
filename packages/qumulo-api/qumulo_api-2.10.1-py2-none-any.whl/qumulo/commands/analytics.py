...
class GetEventLogCommand(qumulo.lib.opts.Subcommand):
    NAME = "event_log_get"
    DESCRIPTION = "Get the logged Events since a given time."

    @staticmethod
    def options(parser):
        parser.add_argument(
            '-t', '--begin-time', type=int,
            help='Time period from which to retrieve events, in epoch seconds.')

    @staticmethod
    def main(conninfo, credentials, args):
        print analytics.event_logs_get(conninfo, credentials,
            args.begin_time)
