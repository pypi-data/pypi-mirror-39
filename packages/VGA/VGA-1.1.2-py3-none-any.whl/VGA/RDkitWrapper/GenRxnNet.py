...
def GenerateRxnNet(initial_reactant, reaction_rules):
    ...
    - reaction_rules:       can be smarts string or rdkit.Chem.rdChemReactions.ChemicalReaction object
    ...
    A = generate_rxn_net('CC',['[C:1][H:2]>>[C:1].[H:2]','[C:1][C:2]>>[C:1].[C:2]'])
    for product in A:
            ...
            for reactant_indexes in reactant_list:
                ...
                for mol1 in products:
                    ...
                    for mol2 in processed:
                        # first check the nubmer of atoms and then look for substructure match
                        if mol1.GetNumAtoms() == mol2.GetNumAtoms() and \
                            mol1.GetNumAtoms() == len(mol1.GetSubstructMatch(mol2)):
