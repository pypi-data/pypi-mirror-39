...
ReactionQuery class is equipped with enhanced query comparison over RDkit query comparison
...
class BondForm(transformation):
    ...
    def __call__(self,comb_mol,mapped_index):
        comb_mol.AddBond(mapped_index[self.idx1],mapped_index[self.idx2],self.bondtype)
...
class BondIncrease(transformation):
    ...
    def __call__(self,comb_mol,mapped_index):
        bondtype = self.getbondtype(comb_mol.GetBondBetweenAtoms(mapped_index[self.idx1],mapped_index[self.idx2]))
        comb_mol.RemoveBond(mapped_index[self.idx1],mapped_index[self.idx2])
        comb_mol.AddBond(mapped_index[self.idx1],mapped_index[self.idx2],bondtype)
...
class BondDecrease(transformation):
    ...
    def getbondtype(self,bondtype):
        if bondtype.GetBondType().__str__() == 'SINGLE':
            ...
            raise ReactionQueryError('BondDecrease: Decreasing bond order of aromatic bond is not supported')
        elif bondtype.GetBondType().__str__() == 'DATIVE':
            raise ReactionQueryError('BondDecrease: Decreasing bond order of partial bond is not supported')
        else:
            raise ReactionQueryError("BondDecrease: Unsupported bond type:'"+bondtype.GetBondType().__str__()+"'")
    def __call__(self,comb_mol,mapped_index):
        bondtype = self.getbondtype(comb_mol.GetBondBetweenAtoms(mapped_index[self.idx1],mapped_index[self.idx2]))
...
class ReactionQuery(object):
    ...
    def AppendReactantQuery(self, reactant):
        if not isinstance(reactant,(Chem.Mol,MolQuery)):
            raise ReactionQueryError("Unrecognized instance'"+type(reactant)+"'")
    ...
    def ModifyMolInPlace(self, reactants):
        ...
        if len(self.reactantquery) != 1:
            raise ReactionQueryError('ModifyMolInPlace: Only usable for monomolecular transformation')
        if isinstance(reactants,Chem.Mol):
            reactants = [reactants]
        elif isinstance(reactants,tuple):
            reactants = list(reactants)
