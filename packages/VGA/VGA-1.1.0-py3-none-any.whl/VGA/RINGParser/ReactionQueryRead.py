...
class ReactionQueryReader(object):
    ...
    def ReadBondType(self, tree):
        if tree[0] == 'single':
            ...
            raise RINGReaderError("BondType: Unsupported bond type: '"+tree[0]+"'")
    ...
    def ReadBondForm(self, tree, reactionquery):
        ...
        try:
            ...
            raise RINGReaderError("BondForm: Unrecognized atom label: '"+tree[i][1]+"'")
        try:
    ...
    def ReadBondBreak(self, tree, reactionquery):
        ...
        try:
            ...
            idx1_in_query = reactionquery.reactantquery[reactant_name1].atom_names.index(atom_name1)
            i += 1
        except:
            raise RINGReaderError("BondBreak: Unrecognized atom label: '"+tree[i][1]+"'")
        try:
            atom_name2 = tree[i][1]
            idx2 = self.atom_names.index(atom_name2)
            reactant_name2 = self.atom_belonging_mol[idx2]
            idx2_in_query = reactionquery.reactantquery[reactant_name2].atom_names.index(atom_name2)
        except:
            raise RINGReaderError("BondBreak: Unrecognized atom label: '"+tree[i][1]+"'")
        if reactant_name1 != reactant_name2:
            raise RINGReaderError("BondBreak: Two supplied atom labels aren't in the same molecule")
        bond = reactionquery.reactantquery[reactant_name1].mol.GetBondBetweenAtoms(idx1_in_query,idx2_in_query)
        if not bond:
            raise RINGReaderError("BondBreak: No bond between two atom (" +atom_name1+","+atom_name2+")")
        elif bond.GetBondType().__str__() == 'UNSPECIFIED':
            raise RINGReaderError("BondBreak: Break of unspecified bond is not supported ")
        elif bond.GetBondType() != bondtype:
            raise RINGReaderError("BondBreak: Bond mismatch between two atom ("\
            +atom_name1+","+atom_name2+"). (Bond in mol:" + bond.GetBondType().__str__() + \
    ...
    def ReadBondModify(self, tree, reactionquery):
        try:
            ...
            raise RINGReaderError("BondModify: Unrecognized atom label: '"+tree[0][1]+"'")
        try:
            atom_name2 = tree[1][1]
            idx2 = self.atom_names.index(atom_name2)
            reactant_name2 = self.atom_belonging_mol[idx2]
            ...
            raise RINGReaderError("BondModify: Unrecognized atom label: '"+tree[1][1]+"'")
        if reactant_name1 != reactant_name2:
            raise RINGReaderError("BondModify: Two supplied atom labels aren't in the same molecule")
        bond = reactionquery.reactantquery[reactant_name1].mol.GetBondBetweenAtoms(idx1_in_query,idx2_in_query)
        if not bond:
            raise RINGReaderError("BondModify: No bond between two atom (" +atom_name1+","+atom_name2+")")
        elif bond.GetBondType().__str__() == 'UNSPECIFIED':
            raise RINGReaderError("BondModify: BondModify: modification of unspecified bond is not supported ")
        ...
        if existingbond == 'SINGLE':
            ...
            raise RINGReaderError("BondModify: Unsupported bond type for modifiation: '"+existingbond+"'")
    ...
    def ReadBondIncrease(self, tree, reactionquery):
        try:
            ...
            raise RINGReaderError("BondIncrease: Unrecognized atom label: '"+tree[0][1]+"'")
        try:
            idx2 = self.atom_names.index(tree[1][1])
        except:
            raise RINGReaderError("BondIncrease: Unrecognized atom label: '"+tree[1][1]+"'")
    ...
    def ReadBondDecrease(self, tree, reactionquery):
        try:
            ...
            raise RINGReaderError("BondDecrease: Unrecognized atom label: '"+tree[0][1]+"'")
        try:
            idx2 = self.atom_names.index(tree[1][1])
        except:
            raise RINGReaderError("BondDecrease: Unrecognized atom label: '"+tree[1][1]+"'")
    ...
    def ReadAtomLabel(self,tree,reactionquery):
        try:
            ...
            idx_in_query = reactionquery.reactantquery[reactant_name].atom_names.index(atom_name)
            atom = reactionquery.reactantquery[reactant_name].mol.GetAtomWithIdx(idx_in_query)
        except:
            raise RINGReaderError("AtomLabel: Unrecognized atom label: '"+tree[0]+"'")
    ...
    def ReadAtomTypeModify(self, tree, reactionquery):
        ...
        if atom.GetSymbol() != symbol:
            raise NotImplementedError("AtomTypeModify: Atom Label change not supported")
    ...
    def ReadRadicalModify(self, tree, reactionquery):
        ...
        radical = tree[1]
        if radical <0:
            raise RINGReaderError("RadicalModify: Number of radical electrons cannot be below 0")
        self.electronbalance[idx] -= radical - atom.GetNumRadicalElectrons()
    ...
    def ReadReactantGroup(self, tree, reactionquery):
        ...
        for i in range(0,len(reactionquery.reactantquery[tree[0][1]].atom_names)):
            try:
                ...
                s += ' in reactant ' + reactionquery.reactantquery[tree[0][1]].name
        ...
        self.electronbalance += [0]*len(reactionquery.reactantquery[tree[0][1]].atom_names)
    ...
    def ReadDuplicates(self, tree, reactionquery):
        ...
        assert tree[2][0] == 'LabelMapping'
        labelmapping = self.LabelMapping(tree[2][1:])
        if len(labelmapping) != len(reactionquery.reactantquery[tree[1][1]].atom_names):
        ...
        for i in range(0,len(reactionquery.reactantquery[tree[0][1]].atom_names)):
            try:
                ...
                s = 'Unrecognized label '
                s += reactionquery.reactantquery[tree[0][1]].atom_names[i]
                s += ' in reactant ' + reactionquery.reactantquery[tree[1][1]].name
                s += ' for duplication opreation for reactant ' + tree[0][1]
                raise RINGReaderError(s)
        self.atom_names += reactionquery.reactantquery[tree[0][1]].atom_names
        self.electronbalance += [0]*len(reactionquery.reactantquery[tree[0][1]].atom_names)
        self.atom_belonging_mol += reactionquery.reactantquery[tree[0][1]].name \
