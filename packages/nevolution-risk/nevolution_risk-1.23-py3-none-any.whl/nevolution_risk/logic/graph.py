...
from nevolution_risk.logic import node, player


class Graph(object):
    nodes = []
    player1 = player.Player('joern', 0, colors.green)
    player2 = player.Player('heinrich', 0, colors.blue)

    def __init__(self):
        self.nodes = []
        self.init_graph()
        self.nodes[4].player = self.player1
        self.nodes[8].player = self.player2
...
if __name__ == '__main__':
    player1 = player.Player('joern', 0, colors.black)
    graph1 = Graph()
    ...
    print(graph1.is_graph_conquered())
