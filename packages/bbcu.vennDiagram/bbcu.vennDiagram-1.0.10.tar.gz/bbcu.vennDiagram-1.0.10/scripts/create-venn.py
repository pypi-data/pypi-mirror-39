    ...
    def find_intersections(self):
        ...
        for i in range(2, len(self.samples_list.keys()) + 1):
            for samples in itertools.combinations(self.samples_list.keys(), i):
                ...
                percents_string = ','.join(["%s (%.2f%%)" %(sample, percent) for sample, percent in zip(samples, percents)])
                stat = '%s,%s' % (size_intersected,percents_string)  if len(samples) > 2 else '%s,%s,%s,%s,%s' % (size_intersected, percents_string, pvalue, oddsratio, expected)
                ...
                self.summary_json.append({"label": "("+",".join([sample for sample in samples])+")",
                                          ...
                                          "size": size_intersected, "percent": "%s" % percents_string})
