...
class VennDiagram(object):
    def __init__(self, input_dir, output_dir, min_abs_log_fc, max_abs_log_fc, min_pv, max_pv, total_gene_numbers, package_dir, js_venn_name, js_d3_name):
        self.input_dir = input_dir
        self.output_dir = output_dir
        self.min_abs_log_fc = min_abs_log_fc
        self.max_abs_log_fc = max_abs_log_fc
        self.min_pv = min_pv
        self.max_pv = max_pv
        ...
        for input_file in input_files:
            ...
            with open(os.path.join(output_filtered_dir, file_name + '_filtered.csv'), 'w') as filtered_file:
                ...
                for row in self.read_file(input_file):
                    if row['pv'] <= self.max_pv and row['pv'] >= self.min_pv and abs(row['log2FC']) >= self.min_abs_log_fc and abs(row['log2FC']) <= self.max_abs_log_fc:
                        num += 1
...
def parse_args():
    ...
    parser.add_argument('--max-p-value', type=float, help='Maximum p-value. Default: 0.05', default=0.05,
                        required=False)
    parser.add_argument('--min-p-value', type=float, help='Minimum p-value. Default: 0.0', default=0.0,
                        required=False)
...
if __name__ == '__main__':
    ...
    print 'Minimum fold change is %s, Maximum fold change is %s, Minimum p-value is %s, Maximum p-value is %s' % (args.min_abs_log_fc, args.max_abs_log_fc, args.min_p_value, args.max_p_value)
    os.makedirs(args.output_dir) if not os.path.isdir(args.output_dir) else None
    with open(os.path.join(args.output_dir, 'log.txt')) as log:
    ...
    vd = VennDiagram(args.input_dir, args.output_dir, float(args.min_abs_log_fc), float(args.max_abs_log_fc), float(args.min_p_value), float(args.max_p_value), args.total_gene_numbers,
                     args.package_dir, JS_VENN_NAME, JS_D3_NAME)
    vd.filter_files()
