...
from vennDiagram import venn


class VennDiagram(object):
    def __init__(self, input_dir, output_dir, min_log_fc, max_pv, total_gene_numbers):
    ...
    def filter_files(self):
        ...
        self.samples_list = sorted(self.filtered_genes.keys())
    ...
    def find_intersections(self):
        ...
        summary = {}
        for i in range(2, len(self.samples_list)+1):
            for samples in itertools.combinations(self.samples_list, i):
    ...
    def create_venn_diagram(self):
        genes_groups = [[details[0] for details in self.filtered_genes[sample] ] for sample in self.samples_list]
        labels = venn.get_labels(genes_groups, fill=['number', 'percent'])
        samples_num = len(self.samples_list)
        exec("fig, ax = venn.venn%d(labels, names=[sample for sample in self.samples_list])" %samples_num)
        # fig, ax = venn_function(labels, names=[sample for sample in self.samples_list])
        fig.savefig(os.path.join(self.output_dir, 'venn.png'), bbox_inches='tight')
        plt.close()
