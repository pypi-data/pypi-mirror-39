...
class VennDiagram(object):
    def __init__(self, input_dir, output_dir, min_log_fc, max_log_fc, min_pv, max_pv, total_gene_numbers, package_dir, js_venn_name, js_d3_name):
        ...
        for input_file in input_files:
            ...
            with open(os.path.join(output_filtered_dir, file_name + '_filtered.csv'), 'w') as filtered_file:
                ...
                for row in self.read_file(input_file):
                    if row['pv'] <= self.max_pv and row['pv'] >= self.min_pv and row['log2FC'] >= self.min_log_fc and row['log2FC'] <= self.max_log_fc:
                        num += 1
...
def parse_args():
    ...
    parser.add_argument('--output-dir', metavar='path', type=str, help='Name of existing output directory', default=os.curdir,
                        required=True)
    ...
    parser.add_argument('--min-p-value', metavar='0.0', type=float, help='Minimum p-value. Default: 0.0', default=0.0,
                        required=False)
    parser.add_argument('--max-p-value', metavar='0.05', type=float, help='Maximum p-value. Default: 0.05', default=0.05,
                        required=False)
...
if __name__ == '__main__':
    ...
    print 'Minimum fold change is %s, Maximum fold change is %s, Minimum p-value is %s, Maximum p-value is %s' % (args.min_log_fc, args.max_log_fc, args.min_p_value, args.max_p_value)
    ...
    vd = VennDiagram(args.input_dir, args.output_dir, float(args.min_log_fc), float(args.max_log_fc), float(args.min_p_value), float(args.max_p_value), args.total_gene_numbers,
