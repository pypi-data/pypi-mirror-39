...
def select_ami(region, description_includes, description_excludes):
    '''
    Select a suitable ami based on the description string.
    '''
    client = boto3.client('ec2', region_name=region)
    ...
    if description_excludes in [None, '']:
        description_excludes = 'QWERTYUIOPhjkl'
    newest = None
    newest_image = None
    for i in result['Images']:
        if 'Description' in i:
            if description_includes in i['Description']:
                if not description_excludes in i['Description']:
                    creationdate = datetime.datetime.strptime(i['CreationDate'][:-1], '%Y-%m-%dT%H:%M:%S.%f')
                    if newest is None or creationdate > newest:
                        newest = creationdate
                        newest_image = i
                
    if newest_image is None:
        raise ValueError('Error: cannot find a suitable image')
    image_id = newest_image['ImageId']
