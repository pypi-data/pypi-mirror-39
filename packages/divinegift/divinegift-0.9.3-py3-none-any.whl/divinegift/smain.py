...
def get_list_files(path, filter=None, add_path=False, path2=''):
    ...
    :param path: Folder witch contains files
    :param filter: filter, could be list of str or just str. None by default
    :param add_path: bool, should add path to file
    :param path2: path witch need to add
    :return: return list of files with path to it
    """
    list_files = []
    if add_path:
        if path2 == '':
    ...
    if filter:
        if type(filter) == list:
            for f in filter:
                list_files.extend([os.path.join(path2, x) for x in os.listdir(path) if f in x])
        if type(filter) == str:
            list_files = [os.path.join(path2, x) for x in os.listdir(path) if filter in x]
    else:
