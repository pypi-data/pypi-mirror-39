...
def connect(host='localhost', port=7600, timeout=10, quiet=False):
    ...
    .. warning::
        ...
        bound to one instance of the TecUtil Server. You may still add this to
        the ``tecplot.add`` file which will be run everytime, but it must only
        be activated when Tecplot 360 is running interactively, i.e. not in
        batch mode. To do this, check the value of ``|INBATCHMODE|``:
...
def clear_suspend():
    ...
    cause the TecUtil Server to break out of a suspended state which may have
    been the result of a script not properly exiting from a `suspend()`
    context, possibly due to a segmentation fault. Example usage::
