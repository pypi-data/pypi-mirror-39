...
class Linemap(session.Style):
    ...
    @property
    def sort_mode(self):
        """`LineMapSort`: Mode controlling which `Variable` to use when sorting lines.
    ...
    @property
    def curve(self):
        """`LinemapCurve`: `LinemapCurve` style and fitting-method control for lines.
        """
        return LinemapCurve(self)

    @property
    def indices(self):
        """`LinemapIndices`: `LinemapIndices` object controlling which lines are shown.
        """
        return LinemapIndices(self)

    @property
    def line(self):
        """`LinemapLine`: `LinemapLine` style for lines to be drawn.
        """
        return LinemapLine(self)

    @property
    def symbols(self):
        """`LinemapSymbols`: `LinemapSymbols` style for markers at points along the lines.
        """
