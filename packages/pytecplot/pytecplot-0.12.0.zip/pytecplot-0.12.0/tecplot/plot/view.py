...
from ..tecutil import RectTuple, XYZPosition, lock, lock_attributes, sv
...
class LineView(View):
    ...
    def extents(self):
        ...
        """
        return RectTuple(self._get_style(float, sv.EXTENTS, sv.X1),
                         self._get_style(float, sv.EXTENTS, sv.Y1),
                         self._get_style(float, sv.EXTENTS, sv.X2),
                         self._get_style(float, sv.EXTENTS, sv.Y2))
...
class Cartesian3DView(FieldView):
    ...
    def rotation_origin(self):
        ...
        return style._get_style(XYZPosition, sv.ROTATEORIGIN)

    @rotation_origin.setter
    def rotation_origin(self, value):
        style = session.Style(sv.GLOBALTHREED, uniqueid=self.plot.frame.uid)
        style._set_style(XYZPosition(*value), sv.ROTATEORIGIN)
    ...
    def rotate_viewer(self, angle, normal=(0, 0, 1)):
        ...
        This rotates the viewr about some **normal** using the right-hand rule
...
class PolarView(LineView):
    ...
    def reset_to_entire_circle(self):
        raise TecplotInterfaceChangeError(
            'PolarView.reset_to_entire_circle()'
            ' removed in PyTecplot 0.11.1. Use'
            ' PolarAngleLineAxis.set_range_to_entire_circle() instead. Example:'
