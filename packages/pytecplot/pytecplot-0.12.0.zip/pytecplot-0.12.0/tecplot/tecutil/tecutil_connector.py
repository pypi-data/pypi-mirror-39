...
class TecUtilConnector(object):
    ...
    def load_windows(self):
        def _load_library(libbatch, libchecked=None):
            try:
                ...
                if libchecked:
                    ...
                    self.tecutil_handle = self.handle
                return True, find_library(os.path.splitext(libbatch)[0])
            except OSError as e:
                exc_type, exc_obj, exc_tb = sys.exc_info()
                fname = path.split(exc_tb.tb_frame.f_code.co_filename)[1]
        ...
        libfmt = '{name}.dll'  # set to '{name}d.dll' for debug libraries
        libbatch = libfmt.format(name='tecutilbatch')
        libchecked = libfmt.format(name='tecutilchecked')
        libinterproc = libfmt.format(name='tecinterprocess')

        cdllexcept = []
        for libs in ((libbatch, libchecked), (libinterproc,)):
            ...
            if success:
                ...
                libpath = find_file(libs, os.environ['PATH'])
                if libpath:
                    with _patch_path(libpath):
                        success, msg = _load_library(*libs)
                    if success:
                        self.libbatch_path = msg
                        return
    ...
    def translate_macro_to_python(self, command):
        ...
        try:
            ...
            result = None
        self.tecutil_handle.tecUtilStringDealloc(tmp_result)
