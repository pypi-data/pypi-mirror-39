#! /usr/bin/env python
# -*- coding: utf-8 -*-
"""TODO"""


# This file is part of Linshare cli.
#
# LinShare cli is free software: you can redistribute it and/or modify
# it under the terms of the GNU General Public License as published by
# the Free Software Foundation, either version 3 of the License, or
# (at your option) any later version.
#
# LinShare cli is distributed in the hope that it will be useful,
# but WITHOUT ANY WARRANTY; without even the implied warranty of
# MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
# GNU General Public License for more details.
#
# You should have received a copy of the GNU General Public License
# along with LinShare cli.  If not, see <http://www.gnu.org/licenses/>.
#
# Copyright 2018 Frédéric MARTIN
#
# Contributors list :
#
#  Frédéric MARTIN frederic.martin.fma@gmail.com
#

from __future__ import unicode_literals

from argtoolbox import DefaultCompleter as Completer
from linshareapi.cache import Time
from linsharecli.common.core import add_list_parser_options
from linsharecli.common.core import hook_file_content
from linsharecli.common.core import CreateAction
from linsharecli.common.filters import PartialOr
from linsharecli.admin.core import DefaultCommand
from linsharecli.common.core import add_delete_parser_options
from linsharecli.common.formatters import DateFormatter


class JwtLTSCommand(DefaultCommand):
    """TODO"""

    IDENTIFIER = "issuer"
    DEFAULT_SORT = "issuer"
    DEFAULT_SORT_NAME = "issuer"
    RESOURCE_IDENTIFIER = "uuid"

    # pylint: disable=line-too-long
    DEFAULT_TOTAL = "JwtLTS found : %(count)s"
    MSG_RS_NOT_FOUND = "No JwtLTS could be found."
    MSG_RS_DELETED = "%(position)s/%(count)s: The JwtLTS '%(issuer)s' (%(uuid)s) was deleted. (%(time)s s)"
    MSG_RS_CAN_NOT_BE_DELETED = "The JwtLTS '%(issuer)s'  '%(uuid)s' can not be deleted."
    MSG_RS_CAN_NOT_BE_DELETED_M = "%(count)s JwtLTS(s) can not be deleted."
    MSG_RS_UPDATED = "The JwtLTS '%(issuer)s' (%(uuid)s) was successfully updated."
    MSG_RS_CREATED = "The JwtLTS '%(issuer)s' (%(uuid)s) was successfully created. (%(_time)s s)"

    def complete(self, args, prefix):
        super(JwtLTSCommand, self).__call__(args)
        json_obj = self.ls.jwt_lts.list()
        return (v.get(self.RESOURCE_IDENTIFIER)
                for v in json_obj if v.get(self.RESOURCE_IDENTIFIER).startswith(prefix))

    # pylint: disable=unused-argument
    def complete_fields(self, args, prefix):
        """TODO"""
        super(JwtLTSCommand, self).__call__(args)
        cli = self.ls.jwt_lts
        return cli.get_rbu().get_keys(True)

    def complete_domain(self, args, prefix):
        """TODO"""
        super(JwtLTSCommand, self).__call__(args)
        json_obj = self.ls.domains.list()
        return (v.get('identifier')
                for v in json_obj if v.get('identifier').startswith(prefix))


class JwtLTSListCommand(JwtLTSCommand):
    """ List all JwtLTS token."""

    @Time('linsharecli.publickeys', label='Global time : %(time)s')
    def __call__(self, args):
        super(JwtLTSListCommand, self).__call__(args)
        cli = self.ls.jwt_lts
        table = self.get_table(args, cli, self.IDENTIFIER, args.fields)
        json_obj = cli.list(args.domain)
        # Filters
        filters = [PartialOr(self.IDENTIFIER, args.identifiers, True)]
        formatters = [DateFormatter('creationDate')]
        return self._list(args, cli, table, json_obj, formatters=formatters,
                          filters=filters)


class JwtLTSCreateCommand(JwtLTSCommand):
    """Create JwtLTS token."""

    def __call__(self, args):
        super(JwtLTSCreateCommand, self).__call__(args)
        act = CreateAction(self, self.ls.jwt_lts)
        act.add_hook("publicKey", hook_file_content)
        return act.load(args).execute()


class PublicKeyDeleteCommand(JwtLTSCommand):
    """Delete JwtLTS token."""

    def __call__(self, args):
        super(PublicKeyDeleteCommand, self).__call__(args)
        cli = self.ls.jwt_lts
        return self._delete_all(args, cli, args.uuids)


def add_parser(subparsers, name, desc, config):
    """Add all JwtLTS token sub commands."""
    parser_tmp = subparsers.add_parser(name, help=desc)
    subparsers2 = parser_tmp.add_subparsers()

    # command : list
    parser = subparsers2.add_parser(
        'list', help="list JwtLTS token.")
    parser.add_argument('identifiers', nargs="*", help="")
    add_list_parser_options(parser, delete=True, cdate=False)
    parser.add_argument('--domain',
                       ).completer = Completer("complete_domain")
    parser.set_defaults(__func__=JwtLTSListCommand(config))

    # command : delete
    parser_tmp2 = subparsers2.add_parser(
        'delete', help="delete JwtLTS token.")
    add_delete_parser_options(parser_tmp2)
    parser_tmp2.set_defaults(__func__=PublicKeyDeleteCommand(config))

    # command : create
    parser_tmp2 = subparsers2.add_parser(
        'create', help="create JwtLTS token.")
    parser_tmp2.add_argument('issuer')
    parser_tmp2.add_argument('-f', '--format', choices=['PEM', 'RSA'],
                             default="PEM", dest="format")
    parser_tmp2.add_argument('--key', action="store",
                             required=True)
    parser_tmp2.add_argument('--cli-mode', action="store_true", help="")
    parser_tmp2.add_argument('--domain', default="LinShareRootDomain"
                            ).completer = Completer("complete_domain")
    parser_tmp2.set_defaults(__func__=JwtLTSCreateCommand(config))
