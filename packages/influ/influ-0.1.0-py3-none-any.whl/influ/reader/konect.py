...
class KonectReader:
    def __init__(self, config=None) -> None:
        ...
        self.list = [name for name in self.datasets.keys()]
    ...
    def load(self, dataset_name: str) -> ig.Graph:
        ...
        :type dataset_name: name of the dataset to load; awailable datasets are in 'list' attribute
        :return: loaded graph
        """
    ...
    def download(self, dataset_name):
        ...
        if not pathlib.Path(file_path).exists():
            with open(file_path, "wb") as file:
                response = get(donwload_url)
                file.write(response.content)

    def extract_tbz2(self, dataset_name):
        """Extracts donwloaded dataset"""
    ...
    def apply_vs_attributes(self, dataset_name, graph: ig.Graph):
