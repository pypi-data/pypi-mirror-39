...
from influ.merge import merge_edges


def read_graph(filepath, format=None, *args, **kwargs):
    '''
    ...
    :param format: optional (but recommended) format name how graph is stored
    :param args:
    :param kwargs:
    :return: igraph Graph object
    '''
    if format == 'events':
        g = _read_events(filepath, *args, **kwargs)
    else:
        g = ig.Graph.Read(f=filepath, format=format, *args, **kwargs)
...
def _read_events(filepath, sep=',', directed=True):
    df = pd.read_csv(filepath, sep)
    return _df_to_graph(df, directed)


def _df_to_graph(data: pd.DataFrame, directed=True) -> ig.Graph:
    '''
