...
class Node(object):
    ...
    def __str__(self, parent_value=0):
        """Recursively generate the string representation of the node and its children, by passing the values of the higher bits"""
...
class Radix(object):
    ...
    def exist(self, ip, is_network=False):
        ...
        # Is the tree empty?
        if cur_node.node0 is None and cur_node.value == 0 and cur_node.length != cur_node.bit_width:
            return None
    ...
    def remove(self, ip, is_network=False):
        ...
        while current_offset > 0:
            ...
            if next_node.is_network:
                ...
                if target_value & ((1 << (current_offset + 2 - next_node.length)) - 1):
                    return False

                # Other case of removing a subnet
    ...
    @staticmethod
    def string_for_match(ip, node, ip_is_network):
        ...
        full_ip_value = Radix.get_value(ip, ip_is_network)
