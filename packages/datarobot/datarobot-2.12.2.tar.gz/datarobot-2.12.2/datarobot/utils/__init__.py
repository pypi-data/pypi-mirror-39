...
def from_api(data, do_recursive=True, keep_attrs=None):
    if type(data) not in (dict, list):
        return data
    if isinstance(data, list):
        return _from_api_list(data, do_recursive=do_recursive)
    return _from_api_dict(data, do_recursive=do_recursive, keep_attrs=keep_attrs)


def _from_api_dict(data, do_recursive=True, keep_attrs=None):
    ...
    for k, v in six.iteritems(data):
        ...
        if v is None and k_under not in current_level:
            continue
        if do_recursive:
