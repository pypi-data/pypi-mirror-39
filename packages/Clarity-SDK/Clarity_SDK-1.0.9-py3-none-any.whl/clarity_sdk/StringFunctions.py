...
def after(source_string, regex_pattern, return_all_on_no_match = False):
    ...
    source_string = html.escape(source_string, True)
    regex_pattern = html.escape(regex_pattern, True)
    regex_pattern = regex_pattern.replace("\\","\\\\")

    parametersValueList = [repr(source_string)[1:-1], regex_pattern, return_all_on_no_match]
    ...
    return html.unescape(parsed_json['value'])
...
def before(source_string, regex_pattern, return_all_on_no_match = False):
    ...
    # Create JSON Payload to send
    source_string = html.escape(repr(source_string)[1:-1], True)
    regex_pattern = html.escape(regex_pattern, True)
    regex_pattern = regex_pattern.replace("\\","\\\\")
...
def best_match(source_string, string_list):
    ...
    # Create JSON Payload to send
    source_string = html.escape(source_string, True)
    string_list = [repr(html.escape(s.replace('\\', '\\\\'), True)) for s in string_list]
    parametersValueList = [repr(source_string)[1:-1], string_list]
...
def clean_html(source_string):
    ...
    # Create JSON Payload to send
    source_string = html.escape(source_string, True)
    parametersValueList = [repr(source_string)[1:-1]]
...
def lev_distance(source_string, compare_string):
    ...
    # Create JSON Payload to send
    source_string = html.escape(source_string, True)
    compare_string = html.escape(compare_string, True)
    parametersValueList = [repr(source_string)[1:-1], repr(compare_string)]
...
def regex_chunk(source_string, regex_string, case_sensitive=False, RemoveEmpty=False):
    ...
    # Create JSON Payload to send
    source_string = html.escape(source_string, True)
    regex_string = html.escape(regex_string, True)
    regex_string = regex_string.replace("\\","\\\\")
    parametersValueList = [repr(source_string)[1:-1], regex_string, case_sensitive, RemoveEmpty]
    ...
    for val in parsed_json['output']:
        output.append(html.unescape(val['value']))
...
def regex_count(source_string, regex_string, case_sensitive=False):
    ...
    regex_string = html.escape(regex_string, True)
    regex_string = regex_string.replace("\\","\\\\")
    parametersValueList = [repr(source_string)[1:-1], regex_string, case_sensitive]
    parametersTypeList = ['System.String','System.String', 'System.Boolean']
    jsonInputList = [{"type": pt ,"value": pv} for pt, pv in zip(parametersTypeList, parametersValueList)]
    payload ={"parameters": jsonInputList}
...
def regex_extract_each(source_string, regex_string, case_sensitive=False, reverse_search=False, return_nothing_on_pattern_fail=False):
    ...
    regex_string = html.escape(regex_string, True)
    regex_string = regex_string.replace("\\","\\\\")
    parametersValueList = [repr(source_string)[1:-1], regex_string, case_sensitive, reverse_search,return_nothing_on_pattern_fail]
...
def regex_extract(source_string, regex_string, output_string="", case_sensitive=False, reverse_search=False, return_nothing_on_pattern_fail=False):
    ...
    output_string = html.escape(output_string, True)
    parametersValueList = [repr(source_string)[1:-1], regex_string, repr(output_string)[1:-1], case_sensitive, reverse_search,return_nothing_on_pattern_fail]
...
def regex_extract_all(source_string, regex_string, outputstring="", case_sensitive=False):
    ...
    parametersValueList = [repr(source_string)[1:-1], regex_string, repr(outputstring)[1:-1], case_sensitive]
...
def regex_replace(source_string, regex_string, replace_string, case_sensitive=False):
    ...
    parametersValueList = [repr(source_string)[1:-1], regex_string, repr(replace_string)[1:-1], case_sensitive]
    parametersTypeList = ['System.String','System.String','System.String', 'System.Boolean']
    jsonInputList = [{"type": pt ,"value": pv} for pt, pv in zip(parametersTypeList, parametersValueList)]
    payload ={"parameters": jsonInputList}
