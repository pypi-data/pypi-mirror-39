...
def mergeDataFrames(data, cols, axis, strategy, dryrun=False):
    ...
    :arg cols:     List of lists - :class:`.Column` objects representing the
                   columns in each data set in ``data``. The first ``Column``
                   in ``cols`` is assumed to describe the index column.
    ...
    if naive:
        ...
        if dryrun:
            if axis == 0:
                ...
                cols = cols[0] + list(it.chain([c[1:] for c in cols[1:]]))
        ...
        # the index column
            ...
            cols   = [idxcol] + list(it.chain(*[c[1:] for c in cols]))
