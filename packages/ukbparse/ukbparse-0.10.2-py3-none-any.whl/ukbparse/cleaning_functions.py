...
@custom.cleaner()
def remove(vid, columns):
    """Remove (do not load) all columns associated with ``vid``.

    :arg vid:     Integer variable ID.
    :arg columns: List of all :class:`.Column` objects associated with ``vid``.
...
@custom.cleaner()
def keepVisits(vid, columns, *tokeep):
    """Only load columns for ``vid`` which correspond to the specified visits.

    :arg vid:     Integer variable ID.

    :arg columns: List of all :class:`.Column` objects that are associated with
                  ``vid``.
...
def convertICD10Codes(dtable, vid):
    """Converts values which are assumed to contain ICD10 codes into numerical
    values.
    """
