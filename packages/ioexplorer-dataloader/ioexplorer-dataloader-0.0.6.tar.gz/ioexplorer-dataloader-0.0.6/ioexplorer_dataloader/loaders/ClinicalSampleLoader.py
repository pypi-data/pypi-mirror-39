...
class ClinicalSampleLoader(BaseLoader):
    ...
    def inspect(self):
        ...
        loadable_columns = [str(column.name) for column in table.c if column.name in D.columns.values]
    ...
    def validate(self):
        ...
        if 'sample_label' not in D.columns:
            print('No column named "sample_label". This is required, and must be the unique sample label.')
            return False
        if 'subject_label' not in D.columns:
            print('No column named "subject_label". '
    ...
    def ingest(self, D):
        D['subject_id'] = [self.subject_label_to_id[label] for label in D['subject_label']]
        table = Table('samples', MetaData(), autoload=True, autoload_with=self.engine)
        ...
        res = self.conn.execute("SELECT id FROM samples WHERE sample_label in {};"
                                .format(tuple(D['sample_label'])))
