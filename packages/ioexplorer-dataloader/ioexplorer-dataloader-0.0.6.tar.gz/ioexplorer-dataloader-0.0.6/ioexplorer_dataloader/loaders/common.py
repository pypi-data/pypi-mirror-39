...
def df_to_sql(df, table_name, engine):
    if not df.empty:
        ...
        res = curs.copy_from(output, '"{}"'.format(table_name), null='', columns=columns)
...
ingesters = _AttrDict(
    df_with_progress = df_to_sql_with_progress
)

def df_lowercase_columns_transform(D):
    # Doesn't lowercase foreign keys, with end with 'Id'.
    D.columns = map(lambda x: x.lower() if not x.endswith("Id") else x, D.columns.values)
    return D
