...
class TimelineLoader(BaseLoader):
    ...
    def pre_ingest_transform(self, data):
        ...
        data['start_date'] = data['start_date'].apply(lambda x: x if pd.isnull(x) else str(int(x)))
        data['stop_date'] = data['stop_date'].apply(lambda x: x if pd.isnull(x) else str(int(x)))
        return data
