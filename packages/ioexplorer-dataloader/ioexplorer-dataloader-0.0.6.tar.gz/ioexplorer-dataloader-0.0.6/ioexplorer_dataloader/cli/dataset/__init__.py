...
def ingest(conn, engine, _env):
    ...
    try:
        ...
        error("\n\n===START SQL ERROR LOGS===\n\n{}\n===END SQL ERROR LOGS===\n\n"
              .format(e),
              exit=False)
        error('Oh no! Something bad happened during ingestion. '
              'SQL logs should be above to help you debug! '
              'I am deleting everything that was ingested prior to the error!', exit=False)
...
@add_env_variables
@connect_to_db
@click.confirmation_option(prompt=click.style("Are you sure you want to delete the dataset?", fg='red'),
...
def delete(dataset, conn, engine, _env):
    ...
    if deleted:
        ...
        error("Could not delete the `{}` dataset. Are you sure it exists?".format(dataset))
