...
from ..common.constants import DATATYPE_NAMES, REQUIRED_DATATYPES, CONF_PATH
...
def ask_for_dataset_info():
    questions = [
        ...
        },
        {
            'type': 'input',
            'name': 'paper_url',
            'message': 'Enter link to paper.'
        },
        {
            'type': 'input',
            'name': 'uploader',
            'message': 'Who are you (person uploading data)?'
        }
    ]
...
def create_default_ui_config(dataset_info):
    _ = lambda x: get_in(['data_type_summaries', x, 'variables'],
                         dataset_info,
                         default=[])
    default_variables = {n: _(n) for n in DATATYPE_NAMES}
    default_variables = {k: v for k, v in default_variables.items() if v}
    return {
        'FilterVariables': default_variables,
        'Correlation': default_variables,
        'Survival': default_variables,
        'Expression': default_variables
    }

def init_dataset(conn, engine):
    if os.path.exists('config.yaml'):
        error("A `config.yaml` file already exists in this directory. "
    ...
    for data_name in DATATYPE_NAMES:
        ...
        if not loader.validate():
            if data_name in REQUIRED_DATATYPES:
                message = ('Could not validate `{0}` data, (errors are shown above). '
                           'This data type is required, so I am exiting!'
                           .format(data_name))
                error(message)
            ask_continue_on_invalid(data_name)
        else:
            success("I was able to find the `{}` data! Please wait while I load it...".format(data_name))
    ...
    success("Thanks! I made a file called `config.yaml` in this directory! "
...
def create_dataset(dataset_info, conn):
    ...
    result = conn.execute("""
        INSERT INTO datasets (name, description, url, uploader, config)
        VALUES ('{name}',
                '{description}',
                '{url}',
                '{uploader}',
...
def ingest_dataset(name, conn, engine, dataset_config):
    dataset_info = {
        ...
        'description': get_in(['dataset_info', 'description'], dataset_config, default=''),
        'url': get_in(['dataset_info', 'paper_url'], dataset_config, default=''),
        'uploader': get_in(['dataset_info', 'uploader'], dataset_config, default=''),
        'config': json.dumps(get_in(['ui_config'], dataset_config, default='{}')),
    }
    if not dataset_info['name']:
        error('config.yaml had no conf["dataset_info"]["dataset_name"] entry. This is required!')
    ...
    for datatype_name in DATATYPE_NAMES:
        ...
        if datatype_name == 'ClinicalSubject':
            ...
            loader = loader_class(conn=conn, engine=engine, subject_label_to_id=subject_label_to_id)
        else:
            loader = loader_class(conn=conn, engine=engine, sample_label_to_id=sample_label_to_id)
        if not loader.validate():
            if datatype_name in REQUIRED_DATATYPES:
                error('Could not validate {}. '
                      'This is a required data type, so I am exiting!'
                      .format(datatype_name))
                sys.exit(1)
        else:
            if datatype_name == 'ClinicalSubject':
