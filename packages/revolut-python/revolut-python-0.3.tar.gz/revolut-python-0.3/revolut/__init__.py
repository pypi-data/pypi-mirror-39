...
class Account(_UpdateFromKwargsMixin):
    ...
    def send(self, dest, amount, currency, request_id, reference=None):
        ...
        if destid in self.client.accounts:
            return self._transfer_internal(destid, amount, currency, request_id, reference)
        try:
    ...
    def _transfer_internal(self, destid, amount, currency, request_id, reference):
#        if self.currency != currency:
#            raise ValueError('Currency {} does not match the source currency: {}'.format(
#                    currency, self.currency))
#        if self.client.accounts[destid].currency != currency:
#            raise ValueError('Currency {} does not match the destination currency: {}'.format(
#                    currency, self.client.accounts[destid].currency))
        reqdata = {
            'request_id': request_id,
