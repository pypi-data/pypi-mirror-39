...
Config = typ.NamedTuple('Config', [('current_version', str), (
    'pep440_version', str), ('tag', bool), ('commit', bool), ('push', bool),
...
def _debug_str(cfg):
    cfg_str_parts = ['Config Parsed: Config('.format(),
        ...
        "pep440_version='{0}'".format(cfg.pep440_version), 'tag={0}'.format
        (cfg.tag), 'commit={0}'.format(cfg.commit), 'push={0}'.format(cfg.
        push), 'file_patterns={']
...
def parse(ctx):
    ...
    if not ctx.config_filepath.exists():
        log.error('File not found: {0}'.format(ctx.config_filepath))
        return None
    try:
