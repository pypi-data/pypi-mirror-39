...
class JemuI2cSlave(object):
    ...
    def _send_master_read_response(self, ack, response):
        self._jemu_connection.send_response(self._master_read_response_json(ack, response), self._id,
                                            self._EXTERNAL_PERIPHERAL)

    def _send_master_write_response(self, ack):
        self._jemu_connection.send_response(self._master_write_response_json(ack), self._id,
                                            self._EXTERNAL_PERIPHERAL)
    ...
    def set_address(self, address, mask=None):
        ...
        self._jemu_connection.send_command(self._address_json(), self._id, self._EXTERNAL_PERIPHERAL)
