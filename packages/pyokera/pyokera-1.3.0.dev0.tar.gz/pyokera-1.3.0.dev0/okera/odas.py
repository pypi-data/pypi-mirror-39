...
class PlannerConnection():
    ...
    def open(self, path, preload_content=True):
        ...
        try:
            ...
            if urls and len(urls) != 1:
                raise ValueError(
                    "Expected result from server. Expecting at most one url.")
            return self.http_pool.request('GET', urls[0],
                                          preload_content=preload_content)
        except:
            # TODO: the only exception that makes sense is authorization error,
            # otherwise, the user could access the path but there's nothing there.

            # TODO: look up the table registered at this path
            object_params = TGetRegisteredObjectsResult()
            object_params.prefix_path = path
            objects = self.service.client.GetRegisteredObjects(object_params)
            if path not in objects.object_names:
                # If it doesn't exist, then nothing we can do, fail the call
                raise ValueError("Invalid path: " + path)

            for catalog_obj in objects.object_names[path]:
                if '.' not in catalog_obj:
                    # This is a DB, can't read from this
                    continue
                # If it does exist (meaning there is a table/view accessible to this
                # user), read it from the scan api and return those results back.
                print(catalog_obj)
        return "Hi kevin"
    ...
    def list_dataset_names(self, db, filter=None):
        ...
        ...     datasets
        ['okera_sample.sample', 'okera_sample.users']
        """
...
def _columnar_batch_to_python(schema, columnar_records, num_records, ctx_tz=pytz.utc):
...
class PandasScanTask(ScanTask):
  def __init__(self, ctx, plan_hosts, task, max_records, options):
