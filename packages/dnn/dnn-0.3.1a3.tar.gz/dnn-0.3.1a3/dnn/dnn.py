...
class DNN:
    ...
    def write_summary (self, writer, feed_dict, verbose = True):
        ...
        output.sort ()    
        if self.on_ftest:
            head = tc.primary ("[final:test]")
            print ("%s %s" % (head, " | ".join (output)))
        else:
            if writer != "saved":
                if self.is_overfit:
            ...
            if verbose and self.verbose:
                if writer == "saved":
                    head = tc.warn ("[%04d:%5s]" % (epoch, writer))
                else:
                    head = tc.debug ("[%04d:%5s]" % (epoch, writer))                    
                print ("%s %s" % (head, " | ".join (output)))
    ...
    def add_accuracy (self):
        try:
            ...
            try:
                self.calculate_accuracy_v2 (None)
            except NotImplementedError:
                raise NotImplementedError ("You should be overidden calculate_accuracy or calculate_accuracy_v2")
            except:
                pass    
    ...
    def resub (self, x, y, ops = [], **kargs):
        self.is_validating = False
        return self.evaluate (x, y, True, ops, **kargs)        
    train = resub
    ...
    def lstm (
        ...
        for i in range (lstm_layers):            
            ...
            if dropout:
                lstm = rnn.DropoutWrapper (lstm, input_keep_prob = 1.0 - self.dropout_rate, output_keep_prob = 1.0 - self.dropout_rate)
            cells.append (lstm)
        ...
        if dynamic:     
            outputs, final_state = type_rnn (cell, lstm_in, time_major = time_major, dtype = tf.float32, sequence_length = self.seq_length)
    ...
    def custom_accuracy (self, r, *args, **karg):
        try:
            acc = self.calculate_accuracy_v2 (r, *args, **karg)            
        except NotImplementedError:
            acc = self.calculate_accuracy (r.logit, self.filter (r.y), *args, **karg)
        if self.is_validating and not self.on_ftest:
    ...
    def calculate_accuracy_v2 (self, result, *args, **karg):
        raise NotImplementedError
