...
class DNN:
    ...
    def lstm (self, n_input, hidden_size, lstm_layers = 1, activation = None, dynamic = True, dropout = False, basic = True, kreg = None):
        ...
        if dynamic:            
            output, final_state = type_rnn (cell, lstm_in, time_major = True, dtype = tf.float32, seqence_length = self.seq_length)
        else:
            seq_len = shape [1]
            ...
            output, final_state = type_rnn (cell, lstm_in, dtype = tf.float32, initial_state = initial_state, sequence_length = sel.seq_length)
