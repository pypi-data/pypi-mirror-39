...
class CNV(object):
    ...
    """
    def __init__(self, raw_text, defaults=None, logger_name=None):

        logger = logging.getLogger(logger_name or "seabird.CNV")
        logger.debug('Initializing CNV class')
    ...
    def as_DataFrame(self):
        ...
        try:
            ...
            logger.warning("I'm not able to import pandas")
            return
...
class fCNV(CNV):
    ...
    def __init__(self, filename, defaultsfile=None, logger=None):
        ...
        logger = logging.getLogger(logger or "seabird.fCNV")
        logger.debug('Initializing fCNV class with file: %s' % filename)
