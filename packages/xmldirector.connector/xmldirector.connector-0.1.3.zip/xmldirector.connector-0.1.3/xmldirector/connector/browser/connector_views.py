...
def default_html_handler(get_handle, filename, view_name, request):
    ...
    base_url = '{}/@@view/{}'.format(
        request.context.absolute_url(1), '/'.join(request.subpath[:-1]))
    # get HTML
    ...
    return html_template.pt_render(
        dict(
            template='html_view',
            request=request,
            context=request.context,
            options=dict(base_url=base_url, html=html)))
...
def ace_editor(get_handle,
    ...
    view_url = '{}/view-editor-readonly/{}'.format(
        request.context.absolute_url(), '/'.join(request.subpath))
...
def ace_editor_readonly(get_handle,
                        filename,
                        view_name,
                        request,
                        readonly=True,
                        template_name='ace_editor.pt'):
    return ace_editor(get_handle, filename, view_name, request, readonly,
                      template_name)
...
def default_view_handler(get_handle, filename, view_name, request):
    ...
    if 'filename' in request:
        request.response.setHeader(
            'Content-Disposition',
            'attachment;filename={}'.format(request['filename']))
    if 'size' in info:
...
precondition_registry.register(
    Precondition(
        suffixes=['.xml', '.html', '.htm', '.css', '.json'],
