...
class EEClient(object):
    ...
    def __init__(self, hostname, port):
        ...
        if TRACE_ENABLED:
            ...
            self.channel = grpc.intercept_channel(
                self.channel,
                # TODO: Change DS with the right name
                ParaverClientInterceptor("DS", hostname, port)
            )
        try:
    ...
    def activate_tracing(self, sync_time):
        request = dataservice_messages_pb2.ActivateTractingRequest(
            time=sync_time
        )
    ...
    def create_paraver_traces(self):        
        try:
            response = self.ds_stub.createParaverTraces(CommonMessages.EmptyMessage())
