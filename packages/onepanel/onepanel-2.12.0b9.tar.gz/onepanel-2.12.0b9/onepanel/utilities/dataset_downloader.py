...
class DatasetMountProgressUpdater:
    ...
    def update(self):
        logging.getLogger(LOGGER).info('Sending Update to API')
...
class AWSDownloadFileTracker(FileSystemEventHandler):
    ...
    def on_moved(self, event):
        ...
        logging.getLogger(LOGGER).info('File moved from {} to {}'.format(event.src_path, event.dest_path))
    ...
    def update_statistic(self, file_path):
        ...
        try:
            ...
            logging.getLogger(LOGGER).info('{} is {} bytes'.format(file_path, os.stat(file_path).st_size))
