...
from onepanel.awss3Dataset import AwsS3Dataset
...
@datasets.command('push', help='Push up changes')
@click.pass_context
@login_required
def datasets_push(ctx):
    ...
    next_version = vc.get(field_path='/version/1')
    ...
    # Here we can load a concrete class of an abstract dataset worker class
    datasetManager = AwsS3Dataset()

    datasetManager.push('./', next_version['data']['version']['path'])

    vc.put(field_path='/update_version')

    click.echo('Pushed up version ' + str(next_version['data']['version']['version']))
...
def datasets_clone(ctx, path, directory, include, exclude):
    ...
    current_version = vc.get(field_path='/version/0')
    ...
    # TODO - logic to decide which provider to use. Probably based on a response from API
    dataset_manager = AwsS3Dataset()
    dataset_manager.clone(home, current_version['data']['version']['path'])
