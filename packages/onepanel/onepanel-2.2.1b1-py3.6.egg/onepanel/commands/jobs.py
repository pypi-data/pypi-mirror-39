...
import boto3
import botocore
from onepanel.utilities.boto3_utility import Boto3Helpers
import click
import configobj
import websocket
...
def jobs_download_output(ctx, path, directory,archive_flag):
    ...
    if is_legacy:
        ...
        boto3_util = Boto3Helpers()
        client = boto3.client('s3')
        resource = boto3.resource('s3')
        s3_download_to_path = '.'  # Current directory that the script is executing from
        if archive_flag is True:
            print("Attempting to download the compressed output file to current directory.")
            compressed_file_name = 'job-{job_uid}-output.tar.gz'.format(job_uid=job_uid)
            s3_path_to_download_from = jvc.get_s3_compressed_file_for_job_output(account_uid, project_uid, job_uid)
            boto3_util.download_compressed_file(client, resource, s3_path_to_download_from, s3_download_to_path, jvc.s3_bucket_name,compressed_file_name)
        else:
            print("Attempting to download output to current directory.")
            s3_path_to_download_from = jvc.get_s3_root_for_job_output(account_uid, project_uid, job_uid)
            boto3_util.download_dir(client, resource, s3_path_to_download_from, s3_download_to_path,jvc.s3_bucket_name)
