...
def datasets_push(ctx):
    ...
    boto3_helper = Boto3Helpers()

    dataset_dir = os.curdir
    s3_push_to_dir = next_version['data']['version']['path']
    boto3_helper.push(s3,dataset_dir,s3_push_to_dir)
...
def datasets_clone(ctx, path, directory, include, exclude):
    ...
    boto3_helper.download_dir(s3,s3resource,s3_from_dir,
      home,boto3_helper.get_dataset_bucket_name(),s3_from_dir)
    return 0
