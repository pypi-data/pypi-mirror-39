...
from onepanel.utilities.dataset_downloader import DatasetDownloader, DatasetMountIdentifier, DatasetDownloadListener
...
# TODO - for each dataset mount add an alias
...
@click.pass_context
@login_required
def start_dataset_mount_downloader(ctx, account_uid, project_uid, resource_type, resource_uid, download_path):
    click.echo('Starting dataset mount downloader...')
    # TODO its own connection?
    downloader = DatasetDownloadListener(ctx.obj['connection'], download_path, account_uid, project_uid, resource_type, resource_uid)
    downloader.start()
    downloader.join()
