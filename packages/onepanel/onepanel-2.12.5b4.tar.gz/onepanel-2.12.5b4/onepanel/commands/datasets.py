...
@click.pass_context
@login_required
def start_dataset_mount_downloader(ctx, account_uid, project_uid, resource_type, resource_uid, download_path, verbose):
    downloader = DatasetDownloadListener(ctx.obj['connection'], download_path, account_uid, project_uid, resource_type, resource_uid, verbose)
    downloader.start()
    downloader.join()
...
def datasets_init(ctx):
    ...
    # Please enter a valid name [<name-suggestion>]:
    # <blue>To use suggested name "<name-suggestion>", just press enter.</blue>
    ...
    if not vc.is_uid_valid(dataset_uid):
        suggested = dataset_uid.lower()
        suggested = suggested.replace('_', '-')
        # <red>The name you entered is invalid, it should be...</red>
        # Please enter a valid name [<name-suggestion>]:
        ...
        prompt_msg += click.style('\nTo use suggested "{alt}", just press enter.'.format(alt=suggested),fg='blue')
        try:
            dataset_uid = click.prompt(prompt_msg, default="", type=str, show_default=False)
        except click.Abort:
            return None
