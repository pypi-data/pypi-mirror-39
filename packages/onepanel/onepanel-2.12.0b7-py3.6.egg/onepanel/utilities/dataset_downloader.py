...
class AWSDownloadFileTracker(FileSystemEventHandler):
    ...
    def on_moved(self, event):
        ...
        logging.getLogger(LOGGER).info('File moved from', event.src_path, ' to ', event.dest_path)

        self.update_statistic(event.dest_path)

    def update_statistic(self, file_path):
        logging.getLogger(LOGGER).info('Updating statistics with file', file_path)

        self.stats.add_files(1)

        try:
            self.stats.add_bytes(os.stat(file_path).st_size)
            click.echo(file_path, 'is ', os.stat(file_path).st_size, 'bytes')
        except Exception:
            # Don't do anything
            pass
