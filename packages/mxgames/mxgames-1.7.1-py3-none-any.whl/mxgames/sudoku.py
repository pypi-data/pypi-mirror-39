...
class Sudoku(game.Game):
    def __init__(self, title, size, fps=30):
        ...
        while not self.make_digits():
            pass
        self.answer = [[self.digits[i][j] for j in range(ROWS)] for i in range(ROWS)]
        self.blank = [[False for i in range(ROWS)] for j in range(ROWS)]
        self.blank_num = self.make_blank(randint(45, 65))
    ...
    def make_blank(self, max_num):
        num = 0
        rest = [(x, y) for x in range(ROWS) for y in range(ROWS)]
        while True:
            if rest == [] or num == max_num:
                print("blank: %d / %d" % (num, 81))
                break
            x, y = choice(rest)
            rest.remove((x, y))
            if self.blank[x][y] or self.answer[x][y] == 0:
                continue
            if self.multi_solve(x, y, self.digits[x][y]):
                self.answer[x][y] = self.digits[x][y]
                continue
            self.blank[x][y] = True
            self.answer[x][y] = 0
            num += 1
