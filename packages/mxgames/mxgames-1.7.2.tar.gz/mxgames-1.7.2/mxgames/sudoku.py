...
class Sudoku(game.Game):
    def __init__(self, title, size, fps=30):
        ...
        while self.blank_num < 48:
            ...
            self.blank_num = self.make_blank()
        print("blank: %d / %d" % (self.blank_num, 81))
    ...
    def make_blank(self):
        ...
        rest = [[(j, i) for i in range(ROWS)] for j in range(ROWS)]
        shuffle(rest)
        for row in rest:
            for i, j in row:
                if i % 2 == 1:
