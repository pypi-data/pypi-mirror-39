## $Id: test_applicantcopier.py 14952 2018-02-09 19:52:16Z henrik $
##
...
class ApplicantCopierFunctionalTests(ApplicantsFullSetup):
    ...
    def test_copier(self):
        ...
        # Copy te file to samples_dir
        path = os.path.join(samples_dir(), 'aplication_slip.pdf')
        open(path, 'wb').write(pdf)
        print "Sample PDF aplication_slip.pdf written to %s" % path
    ...
    def test_hidden_batch_copying_container(self):
        ...
        self.browser.open(self.container_path + '/createallstudents')
        self.assertTrue('No course admitted provided' in self.browser.contents)
        self.assertFalse('Successfully created records' in self.browser.contents)
