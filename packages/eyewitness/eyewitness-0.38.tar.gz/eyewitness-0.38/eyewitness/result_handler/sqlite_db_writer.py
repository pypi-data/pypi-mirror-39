...
class BboxPeeweeSQLiteDbWriter(DetectionResultHandler, ImageRegister):
    def __init__(self, db_path):
        """
        Parameters:
        -----------
...
class FalseAlertPeeweeSQLiteDbWriter(FeedbackMsgHandler, AudienceRegister, ImageRegister):
    def __init__(self, db_path):
        """
        Parameters:
        -----------
        ...
        # setup database for models
        self.database = SqliteDatabase(db_path)
        feedback_models.DATABASE_PROXY.initialize(self.database)
        self.create_db_table()
    ...
    def check_proxy_db(self):
        if not (self.database is feedback_models.DATABASE_PROXY.obj):
            feedback_models.DATABASE_PROXY.initialize(self.database)
