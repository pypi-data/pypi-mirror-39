...
import os, copy, commands, datetime, time
...
class TarGpgBackup:
    ...
    def __init__(self, vmname, targfoldr, vmfoldr, filelist=[], \
        ...
        """ settings
            ...
            'targetserver' = SFTP-server as backup-target, if used: 'targfoldr' is server-path,
            'targetserverlogin' = user:password for the SFTP-server,
    ...
    def paraCheck(self):
        ...
        try :
            ...
            if not isinstance(self.__targetserver, type(None)):
                self.__log.info('|- backup will be uploaded to SFTP-server "%s"' % self.__targetserver)
                if isinstance(self.__targetserverlogin, type(None)):
                    self.__log.warning('|- no login data given for SFTP-server')
                    retval = False
                else :
                    self.__log.info('|- SFTP-login data given')
        ...
        try :
            ...
            if not isinstance(self.__targetserver, type(None)):
                ...
                if len(cmd1) > 0: # if using gpg, we use pipe as in-file
                    cmd1 += ' | '
                    fname = '-'
                cmd1 += 'curl --upload-file %s --insecure --user %s sftp://%s%s' % \
                          (fname, self.__targetserverlogin, self.__targetserver, targetfile)
            ...
            if isinstance(self.__targetserver, type(None)):
                ...
                self.__log.info('|- --> sftp://%s%s' % (self.__targetserver, targetfile))
    ...
    def __packOnTheFly(self, packdateien, filenumber=None):
        ...
        try :
            ...
            ziel1 = ''
            if not isinstance(self.__targetserver, type(None)):
                ziel1 = 'sftp://%s' % self.__targetserver
            ...
            # upload
            if not isinstance(self.__targetserver, type(None)):
                cmd1 += '| curl --upload-file - --insecure --user %s sftp://%s%s' % \
                          (self.__targetserverlogin, self.__targetserver, \
