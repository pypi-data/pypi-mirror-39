...
def get_tree(input, output=[], prefix=u'│', show_id=False, show_only_folder=False, root=True):
    ...
    if input.get('result') is not None:
        ...
        if input['children'].__len__() is not 0:
            for index, i in enumerate(input['children']):
                j = {}
                if i.get('isFolder') == 1:
                    j = _list_files(i.get('filePath')+i.get('name')+'/', False, config.current_project_id, show_only_folder)
                i.update(result=j.get('result'))
                if root == False:
                #if input.get('name'):  # 判断是否是第一次函数
                    if index + 1 == len(input['children']):  # 判断是否是最后一次递归
                        get_tree(i, output, prefix + u'     ', show_id, show_only_folder, False)
                    else:
                        get_tree(i, output, prefix + u'   │ ', show_id, show_only_folder, False)
                else:
                    if index + 1 == len(input['children']):  # 判断是否是最后一次递归
                        get_tree(i, output, "", show_id, show_only_folder, False)
                    else:
                        get_tree(i, output, prefix, show_id, show_only_folder, False)
...
def mv(args):
    ...
    try:
        if args.sources[-1] == '/':
            data = {'filePath': get_fully_path(args.sources, is_end=args.sources[-1] is not '/'),
...
def tree(args):
    ...
    get_tree(input=file_list, output=gettree, show_id=args.id, show_only_folder=(not args.file))
    print(to_zhcn('\n'.join(gettree)))
